 ...
 * goes away (quit, window close). The renderer's job is to refresh the durable
 * primary-profile store, never to ship per-tab state: the host suspends the
 * session to dormant on route loss and re-materializes it later from the
 * durable tab URLs plus that store.
 */
