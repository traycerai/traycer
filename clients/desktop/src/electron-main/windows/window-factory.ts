...
export function createMainWindow(options: MainWindowOptions): BrowserWindow {
  ...
  //
  // Gated to slots where DevTools are permitted (dev + staging): those are the
  // dogfood/diagnosis builds. Production stays out of the all-level firehose so
  // a chatty renderer never inflates the shipped log.
  if (canOpenDevTools) {
    window.webContents.on(
      "console-message",
