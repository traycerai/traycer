...
export class BrowserViewManager {
  ...
  private closeEntry(entry: BrowserViewEntry): Promise<void> {
    ...
    // Mark the entry as closing BEFORE any teardown runs, never after. This
    // ordering used to hold only by accident: `destroyEntry` awaited the
    // handoff capture early, so the assignment landed while the entry was
    // still in the registry. With the capture gone the destroy is fully
    // synchronous, and assigning afterwards would leave `closePromise` null
    // for the entire close - which is what `closeEntry`'s own idempotence
    // guard, `findExactNativeEntry`'s "skip a closing entry" check, and
    // provisioning's "chain an ensure behind the in-flight close" branch all
    // read. They must not depend on a teardown step happening to await.
