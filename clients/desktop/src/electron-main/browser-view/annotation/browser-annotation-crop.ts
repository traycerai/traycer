...
/** CSS-pixel viewport size - same space as `BrowserAnnotationCssRect`. */
export interface AnnotationCssDimensions {
  readonly width: number;
  readonly height: number;
}

/** Device-pixel captured image size - same space as `BrowserViewCropRect`. */
export interface AnnotationDeviceDimensions {
  readonly width: number;
  readonly height: number;
}
...
export function computeAnnotationCropRect(input: {
  ...
  readonly imageSize: AnnotationDeviceDimensions;
}): BrowserViewCropRect | null {
...
export function cropAnnotationPng(
  image: BrowserViewCapturedImage,
  unionRect: BrowserAnnotationCssRect,
  viewport: AnnotationCssDimensions,
): Uint8Array | null {
