...
function runDeferred(state: BootState, services: AppServices): void {
  ...
  void timed("deferred", "host-watcher", () => {
    services.host.on("error", (err: HostStartupError) => {
      log.error("[desktop] host startup error", err);
      notifyHostError(err, (command) => {
        state.bridge?.dispatchMenuCommand(command);
      });
    });
  ...
  void timed("deferred", "registry-probe", async () => {
    ...
    if (result.updateAvailable && result.latestVersion !== null) {
      services.menu.setHostUpdateAvailableVersion(result.latestVersion);
    } else {
      services.menu.setHostUpdateAvailableVersion(null);
    }
