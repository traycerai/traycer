...
async function ensureHost(
  ...
  // Fast path - the persistent service is already up from a prior launch,
  // reachable, and on this build. Unified build-identity gate (all
  // environments, Tech Plan Decision 8), mirroring
  // `HostLifecycle.isBuildStampCompatible`: a running host whose published
  // version differs from this build's `config.version` is the reinstall-
  // without-uninstall case (protocol may have changed with no semver bump), so
  // we fall through to `traycer host ensure`. The CLI's busy probe then keeps
  // a host that has work in progress (returning E_HOST_BUSY) instead of
  // tearing it down - that busy + compat gate is what makes restarting the
  // shared host (one per machine, possibly serving a co-installed CLI) safe,
  // so production no longer needs the version-agnostic hotfix exemption.
  // `prePid` lets the readiness poll below skip the still-dying old host's
  // pid.json during the SMAppService unregister→register cycle.
  ...
  const stampMatches = serviceStatus.version === config.version;
  if (
    !force &&
  ...
  if (serviceStatus.state === "running" && !stampMatches) {
    log.info(
      "[host-ensure] running host build differs from bundle - ensuring (busy check keeps in-progress work)",
      { running: serviceStatus.version, bundled: config.version },
    );
  }
...
// Keep a running-but-busy host instead of restarting it: tolerate its
// mismatched stamp (scoped to this exact `version` - #5) and surface it so the
...
// ours and leave `getSnapshot()` momentarily stale (#6). Returns the
// `host-busy` result when a reachable snapshot was surfaced (#4), else `null`
// so the caller routes to normal recovery / restart.
async function surfaceBusyHostKeep(
  bridge: RunnerIpcBridge,
  version: string,
): Promise<HostEnsureIpcResult | null> {
  bridge.options.host.tolerateStampMismatch(version);
