...
export class HostLifecycle extends EventEmitter {
  ...
  // The published version of a mismatched-but-busy host we chose to keep (the
  // CLI returned host-busy): we surface ONLY that specific version so the
  // renderer can run its compat probe, instead of nulling it (which would
  // trigger a reinstall). Scoped to the version so a *different* stale host is
  // still gated. `null` = gate every mismatch. Cleared once a stamp-matching
  // host appears (Force restart / natural upgrade).
  private stampMismatchToleratedVersion: string | null = null;
  ...
  /**
   * Tolerate a build-stamp mismatch for the host publishing `version`, so a
   * mismatched-but-busy host (the CLI returned `host-busy`) is surfaced to
   * the renderer instead of being nulled by the stamp gate. Scoped to the
   * specific version so a *different* stale host is still gated. The renderer
   * then runs its compat probe and either keeps the host or prompts for a
   * restart. Call `reloadSnapshotFromDisk()` afterwards to re-emit the
   * now-tolerated snapshot (and use ITS return value, not `getSnapshot()`, to
   * judge whether the host was surfaced). Cleared automatically as soon as the
   * tolerated host is no longer the one on disk - it upgraded to a matching
   * stamp, was replaced by a different build, or exited - so the tolerance never
   * outlives the specific busy host it was granted for.
   */
  tolerateStampMismatch(version: string): void {
    this.stampMismatchToleratedVersion = version;
  }
   ...
   * websocket URL shape, and build-stamp checks happen in `reloadSnapshot()`
  ...
  /**
   * Build-identity gate (all environments). A running host whose published
   * stamp differs from this build's `config.version` is treated as not-ours -
   * it survived a reinstall-without-uninstall and serving it could surface
   * protocol mismatches - so the gate nulls it and the ensure flow decides
   * whether to keep it (busy + compatible) or restart it. Production is no
   * longer exempt: the CLI busy probe + the renderer compat probe (Tech Plan
   * Decision 8) are what make restarting the shared host safe, so the same
   * flow runs everywhere. Sync (no reachability probe) so the snapshot-emission
   * path can apply it cheaply.
   */
  private isBuildStampCompatible(snapshot: DesktopLocalHostSnapshot): boolean {
    // The CLI reported a host on this exact version busy on a stamp mismatch;
    // surface that specific host so the renderer can run its compat probe (and
    // keep it if compatible). A different stale version stays gated. Reset in
    // `reloadSnapshot` as soon as the tolerated host is no longer the one on
    // disk (upgraded, replaced, or exited).
    if (
      this.stampMismatchToleratedVersion !== null &&
      snapshot.version === this.stampMismatchToleratedVersion
    ) {
      return true;
    }
    return snapshot.version === config.version;
  }
  ...
  private async reloadSnapshot(): Promise<DesktopLocalHostSnapshot | null> {
    ...
    // Stop tolerating the kept mismatched host once it is no longer the host
    // on disk: it upgraded (raw.version === config.version), was replaced by a
    // different stale build, or exited (raw === null). Only while the exact
    // tolerated host is still publishing do we keep surfacing it; otherwise a
    // later wrong-stamp host - or a flap of the same stale version after it
    // left - is gated again until a fresh busy-keep re-tolerates it. (Without
    // this, a tolerance set once would linger for the whole process and could
    // surface a different stale host that was never busy-checked.)
    if (
      this.stampMismatchToleratedVersion !== null &&
      (raw === null || raw.version !== this.stampMismatchToleratedVersion)
    ) {
      this.stampMismatchToleratedVersion = null;
    }
    // Filter a stale (wrong-stamp) host out of what the renderer sees, so the
    // host gate treats it as not-ready and fires `ensureHost` (reinstall +
    // restart) instead of serving the previous build's host. The freshly
    // installed host publishes a matching stamp and passes through.
    ...
    if (!snapshotEquals(prev, next)) {
      if (next === null && raw !== null) {
        log.info(
          "[host] ignoring pid metadata until the local host is reachable and compatible",
          {
            hostId: raw.hostId,
  ...
  private async toReachableSnapshot(
    ...
    if (!this.isBuildStampCompatible(raw)) {
      return null;
    }
  ...
  private async waitForReady(): Promise<void> {
    ...
    throw new HostStartupException(
      "HOST_NOT_READY",
      `Local host did not publish reachable PID metadata at ${this.options.layout.pidMetadataFile} within ${this.readyTimeoutMs}ms - run \`traycer host doctor\` to recover.`,
    );
  }
