...
/**
 * The checks mirror the host's - `traycer-host/src/transport/rpc/auth/ host-access-token.ts` and `jwks-key-source.ts`: RS256 only (so `alg: none` and HMAC-with-a-public-key are.
 * What it deliberately does NOT carry over is the host's persisted-JWKS fallback: the renderer cannot obtain a bearer at all without reaching authn-v3, so an unreachable key source.
 */
