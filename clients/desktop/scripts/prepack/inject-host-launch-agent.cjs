...
/**
 * This mirrors (and must stay in lockstep with) the internal repo's `scripts/desktop-install-cloud.js` (`installHostHelperApp` / `buildInAppLaunchAgentPlist` /.
 * The internal script only wires this into the internal cloud/local-install path.
 */
...
// This name is NOT the CLI's `HOST_START_LAUNCHER_BASENAME`, and must not be unified with it.
// The bundled agent below never matches that arm (its launcher's parent directory is `MacOS`, not the label id) - it attests through its label signal.
...
// The CLI label's plist still ships, but INERT: never registered, present only so the desktop's transition cleanup (`unregister` of the old serviceName in `host-login-item.ts`) can.
// The `.agent` derivation must stay in lockstep with `host-paths.ts:smAppServiceAgentLabelId`, the CLI's `service/label.ts:smAppServiceAgentLabelId`, and the internal repo's.
...
function writeHostStartCompatibilityLauncher(helperMacOSDir) {
  ...
  // The supplied label is data, never shell code; the launcher never evaluates it.
  fs.writeFileSync(
    launcher,
    `#!/bin/sh
set -eu
...
exports.afterPack = async function afterPack(context) {
  ...
  // Identical bodies apart from the Label; the old one is never loaded by anything - the desktop only ever `unregister`s its serviceName.
