...
/** Hosts are not a series in any chart on this page, so one neutral accent across every row is the honest
 * encoding: the bar carries magnitude, and nothing about the color claims an identity. */
...
function UsageHostSplitItem(props: {
  ...
  return (
    ...
    >
      {/* The full id is worth reaching for whichever way the label reads: a truncated id needs its remaining
         characters, and a named host is the case where the id is otherwise nowhere on screen at all. */}
      <TooltipWrapper
        label={row.hostId}
        side="top"
