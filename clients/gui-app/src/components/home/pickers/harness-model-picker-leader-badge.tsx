...
interface PickerLeaderBadgeProps {
  ...
  /**
   * Both placements are absolute / out of flow, so revealing a badge never
   * reflows its surface. `corner` floats a tiny number in the icon's top-right
   * (rail, no label room); `trailing` floats just past a label's right edge
   * (reasoning pills), landing in the pill's existing trailing whitespace.
   */
  readonly placement: "corner" | "trailing";
}
 ...
 * trailing chip, fading in while the leader is held. Both placements are
    ...
    <AnimatePresence initial={false}>
      {show ? (
        <m.span
          ...
          aria-label={leaderHint(index, hintAction, hintTarget)}
          data-testid={testId}
          className={cn(
