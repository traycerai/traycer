...
/** The tab's bound host is resolved at mutation time; it never falls through to a newly selected app-default
 * host. */
  ...
  return useMutation(
    withHostMutationLifecycleBoundary("terminal.kill", {
      ...
      onSuccess: (response, variables) => {
        // An acknowledgement is the durable boundary: only now can a tombstone be cleared without reopening adoption
        // to a still-running PTY. The reprieve is a delay, not a reprieve without end.
...
function clientForLandingTerminal(
  ...
  // Always use a transient client pinned to the tab host, even when that host is selected, so a host switch
  // during backoff can never redirect a destructive RPC.
