...
export function LandingComposer(props: LandingComposerProps) {
  ...
  // Not the app-wide selection - the picker no longer moves that - so every RPC this surface makes, and every
  // create it performs, resolves through here.
  const placement = useComposerPlacement(null);
  // Read target for this surface's queries; submit target (host-frozen client) for anything that creates - see
  ...
  // Restore the caret to where it was when the draft was last persisted (decision A3).
  ...
  // Only the toolbar slot swaps, so the editor keeps its position in the tree and never remounts when the
  ...
  // This unblocks the landing image GC's deleting sweep, which is otherwise deferred so a cold-start reconcile
  // (empty session cache, possibly stale-empty projected roots) can't reap freshly-restored bytes.
  useEffect(() => {
    markLandingEditorMounted();
  }, []);

  // effective): under the surface-pin model the settings memory follows what this draft will actually create on,
  // not an app-wide "active" the picker no longer moves.
  ...
  // Never authoritative: the landing composer has no reauth gate of its own to defend a dead pin with a banner.
  ...
  // Deleted rather than re-pointed: they also cost two live mutation subscriptions for an answer they could not
  // give.
  ...
  // Purely informational: it never blocks epic creation.
  ...
  // Keeps the banner's `providers.list` read converging with a turn's passive rate-limit capture from any
  // running epic on this host.
  ...
  // Editor-gone / store-failure paths drop the node (if still present) and reclaim the bytes.
      ...
      runPendingImageJob(async (signal) => {
        ...
        try {
          ...
          if (signal.aborted || handle === null || !handle.isReady()) {
            // (If a remount kept the b64 node, its own mount-time re-entry re-ingests and re-roots this hash before the
            // debounced sweep runs.)
            scheduleLandingImageReconcile();
            return;
          }
          ...
          if (!handle.rewriteImageAttachmentHashById(id, hash)) {
            // The pending node was removed (the user deleted it before the write settled), so the just-stored bytes are
            // unrooted - reclaim them.
            scheduleLandingImageReconcile();
          }
        } catch {
          if (signal.aborted) {
            // Don't toast for a surface the user already left (matching the shared file-paste path's abort handling) -
            // just reclaim the bytes.
            scheduleLandingImageReconcile();
            return;
          }
  ...
  // Synchronously validate a landing paste's inline-base64 images (decode, mime/5MB, budget), mint a fresh id +
  // start the background job for each accepted one, and report a verdict per image.
  const ingestPastedComposerImages = useCallback(
    (
      ...
      // Reserve atomically as a batch, but retain one handle per image so an unmounted job releases its own
      // anonymous charge before that image's remount re-entry acquires the now-hash-aware reservation.
      ...
      // A valid image blocked solely by the aggregate budget is already covered by `reserveLandingImageBudget`'s own
      // accurate budget toast, so adding this one would double-toast it with a false cause.
  ...
  // Mount-time re-entry completes the pending-node model: the b64 image node IS the work token, so whichever
  // mount owns the editor restarts its ingest.
  ...
  // Its runtime owns an independent revision counter, so treating both phases as one identity could let equal
  // counter values clear content written after promotion.
  ...
  // Folded into `canSubmit` rather than checked separately at submit, so the button and its hint can never
  // disagree.
  ...
  // Submit-time refusal copy (selection model §54).
  const {
    notice: hostNotice,
    raise: raiseHostNotice,
    dismiss: dismissHostNotice,
  } = useComposerHostNotice(resolvedHostId);
  // G4: this composer follows the effective host only when nothing else answered its placement - no override, no
  // pin IN force.
  useEffect(() => {
    return subscribeFollowingSurfaceReset(({ nextEffectiveHostId }) => {
      ...
      // The modal's equivalent asks across every host because it clears across every host; pairing the two is what
      // keeps "we warn exactly when we destroyed something" true on both surfaces.
  ...
  /** The one mint path for an unbound landing composer: the first substantive edit calls it, and so does the
   * picker's sign-in button, whose terminal panel does not exist until a draft anchors it. */
  const ensureBoundDraftId = useCallback((): string => {
    // `handleDocumentChange` reaches this only while unbound (a bound composer returns on its own runtime first),
    ...
    // Reuse the parent-pre-minted mount key (props.pendingCreateId) as the new draft's id so activeDraftId's null
    // -> id flip lines up with the key the parent already mounted under (no editor remount).
    const createdDraftId = useLandingDraftStore.getState().createDraftWithId(
      props.pendingCreateId ?? uuidv4(),
      useComposerRunSettingsStore
        .getState()
        // The composer's own resolved placement host - not the app binding: the seed should describe the machine this
        // draft will create on.
    ...
    // Carry any staged worktree intent over synchronously, before React re-renders the picker on the new key.
    useWorktreeIntentStagingStore
      .getState()
  ...
  // It binds the page rather than naming it: an unbound start page renders no pane anchor, so its panel has
  // nowhere to mount and the sign-in terminal would exist unseen.
  ...
  const handleSubmit = useCallback(() => {
    ...
    const refusal = actions.submit({
      // Without this fallback `ensureSubmissionDraft` would mint a second draft and strand the one already holding
      // the user's content.
    ...
    // Nothing was created when a refusal comes back - the draft, its content and its staged workspace are exactly
    // as the user left them, and the reason is stated inline above the composer rather than as a toast.
    raiseHostNotice(
  ...
  return (
    <ComposerBody
        ...
        <>
          ...
          rateLimitPrompt.kind === "visible" ? (
            <ProfileRateLimitSwitchBanner
              ...
              // The composer's resolved host (pin, else effective), matching the `hostId` this surface hands `ComposerBody`
              // below - so the usage sidecar / R-key refresh reads the machine the turn will actually run on.
      ...
      // `null` reaches here only in the ∅ case - nothing usable to create on - which submit re-validation refuses
...
/** Returned as a pair for the same reason `resolveSendBlock` does it in `chat-composer.tsx`: a reason that
 * kills the button without supplying its hint leaves a grey button that reads as broken. */
