...
import {
  ...
  type ImportLoginsFrame,
} from "@/components/settings/import-logins-frame";
...
export function ImportLoginsFlow(props: {
  ...
  return <FrameContext value={props.frame}>{renderStep()}</FrameContext>;
}
...
function PickStep(props: {
  ...
  return (
    <>
      <Frame.Header>
        <Frame.Title>Import logins from another browser</Frame.Title>
        <Frame.Description>
...
/**
 * `onClose` is `null` on a surface with nowhere to go: the tour's stage, whose
 * own Continue is the way on. Done then shows its counts and, for a blocked
 * import, only Try again.
 */
function DoneStep(props: {
  ...
  return (
    <>
      <Frame.Header>
        <Frame.Title>Logins imported</Frame.Title>
        <Frame.Description>{describeImported(result)}</Frame.Description>
      </Frame.Header>
...
function BlockedExplainer(props: {
  ...
  switch (props.reason) {
    ...
    case "profile-too-large":
      return (
        <Notice tone="warning">
          This profile's cookie database is far larger than a browser normally
          keeps, so Traycer won't read it. Clear old cookies in that browser and
          try again, or export the sites you need with a cookie extension and
          import the file.
        </Notice>
      );
    case "incomplete":
...
function Notice(props: {
  ...
  // reader mid-flow - and re-announce the Google warning on every toggle.
  return (
    <div
      role="note"
      className={cn(
