...
// Only these capture ⌃ vs ⌘ distinctly; every other action keeps the lenient capture (Control → `mod`) so
// rebinding a normal action by pressing Control on macOS still produces a binding that fires on ⌘ as before.
...
/** Click the chip to enter capture mode; the next full chord keydown becomes the new binding. Escape cancels
 * capture, Backspace clears the binding. */
export function ChordCaptureInput(props: ChordCaptureInputProps) {
  ...
  // Reserved by persisted intent, not live OS status (amended decision 6).
  ...
  return (
    <ChordCaptureCore
      ...
      checkConflict={(candidate): ChordCaptureCheck | null => {
        // For a Control-aware action on macOS, reject a Command (`mod`) capture: ⌘ chords can't drive hold/toggle
        // there (no key-release event).
        if (
          controlAware &&
          isMac() &&
