...
/** We deliberately do not copy that rule unguarded - see the URL case below, which is the one that made it
 * worth diverging. */
describe("extractConfirmationCode", () => {
  ...
  it("DEGRADES rather than reporting a URL tail as a code", () => {
    // The whole reason for diverging from upstream: splitting on `:` here yields "//example.test/device and enter
    // abcd-1234", which their rule would present in a monospace field as though it were the code.
    expect(
      extractConfirmationCode(
...
describe("source badges", () => {
  it("speaks the upstream app's vocabulary", () => {
    // "Saved in OpenCode" is true and useful in a sentence, but not in the one slot that has to stay scannable
    // across ~180 rows.
  ...
  it("splits one source into two badges on the host's flag", () => {
    // `config` covers a provider the user declared as a custom endpoint and one a config file merely supplies a
    // key for.
