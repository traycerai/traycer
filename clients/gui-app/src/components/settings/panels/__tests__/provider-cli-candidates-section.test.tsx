...
/** `null` is a real value here, not a widening for convenience: the hook answers null while support is still
 * unknown, and the panel must stay silent for it rather than claim the host is too old. */
type SectionMocks = {
  ...
  // Concretely typed, not bare `Mock`: a bare one is `Mock<Procedure>`, whose return is `any`, and the two call
  // sites below then trip `no-unsafe-return`.
...
// Three-valued on purpose: null means "handshake has not landed", which this surface must not render as "your
// host is too old".
vi.mock(
...
// Capability gate for Manage versions - evaluated against the scoped hostId prop, never an ambient active-host
// hook.
...
/** Table coverage: managed install progress, retired standalone notices, PATH-row advisory tooltip, the
 * substitution note derived from `nextRunBinary`, and version-menu capability gating. */
...
// Second helper rather than a default/optional param - the monorepo forbids `fn(x?: T)` and `fn(x = …)`.
...
describe("ProviderCliCandidatesSection: empty-candidate notice (F2 route-back)", () => {
  it("shows amp's install notice with the ampcode.com/manual link when candidates are empty", () => {
    // Amp joined PROVIDERS_WITHOUT_BUNDLED_BINARY: with nothing on PATH the host sends an empty candidate list.
    // The notice is the only on-screen route back from the silent MCP binary-absent dead end.
  ...
  it("waits for the PATH probe instead of declaring the binary missing", () => {
    // `availabilityPending` means the host's shell/PATH probe has not settled, and the protocol is explicit that
    // `candidates` must not be trusted until it does.
    const state: ProviderCliState = {
      ...providerState({
        providerId: "amp",
        selected: { kind: "path" },
...
describe("ProviderCliCandidatesSection: former standalone notice lines are gone", () => {
  it("never renders the retired PATH-unblock sentence", () => {
    // Replaced by the host-driven substitution note.
    const state = providerState({
      selected: { kind: "bundled" },
      candidates: [
...
describe("ProviderCliCandidatesSection: managed install status line", () => {
  it("shows indeterminate Installing v<version>… when percent is null", () => {
    // Null must never stall or error - it is an honest indeterminate install.
    const state = providerState({
      selected: { kind: "bundled" },
      candidates: [bundledCandidate({ available: false })],
      managedInstallState: {
  ...
  it("states a retryable error as readable text, not as a tooltip's accessible name", () => {
    // It is the one sentence that decides whether a retry is worth attempting - four of the eight reasons are
  ...
  it("keeps the row's version and leads the failure with what actually runs", () => {
    // The regression, and the shape that replaced its first fix.
    const state = providerState({
      selected: { kind: "bundled" },
    ...
    // Leads with what runs, then names the version that actually failed - which is the whole difference between
    // "retry" and "this pin was never published".
  ...
  it.each([["unrepairable" as const], ["trust-unavailable" as const]])(
    ...
    (reason) => {
      // `unrepairable` is terminal host-side and a `trust-unavailable` host has no install machinery at all, so a
      // button here would be offered-then-failed on the one screen a stuck user was told to open.
      const state = providerState({
        selected: { kind: "bundled" },
        candidates: [bundledCandidate({ available: false })],
        managedInstallState: {
      ...
      renderSection(state);
      // Both reasons render copy naming the build, so one matcher covers the pair without asserting reason-specific
      // wording twice.
...
/** A pack that failed behind a working PATH binary. */
describe("ProviderCliCandidatesSection: a pack that failed behind a working binary", () => {
  ...
  it("does not paint the non-blocking row's version as an error colour", () => {
    // Anchored on the version cell, which is the only element that carries the dimming - asserting it on the
    // status line would pass trivially in both directions, since the line never carries that class.
    renderSection(failedPackWithPathFallback());
  ...
  it("reports the managed row's install progress AND what runs meanwhile", () => {
    // What runs in the meantime is folded onto the same line rather than dropped: with the Active chip deleted,
    // the row that would have worn it is the PATH row, which says nothing now.
    const state = providerState({
      selected: { kind: "bundled" },
      candidates: [
        bundledCandidate({ available: false }),
...
/** The closure-carve advisory - the only advisory kind a Phase-1 host populates. After the table rework the
 * host detail rides a PATH-row info tooltip rather than a bare line above the table. */
describe("ProviderCliCandidatesSection: row-incompatibility advisory", () => {
  ...
  it("surfaces the host advisory through a PATH-row tooltip, not a standalone line", () => {
    ...
    // Nested under FilePathTooltip: focusing the info icon can open both tooltips, so assert the advisory is among
    // open tooltips rather than the unique role=tooltip.
...
/** That put a success-shaped badge on a row that had just failed its install, and answered "which row wins?" as
...
describe("ProviderCliCandidatesSection: substitution note from nextRunBinary", () => {
  it("notes the PATH substitution on the SELECTED managed row, not on the PATH row", () => {
    ...
    // The chip's tooltip is gone with it - the same fact is now readable text rather than something a pointer has
    // to uncover.
    expect(
      anyTooltipHasText(
    ...
    // The note rides the row the radio picks - the Managed row - and it is that row, not the PATH row, that
  ...
  it("tells the selected PATH row that the bundled build will start instead", () => {
    ...
    // On the PATH row - the one the radio picks - not on the bundled row that actually wins, which is where the
    // chip used to land.
  ...
  it("still notes the substitution when selection is Managed and nextRun is the inline bundled fallback", () => {
    // Non-null managedInstallState means the row is Managed. The selected managed install and the next-run inline
    // bundled build share that row but are different binaries - the note must stay.
    const state = providerState({
      selected: { kind: "bundled" },
      candidates: [
        bundledCandidate({ available: true, version: "1.2.3" }),
  ...
  it.each([
    ...
    (_label, managedInstallState) => {
      // Only a non-null managedInstallState makes the bundled next-run a distinct fallback.
      const state = providerState({
        selected: { kind: "bundled" },
        candidates: [
          bundledCandidate({ available: true, version: "1.2.3" }),
...
/** Each reported defect is closed here by a property rather than by copy: one subject per column, one status
 * per row. */
describe("ProviderCliCandidatesSection: one subject per column, one status per row", () => {
  ...
  it("shows only the highest-priority status when several are true at once", () => {
    // The row shows one line: a second would reintroduce the crowding the whole revision removes, so the session
    // count is deliberately starved.
    renderSection(
      providerState({
        selected: { kind: "bundled" },
        candidates: [bundledCandidate({ available: true, version: "0.146.0" })],
...
/** Manage versions is capability-gated against the scoped hostId prop. Pack fields alone are not enough - an
 * older host must never be offered the panel entry point (offered-then-failed). */
describe("ProviderCliCandidatesSection: version menu capability gate", () => {
  ...
  // Hiding it is what the user reported: a settings tab with no control and no reason reads as "this feature
  // does not exist on my machine". An old host now says so instead of vanishing.
  ...
  it("forwards scoped hostId, packId, pack-scoped title, and managedVersions on open", () => {
    ...
    const state = providerState({
      ...
      managedInstallState: { status: "installed", version: "1.0.0" },
      // Shared-store pack id, not the provider display name - the panel title must stay pack-scoped (e.g. opencode
      // CLI from an openrouter row).
      packId: "opencode",
      managedVersions,
    });
    renderSection(state);
...
describe("ProviderCliCandidatesSection: keyboard reachability", () => {
  it("exposes the PATH advisory and the version menu as focusable buttons", () => {
    // It is gone, but the version menu took its place as a control with NO visible label - a bare chevron in the
    // action column - so its keyboard reachability and its accessible name are now the thing that has to hold.
    mocks.hostSupportsMethod = true;
    const DETAIL =
      "Traycer is built to run opencode 1.2.3 specifically, so this copy on your PATH is not used. Select this row to use it anyway.";
    const state = providerState({
