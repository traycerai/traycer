...
/** Everything here answers a question the JSX would otherwise answer inline - which entry points to render,
 * where a write lands, what the file will look like. */

export const SKILL_NAME_PATTERN = /^[a-z0-9]+(-[a-z0-9]+)*$/;

/** Not enforced - the field is what the agent reads to decide whether to load the skill at all, and a hard stop
 * mid-sentence is worse than a long line. */
...
/** `includes(effectiveScope)` rather than `length > 0`: a verb advertised only for project must not light a
 * button while the user is viewing Global (and vice versa). */
...
/** Keyed on advertised create/import scopes, not on existing rows: a project listing with zero provider-sourced
 * skills must still be able to create the first one. */
...
export interface SkillDestination {
  /** The directory a write lands in, as shown to the user. */
  readonly display: string;
  /** False when the directory could not be determined and `display` is prose rather than a path. A guess rendered
   * as a path would be worse than an honest sentence: this line's whole job is to be trustworthy. */
  readonly exact: boolean;
}

/** A provider's own root is provider-specific and lives in host code. */
...
/** Only `source: "provider"` rows qualify - a `shared` row sits in the cross-provider root and a
 * `plugin`/`managed` row is somewhere we never write. */
...
/** Not a placeholder - real text in the field. An empty box is the single biggest reason a first skill never
 * gets written: the headings are the part that is hard to invent and cheap to delete. */
...
/** Mirrors the host's `formatSkillMd`, including its JSON-quoted description and its empty-body fallback - a
 * preview that pretty-printed the frontmatter differently would be a preview of a file that does not exist. */
...
/** Why the submit button is disabled, or null when it is ready. Returned as a message rather than a boolean so
...
/** Host maps `NativeWriteError("external_drift")` onto the wire for both inspect SHA drift (expired-token
 * re-clone moved) and update-from-source when canon hash ≠ recorded `installedHash`. */
...
/** Absent `actionScopes.edit` / `update` is the old-host skew gate: hide the feature rather than offering a
 * verb the host will refuse. */
...
/** The destination strings this joins are either the standard's own `~/.agents/skills` or a path the host
 * already reported, so a separator is never invented - only appended. */
function joinPath(left: string, right: string): string {
  if (left.endsWith("/")) return `${left}${right}`;
  return `${left}/${right}`;
}

/** The parent of a skill directory, honouring whichever separator the host's path already uses. Returns null
 * when there is no parent segment to take. */
