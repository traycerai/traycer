 ...
 *    nothing syncs until the sign-in is confirmed, so the sentence names that
 *    instead. A plan without sync keeps the row here for good, which the
 *    sentence admits rather than promising a sync that never comes.
 *
...
export function historyRowProvenanceTitle(
  ...
  if (!cloudAuthorized) {
    return "Not synced yet. Sign in again to sync this task.";
  }
