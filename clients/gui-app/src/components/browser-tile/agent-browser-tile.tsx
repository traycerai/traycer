...
export function ElectronTabSurface(props: ElectronTabSurfaceProps) {
  ...
  return (
    ...
    >
      ...
      <BrowserTileToolbar
        controller={chromeController}
        captureTarget={browserTileCaptureTarget({
          // The placement carries both halves of the address, so a Start Page
          // surface (no epic, no canvas tab) yields `null` from one read.
          epicId,
          hostId: props.binding.hostId,
          tabId: props.binding.tabId,
          viewTabId:
            props.placement.kind === "canvas"
              ? props.placement.viewTabId
              : null,
        })}
