...
/**
 * The target, or `null` when either half of its address is missing.
 *
 * Both halves are load-bearing and each goes missing on its own. A tile
 * outside an epic - the Start Page's device inventory - has no authorization
 * subject; a tile outside a canvas tab has nowhere to open the saved file's
 * tile or its "attach to chat" conversation. Every tile shape reaches this one
 * gate rather than spelling the pair out, because the two tiles read the pair
 * from different places: the Electron surface from its placement, the streamed
 * viewer from its scope plus the view-tab context.
 */
export function browserTileCaptureTarget(args: {
  readonly epicId: string | null;
  readonly hostId: string;
  readonly tabId: string;
  readonly viewTabId: string | null;
}): BrowserTileCaptureTarget | null {
  if (args.epicId === null || args.viewTabId === null) return null;
  return {
    epicId: args.epicId,
    hostId: args.hostId,
    tabId: args.tabId,
