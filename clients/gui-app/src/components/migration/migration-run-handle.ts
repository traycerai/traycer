...
// Module-scoped handle so non-React callers (e.g. settings-panel button) can trigger a migration run without
// prop-drilling.
...
export function startMigrationRun(binding: StreamRuntimeBinding | null): void {
  ...
  if (binding === null || binding.hostId === null) {
    // No stream, or one that cannot name its machine: there is nowhere to run the migration and nothing to file
    // its progress under. The button that asked stays enabled, so the click is lost - do not lose it silently.
    appLogger.error(
