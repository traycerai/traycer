  ...
  "group/tabs-list inline-flex w-fit items-center justify-center rounded-lg p-0.75 text-muted-foreground group-data-[orientation=vertical]/tabs:h-fit group-data-[orientation=vertical]/tabs:flex-col data-[variant=line]:rounded-none",
  {
    variants: {
      variant: {
        ...
        scope:
          "relative grid max-w-full shrink-0 grid-cols-[1fr_2fr_3fr] rounded-lg bg-foreground/4 p-0.5",
    ...
    >
      {variant === "scope" ? (
        <span
          ...
          className="pointer-events-none absolute inset-y-0.5 left-0.5 rounded-md bg-foreground/12 transition-[transform,width] duration-160 ease-[cubic-bezier(0.23,1,0.32,1)] motion-reduce:transition-none"
          // All has no badge; give Tasks and Messages room for their counts.
          // The 1:2:3 columns keep the indicator geometry entirely in CSS.
          style={{
            width: `calc((100% - 4px) * ${((indicatorIndex ?? 0) + 1) / 6})`,
            transform: `translateX(${(indicatorIndex ?? 0) * 50}%)`,
          }}
        />
      ) : null}
      {children}
