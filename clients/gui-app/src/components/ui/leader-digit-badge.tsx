...
interface LeaderDigitBadgeProps {
  /** Callers pick the convention (plain `leaderDigitFor` or the wraparound `singleDigitLeaderDigitFor`) that
   * matches how their scope actually dispatches the chord, so this badge never has to know which one applies. */
  readonly digit: string;
  /** Epic tabs and the settings sidebar only ever bind a bare `mod`/`alt` leader. */
...
/** Shared by the epic tab strip and the settings section sidebar. */
