...
function DialogContent({
  ...
  // A modal dialog kept mounted in the background would keep aria-hiding + scroll-locking the focused split
  // partner, so an unfocused pane un-presents by unmounting only its document portal.
  const { paneFocused, handleCloseAutoFocus } =
    usePaneAwareContentGuard(onCloseAutoFocus);
  // A concealed region's dialog un-presents the same way an unfocused pane's does.
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          // `top-safe-center-y` / `left-safe-center-x`, not `top-1/2` / `left-1/2`: a fixed element centres on the
          // viewport, which on a phone includes the strips the app never paints into.
