/** Hidden via `display: none` when the row is not pinned, not hovered, and not cmdk-selected so it reserves
 * zero layout width - `opacity-0` would still leave a ~24px gap between the title and the trailing shortcut. */
...
export function PinToggle(props: PinToggleProps) {
  ...
  return (
    <button
      ...
      onPointerDown={(event) => {
        // cmdk intercepts clicks on rows to fire `onSelect`; run the toggle on pointerdown before that handler + stop
        // propagation so pin actions don't also dispatch the command.
