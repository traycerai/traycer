...
/** The edge rail beside the content is a hover affordance behind `@media(pointer:fine)`, so on a phone it does
 * not exist at all. */
...
function ContentMinimapDrawer(props: {
  ...
  const { adapter } = props;
  // Drawer content portals to <body>; re-assert the resolved theme so `--popover` resolves inside the portal
  // instead of the light `:root` (same treatment as `TabSwitcherSheet` / `ComposerOptionsSheet`).
  const { resolvedTheme, themePreset } = useResolvedTheme();
  // Subscribed while closed too: the button hides itself on empty content, and an unsubscribed bar would never
  // learn that the first turn landed.
  ...
  return (
    <Drawer direction="bottom" open={open} onOpenChange={handleOpenChange}>
      ...
      >
        ...
        {/* Mounted with the drawer, not with the bar: the card scrolls its current row into view on every change of it,
           and a closed drawer still lives in a portal. */}
        {open ? (
