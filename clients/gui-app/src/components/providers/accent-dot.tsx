...
interface AccentDotProps {
  ...
  /** `corner` floats bottom-right over a relatively-positioned provider icon (the picker rail); `inline` sits in
   * normal flow next to a label (the profile dropdown's trigger and rows). */
  readonly variant: "corner" | "inline";
  readonly size: "compact" | "default";
  readonly className: string | undefined;
}

/** Both variants resolve the same accent color (`resolveProfileAccentColor`) - the dot signals presence of 2+
 * profiles, never the sole identity signal; callers must always pair it with a visible or accessible name. */
