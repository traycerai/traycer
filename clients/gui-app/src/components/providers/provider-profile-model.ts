...
/** Canonical provider-profile identity model (multi-profile UX overhaul): commit-id semantics, ambient display,
 * ordering, status text, and chip/dot projection inputs, consumed by the picker, settings, rate-limit. */

/** The wire array's ambient row keys itself by the literal "ambient" sentinel; every run/session-level
 * profileId (composer selection, rate-limit switching,...) uses `null` for the same concept. */
...
// Dropdown row status suffix: null for a healthy profile (nothing to announce beyond its name), else the same
// wording the Settings profile row uses (`profileAuthStatusText`) so every surface stays consistent.
...
/** `null` when the provider reports no profiles (callers don't render profile-scoped UI then anyway). */
...
/** Chip/dot projection input shared by the rail's corner badge, the trigger's corner badge, and any other
 * surface that renders an `AccentDot` for a resolved profile. */
