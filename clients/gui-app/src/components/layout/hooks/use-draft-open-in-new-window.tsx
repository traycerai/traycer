 ...
 * stage the image bytes for cross-partition handoff, relocate the per-window
 * record via the move IPC, then remove the local copy.
 *
  ...
  const requestOpenInNewWindow = useCallback(
    (request: DraftNewWindowRequest) => {
      ...
      void (async () => {
        ...
        // Step 2: a grouped draft cannot be handed over still paired -
        // separate first, and let step 4's revalidation catch a refused
        // separation, exactly as the epic move does.
        tabCommandCoordinator.separateBeforeMove(ref);
        // Step 3: the durability barriers. Tab-strip separation first, then
        ...
        // Step 4: revalidate after the awaits - still open, unlocked,
        // ungrouped.
        ...
        // Step 6: remove the local copy. The coordinator routes the removal
