...
describe("<TabStrip />", () => {
  ...
  it("keeps the new-tab button outside the scrollable shrinking tab row", async () => {
    openEpicFixture(EPIC_A);
    openEpicFixture(EPIC_B);
    const router = buildRouter("/epics/e-a/e-a");
    render(<RouterProvider router={router} />);
    ...
    expect(tabRow.className).toContain("w-max");
    expect(tabRow.className).toContain("max-w-full");
