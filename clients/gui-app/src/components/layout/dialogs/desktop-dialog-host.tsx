...
export function DesktopDialogHost(): ReactNode {
  ...
  // Confirm re-runs the app-wide unsyncable check before installing (see `confirmAppUpdateInstall`); while it is
  // in flight the dialog is pending so a second click cannot race two checks - and two installs.
  const [updateInstallConfirmPending, setUpdateInstallConfirmPending] =
    useState(false);

  // If guidance disappears while the dialog is open (defensive - there's no normal flow that clears it
  // mid-display), don't leave a stale dialog with no content behind it.
  useEffect(() => {
    if (
  ...
  return (
    <>
      ...
      {/* Confirm does not install off the captured rows: it hands them to `confirmAppUpdateInstall`, which re-checks
         every window and installs only if nothing new would be lost. */}
      ...
      appUpdates.bridge !== null ? (
        <ConfirmDestructiveDialog
          ...
          onConfirm={() => {
            ...
            // The door closes the dialog itself on install, or replaces the rows with the fresh full set and leaves it
            // open when the protected set changed under the user.
...
/** Deliberately says "cannot be saved" rather than "unsynced": the whole point of this prompt is that waiting
 * will not help. */
