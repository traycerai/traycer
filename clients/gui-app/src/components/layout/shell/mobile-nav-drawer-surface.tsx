...
/** It has to be: the drag engine's imperative start forwards to components that have already subscribed, so
 * against an unmounted panel it is a silent no-op. */
export function MobileNavDrawerSurface(
  props: MobileNavDrawerSurfaceProps,
): ReactNode {
  const { open, onOpenChange, children } = props;
  const dragControls = useDragControls();
  // The wrapper carries the -100% that parks the panel off screen, so this value never has to be seeded from a
  ...
  // A pointer owns the panel from the instant a gesture claims it, which is earlier than motion reports a drag:
  // the engine only calls back once the pointer has travelled past its own threshold.
  ...
  // Collapsing the two would announce a dialog to assistive technology while it is off screen, and hand the app
  // back while a drawer is still on top of it.
  ...
  /** Leaving those to skip the call would strand `inFlight` true, and the scrim is only inert to pointers while
   * it is both closed and at rest. */
    ...
    (toOpen: boolean): void => {
      ...
      settleRef.current = animate(x, target, {
        ...(reducedMotionRef.current === true
          ? NAV_DRAWER_SETTLE_REDUCED
          : NAV_DRAWER_SETTLE),
        // Only a settle that arrives flips the semantics.
        onComplete: () => {
          settleRef.current = null;
          settleTargetRef.current = null;
          setInFlight(false);
          commitSettled(toOpen);
          // A gesture can settle somewhere the request never asked for, so the store is reconciled to what actually
  ...
  // Measured, never assumed: the panel is fluid-width, and the same build runs on a phone and on a tablet.
  useLayoutEffect(() => {
  ...
  // The request leads and the panel follows, but the semantics still wait for the panel - so this only ever
  // starts a settle, and that settle's completion is what makes the request true.
  useEffect(() => {
    if (gestureRef.current) return;
    settle(open);
  }, [open, settle]);
  ...
  /** A cancelled pointer is never a tap either, whatever it travelled: the system took the gesture away, which is
   * not a decision the user made. */
  const armPointerRelease = useCallback(
    (
      pointer: {
        /** Every other one is somebody else's finger: a second touch that lands and lifts while the arming one is still
         * on the glass says nothing about this gesture. */
        readonly id: number;
        readonly x: number;
        readonly y: number;
      },
      ...
      // One signal retires all three listeners, which keeps the teardown from having to name the handler that calls
      // it - there is no ordering to get wrong and no listener that can be left behind.
    ...
    (event: MouseEvent | TouchEvent | PointerEvent, info: PanInfo): void => {
      ...
      settle(
        resolvesToOpen({
          ...
          // Once the engine has adopted a gesture it reports cancellation through this same callback, as an ordinary
          // release carrying a cancel event - so the distinction survives only if it is read off the event here.
  ...
  useNavDrawerClosePull({
    onActivate: (event, travelPx) => {
      beginPointerGesture();
      // Meet the finger where the drag declared itself, so what follows is 1:1 against the original touch-down
      // rather than against the point 15px later where the classifier made up its mind.
  ...
  // A degenerate range before the first measurement would make the transform undefined rather than merely wrong.
  const scrimOpacity = useTransform(x, [0, Math.max(width, 1)], [0, 1]);

  // Interactive whenever the panel is settled open; also whenever it is moving, so a spring in flight can be
  // caught and dragged the other way.
  ...
  return createPortal(
    ...
    >
      {/* A tap and a drag enter through the same pointerdown, and only the release can tell them apart. */}
      <m.div
        aria-hidden
        className={cn(
          "absolute inset-0 bg-black/40 supports-backdrop-filter:backdrop-blur-xs",
          // Interactive for as long as the scrim is showing anything, which includes a settle still in flight -
          // otherwise a drawer caught mid-open could not be pushed back from the side it is uncovering.
        ...
        onPointerDown={(event) => {
          ...
          armPointerRelease(
            ...
            {
              onTap: dismiss,
              // A flick that the engine never adopted still travelled, and a rightward one asked for the drawer to stay -
              // dismissing it would be the opposite of what the hand did.
              onSettle: () => {
                settle(settledOpenRef.current);
              },
            },
      ...
      />
      {/* Keeping the closed rest position in CSS rather than in the motion value means the very first paint is
         already correct, with no frame at x=0 while a layout effect measures the panel. */}
      <div className="absolute top-safe-top bottom-0 left-safe-left w-3/4 -translate-x-full sm:max-w-sm">
        <m.div
          ...
          // Its built-in axis lock is not the answer either.
          dragListener={false}
          dragConstraints={{ left: 0, right: width }}
          // Rubber-band only at the closed stop, where the give happens off screen. Elasticity at the open stop would
          // pull the panel past its own edge and open a gap at the screen edge behind it.
          ...
          style={{ x }}
          // `touch-pan-y` and `select-none` are the engine's own doing when it owns the listener; with that off they
          // have to be stated.
