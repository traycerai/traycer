// The suites below build a plain history to pin the unbranded fallback, which is still what a browser tab gets
// and what `goBack` must keep answering on.
...
// `navigate` is published from inside the router so activations run against the same instance the history
...
function buildRouter() {
  ...
  // A splat child rather than the app's real route tree: this suite is about where navigation lands, and every
  // landing here is a top-level surface whose component would only add loaders to wait on.
...
/** Two cross-item activations, which is what makes them pushes rather than focus-replaces - the same shape as a
 * phone leaving one top-level surface for another. */
...
/** The app's real route tree, for the suite that only needs router context - the swipe hook reads
 * `useRouter.history` and never renders a route. */
...
describe("mobile back navigation over a plain history", () => {
  ...
  // The suites here hop between two tabs, so they never land on the landing path - which is resolved by a
  // different branch entirely, and is the one a user reaches by opening one thing from Home and swiping back.
  it("returns to the landing surface on a back step out of the first tab", async () => {
    ...
    // The route is back at the landing; the surface has to follow it.
    await waitFor(() => {
      expect(router.state.location.pathname).toBe("/");
    });
    await waitFor(() => {
  ...
  // The landing draft is named rather than minted, so the second press re-activates the one that exists.
  ...
  // The gesture fires wherever the user is, including on a session that has never navigated.
...
/** The seam between the recognizer and the navigation: that an edge swipe reaches the shared action rather than
 * a second implementation of "go back". */
describe("useMobileHistorySwipes", () => {
  ...
  /** Driven through a real sheet rather than by writing the barrier style directly, because the claim is that the
   * signal the hook reads is the one the app's modal surfaces actually raise. */
...
// Each one constructs its history, so all of them would pass unchanged on a shell whose real history nothing
// ever writes to - which is exactly the shape the feature shipped in.
describe("the history the mobile app actually boots with", () => {
  ...
  // The stack is the phone's, the chrome is not: the same brand that lights up the desktop's arrows, mouse
  // buttons and palette rows must stay dark on a header with no room for them.
  ...
  it("grows an entry when the user changes surface", async () => {
    ...
    // A cold launch starts at the landing with nothing behind it - the state every one of these surfaces is
    // reached from, and the state that makes a seeded fixture unable to see this bug.
    ...
    // The next cold launch, through the same boot path that produced the stack above: a second router, same
    // arguments, and none of it carried over.
