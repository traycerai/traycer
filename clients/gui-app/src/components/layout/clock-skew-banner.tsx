...
/** That claim is true exactly when the local `exp <= Date.now` read can condemn a valid bearer, which is a
 * clock running ahead. */
export function ClockSkewBanner(): ReactNode {
  const state = useServerClockSkew();
  // Mounted app-wide and never conditionally, so the check runs whatever the current verdict is - which it must,
  // since its whole job is to notice the clock being set.
