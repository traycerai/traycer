...
/** So the stub is an identity, not a shape - naming it says that, where casting an empty object to `HostClient`
 * would claim a surface nothing in this file provides. */
...
/** A plain counter rather than `vi.fn` because the return value already flows through the mutable `bars` above. */
...
// `useScopedHostBinding` is left real: it is a pure function of the scope, and stubbing it would hide the one
// thing this suite cares about (that a null binding means the glyph must not draw).
vi.mock("@/hooks/rate-limits/use-rate-limit-host-scope", () => ({
  // The client is spread over the fixture rather than passed into it: the fixture's type demands a real
  // `HostClient`, and the stub deliberately is not one (see `SCOPE_CLIENT_STUB`).
...
vi.mock("@/lib/host", () => ({
  ...
  // Returning null instead would make it answer null for every scope alike, and the re-mount case below turns on
  // that answer changing.
...
// The popover's contents are another suite's subject, but whether it is presented is this one's: the stand-in
// keeps the real `PopoverContent`, which Radix mounts only while the root is open.
...
// Exact class-token membership, not substring containment - the button's base variant classes always carry
// `disabled:opacity-50`.
...
describe("<RateLimitIconButton />", () => {
  ...
  describe("host scope", () => {
    ...
    // Blanking the bars here would make every single-host user worse off for a picker they never opened.
    ...
    // The fetch-against-the-wrong-host guarantee: an explicit pick that has not resolved to its own client must
    // not mount the live hook at all, not just hide its output.
    ...
    // Mounting the scoped `HostRuntimeContext` only when a scoped binding exists did exactly that.
    it("stays open when a pick resolves into its own scoped binding", () => {
      ...
      // Exactly the transition a click in the picker produces: an explicit pick on another host, resolved far enough
      // to have a client of its own - the first scope for which `useScopedHostBinding` returns a binding.
      hasExplicitPick = true;
