...
/** This is the single source of truth for the popover: the click handler consumes it and NEVER re-reads the live `Selection` (Chromium/Electron can collapse or retarget the selection before the click dispatches across the portal). `text` is the raw `Selection.toString()` value - this preserves the browser's own block-boundary blank lines (`\n\n` between paragraphs), which a derived `Range.toString()` would flatten away. */
...
// Tailmark marks the open streaming tail with `data-md-streaming`.
// Keep the legacy `data-md-unstable` token so older fixtures / intermediate DOM still exclude correctly during the migration window.
const EXCLUDED_SELECTOR =
  "[data-md-streaming],[data-md-unstable],[data-quote-exclude]";

/** The listeners are passive observers - the hook never calls `focus()`, `blur()`, or `preventDefault()`. */
export function useQuoteSelection(params: {
  ...
  useEffect(() => {
    // While disabled, attach nothing.
    // The returned snapshot is also gated on `enabled` below, so toggling the setting off hides any open popover immediately without a state write here (a lingering snapshot self-heals: on re-enable its anchor guard dismisses a now-stale range on mount).
    if (!enabled) return;
    ...
    const handleMouseUp = (event: MouseEvent): void => {
      ...
      // Defer the Selection read by one frame: on double-click-drag and shift-click the browser can finalize (extend) the selection AFTER mouseup, so a synchronous read here would snapshot a stale/incomplete range.
      // Passive: this still never focuses/blurs/preventDefaults.
      cancelPending();
      pendingFrame = requestAnimationFrame(() => {
        pendingFrame = null;
        const next = resolveFromLiveSelection();
        // Summon (or replace) the snapshot; reuse the previous reference when unchanged so an equivalent re-read - e.g. a preceding selectionchange for the same shift-extend gesture - doesn't churn the popover.
    ...
    const handleSelectionChange = (): void => {
      const next = resolveFromLiveSelection();
      setSnapshot((prev) => {
        // Release-only trigger: a selection change never summons the popover on its own - it only tracks or dismisses one that mouseup already opened.
        // This also keeps a purely keyboard-built selection (no prior mouseup) from showing it.
        if (prev === null) return null;
        // Re-snapshot so the anchor AND captured text stay consistent with the live highlight when the user extends/adjusts the selection (Shift+ Arrow/End/click).
        // A null result - collapsed, multi-range, moved out of the root, intersecting excluded/unstable, or whitespace - dismisses.
...
/** A selection yields a quote only when it holds exactly ONE, non-collapsed range. Multi-range selections (Firefox Ctrl/Cmd-select) are rejected: the native highlight would span more than the single `getRangeAt(0)` we capture, so the popover could promise more than it quotes. */
...
/** Preserves referential stability: when `next` is equivalent to `prev` (same root, text, fence, and range boundary points) returns `prev`, so a no-op selectionchange doesn't tear down and re-arm the popover effect/autoUpdate. */
...
// Compare boundary NODE identity + offsets rather than `range.compareBoundaryPoints`: both ranges are clones of the same live selection, so their boundary nodes are the same objects when unchanged, and this never throws a WrongDocumentError when `a`'s nodes were disconnected by a streaming re-render while `b` is a fresh live range (different range roots).
...
/** Resolves a live selection range into a validated snapshot, or `null` when the gesture is not a legal quote. Rules: - text non-empty after trim; - both endpoints resolve to the SAME `[data-quotable]` root (boundary-aware: element-node containers resolve to a child at/before the offset, then climb; a triple-click whose end lands at offset 0 outside the root is clamped back to the root's end instead of being rejected); - the range does not intersect `[data-md-streaming]` / `[data-md-unstable]` or `[data-quote-exclude]`. */
export function resolveQuoteSelection(
  ...
  // Text found under a [data-quote-exclude] ancestor in the clamped-away region, one entry per rendered descendant - real `Selection.toString()` includes it (unlike non-rendered SVG chrome), so it's stripped from the tail of `selectionText` below.
  let excludedTailParts: ReadonlyArray<string> = [];
  if (end.clampToRootEnd) {
    // The clamp exists for the triple-click tail, where the clamped-away region holds no USER-VISIBLE, non-excluded text (only whitespace, non-rendered SVG chrome like <title>/<desc>, or a [data-quote-exclude] sibling).
    // A drag that genuinely selected visible content past the root rejects instead of clamping.
...
function quotableRootForEnd(
  ...
  if (isTextNode(container)) {
    ...
    // A double-click's word selection can absorb the block's trailing whitespace and finalize at offset 0 of the NEXT text node - on a turn's LAST block that node sits outside the quotable root. offset 0 means no character of this text node is selected, so it's the same tail landing as the triple-click element@0 case below: clamp back into the root rather than rejecting, and let the caller's clamped-away guard reject a drag that genuinely selected content past the root.
  ...
  if (offset === 0) {
    // If that node is outside the start root, clamp the end back into the root rather than rejecting the gesture.
...
// An element-node boundary belongs to the child just inside it (at the offset for a start, before it for an end), not the container itself.
// Without this a clamped triple-click end - which sits on the quotable root - would never resolve into the trailing code block the selection actually covers.
...
// SVG element names whose subtree is never painted even though it appears in `Range.toString()` - a provider icon's `<title>Claude</title>`, `<desc>`, and definition containers (`<metadata>`, `<defs>`) that only exist to be referenced via `<use>`.
// `desc` isn't mistaken for one.
const NON_VISIBLE_SVG_TAGS = new Set(["title", "desc", "metadata", "defs"]);

/** Whether `range` covers any user-visible, non-whitespace text - i.e. text outside SVG `<title>`/`<desc>` and `[data-quote-exclude]` subtrees. Walks a `cloneContents()` snapshot (which already slices partial boundary text nodes to just their in-range portion) instead of `Range.toString()`, so invisible accessibility text doesn't get counted as real tail content by the clamped-away guard above. */
...
/** Collects the text found under a `[data-quote-exclude]` ancestor within `range`'s content, ONE ENTRY PER TEXT NODE (document order) rather than a single joined string - real `[data-quote-exclude]` chrome (e.g. Joining the parts directly would lose that boundary and make the tail unmatchable. */
...
/** A miss (the parts don't actually form a trailing suffix) safely no-ops rather than mangling `selectionText`. */
...
// Overlap via boundary-point comparison (robust across jsdom, whose `Range.intersectsNode` has been spec-noncompliant).
// Two ranges overlap iff A.end is after B.start AND A.start is before B.end; touching boundaries do not count as an intersection.
