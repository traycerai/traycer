...
export interface TerminalQuote {
  ...
  /** The shell's launch directory, shown as the chip's tooltip - the same secondary line the `@`-picker gives a terminal row. `null` while the host's `terminal.list` row has not arrived; the chip then carries no directory rather than an invented one. */
  readonly terminalCwd: string | null;
  /** The selected lines, exactly as xterm reported them. */
  readonly text: string;
}

/** The two blocks a terminal quote adds to a chat draft: 1. a `sourcedQuote` wrapping the selection as a code block - the excerpt, plus the terminal it came from, so the coding agent can go read the live session instead of reasoning from a frozen snippet; 2. a paragraph holding the terminal's mention chip - the same pointer the `@`-picker inserts, so the reference reads identically however it got there, and so the user has a line to start typing on. The chip follows the quote rather than leading it because the quote is what the user selected and the chip is the citation for it. */
...
/** Never replaces the draft and never sends: the user reviews and submits. A composer that is not mounted yet picks the same draft up as its initial content when it opens. */
...
/** The same two blocks, appended to the new-conversation modal's per-epic draft instead of an existing chat's - the quote path for a chat that does not exist yet. The caret is dropped rather than moved: the blocks land at the end of the draft, and the composer's `autofocus: "end"` then puts the user straight after the chip's trailing space - the same place the chat-draft path leaves them. */
...
/** Terminal output is preformatted: column alignment and indentation are the content. A code block preserves both, and `language: null` keeps the highlighter from guessing a language for shell output. */
