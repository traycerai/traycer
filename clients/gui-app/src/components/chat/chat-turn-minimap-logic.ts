...
/** On a phone viewport it stays mounted and suppresses only its own rail, because the tile bar's button reads the outline it registers and that button deliberately does not obey `hide`. */
...
/** The rail is chrome for a pointer that can hover: the tile hides it below the `md` breakpoint and its own markup is behind `@media(pointer:fine)`, so on a phone it never draws a pixel. A pointer resolves to exactly one of `none`, `coarse` and `fine`, and the question here is only whether a touch device is driving, so this asks the negative: everything that is not coarse either paints the rail or has no pointer at all. */
...
/** A visible user query is current. Otherwise, the reply occupying most of the viewport keeps its triggering query current. */
...
/** One rail entry: a human-sent turn, and the row span it covers. `rowIndex` / `endRowIndex` are LIST indexes - what `positionAtIndex` takes - and not transcript ordinals, which on the windowed line differ wherever a renderer policy withheld a row. */
...
/** Whether a row is a HUMAN-sent user turn - the only kind the rail lists. Answerable for unhydrated rows too: the skeleton carries `role`, `sentByAgent` and `preview` precisely so the minimap can list a turn whose body has not arrived (see `row-skeleton.ts`). */
...
/** What must move for the derive to re-run, beyond the window's own identity. `rows.length` is here because a PLACED row can leave the list without the window changing at all: `rendered` is post-filter, and the pinned-todo pass drops an assistant row whose only segments were lifted into the dock (`transcript-list-rows.ts` documents this as OMITTED, not placeholder'd). */
function deriveCacheKey(rows: ReadonlyArray<TranscriptListRow>): string {
  return `${rows.length}:${unplacedRowKeys(rows)}`;
}

/** The keys of the rows that own no ordinal - the live turn, pending sends, and records the index has not placed yet. Bounded, and that is the whole reason it exists: `transcriptListRows` appends every ordinal-less row AFTER all the placed ones, so this walks back from the end and stops at the first placed row rather than scanning the transcript. */
...
/** One slot rather than a map because the key it guards moves only when an unplaced row is added or removed, which is not a churn pattern worth retaining alternatives for. */
...
/** Why this is not `useMemo(..., [rows])` `rows` is rebuilt on every streaming token - `transcriptListRows` is memoized on `[transcriptWindow, messages]` and `messages` moves per token - so keying on it ran a whole-transcript scan, a per-turn object allocation and a per-turn preview compaction dozens of times a second. A block delta - the per-token event - folds into `messages` alone and never touches the window (`applyBufferedDeltas`), so the window's identity is stable across exactly the churn this is avoiding, while every input that CAN change a rail entry moves it: - a row's label, and whether it is a human turn at all, comes from the skeleton entry for a cold row and from the model for a hydrated one - and a user row's body changes only by an edit, which arrives as an index `updated` and rebuilds the window; - which ordinals a renderer policy withholds is decided by the models the spans hold, and those arrive in range responses, which rebuild the window; - rows the index has not placed are an input the window does not version, so their keys are compared directly. */
