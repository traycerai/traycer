 ...
 * focus or clicks while hidden. Neutral elevation keeps it legible without
 * turning every tiled chat into an attention beacon; the persistent chevron
 * keeps the action clear even when the visible label describes live work. The
...
export function ScrollToEndPill({
  ...
  return (
    <button
      ...
      className={cn(
        "pointer-events-auto absolute left-1/2 z-10 flex h-8 -translate-x-1/2 select-none items-center gap-2 whitespace-nowrap rounded-full border border-foreground/20 bg-[color-mix(in_oklch,var(--foreground)_9%,var(--background))] px-3.5 font-medium text-foreground/85 text-ui-xs shadow-md ring-2 ring-background/70",
        "outline-none transition-[opacity,background-color,border-color,box-shadow,color] duration-150 hover:border-foreground/30 hover:bg-[color-mix(in_oklch,var(--foreground)_12%,var(--background))] hover:text-foreground hover:shadow-lg focus-visible:border-ring/70 focus-visible:ring-2 focus-visible:ring-ring/70",
