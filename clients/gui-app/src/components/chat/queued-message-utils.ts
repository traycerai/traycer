...
export function queueItemCanPauseFromQueueHeader(
  ...
  // The header's Pause button holds the user's own backlog.
  // A managed-command digest is system-owned, so it is not a "human queued message" the button speaks for - it rides the queue's pause state, but never justifies offering the button.
