...
/** Shared `vi.mock("@/lib/browser-view/sessions/browser-sessions-coordinator")` factory: both the mention-chip and annotation-card tests resolve their session across the coordinator REGISTRY rather than a surrounding sessions context, so each seeds this lookup instead of the live subscription. */
