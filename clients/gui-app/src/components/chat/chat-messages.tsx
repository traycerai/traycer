...
interface ChatMessagesProps {
  ...
  /** The transcript index on the windowed line (`chat.subscribe@1.8`), or `null` on the legacy line where `messages` IS the whole transcript. The merge with `messages` happens here (`transcriptListRows`), so everything below this component - the timeline, the minimap, every saved or computed LIST index - lives in one index space that includes placeholder rows. */
  transcriptWindow: TranscriptWindow | null;
  /** Reports which ordinals the viewport is showing, for viewport-driven hydration. Called with `null` when no placed row is visible (the pending tail, or the legacy line where rows own no ordinals) - the store treats that as "no viewport obligation", never as a request. */
  onVisibleOrdinalRangeChange: (range: OrdinalRange | null) => void;
  /** `ChatSessionState.transcriptBaselineEpoch` - which connection's snapshot established these rows. The polite-announcement deriver needs it to tell a live arrival from (re)hydrated history without guessing from row shape. */
  baselineEpoch: number;
  /** `ChatSessionState.transcriptHydrationSequence` - bumped when a range response seated rows the reader scrolled to, so the deriver can absorb them as history rather than announce them as arrivals. */
  hydrationSequence: number;
  /** `ChatSessionState.coldRewrittenMessageIds` - rows rewritten while their span was evicted. The announcement deriver exempts these from the history rule above, once each, because a row updated while cold first appears during a hydration and is otherwise indistinguishable from old scrollback. */
...
/** `awaitScrollSettle`'s fallback timeout when `scrollend` never fires (jsdom, some browsers) - exported so tests can wait past it rather than hardcoding a copy of this number. Used only by the DOM-event-based `scrollToEnd`/`navigateToMessage` settle paths - the anchor engine's Promise-based settle uses its own, longer, `CHAT_TIMELINE_ANCHOR_SCROLL_ PROMISE_TIMEOUT_MS` below (review finding: this shorter window is unsafe as a validate-and-settle deadline for an animated anchor scroll). */
export const CHAT_ANCHOR_SETTLE_FALLBACK_MS = 750;
/** NOTE: correctness no longer depends on this exact number - `awaitChatTimelineScrollPromiseSettle`'s fallback now ALWAYS routes into the validate-failure path on expiry (never a blind settle), so a future LegendList bump past this value degrades to an extra reissue cycle, not a false-settled anchor. Kept contract-accurate anyway so an expiry remains the rare, truly-abnormal case rather than routinely consuming a retry against a scroll that was always going to finish on its own - if either cited constant changes, this comment (not just the number) needs updating. */
export const CHAT_TIMELINE_ANCHOR_SCROLL_PROMISE_TIMEOUT_MS = 2_600;
/** pixel tolerance for the settle/re-issue validation below - a navigation whose landing is off by more than this is treated as a real undershoot, not float/rounding noise. */
const CHAT_TIMELINE_NAVIGATION_LANDING_EPSILON_PX = 1;
/** bounded retry count for the settle/re-issue loop (ticket text: "max 2-3") - the upper end, since the field bug this fixes needed multiple manual pill re-clicks to converge and the goal is to absorb that automatically in one operation. */
...
/** The scroll offset that lands `index`'s row at `viewOffset` px from the viewport top - the restoration convergence loop's issue/reissue target. Inverse of `captureChatFreeScrollingOffset`. */
...
/** A snapshot of the list geometry a mount-time free-scrolling restore's target row depends on - read fresh from the list at either issue or abort time, so both call sites stay in sync by construction. `rowPosition` is the target row's own measured position, independent of header/footer padding: LegendList reports its real header size via a metrics callback shortly after every mount, settling from an initial 0 as a routine bootstrap unrelated to content - comparing that pad directly would treat that normal settling as a geometry change on every restore. */
...
/** LegendList's web implementation can return null before its scroll element attaches even though the cross-platform public type is non-null. Keep that runtime boundary explicit for mount-time restoration reads. */
...
/** The target and content/geometry fingerprint actually issued by a mount-time free-scrolling restore, captured once - never recomputed from the latest list state, or automatic content/layout movement during settle (append, in-place growth, reorder) would always agree with wherever the geometry currently sits. */
...
/** Require row identity/order, scroll height, viewport height, and the target row's own position to still match what was issued before trusting the raw scrollTop comparison. */
...
/** Arrows aimed at those must reach them, so the transcript does not claim them. */
const ARROW_KEY_OWNER_SELECTOR = [
  ...
  '[role="tree"]',
  '[role="treeitem"]',
  // A `dialog` popup (Radix `PopoverTrigger`, e.g. the composer's context-usage chip) activates on Enter/Space, so it must not hold the arrows hostage.
  '[aria-haspopup="true"]',
  '[aria-haspopup="menu"]',
...
/** The minimap rail's single hit-target implements its own roving-selection keyboard contract (arrows/Home/End/Enter/Space) while focused - unlike `ownsArrowKeys`, plain Home/End otherwise has NO owner exemption at all (they scroll the transcript unconditionally on macOS), so without this the rail's own `onKeyDown` never sees them: this capture-phase listener runs first and calls `stopPropagation`. */
function ownsBoundaryKeys(target: EventTarget | null): boolean {
  if (!(target instanceof Element)) return false;
  return target.closest(CHAT_TURN_MINIMAP_KEYBOARD_OWNER_SELECTOR) !== null;
}

/** A hosted chat's own DOM lives in `StableTileSurfaceHost`'s plane, not inside its canvas pane's `[data-group-id]` subtree - the physical ancestry lookup misses for it (and for any target inside it), so a miss falls back to the hosted resolver, which walks the SAME node up to its hosted-record ancestor's stamped pane id instead. */
...
/** Whether `target` sits in the same canvas pane as `tile` - the pane's tab strip is a SIBLING of the tile, so containment alone cannot tell "this pane's own chrome" apart from an unrelated surface. */
...
/** Waits for `scrollNode` to settle after an issued scroll: the native `scrollend` event, or a `timeoutMs` fallback (some environments never fire `scrollend`), whichever comes first - then calls `onSettle` exactly once. Returns a cancel function that tears down the pending listener/timeout WITHOUT calling `onSettle`, for when something else pre-empts the wait (a real user gesture, a newer operation superseding this one). */
...
/** Schedules `callback` two animation frames out - the reveal pass's own post-layout timing convention (waits for LegendList's measurement pass to settle before reading geometry). Returns a cleanup that cancels whichever frame is still pending. */
...
/** `getPendingScroll` is re-invoked on every wait (never captured once) - a reissue creates a NEW promise each attempt, and this must await THAT one, not a stale earlier one. `onSettle` now receives `timedOut` - `false` when the library's own promise genuinely resolved (or - defensively, it never actually rejects today - if it somehow rejected), `true` only when the local fallback timer fired first. */
...
/** After `awaitSettle` calls back, `shouldYieldToReader` (a real gesture - e.g. an OS scrollbar drag that fires no wheel/touch/pointerdown of its own - must still win over a still-in-flight correction, never yanking the reader back) takes priority over `validate`; if it yields, `onSettledInvalid` runs directly, same remedy as exhausting retries. `isAborted` (checked before every check) is the caller's own ownership check - a generation bump or a real gesture supersedes a still-in-flight operation; it must stop correcting a position nobody wants anymore - a fired `isAborted` abandons silently (someone else is already driving the UI). */
...
/** Virtualized chat transcript. The full derived row history is handed to `ChatTimeline` (LegendList), which windows the mounted DOM to the viewport. */
export function ChatMessages(props: ChatMessagesProps) {
  // Stable for the component's whole lifetime (a chat tile fully remounts on any real identity change - decision #17), so a `useState` initializer is enough; no need to react to prop changes.
  ...
  // The transcript's own chat, for segments that ask its live session whether a shell still exists.
  // Memoised so the provider does not re-render every consumer per transcript render; `null` with no bound host, where no live session can exist to ask.
  ...
  // opening a chat clears its own tombstone (a prior deletion is over; this is the SAME chatId only if the host has genuinely recreated it, which mints a fresh chatId in practice - this clear is a no-op then, but cheap and correct either way).
  // Also clears the EPIC-level tombstone: `handleEpicAccessLoss` tombstones by epic PREFIX (not an exact chat key - see chat-tab-persistence- tombstone.ts), and unlike a chat delete, access loss is not necessarily terminal (access can be regained).
  useLayoutEffect(() => {
  ...
  // registry-backed, keyed by tile instance id, so expanded A2A cards survive a remount of this SAME instance - retention-cap or top-level eviction, or a hosted-eligibility flip; no longer an inner tab switch (decision #17 was reversed by pane chat retention) - and are evicted when the tab permanently closes (canvas store's tile-removal subscriber), never on a mere remount.
  // That single point covers both an active view's close AND an inactive (never-mounted) view's close, which a component-owned commit structurally cannot (nothing here ever runs for a view that never rendered).
...
/** non-null only when the mount-time restore had to clamp away from the true saved anchor - i.e. `restoreChatTabState` silently substituted a neighbor because `messages` was still mid-hydration, not because the anchor is genuinely gone. */
...
function ChatMessagesInner(props: ChatMessagesInnerProps) {
  ...
  // The array the list actually renders: hydrated bodies and placeholders in one sequence.
  // On the legacy line (`transcriptWindow === null`) this is the identity mapping over `messages`, so nothing below behaves differently there.
  const listRows = useMemo(
    () => transcriptListRows({ window: transcriptWindow, rendered: messages }),
    [transcriptWindow, messages],
  );

  // Restore the persisted reading position once, on mount (tries the tab-key entry first, then the durable chat-key entry - RESTORE-FIRST, decision #29).
  // The identity is stable for the mount, so re-reading per render would only repeat an O(n) row scan whose result the initializers below already captured.
  ...
  // a restored row becomes LegendList's own `initialScrollIndex` measurement bootstrap - the same self-correcting path `initialScrollAtEnd` uses, carrying the saved pixel offset.
  // `null` for a fresh open or a saved bottom-following view (LegendList's own `initialScrollAtEnd` covers those).
  ...
  // Raw saved state, peeked WITHOUT `restoreChatTabState`'s messages-dependent clamp.
  // Hydration retry needs the original anchor id to replay a measured location once the transcript has hydrated enough to contain it.
  ...
  // Non-null only when the mount-time restore above had to clamp away from the true saved anchor - the hydration-retry effect below (declared after `navigateToMessage`, which it needs) resolves this against `messages` as it grows, then nulls it out permanently, so an ordinary NEW live message arriving later can never re-trigger a jump back here.
  ...
  // Per mounted transcript, exactly like the row-stability caches: row ids are chat-scoped, and a memory that outlived its tile would hold heights measured at a width this one may not share.
  // Same shape as `restoredTabState` above - a value computed once for the mount and never set again.
  ...
  // A layout effect rather than a render-time call: this writes, and the memory must not be advanced by a render React discards.
  useLayoutEffect(() => {
    rowHeightMemory.observeSkeleton(rowSkeleton);
  }, [rowHeightMemory, rowSkeleton]);
  // The effective root font size the rows lay out at - `theme-provider` writes it to `document.documentElement.style.fontSize`, and this is the setting it writes from.
  // Read here rather than via `getComputedStyle` so a change is reactive: the height memory has to be told, and nothing else would.
  ...
  // A ResizeObserver on the container rather than React state, for the reason the memory itself is not state: a resize must not re-render a mounted transcript, and the basis is only wanted as a hint for the placeholders that mount next.
  // `uiFontSize` is a DEPENDENCY, not just a read: changing it re-flows every row without necessarily changing the container's width, so the ResizeObserver may never fire and this effect re-running is the only thing that reports the new basis.
  useLayoutEffect(() => {
  ...
  // Unlike `listRowsRef`, no descendant layout callback consumes this map, so it does not need insertion-phase freshness.
  ...
  // Read only by find's coverage supplier, and through a ref for the same reason `messages` is: the window changes identity on every hydration, and a value dependency would re-register the find adapter each time.
  const transcriptWindowRef = useRef<TranscriptWindow | null>(transcriptWindow);
  // LegendList's measured header size (the main component of getTopOffsetAdjustment).
  // Capture folds this into the saved viewOffset so initialScrollIndex restore lands on the same pixel - bare positionAtIndex - scroll under-counts by exactly this pad.
  ...
  const resolveSuppressedEndLanding = useCallback((): boolean => {
    ...
    // Stop that placeholder restore before its valid/exhausted callbacks normalize durable state.
  ...
  // Native smooth scrolling outlives the JavaScript call that starts it.
  // Track the exact animated imperative operation that currently owns that motion so a physical reader gesture can freeze it without issuing disruptive same-offset writes during ordinary, already-settled follow mode.
  ...
  // --- Follow-vs-free scroll state (behavior contract: one edge, one rule) -- Bottom permission is geometry, not a mode machine: `scrollMode` and `isAtEndRef` are render/persistence mirrors of the latch's fresh-DOM strict edge (the library's same 1px epsilon).
  // Transient correction and explicit-navigation ownership live only inside that latch.
  ...
  // A generation bump marks any in-flight settle/navigation loop (explicit navigation, restoration convergence) superseded - decoupled from follow ownership, which no longer exists as a separate concept.
  const anchorUserScrollGenerationRef = useRef(0);
  // Persistence is a publication step, not an unmount side effect that may sample arbitrary bootstrap geometry.
  // Until a saved pixel restore validates, the cache entry read at mount remains the authoritative reader state.
  const restorePersistencePendingRef = useRef(
    savedRestoreRequiresPersistenceGate(rawSavedTabState),
  );
  // A restored free-reading chat can briefly report measurable strict-bottom geometry when a hidden task canvas becomes visible, before LegendList has reinflated its rows and applied the saved anchor.
  // Maintenance callbacks in that window are bootstrap layout, not evidence that the reader returned to the tail.
  ...
  // A top-level pane hide zeroes this tile's scroll container's measured geometry in the SAME commit as the `visible` prop flipping false - a hide-time read would already observe zeroed geometry, so the visibility-handoff effect publishes THIS ref's last-good value instead of reading live DOM at that moment.
  // See `hooks/scroll/use-scroll-restoration.ts`'s doc comment for the same hazard already solved this way for other tile kinds.
  const lastVisibleScrollSnapshotRef = useRef<Omit<
    SaveChatTabStateInput,
    "identity"
  > | null>(null);
  // Mirrors `timelineScrollModeRef.current` into render - the pill formula and `data-scroll-mode` derive from this single value.
  // The ref above stays the authoritative value every imperative callback reads/writes synchronously; this is purely a render-time mirror of it.
  ...
  // A turn completed while the reader was away from the tail and they have not returned since - drives the pill's "New reply" state. Reset on returning to the tail.
  const [hasUnseenTurnCompletion, setHasUnseenTurnCompletion] = useState(false);
  // Ref-only (no React state): the minimap derives in-view highlighting from list state, not a stored active id, so nothing renders off this value anymore - only the ticket-5 unmount-save effect reads it imperatively.
  // A state setter here would just re-render the whole component on every scroll tick for no observable effect.
  ...
  // `releasePersistenceGate` decouples "publish following-end" from "the saved coordinate is resolved" (fixup: hydration-transaction).
  // A temporary landing at the strict edge while `pendingHydrationRestoreAnchorIdRef` is still unresolved is not reader intent and must not release the persistence gate - every caller that already represents a real gesture or explicit navigation passes `true` (their own code already cleared the pending id before reaching here, or never gated it in the first place).
  ...
  // Must NOT clear `pendingHydrationRestoreAnchorIdRef`: a temporary hydration-clamp restore landing at the current (still partial) end reports the same `isAtEnd=true` as a genuine reader reaching the tail, but is not reader intent and must not discard an unresolved saved coordinate.
  // Same reasoning gates the persistence release below: a passive geometry report must not publish the temporary tail as durably authoritative while that coordinate is still pending.
  ...
  // Decision #6/#5/#7: reader input freezes an in-flight native smooth-scroll and re-syncs ownership against the actual physical position.
  // Transcript pointerdown is a preflight because a disclosure click can resize content without publishing a reading position; wheel/touch/keyboard and scrollbar input publish, so their subsequent scroll report may detach the latch.
  const handleTimelineReaderGesture = useCallback(
    ({
      ...
      const hadActiveAnimatedImperativeScroll =
        activeAnimatedImperativeScrollGenerationRef.current !== null;
      // This cancel supersedes the operation immediately. Its eventual settle callback is generation-guarded and cannot clear ownership belonging to a newer operation.
      activeAnimatedImperativeScrollGenerationRef.current = null;
      ...
      // A preflight is not a departure.
      // The latch remains authoritative and publishes a mode change only after the gesture/navigation produces a measurable scroll, so a disclosure click cannot detach a reader and an owned under-landing cannot be mistaken for reader motion.
  ...
  const handleTranscriptPointerDown = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>): void => {
      // Inline artifact/A2A navigation starts with pointerdown and can unmount this tile before a later passive scroll snapshot runs.
      // Capture the exact source viewport synchronously, before cancellation changes mode.
      persistCurrentScrollRef.current();
      const scrollNode = chatTimelineRef.current?.getScrollableNode();
      // A disclosure/card click is only a correction-cancelling preflight: its ensuing ResizeObserver/MVCP movement is layout-owned and must not detach follow.
      // Pointer input targeting the scroll node itself is the scrollbar interaction shape; it publishes the ensuing scroll position.
  ...
  // ChatTimeline unmounts LegendList entirely for an empty transcript (ChatEmptyState instead), so this - not just `messages` identity - is the signal that tracks whether a real scroll node can exist right now.
  ...
  // scrollToEnd reset (pill click / any future explicit "go live" action). this is an explicit user action - the `setTimelineMode` call below is decision-sanctioned and unconditional, same as before; the settle/re-issue only corrects the LANDING it produces, never re-decides whether the click counts as "following".
  // Pill action is an explicit supersession (fixup: hydration-transaction), so the pending hydration id is cleared above and the gate always releases.
  const scrollToEnd = useCallback(
    (animated: boolean): void => {
      ...
      activeNavigationSettleCleanupRef.current = settleChatTimelineNavigation({
        ...
        // The pill-click "go live" path has no reader-departure detection of its own - a real gesture already bumps the generation and is caught by `isAborted` above.
        ...
        // free-scrolling with the pill visible beats silently claiming ownership from an invalid landing.
        onSettledInvalid: () => {
          finishImperativeScrollOperation(imperativeScrollGeneration);
  ...
  const onFollowIntentChange = useCallback(
    (isFollowing: boolean): void => {
      // Cross-check the rendered mode, not just the cached ref, before taking the fast path: `setTimelineMode` always keeps both in sync, but this guards against any future path that could otherwise leave them briefly out of step (behavior contract: an equality fast path must never let `{mode: free-scrolling, isAtEnd: true}` stand).
      ...
      if (isFollowing) {
        ...
        // Clearing the pending coordinate here would resolve the transaction before validation and defeat the whole point of routing hydration catch-up through a validated retry.
        // Bare edge geometry must never supersede - only a validated landing or a recognized reader/ explicit-navigation path (their own direct clears) may.
        setTimelineMode("free-scrolling", true);
        setShowScrollToBottom(true);
      }
    },
  ...
  const handleKeyDownCapture = useCallback(
    (event: globalThis.KeyboardEvent): void => {
      ...
      // Freeze an owned native smooth-scroll at its current pixel first, then apply this key's deterministic step as the replacement movement.
      // The resulting native `scroll` event is what determines follow, via the latch's geometry observer - no separate reattach check needed here.
      cancelTimelineLiveFollowForUserNavigation({
        direction:
  ...
  // tool/subagent open state now survives a chat tile's remount (module-scope stores scoped by `instanceId` - see `tool-open-store.ts` / `subagent-open-store.ts`), so mount no longer resets it.
  // A permanently closed tab's entries are reclaimed by the canvas store's tile-removal subscriber (`stores/epics/canvas/store.ts`) instead.

  // Without the proactive flush, React would mount the replacement fiber's `restoreChatTabState` render-time state initializer BEFORE this component's own unmount cleanup ever runs - render happens before ANY commit-phase effect, including a removed fiber's layout-effect cleanup (painted-chat lifecycle audit finding 1: a disposable probe recorded `initialize:stale` then `cleanup:fresh`).
  // One function backs both call sites so the coherent-snapshot invariant below can never drift into two independently maintained copies.
  ...
  > | null => {
    ...
    const list = chatTimelineRef.current;
    // `scrolledActiveUserMessageIdRef` is an rAF-throttled mirror (`scheduleActiveViewportUpdate`) of "which row is at the reading line" - it can lag a scroll that happened in the same tick as this capture (rAF never runs for a backgrounded/closing tab, but the race exists in a visible renderer too: scroll then close/move before the pending frame).
    // Recompute the anchor row SYNCHRONOUSLY from the SAME live list snapshot the offset capture below reads, so both halves of the pair are drawn from one coherent, never-mixed-time snapshot - restoring the invariant the pre-LegendList `chat-scroll-state-cache.ts` `saveChatScrollState` documented ("captures any reading position the last animation-frame update had not yet committed").
    ...
    const resolvedAnchorMessageId =
      mode === "free-scrolling"
        ...
        : null;
    // Saving no anchor at all is the better answer - restore falls back to the offset and to its pending-hydration correction, both of which are built for "the anchor is not resolvable yet".
      ...
      list === null
        ...
        : {
            getState: () => ({
              ...
              // LegendList's tracked scroll can lag the DOM while an animated navigation is still settling.
              // Persist the pixels the reader can actually see right now; row positions still come from the library's measured state.
              scroll: list.getScrollableNode().scrollTop,
              topOffsetAdjustment: listTopOffsetAdjustmentRef.current,
            }),
          };
  ...
  // Writing the tab-key unconditionally would resurrect the entry that sweep just cleared (mirrors `use-scroll-restoration.ts`'s `commitIfTileLive` guard) - but skipping the save ENTIRELY on a non-live unmount was the bug: it is the field symptom's actual trigger, since a genuine direct close (no prior switch-away) then wrote NOTHING anywhere, and a reopen either got no saved state or - worse - a STALE durable entry from some earlier session.
  useLayoutEffect(() => {
    const persistCurrentScroll = (): void => {
      ...
      if (restorePersistencePendingRef.current) {
        // Do not overwrite them from a transient bootstrap viewport.
        if (!isLive && rawSavedTabState !== null) {
          commitChatTabStateToDurable({ identity, ...rawSavedTabState });
        }
        return;
      }
      // Fall back to that already-known-good mirror instead of a live read whenever the surface cannot be measured right now.
      const list = chatTimelineRef.current;
  ...
  // Structural moves can mount the replacement fiber before this fiber's unmount cleanup.
  // Publish the same coherent snapshot synchronously so the replacement restores current geometry, while the restore gate still rejects transient bootstrap measurements.
  useLayoutEffect(
    () =>
  ...
  // Quote-to-composer: track selections inside the transcript wrapper below and surface the floating quote button.
  // The hook attaches no listeners while the setting is off, so a disabled affordance costs nothing.
  ...
  // registry-backed, keyed by tile instance id, so expanded activity groups survive a remount of this SAME instance - retention-cap or top-level eviction, or a hosted-eligibility flip; no longer an inner tab switch (decision #17 was reversed by pane chat retention) - and are evicted when the tab permanently closes (canvas store's tile-removal subscriber), never on a mere remount.
  // A reopen is a new instance, not a revival. no longer commits to durable on its own unmount - see the matching comment on `a2aOpenStore` above.
  const [activityGroupOpenStore] = useState(() =>
    getOrCreateActivityGroupOpenStore(identity),
  );

  // dual-keys the global tool/subagent open stores for this tab - seeds this tab's expanded cards from the durable chat-key snapshot on a genuinely fresh (never-initialized) scope only.
  // Round 3: the durable commit moved to the canvas sweep's promotion choke point - see the hook's own doc comment.
  ...
  // Recompute the ticket-5 free-scrolling save anchor (nearest human user message to the reading line) from LegendList's own measured positions - no DOM rect probing - coalesced to one read per frame.
  const scheduleActiveViewportUpdate = useAnimationFrameThrottle(
    useCallback(
      (atBottom: boolean): void => {
        if (atBottom) {
          // P4: same zero-human-row fallback as `viewportActiveUserMessageId` (see its own doc comment) - an A2A-only transcript has no candidate for the human-only gate, so at the tail the natural role-agnostic anchor is simply the last row, any role.
        ...
        // list.getState() does not expose headerSize/topOffsetAdjustment - fold in the live measured value (decision #18), same as the ticket-5 save path's measurementSource below.
        const state = {
          ...rawState,
          topOffsetAdjustment: listTopOffsetAdjustmentRef.current,
        };
  ...
  // A reader who scrolls then immediately switches away hides the pane in the SAME tick the scroll event was dispatched - a throttled capture scheduled for the next frame never runs before the hide-time effect below reads the mirror, which would publish the previous (stale) position instead of the new one.
  const captureLastVisibleScrollSnapshot = useCallback((): void => {
    const list = chatTimelineRef.current;
    // A concealed container reports a zero-height box; ignore so a hidden-state read never clobbers the last known-good snapshot (same guard `use-native-div-scroll-restoration.ts` uses).
  ...
  const handleScroll = useCallback((): void => {
    // O2 (drives the minimap's in-view highlighting off THIS existing LegendList scroll callback instead of a second scroll-listener lifecycle the minimap used to attach itself (rAF-polling attach + native listener + detach).
    // Called unconditionally, ahead of the `visible` early-return below - the minimap's own previous native listener never gated on this component's `visible` prop either, and there is no reason a background/not-yet-selected tile's scroll (e.g. a still-settling restore) should leave its in-view dots stale.
    minimapInViewRefreshRef.current();
    if (!visible) return;
  ...
  // Explicit navigation (find/minimap/deep-link/restoration) is programmatic, not a gesture.
  // Bare scrollTop direction is never used as intent because MVCP and layout compensation produce the same browser signal. settle/re-issue against the CURRENT geometry - an ANIMATED long jump targets ESTIMATED heights; no mid-flight retargeting in the installed LegendList.
    ...
    (location: ChatTimelineNavigationLocation): void => {
      ...
      activeNavigationSettleCleanupRef.current = settleChatTimelineNavigation({
        ...
        onSettledInvalid: () => {
          ...
          // Accept - already correctly free-scrolling/suppressed wherever the bounded retries landed; nothing claims to be "at this exact spot" the way following-end does, so there is no mode to reconcile.
        },
        maxRetries: CHAT_TIMELINE_NAVIGATION_MAX_RETRIES,
      });
    },
  ...
  // Find does NOT go through this function at all - `use-chat-find-controller.ts`'s own `scrollToMessageForFind` independently builds its location with `animated: false` already, unchanged by this ticket.
  // The late-hydration catch-up effect below is different from either: it is a RESTORE landing where the transcript simply grew to finally contain a position already decided at mount, not a fresh navigation the reader initiated - it must reposition in a single frame like any other mount-time restore, never visibly scroll/animate to get there.
  /** Restoration is a measured convergence operation, not a one-shot index guess. */
  const restorePersistedTimelineLocation = useCallback(
    (
      ...
      activeNavigationSettleCleanupRef.current = settleChatTimelineNavigation({
        ...
        // `handleTranscriptPointerDown` bumps `anchorUserScrollGenerationRef` on every pointerdown, including one that goes on to scroll past this restore's issued target to true bottom - with the generation check first, `||` short-circuits and the caller's atomic frozen-target predicate (which performs the pending-id/registry/gate clear as a side effect when it resolves) never runs at all, leaving all three pieces armed after a pointer-driven reach-the-edge.
        // Callers whose own predicate never has a resolving side effect (`() => false`, the hydration-retry and visibility-replay callers) are unaffected by the reorder - `false || X` and `X || false` are equivalent there.
        isAborted: () =>
          isAborted() ||
          anchorUserScrollGenerationRef.current !== generationAtIssue,
        shouldYieldToReader: () => false,
        ...
        onSettledValid: () => {
          ...
          // Publish that semantic result through the latch explicitly so a temporary partial-transcript end report cannot remain the live follow authority after the saved row has actually landed.
          setTimelineMode("free-scrolling", false);
          acceptExhaustedPersistedRestoreFallback(
            restorePersistencePendingRef,
            pendingMeasuredFreeRestoreRef,
          );
          // Without this, a rapid hide landing between "restoration just validated" and "the next scroll-driven capture" would find the mirror still unwarmed and fall back to the last-persisted value - harmless (still never a jump to row 0), but not the freshest coherent snapshot the acceptance criterion asks for.
  ...
  useLayoutEffect(() => {
    ...
    // The React commit that mounts restored reserve geometry owns the final measured landing synchronously; correctness cannot depend on rAF, which is legitimately paused for background/unfocused Electron renderers.
    restorePersistedTimelineLocation(pending.messageId, pending.viewOffset, {
      isAborted: () => {
        // A scrollbar/manual strict-end landing can be observed by the restoration settle before the browser publishes its native scroll event.
        // Reconcile that fresh DOM geometry through the latch (the sole authority) before deciding whether the restore still owns the viewport.
        followLatchRef.current?.observeLiveGeometry();
        if (timelineScrollModeRef.current === "free-scrolling") return false;
        // But hydration-transaction P1 (chat-messages.test.tsx "close-before-hydration"/"bare pointerdown... before hydration") needs the OPPOSITE outcome for a tail-only partial snapshot, whose clamped restore target IS the current tail - that landing reports following-end purely because not enough content has hydrated yet, not because the reader moved anywhere, and must NOT release the gate (a later remount still needs the ORIGINAL saved row, not this transient tail).
        // `isDemonstrablyPastIssuedFreeRestoreTarget` distinguishes the two against the frozen `issuedTarget` above, not a recomputed one.
        const isPastTarget = resolvePendingEndLanding();
        if (isPastTarget) {
          // Leaving either armed after a proven reader move would let a later arrival of the formerly missing row yank the reader back off the bottom they deliberately reached, or let a permanent close before that arrival re-commit the stale coordinate to durable storage.
          pendingHydrationRestoreAnchorIdRef.current = null;
          forgetPendingHydrationRestore(identity);
          restorePersistencePendingRef.current = false;
        }
      ...
      onExhausted: () => {
        resolvePendingRestoreEndLandingRef.current = null;
        // The bounded restore has accepted the browser-clamped position as its safe fallback.
        // Publish that real viewport from now on rather than retaining/replaying an unreachable saved coordinate across every later remount.
        restorePersistencePendingRef.current = false;
        pendingMeasuredFreeRestoreRef.current = null;
        reconcileInvalidTimelineLanding();
      },
  ...
  // A hide eventually zeroes this tile's measured geometry (`PaneVisibilityContext`'s own doc comment: "size-measuring surfaces... read a 0x0 box while hidden"), so this effect never TRUSTS a live DOM read on the transition - it publishes through the SAME chat-tab-state-cache/restoration seam mount-time restore uses, not a second store or a new automatic-scroll policy.
  // On show, re-deriving via `restoreChatTabState` (not the frozen mount-time `restoredTabState`) picks up the SAME clamp-to-surviving- neighbor/natural-clamp fallback a destructive mutation while hidden already gets on an ordinary remount.
  ...
  useLayoutEffect(() => {
    ...
    if (!visible) {
      // Deliberately NOT a live read here: LegendList's OWN internal scroll/position bookkeeping (`getState().scroll`, `positionAtIndex`, and the rendered-window bounds `viewportAnchorMessageId` searches) can still be internally INCONSISTENT for a handful of milliseconds after a raw `scrollTop` write, even while `clientHeight` already reads non-zero - a guarded live read in that window does not fail safely (return null); it can silently resolve to the WRONG row (verified empirically: row 0, not the target).
      // The mirror is only ever written from `handleScroll`, after LegendList's own state is internally coherent, so it never carries that failure mode - at worst it is one scroll event behind, which is still a position the reader genuinely was at.
    ...
    // A top-level task canvas is kept mounted, so showing it again can deliver ResizeObserver maintenance while LegendList still exposes its temporary zero/incomplete geometry.
    restorePersistencePendingRef.current = true;
    // Unlike the hydration catch-up above, nothing re-attempts a failed visibility replay (this effect only fires on hide/show transitions), so retaining the gate on failure would leave follow reconciliation and scroll persistence suppressed until an unrelated reader gesture.
    // Reuse the mount-time exhaustion fallback instead: accept wherever the bounded retries (or an unavailable list/anchor) left the viewport as the new truth and release the gate.
  ...
  const navigateToMessage = useCallback(
    (messageId: string, highlight: boolean, animated: boolean): void => {
      // Decision #21: minimap/find/deep-link navigation all perform manual-navigation cancellation first.
      // Not a real gesture - a plain release, no freeze: the navigation's own scroll (right below, via scrollToTimelineLocationSuppressingFollowRestore) takes over immediately regardless.
      forgetPendingHydrationRestore(identity);
      pendingHydrationRestoreAnchorIdRef.current = null;
      cancelTimelineLiveFollowForUserNavigation({
        direction: "indeterminate",
  ...
  // `chat.subscribe`'s snapshot can still grow after this tile's own `snapshotLoaded` first flips true (a reconnect resends a fuller snapshot; backfill can trail the flag - see chat-session-store.ts's reconnect/rehydrate comments), so a mount that races that growth silently clamps to whatever short prefix had arrived, landing near the wrong end of a still-growing transcript - and nothing ever revisited that decision even once the real transcript caught up.
  // Disarms permanently the first time it lands - a NEW live message arriving afterward can never re-trigger a jump back here.
  useEffect(() => {
  ...
  // Find scans the records the client HOLDS.
  // On the windowed line that is a subset, so the bar has to say so - a count over a subset presented as a total is wrong in the one direction a reader cannot detect.
  ...
  // The library can run a new-data layout pass before installing a changed callback prop; closing over `listRows` would therefore pair new indexes with old rows.
  ...
  // The controller does not diff message arrays to decide scrolling - append, prepend, reorder/weave, in-place update, and suffix replacement all flow directly into LegendList's own static `maintainScrollAtEnd`/ `maintainVisibleContentPosition` configuration (behavior contract: "one edge, one rule").
  // This effect only keeps the restoration viewport-tracker and find's rendered-data index in sync with a new `messages` reference.
  useLayoutEffect(() => {
    scheduleActiveViewportUpdate(
      timelineScrollModeRef.current === "following-end",
    );
  ...
  useLayoutEffect(() => {
    ...
    if (activityGroupId !== null) {
      // NOT routed through requestMeasuredItemChange: this effect always runs from a layout effect, and flushSync is a documented no-op (with a console warning - "React cannot flush when React is already rendering") when called from inside React's own commit/effect pass, since it cannot re-enter a synchronous work loop.
      // The store update still applies (React schedules and commits it before paint, same as any layout-effect-triggered update) - it just isn't guaranteed synchronous by the time navigateToMessage runs right below, so its scrollToIndex can race a still-collapsed measurement on the first frame.
      activityGroupOpenStore.getState().setOpen(activityGroupId, true);
    }
    // Cross-tile jumps use the same programmatic-navigation choke point as every in-tile navigation: suppression, settle validation, and bounded re-issue are all armed before the scroll.
    // The highlight is visual only.
  ...
  // Liveness is NOT inferred here: `useChatAnnouncements` reads the store's transcript baseline (which connection hydrated these rows) and reports the semantic transition.
  ...
  // Recomputing would rewrite the text inside the already-announced live-region node, and a screen reader re-announces on content change - a phantom completion for a rename.
  ...
  useLayoutEffect(() => {
    ...
    // Decision #10/#16: turn completion below the fold stays anchored - no auto-reveal.
    // The pill flips to "New reply" instead, unless the reader is already at the tail (nothing to signal).
  ...
  // Behavior contract: pill visibility mirrors the latch's single live follow authority: immediate hide at the edge and immediate show on the first confirmed reader departure.
    ...
    <ChatOpenStoreScopeProvider value={instanceId}>
      <ActivityGroupOpenStoreProvider store={activityGroupOpenStore}>
        <div
          ...
          // Marked here rather than on the chat tile's transcript wrapper: that wrapper also holds the absolutely-positioned lower-surfaces dock (composer, approvals, todo), which must stay out of the selection.
          data-selection-root=""
        ...
        >
          ...
          />
          {/* The minimap rail is untappable on touch and its hover-expand never fires; hide it below md and reclaim the right edge. The `side` setting is a user preference, not a viewport rule, so it cannot stand in for this. */}
          {shouldMountChatTurnMinimap({
            hasContent,
