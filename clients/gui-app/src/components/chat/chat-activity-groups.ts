...
// Reasoning folds into the surrounding activity group for its whole life - streaming AND completed - and its duration accumulates into the "Thought for Xm Ys" summary clause.
// Admitting it unconditionally is the point: a block that changed container the instant it stopped streaming (born standalone, absorbed into a COLLAPSED group, so it vanished whole in one frame) is what made #597 jarring enough to revert.
...
export interface ActivityGroupModel {
  ...
  /** Start (epoch ms) of the currently-streaming tool/command child, or null when nothing nestable is in flight. Drives the elapsed heartbeat on the (collapsed-by-default) active group header - the only always-visible surface for a top-level tool's "still working / how long". */
...
interface ActivitySummaryCounts {
  ...
  // The group contains reasoning AT ALL.
  // Tracked separately from the duration because a completed block can legitimately have none: `completedDurationMs` returns null when the block carries no `startedAt` (persisted history), and 0 when it started and finished inside the same millisecond.
  thinkingPresent: boolean;
  // Summing alone cannot tell them apart, and the child's own formatter can - which is how the group header and the block it stands in for came to disagree.
  thinkingDurationKnown: boolean;
  // At least one FINISHED block contributed nothing to the sum, so `thinkingDurationMs` is a floor rather than a total.
  // A block is finished with no duration when it was interrupted, superseded or errored (its timestamp is the turn-end, not the real finish) or when it predates `startedAt` - see `completedDurationMs`.
  thinkingDurationPartial: boolean;
  // At least one reasoning block has finished.
  // Once that is true the clause says "thought", never "thinking", however many new blocks start streaming - the header describes the whole run, and a run that has already thought does not stop having done so.
  thinkingCompleted: boolean;
  exploredFiles: number;
  readFiles: number;
  searched: number;
  // Distinct edited-file keys (path when extractable, else the tool-use id).
  // Mutated in place across the single counting pass - the accumulator is built fresh per `activityGroupSummary` call (see `createEmptyCounts`) and never shared, so this avoids the O(n^2) full-Set copy a per-add clone would cost.
...
// Timeline is purely a function of `segments`.
// The base walk is turn-state independent - only the trailing-group "active" marker differs - so we cache the base once per `segments` identity and derive the active variant on demand.
...
function buildChatActivityTimelineImpl(
  ...
  // Everywhere else the block keeps its own header: that header carries its label, its collapse on completion and its find anchor, and a group reading "Thought for 2s, ran 3 commands" is NOT standing in for it.
  // This builder does not try to track it; the component that stays mounted across the shrink does.
  const flushRun = (): void => {
  ...
  for (const segment of segments) {
    ...
    // Either promotion can start returning true MID-TURN for a segment already sitting in a rendered group: the durable marker is fixed, but the host's `backgroundItems` set is transient and grows while the turn runs.
    // The run then splits around a member it used to contain.
    if (
      segment.kind === "tool" &&
    ...
    // Everything else - text, errors, plans, etc. - stands on its own in chronological position rather than folding into the activity group.
...
/** Keying this on the reasoning count alone also caught the far more common `[reasoning, tool, tool]` shape, where the group header reads "Thought for 2s, ran 3 commands" and is NOT that block's label - so dropping the row left a paragraph of thinking floating between tool rows with no title on it at all. PURE FUNCTION OF THE CURRENT SHAPE, and it can flip in BOTH directions - runs do not only grow. */
...
/** What a reasoning block calls itself, in one place. A streaming block reads "Thinking" and ignores whatever duration it may already carry; a finished one reads its total. */
...
function shouldPromoteToolSegment(
  ...
  // A shell the agent ran or restarted (`traycer_run_shell` / `traycer_restart_shell`, host-stamped) is a background process that outlives the turn, exactly what the backgrounded-command rule below is for: its card is the shell's own surface in the feed - live status, the door to its output - and folding it into "Used N tools" would hide the one thing in the group that keeps going after the turn ends.
  // Keyed on the durable block marker, so it holds after reload and for legacy-free stamped blocks.
  if (segment.managedCommand !== null) return true;
  // A backgrounded command/Monitor stays a standalone card for its whole life - running, completed, stopped, errored, and after reload - keyed on the persistent block marker.
  // This is the durable signal: it is stamped at birth from the tool input (`run_in_background` / Monitor) and is sticky, so it covers the running phase too.
  if (segment.backgroundTask) return true;
  if (promotedToolBlockIds.has(segment.id)) return true;
  // Foreground commands never capture background output, so this never promotes them.
  // Deliberately NOT keyed on `isStreaming`/`error` - those fire for foreground commands too and would flash every normal command into a standalone card while it runs, then collapse it back into the activity group on completion.
  return (
    isCommandLikeTool(segment.toolName) && segment.backgroundOutput !== null
  );
}

// A backgrounded command (Codex yields a long-running exec to the background and keeps it alive past the turn that started it) stays a standalone card for its whole life - running, settled, and after reload - on the same two signals as a backgrounded tool call: the durable block marker, and the transient host `backgroundItems` set that keeps the card live while the host tracks it.
// There is no `backgroundOutput` fallback here - command stdout is never persisted, so the marker is the only durable evidence.
...
/** Start of the currently-running tool/command (the most recently started still- streaming one - tools run sequentially, so this tracks the current operation). null when nothing nestable is in flight (a streaming subagent/file_change, or a trailing group kept active only because the turn continues, carry no start). */
...
function isActivitySegment(
  ...
  // Unconditional, streaming included - see `ActivityGroupDetailSegment`.
  // A reasoning block must occupy the same container from its first token to the end of the turn.
...
/** Split out of `countActivitySegment` purely to keep that switch under the complexity ceiling - reasoning is the one child kind that contributes two independent facts to the summary rather than a single count. */
function countReasoningSegment(
  ...
  if (segment.isStreaming) {
    // A block still streaming contributes nothing but its presence, even if it already carries a duration: `ReasoningSegment` labels a streaming block "Thinking" and ignores the number, so counting it here would let the group say "Thought for 3s" over a child still saying "Thinking" - and `hidesSoleReasoningHeader` would then hide a header that did NOT say the same thing.
    return;
  }
...
/** The label a reasoning block shows for itself. Lives here, next to the group clause it has to agree with, and is imported by `ReasoningSegment` (the rendered header) and by `chat-find-projection` (its find unit) so all three spellings of "how long did it think" come from one place. */
...
/** `reasoningSummaryLabel` (the block's own header) and `thinkingPhrase` (the group clause standing in for it) both route through here, so the equality `hidesSoleReasoningHeader` bets on is structural rather than two copies of a rounding rule kept in step by hand. A block with no measurement at all is `null` and never reaches here. */
function thoughtClockDuration(durationMs: number): string {
  return formatClockDuration(Math.max(1, Math.round(durationMs / 1000)));
}

/** MONOTONE: `thinking` -> `thought` -> `thought for Xs`, and never backwards. `thinkingCompleted` closes it - a run that has already thought does not stop having done so. */
