// Barrel for the chat find surface.
// The implementation is split into focused modules so each has a single reason to change: - chat-find-projection: transcript -> searchable rows/units + unit ids. - chat-find-adapter: search state, match reconciliation, snapshot lifecycle. - chat-find-highlighter: DOM/CSS Custom Highlight range painting + anchors.
