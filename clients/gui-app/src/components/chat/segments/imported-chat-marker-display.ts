...
/** Find used to build its own string from the raw harness id, which made the row unfindable by the provider name it shows on screen - the exact drift a duplicated format string invites, and the reason this is one function rather than two that happen to agree today. */
