...
/** Everything shown comes off the payload the host stamped from the successful tool RESULT: the effective command the shell relaunched under, the host's own verdict on what changed (command, cwd, both, neither), and - only when the relaunch did not come up - the status the result reported, frozen. The directory itself is not shown - a host-disk detail the output window's popover carries; the phrase is what matters here. */
...
export function ManagedCommandRestartSegment(
  ...
  // Same reading as the start card's: only the owning chat's open stream saying the shell is not there counts as gone; a set that has not arrived yet proves nothing.
  ...
  const header = (
    <>
      ...
      />
      {/* No cwd on the card, same as the start card: the delta phrase says "cwd changed" when that is what this restart did, and the output window's details popover has the effective directory. */}
      ...
      {/* The outcome the result reported, FROZEN - but only when it is news. Never re-read from the live record. */}
