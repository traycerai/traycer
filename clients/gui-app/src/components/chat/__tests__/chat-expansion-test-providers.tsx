...
export function ChatExpansionTestProviders(
  ...
  // Subagent + tool open state live in module-global stores namespaced by the scope string (the tile instance id); activity-group and a2a can be either a fresh per-provider fallback store (`store={null}`, used here so fixed `tileInstanceId`s reused across unrelated `it()` blocks in the same file never leak into each other) or the ticket-5 registry-backed store keyed by tile instance id (production - see `chat-messages.tsx`); find-force is a per-tile provider.
  return (
