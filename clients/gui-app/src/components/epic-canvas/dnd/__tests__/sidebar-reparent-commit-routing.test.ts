...
const seam = vi.hoisted(() => ({
  ...
  recordIds: [] as string[],
  tree: { rootIds: [], childrenByParent: {}, nodeById: {} } as TreeSlice,
...
const { commitSidebarReparentDrop } = await import(
...
beforeEach(() => {
  ...
  seam.recordIds = [];
  seam.hasClient = true;
  seam.tree = { rootIds: [], childrenByParent: {}, nodeById: {} };
});

describe("commitSidebarReparentDrop routes by which plane owns the pointer", () => {
  it("sends a registry-backed terminal agent through epic.reparentChat", () => {
    seam.tree = treeOf([
