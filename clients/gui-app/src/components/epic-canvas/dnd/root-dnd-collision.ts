/**
 * Kept out of the provider component file so the table stays unit-testable and the .tsx file exports only the component.
 */
import {
  pointerWithin,
  rectIntersection,
...
/**
 * Resolves the typed canvas source from the active draggable: Pierre hosts carry a stable marker as `data` and their per-press payload in the module registry (resolved by the sensor at pointer-down); every other source attaches its payload directly.
 */
...
// Only a `sidebar-node` drag may resolve a reparent target.
...
/**
 * `satisfies` keeps the ladder exhaustive over every root drop target kind, so a renamed or newly added kind fails compile here instead of silently dead-zoning its drops.
 */
const TARGET_KIND_PRIORITY = {
  ...
  "empty-shell": 6,
  // Sidebar reparent targets: the row beats the panel so hovering a row picks the row and only true empty space picks the panel (root drop).
  // Sidebar and canvas occupy disjoint regions, so ranking below the canvas chrome is moot for cross-group overlaps - only row-vs-panel matters.
...
/**
 * The collision pass's pointer is the exact point that picked `over`, so every preview/commit computation reads it from here and can never disagree with the hit test.
 */
...
/**
 * Targets incompatible with the active source kind are dropped up front so e.g. a rail drag never lights up a pane body.
 */
