/**
 * A filter is applied as a precomputed *visible-id set* rather than threaded as a prop through the recursive node components.
 */
...
/**
 * Active sort comparator for the panel, or `null` for the projector's default order.
 * The panel body computes it once from the panel's sort mode and publishes it here so root and child levels reorder identically - the sort analogue of {@link SidebarFilterVisibilityContext}.
 */
...
/** Identity is preserved in the pass-through case so memoized callers don't churn. */
...
/**
 * Union the always-expanded ancestor set with the filter's visible ids so a filtered subtree opens to reveal its matches.
 * Returns the ancestor set unchanged (same identity) when no filter is active.
 */
...
/**
 * Child ids of `parentId` that survive both the panel's structural `treeFilter` (chat vs artifact node kinds) and the active visibility filter.
 * `memo` on the row could never have stopped that.
 */
export function useFilteredPanelChildIds(
  ...
  return useEpicStore(
    useShallow((state: OpenEpicState): readonly string[] => {
      ...
      // `childIds` arrive in projector (default) order; re-sort only when the panel has a non-default mode (`comparator !== null`).
      return sortNodeIds(filtered, nodeById, comparator);
    }),
  );
}

/** Shared so the rule can't drift between the chat and artifact panels. */
...
/**
 * A bare printable character with no modifier - the type-to-filter trigger shared by the artifact and chat panels.
 * Excludes space so it can keep its tree-row activation meaning, and anything carrying a modifier so shortcuts still reach their handlers.
 */
...
/**
 * Expand a set of matched node ids to also include every ancestor along each match's parent chain, so the matches remain reachable in the rendered tree.
