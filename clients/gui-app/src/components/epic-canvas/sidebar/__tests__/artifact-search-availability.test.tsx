/**
 * It drives a real `createOpenEpicStore` through the same `EpicSessionContext` the app uses, mutates the Y.Doc the way the host would, and asserts the hook flips WITHOUT any manual rerender.
 */
...
function openStore(): OpenedStoreForTest {
  ...
  // A writable role, the way the host's meta/permission frames would set it: the gate also withholds search from viewers (and from a not-yet-known role), so the artifact-count cases below need write access as a given.
...
describe("useArtifactSearchAvailable against a real Epic store", () => {
  ...
  it("answers no for a viewer even when artifacts exist", () => {
    ...
    // A read-only device never runs epic file sync, so the search RPC's disk mirror never materializes; the gate hides search rather than offering a permanently "still syncing" dead end.
    act(() => {
