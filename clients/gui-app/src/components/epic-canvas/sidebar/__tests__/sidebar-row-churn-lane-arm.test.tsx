...
/**
 * Built as ONE frame rather than row by row so the wire's completeness rules apply here rather than producing a half-populated head that happened to agree on the fields a fixture bothered to set.
 */
function stateSnapshot(
  ...
  const parsed = epicStateSubscribeServerFrameSchemaV10.parse({
    ...
    artifactRecords: ROW_IDS.map((id, index) => ({
      ...
      parentId: null,
      // Revisions must ADVANCE: the row store applies an upsert only when the incoming revision beats the held one, so a replacement frame reusing them would be dropped as stale and this suite would pin nothing.
...
function createLaneRig(): LaneRig {
  ...
  const laneSelection: EpicLaneSelectionSources = {
    // The relay shape: support never resolves, so the probe's own outcome installs the lanes - the way a real remote connection reaches this arm, rather than a manifest a test resolved by hand.
    support: () => "unknown",
    subscribeSupport: () => () => {},
...
describe("the sidebar rows, on the lane arm", () => {
  ...
  it("holds every row still when twelve records are stamped", () => {
    ...
    // THE PIN.
    // An artifact row displays no timestamp, so a flat zero is the honest assertion here - not the 28 bystanders, and not the 12 whose own records moved.
