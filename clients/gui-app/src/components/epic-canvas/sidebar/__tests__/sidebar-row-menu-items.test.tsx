/**
 * The sibling sidebar suites mock `@/components/ui/dropdown-menu` and `@/components/ui/tooltip`, which is fine for wiring but structurally cannot check the things this file exists for: the accessible NAME a browser would compute, the `aria-disabled` Radix derives from `disabled`, and whether a tooltip trigger placed on the menu item via `asChild` actually still fires.
 * If that merge silently dropped them the tooltip would simply never open, and every mocked test would still pass.
 */
...
describe("SidebarDropdownMenuItems against real Radix", () => {
  // This project runs vitest with `globals: false`, so Testing Library's auto-cleanup never registers - without this each test inherits the previous one's DOM and every query matches twice.
  afterEach(() => {
    cleanup();
  });

  it("names a soft-disabled item by its LABEL, not label + reason", () => {
    // The reason lives in an `sr-only` span inside the item so the item can describe itself without depending on the tooltip being open.
    // `sr-only` hides visually but does NOT leave the name-from-content subtree, so without `aria-hidden` the item would be named "Archive Can't archive while..." and then announce the very same sentence again as its description.
    renderMenu();
  ...
  it("leaves Radix's own aria-disabled intact on a HARD-disabled item", () => {
    // Radix renders `"aria-disabled": disabled || undefined` BEFORE spreading caller props, so passing an explicit `false` overrides the `true` it derived and announces an inert, keyboard-skipped item as available.
    // The hard-disabled arm must omit the prop entirely rather than pass `false`.
    renderMenu();
  ...
  it("puts no element between the menu and any item", () => {
    // An intermediate node breaks `menu` -> `menuitem` ownership, and `role="presentation"` cannot save it: ARIA ignores a presentational role on an element carrying a global ARIA property, and Radix Tooltip assigns `aria-describedby` to its trigger while open.
    renderMenu();
  ...
  it("still opens the tooltip from the item itself", () => {
    // If they were dropped, the tooltip would never open and only an unmocked test would notice.
    renderMenu();
