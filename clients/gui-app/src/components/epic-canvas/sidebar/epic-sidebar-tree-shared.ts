...
/**
 * Horizontal offset (from a row's own padding-left edge) to the center of its chevron/icon column.
 * Indent guide rails are drawn at the parent depth plus this offset so each rail sits under the column it descends from.
 */
...
/**
 * Disabled shadcn buttons carry `disabled:opacity-50`, which would otherwise pin these controls visible at rest because the pseudo-class outranks the base `opacity-0`.
 */
...
/**
 * The `disabled:*` overrides are load-bearing: shadcn `Button`'s base `disabled:opacity-50` is a `:disabled` pseudo-class (specificity 0,2,0) and outweighs the plain `opacity-0` rest rule (0,1,0), which would otherwise pin a *disabled* control visible at 50% even when the row is not hovered.
 */
...
/**
 * Per-type icon color customization for a sidebar row's leading glyph, read here rather than threaded from the tree root so the leading icon stays a leaf concern.
 * `ChatProgressIcon` already subscribes to exactly these two settings internally for chat rows; every OTHER sidebar row glyph (terminal agents, cloud rows, static node icons) mirrors it through this hook so one row kind cannot drift muted while another picks up "color by type" in the same column - a chat glyph's tint must depend on the user's icon-color setting, never on which list the row arrived from.
 */
...
/**
 * `revealed` is for a surface whose controls never wait for hover (touch has no hover to wait for): the row must then hold the wider pad at rest, because the controls are already sitting in it.
 * A `pointer-coarse:` variant would say the same thing less reliably - it would have to out-order the `group-hover` rules rather than replace them - so the branch is taken here, in the one place the string is built.
 */
