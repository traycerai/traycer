/**
 * Git badges are overlaid from the existing `git.subscribeStatus` subscription - that pipeline stays the one source of truth - and `ignored` entries are dimmed through the tree's own ignored status, never hidden. - FALLBACK: hosts older than the stream method reject it as unknown, and the panel transparently keeps using the unary `workspace.listFileTree` snapshot (25k files, 10s poll) with the git status that response carries.
 * Filtering has its own source, because lazy coverage makes a local filter structurally incomplete - it can only match rows the panel already loaded.
 */
import {
  useCallback,
  useEffect,
...
/**
 * The only mode that can surface a file the panel never listed, which is the whole point: with lazy coverage the local filter can only ever match rows already loaded. - `local-filter` - a query with no usable host answer (host too old, root refused, response still in flight, or the panel is on the unary snapshot and already holds every path): the tree adapter's own `hide-non-matches` filter runs over the loaded rows, exactly as before.
 * A live tree whose host CANNOT search (the unsupported latch, not a reply merely in flight) has a fourth wrinkle: its loaded rows are just the expanded directories, so a local filter over them silently matches nothing (the adapter shows the full tree when zero rows match).
 */
...
function useFileTreeSource(args: {
  ...
  // A renderer with no stream transport at all can never be served by the stream either, so it takes the same path.
  ...
  // Params are only HALF the precondition, and the other half is invisible here: the registry keys its shared entry on `client.instanceId` too, so sharing also requires both surfaces to hold the same stream CLIENT.
  // They do today only because both read the app-wide `useWsStreamClient()`.
  ...
  const search = useHostPathSearch({
    ...
    // Host search only earns its keep against the LIVE tree, where the loaded rows are just the expanded directories.
    enabled: !useUnaryFallback,
  });
  const localFilterQuery =
    args.searchQuery.trim().length > 0 ? args.searchQuery : null;
  // A live tree whose host cannot search: the local filter would only see the expanded rows (and the adapter shows everything when zero rows match), so the query borrows the whole-workspace snapshot for its lifetime instead.
...
/**
 * "This host cannot serve the method, ever" - the signal that latches the panel onto local filtering for this (host, workspace).
...
interface HostPathSearch {
  ...
  /**
   * The latched verdict that this (host, workspace) cannot serve `workspace.searchPaths` at all - as opposed to a reply merely in flight.
   */
  readonly hostSearchUnavailable: boolean;
}

/**
 * Host-ranked path search for the current filter query, or `null` whenever the panel must keep filtering locally.
 * Latching turns that into one verdict per host+workspace; changing either scope re-probes, since the state is compared against the live scope key rather than reset by an effect.
 */
function useHostPathSearch(args: {
  ...
  // `args.hostId` already keys `unsupportedScopeKey` below and already stamps every ref this panel opens, so taking the CLIENT from anywhere else made the panel search one machine and label the results with another - and cache the "host cannot search" verdict under a host that was never asked.
...
/**
 * That watches the wrong machine's working tree, and it does it without erroring, because the param is a key rather than a route.
 * A separate component ONLY because the provider has to sit above the hooks: `useFileTreeSource` runs in the body, so a provider added inside it would be below its own consumers.
 */
export function FileTreePanelBodyForWorkspace(
  ...
  // No viewport key: pierre bakes `density` / `itemHeight` at construction and offers no setter, so this body used to remount across the breakpoint to rebuild a touch-sized model.
  // Both viewports now build the same geometry, leaving nothing to rebuild - and a remount is not free, since it drops the filter query.
  return (
...
function FileTreeBodyForResolvedHost(
  ...
  // Surface pin (`selection ?? effective`). Opened tabs stamp this host id onto `WorkspaceFileRef`.
  const { hostId, onLatchHost } = props;
  const isMobileViewport = useIsMobileViewport();
  // The box is a filter, not a search field: the query is applied on a pause, and the same debounced value gates both the host RPC and the local row filter so the two can never disagree about what is being filtered for.
  ...
  // The source's path list is the source of truth for "what is an openable file and what is its display name": a path absent from `fileNameByPath` is a directory row and not openable.
  ...
  // Single source of truth for "tree row path -> workspace file ref".
  // Reused by the open handlers and the drag bridge so a row that is not an openable file (no active host, or a directory row) is non-openable everywhere.
  ...
  // Pierre's useFileTree captures the onSelectionChange closure at mount, so we forward the latest props through a ref the closure reads at call time.
  // Same trick for the double-click "open committed" handler.
  ...
  useEffect(() => {
    ...
    // Preview is right on a touch viewport too, even though the double-click that promotes it there has no touch equivalent: that viewport shows ONE tile at a time, none of its lists surfaces an open workspace-file tile, and nothing there can close one - so a permanent open buys nothing visible and leaves an unreachable tile behind.
    // Recycling the single preview is what browsing a tree by tap wants, and the row itself is the way back to a file already visited.
  ...
  // A flag rather than a one-path marker: the rewrite is a deselect of every other row followed by one select, and with two rows selected the first deselect already reports a non-empty selection.
  // Pierre notifies synchronously from inside each `select()` / `deselect()`, so the flag brackets the whole block and a later user click is never suppressed.
  const suppressSelectionOpenRef = useRef(false);
  const { model } = useFileTree({
    paths: treePaths,
    initialExpansion: "closed",
    // Touch used to inflate this to a 44px row because the rows are in a shadow root the mobile hit-area stylesheet cannot reach - the compact pitch is deliberately kept instead, so the phone shows the same tree as the desktop app.
    // If the rows ever need to be easier to hit, the lever is pierre's `--trees-item-padding`, which grows the target without forking the pitch.
    density: "compact",
  ...
  // Runs BEFORE the path reset below, deliberately.
  useEffect(() => {
  ...
  // Only a request for THIS host + workspace is this body's to serve; one naming another workspace is routed by the panel above (it switches the selection, which remounts this body for the right workspace).
  // Declared AFTER the expansion hook on purpose: its per-tick effect must run after the reset that re-seeds the model from a new path list, so it sees the rows that list just added.
  ...
  // Pierre renders the FULL tree when a search matches zero rows - honest for an always-fully-loaded tree, but here it reads as "the filter is broken".
  // Both filter modes surface an explicit empty state instead: host search by an empty result set, the local filter by the model's live match count.
  ...
  const handleDoubleClick = useCallback(
    (event: MouseEvent<HTMLElement>) => {
      // A touch browser synthesises `dblclick` from a rapid double tap, which would promote the preview to a permanent tile - the one outcome this viewport cannot undo, since nothing there lists or closes an open workspace-file tile.
  ...
  // Files keep their canvas-openable source; directory rows carry a composer-only mention source instead of masquerading as file tabs.
  ...
  return (
    ...
    >
      {/* The sidebar's 28px filter row is below the touch target every other phone control meets, and this one is a text field the user has to hit precisely rather than a control with room for invisible hit-slop. */}
      ...
      {/* Pierre's scroller lives in a SHADOW ROOT and a touch inside one retargets to the host, so that walk sees nothing scrollable and would claim a downward swipe as a drawer dismiss. The attribute states what vaul cannot observe: this subtree owns its own scrolling. */}
      {/* The whole container is the context-menu trigger: Pierre's rows live in a shadow root, so the row a right-click landed on is recovered from the event rather than from a per-row handler. */}
...
/**
 * Pierre's `resetPaths` swaps the store but keeps the current search VALUE while never recomputing its match set, and `setSearch` no-ops on an unchanged value - so a filter that is active across a paths reset silently stops filtering: the projection rebuilds against the stale match set from the OLD store (often empty, which Pierre renders as "show everything").
 */
...
/**
 * While a filter query is active - either mode - expansion is driven by the filter (the tree adapter expands its own matches; host search expands the ancestors of the ranked results) rather than by the user, so syncing pauses and the stream's coverage stays exactly where browsing left it.
 */
function useWorkspaceFileTreeExpansion(args: {
  ...
  // Only a NEW path list resets the tree; the expansion sets are dependencies because the reset re-seeds from them, not triggers of their own (re-seeding on every expansion write would fight the user).
  ...
  useEffect(() => {
    ...
    // Host results are a flat ranked list, so nothing is visible until their ancestors are open; browsing re-seeds the durable set instead.
    // Either way the reset owns expansion - the tree adapter's own default would close everything.
    model.resetPaths(treePaths, {
...
/**
 * Ancestors are opened on the MODEL, one listed level per tick, never by writing them into the expansion store directly.
 * A request for a file the workspace never lists (deleted, or a root that stopped serving) stays pending until a newer request for the same view tab replaces it; it is in-memory and per view tab, so that is bounded.
 */
function useWorkspaceFileTreeReveal(args: {
  ...
  /**
   * A dependency of the walk because a listing can add the NEXT folder without adding a file (`fileNameByPath` unchanged), and the walk has to wake up for it.
   */
  readonly treePaths: ReadonlyArray<string>;
