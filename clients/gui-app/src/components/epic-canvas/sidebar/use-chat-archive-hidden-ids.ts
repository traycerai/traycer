/**
 * That pairing is the rule, so it lives in one place and every surface offering the "Show" facet gets it whole rather than reimplementing the easy half.
 * The candidate list is the CALLER's, because each surface knows which rows it renders; what it must not decide for itself is which of those survive.
 */
...
/**
 * The single tier a descendant chat is counted under - its own highest.
 * The attention precedence goes through the shared `attentionTone`, so failure > interview > approval lives in exactly one place.
 */
export function chatDescendantKind(
  ...
  // Terminal failure is demoted only for the exact chat's own glyph, where a newer live turn/Done is a stronger statement of current state.
  // Once this chat is rolled into a collapsed parent it is a distinct failed child and must remain attention-priority over a sibling's activity or completion.
...
/**
 * `chatIds` are the rows the caller renders; `notificationIndicators` is the indicator state it already subscribes to - passed in rather than re-fetched so the reveal reads exactly what the rows read.
 */
