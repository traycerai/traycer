/**
 * Kept apart from any one search UI because the subtleties here are not about presentation and must not be re-derived per surface: which host is asked, which filters compose into the request versus which are applied to the response, when a previous result may still be shown, and how an unsupported host is told apart from a failure.
 * A second surface reimplementing this would agree with the first only until one of them was edited.
 */
...
export function useArtifactSearchResults(args: {
  ...
  // So during a re-point - when the panel stays interactive while only the canvas goes inert - this searched one machine and opened the results as tiles bound to another.
  ...
  // A response must never render for a scope it wasn't fetched in.
  // The query key already isolates late async responses per scope; this signature additionally gates the same-scope retention below so a prior Epic / host / filter result can't linger on-screen after the scope changes (only the query string changing keeps the retained results).
  ...
  // `useEpicSearchArtifacts` intentionally omits `keepPreviousData`, so `query.data` is only ever the current key's result.
  // The setState-during-render idiom keeps the retained value in sync without an effect and never leaks a prior scope.
  ...
  // Resolve each hit's authoritative `updatedAt` from the open-Epic projection; a hit missing from the projection is stale and cannot be classified, so it drops out whenever a read filter is active.
...
/**
 * Silence while no query is active is deliberate: an empty live region must not narrate a search nobody started.
 */
