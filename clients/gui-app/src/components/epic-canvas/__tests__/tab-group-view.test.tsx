...
// Stub it to a no-op so TabGroupView mounts without a HostRuntimeProvider / EpicSessionProvider.
...
vi.mock("@/lib/epic-selectors", () => ({
  ...
  // Chat tab strip icons (ChatProgressIcon) need activity tiers when a chat tile is the active tab - not exercised by the older spec/terminal-only fixtures in this file.
...
// ActiveTabBody's published-copy fallback reads reachability + the active host through these two hook seams.
// Stubbed at the hook boundary (not their query internals) so this provider-less suite can flip a bound host dead per test.
...
// `tab-group-view`'s same-host arm reads `isOwnedByViewer` as well as `identity`, and a stub that enumerates fields answers `undefined` for the ones it forgot - which a boolean predicate reads as "not the viewer's", silently withdrawing the substitution this suite exists to assert.
// The return annotation is the gate: the next field the row gains fails `compile` here instead of quietly turning these tests red in CI.
...
vi.mock("@/components/epic-canvas/renderers/chat-tile", () => ({
  ChatDeadTileBannerContainer: (props: {
    ...
    <div
      ...
      // Surfaced so the substitution arm's owner threading is assertable: the banner must receive the owner the OPENING ROW resolved, not be left to re-derive it from a second cloud lookup.
...
// TabGroupView resolves its tab payloads via `usePaneTabRefs(tabId, pane)`, which reads `canvasByTabId[tabId].tilesByInstanceId`.
// Seed that so the pane's instanceIds resolve to the given refs.
function seedCanvas(
  tabs: ReadonlyArray<EpicCanvasTileRef>,
  activeTabId: string | null,
): void {
...
/**
 * Rendered as the LAST sibling after `TabGroupView` and `StableTileSurfaceHost`, its layout effect only runs once React has finished processing every earlier sibling's own layout effects for the SAME commit - so it observes exactly what has settled by the end of the layout phase, strictly before any passive effect (a `useEffect`-based report, if the fix regresses) gets a chance to run.
 */
...
describe("<TabGroupView />", () => {
  ...
  it("falls back to the first tab when pane.activeTabId is null (resolveActivePaneTab wiring)", async () => {
    // A null activeTabId must still mount/select the first strip tab - same as the previous inline fallback.
    // Inactive never-visited tabs stay unmounted under the keep-alive policy (not display:none).
...
describe("<TabGroupView /> stable tile surface host routing (switch ON)", () => {
  ...
  it("reports the remote-deletion transition into the shared registry so membership can react", async () => {
    ...
    // `tile-surface-membership.ts`'s and `remote-deleted-chat-registry.ts`'s own pure-function pins cover the membership-drop/registry-clear consequence of this report (both are unreachable from this React-mounted harness: `TileSurfaceSlot` never publishes here without a real epic session handle, and no `StableTileSurfaceHost` sibling is mounted to observe).
  ...
  it("design-review F2 residual: a real StableTileSurfaceHost sibling loses the hosted owner in the SAME pre-paint commit as the inline flip", async () => {
    ...
    const tabs = [CHAT];
    seedCanvas(tabs, CHAT.instanceId);
    // `seedCanvas` does not set `openTabOrder`; `getHeaderTabs()` (the top-level retention layer's source of truth) resolves this view tab only through `openTabOrder`, not `tabsById` alone.
    useEpicCanvasStore.setState((state) => ({
      ...state,
      openTabOrder: [VIEW_TAB_ID],
    }));
    ...
    // A real published environment - not a manufactured DOM node - so a real `StableTileSurfaceHost` record is genuinely mounted before the flip.
    act(() => {
      publishTileSurfaceEnvironment(
    ...
    await waitFor(() => {
      // Confirms real eventual settlement - including `StableTileSurfaceHost` actually dropping the stale record - so a genuinely broken transition doesn't silently pass this test.
      expect(isChatRemoteDeleted(CHAT.instanceId)).toBe(true);
      expect(
        document.querySelector(
  ...
  it("design-review F3: completes a deferred hosted activation on a real pointerdown+click on the hosted record itself", async () => {
    ...
    // A real pointerdown directly on the hosted deferred marker - no physical-pane pointerdown involved - arms the deferred flag; the subsequent click on the same hosted element completes it.
  ...
  it("design-review F3: activates the pane immediately on a non-deferred pointerdown on a hosted record", async () => {
    ...
    // No `data-pane-activation-defer` anywhere on this target - ordinary hosted clicks must activate on pointerdown itself, same as an ordinary physical-pane pointerdown does via `handlePointerDownCapture`.
  ...
  it("keeps the hosted claim structurally after document capture and before descendants across a full remount", async () => {
    ...
    // Seeing it before the claim, and the target after the claim, proves the document marker has completed before the plane dispatches ownership.
...
describe("<TabGroupView /> published-copy fallback for an unreachable bound host", () => {
  ...
  it("renders the published copy + dead-tile banner instead of the live chat body", async () => {
    ...
    // The owner the opening row resolved rides into the banner - the ref in PUBLISHED_COPY_TILE_ID names user-1, and the banner's ownership verdict must come from that same row rather than a second lookup.
  ...
  it("drops the instance from hosted-surface membership while the fallback is active (switch ON)", async () => {
    ...
    // Inline copy renders; no slot, no hosted membership - membership keeping the instance would leave the environment registry holding a stale visible snapshot from the unmounted slot ("removal only by membership"), painting the live body over the copy.
    await waitFor(() => {
      expect(
        container.querySelector(
          `[data-testid="tile-${PUBLISHED_COPY_TILE_ID}"]`,
...
describe("<TabGroupView /> published-copy fallback for a same-host chat with no local record (tickets 36 + 49)", () => {
  ...
  it("keeps the LIVE chat surface for a same-host record-less cloud-known chat while its owner is REACHABLE", async () => {
    ...
    // The live tile mounts, so `chat-tile.tsx` gets to attempt `chat.subscribe` at all - which is what makes the CHAT_NOT_VISIBLE terminate below reachable as a signal.
    await waitFor(() => {
      expect(
        container.querySelector(`[data-testid="tile-${CHAT.id}"]`),
      ).not.toBeNull();
    ...
    // Not reaped either - the cloud row still exempts it from `computeIsRemoteDeleted`, which is the other way this tab could have lost its live body.
    expect(
      container.querySelector('[data-testid="deleted-node-body"]'),
    ).toBeNull();
  });
  ...
  it("still substitutes the locked published copy when the same-host owner is UNREACHABLE", async () => {
    ...
    expect(banner).not.toBeNull();
    // SAME host, but nothing answered - reachability outranks the (absent) terminate, so this keeps the host sentence rather than claiming the history "is no longer on this host", which would assert something no one has said while the host is down.
    expect(banner?.getAttribute("data-reason")).toBe("host-offline");
    expect(
      container.querySelector(`[data-testid="tile-${CHAT.id}"]`),
    ).toBeNull();
  ...
  it("substitutes for a same-host record-less chat once the REACHABLE owner's own subscribe terminates CHAT_NOT_VISIBLE", async () => {
    // `chat-tile.tsx` produces this terminate only because its record gate now lets a cloud-known chat open.
    testState.missingArtifactIds.add(CHAT.id);
    testState.sessionHostId = CHAT.hostId;
    testState.cloudKnownChatIds.add(CHAT.id);
    seedCanvas([CHAT], CHAT.instanceId);
  ...
  it("does not substitute when the same-host chat still HAS a local record, even if also cloud-known", async () => {
    // Cloud-known alone is never sufficient; it only matters once there is no local record to explain the tile with.
    testState.sessionHostId = CHAT.hostId;
    testState.cloudKnownChatIds.add(CHAT.id);
    seedCanvas([CHAT], CHAT.instanceId);
    const { container } = render(groupView([CHAT], CHAT.instanceId, true));
...
/**
 * The record table can only report that a row is gone; the delta's reason is what picks between the two honest end states, and the two are NOT interchangeable - one says the work no longer exists, the other says it does and is not yours to read.
 */
describe("<TabGroupView /> open-tab retraction from the record plane", () => {
  ...
  it("renders the no-longer-shared banner, alone, for a REVOKED retraction", async () => {
    // Ablation: remove the `isRetractedAsRevoked` branch and this falls through to the published-copy substitution, which offers to clone a transcript the server has just stopped serving this viewer.
    testState.missingArtifactIds.add(CHAT.id);
    testState.sessionHostId = CHAT.hostId;
    testState.cloudKnownChatIds.add(CHAT.id);
    seedCanvas([CHAT], CHAT.instanceId);
    ...
    // The live region: the banner appears mid-session with no focus move, so a screen-reader user is told nothing about why the tile emptied unless the sentence is announced.
...
describe("<TabGroupView /> Reveal in Sidebar", () => {
  ...
  it("writes a reveal request and switches to the Files panel for a workspace-file tab", () => {
    // The spec tab stays ACTIVE so it is the one mounted body - the workspace-file tab's own tile renderer is heavy and must not mount just to exercise its tab-strip context menu.
