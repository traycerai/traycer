...
function geometryFor(
  ...
  sourceIndex: number,
  mergeable: ReadonlyArray<boolean> | null = null,
  mergeBandPx: number = HEADER_MERGE_BAND_PX,
): StripDragGeometry {
...
describe("header strip drag model", () => {
  describe("hysteresis is derived from the swap rule", () => {
    it("depends on the source width alone, not the mean of the pair", () => {
      ...
      // its own width.
      expect(swapHysteresisPx(191, 191, HEADER_MERGE_BAND_PX)).toBe(
        191 + 2 * bandPxFor(191, HEADER_MERGE_BAND_PX),
      );
      expect(swapHysteresisPx(191, 382, HEADER_MERGE_BAND_PX)).toBe(
        191 + 2 * bandPxFor(382, HEADER_MERGE_BAND_PX),
      );
      expect(swapHysteresisPx(382, 191, HEADER_MERGE_BAND_PX)).toBe(
        382 + 2 * bandPxFor(191, HEADER_MERGE_BAND_PX),
      );
      // Same source, wildly different neighbour: identical hysteresis.
      expect(swapHysteresisPx(120, 300, HEADER_MERGE_BAND_PX)).toBe(
        swapHysteresisPx(120, 900, HEADER_MERGE_BAND_PX),
      );
    });
    ...
    it("measures the real reversal distance on an asymmetric strip", () => {
      ...
      // Source is 120 wide, neighbour 300 -> band clamps at 16, so the derived
      // figure is 152. Both crossings are found by walking integer x against a
      ...
      const derived = swapHysteresisPx(120, 300, HEADER_MERGE_BAND_PX);
      expect(derived).toBe(152);
  ...
  describe("merge disambiguation", () => {
    it("does not arm while passing at an ordinary speed", () => {
      // Chrome's own reference drag is ~168 logical px/s; 200px/s at 60fps is
      // ~3.3px per frame, so a pass crosses the band in a handful of frames and
      // never accumulates the dwell.
      const geometry = geometryFor([191, 191, 191], 0);
      const states = sweep(geometry, range(ORIGIN, ORIGIN + 600, 4), 0);
      expect(states.some((state) => state.kind === "merge-preview")).toBe(
        false,
      );
    });

    it("arms and previews when the pointer stops on a neighbour's centre", () => {
      const geometry = geometryFor([191, 191, 191], 0);
      const laidOutNeighbourCentre = ORIGIN + 191 + 191 / 2;
      const pointerX = pointerForCentre(geometry, laidOutNeighbourCentre);

      const armed = resolveStripDragState({
        geometry,
        contentOriginX: ORIGIN,
        pointerX,
        previous: null,
      ...
      expect(armed.kind).toBe("merge-armed");

      const stillArming = resolveStripDragState({
        geometry,
        contentOriginX: ORIGIN,
        pointerX,
        previous: armed,
        nowMs: 1000 + MERGE_DWELL_MS - 1,
      });
      expect(stillArming.kind).toBe("merge-armed");

      const preview = resolveStripDragState({
        geometry,
        contentOriginX: ORIGIN,
        pointerX,
        previous: stillArming,
        nowMs: 1000 + MERGE_DWELL_MS,
      });
      expect(preview.kind).toBe("merge-preview");
      expect(
        preview.kind === "merge-preview" ? preview.targetItemId : null,
      ).toBe("item-1");
    });

    it("re-anchors the dwell when the pointer drifts past the stillness budget", () => {
      const geometry = geometryFor([191, 191, 191], 0);
      ...
      const drifted = resolveStripDragState({
        geometry,
        contentOriginX: ORIGIN,
        pointerX: pointerX + MERGE_STILLNESS_PX + 1,
        previous: armed,
        nowMs: MERGE_DWELL_MS - 1,
      });
      expect(drifted.kind).toBe("merge-armed");
      // The original deadline passes with no preview, which is the point.
      const atOriginalDeadline = resolveStripDragState({
        geometry,
        contentOriginX: ORIGIN,
        pointerX: pointerX + MERGE_STILLNESS_PX + 1,
        previous: drifted,
        nowMs: MERGE_DWELL_MS,
      });
      expect(atOriginalDeadline.kind).toBe("merge-armed");
    });

    it("keeps a preview until the dragged tab leaves the band, then reorders", () => {
      const geometry = geometryFor([191, 191, 191], 0);
      const centre = ORIGIN + 191 + 191 / 2;
      const pointerX = pointerForCentre(geometry, centre);
      const preview: StripDragState = {
        kind: "merge-preview",
        targetIndex: 0,
        targetItemId: "item-1",
      };
      const insideBand = resolveStripDragState({
        geometry,
        contentOriginX: ORIGIN,
        pointerX: pointerX + 4,
        previous: preview,
      ...
      expect(insideBand.kind).toBe("merge-preview");

      const outsideBand = resolveStripDragState({
        geometry,
        contentOriginX: ORIGIN,
        pointerX: pointerX - bandPxFor(191, HEADER_MERGE_BAND_PX) - 4,
        previous: preview,
        nowMs: 5000,
      });
      expect(outsideBand.kind).toBe("reorder");
    });
    ...
    it("keeps the merge band reachable rather than shadowed by the swap", () => {
      ...
      expect(state.kind).toBe("merge-armed");
  ...
  describe("degenerate strips", () => {
    ...
    it("clamps the band on a very narrow neighbour", () => {
      expect(bandPxFor(40, HEADER_MERGE_BAND_PX)).toBe(10);
      expect(bandPxFor(400, HEADER_MERGE_BAND_PX)).toBe(16);
    });
  });
  ...
  describe("merge dwell advances on time, not on movement (F2 regression)", () => {
    it("reaches merge-preview with the pointer perfectly still", () => {
      // The defect: the model was only resolved on pointer-move events, so a
      // held pointer produced no resolution and the dwell first fired at drag
      // end - committing a split that had never been previewed. The model is
      // time-driven; the caller owes it a timer.
      const geometry = geometryFor([191, 191, 191], 0);
      const pointerX = pointerForCentre(geometry, ORIGIN + 191 + 191 / 2);
      ...
      expect(state.kind).toBe("merge-armed");
      // Not one further pointer event - only time passing.
      ...
      expect(state.kind).toBe("merge-preview");
    });

    it("reports the remaining dwell so a caller can schedule the wake-up", () => {
      const geometry = geometryFor([191, 191, 191], 0);
      const pointerX = pointerForCentre(geometry, ORIGIN + 191 + 191 / 2);
      const armed = resolveStripDragState({
        geometry,
      ...
      expect(armed.kind === "merge-armed" ? armed.armedAtMs : null).toBe(500);
    });
  });

  describe("per-strip band parameterisation (D10.3)", () => {
    it("keeps the header's band at 16 and the tile band at 0", () => {
      // Once bandPx is a per-caller parameter, a regression in the HEADER's
      // value is invisible to every tile test. Assert both values by name.
      expect(HEADER_MERGE_BAND_PX).toBe(16);
      expect(TILE_MERGE_BAND_PX).toBe(0);
    });
    ...
    it("makes merge unreachable on a tile strip", () => {
      // Band 0 plus isMergeTarget false: no pointer position and no dwell can
      // produce a merge state.
      const geometry = geometryFor(
        [130, 101, 192],
        0,
        [false, false, false],
    ...
    it("gives a tile strip hysteresis of exactly the dragged tile's width", () => {
      // D2.5: with bandPx 0 the derived gap is w_s alone, and "w_s" means the
      // DRAGGED tile's measured width - tile widths are unequal.
      expect(swapHysteresisPx(130, 101, TILE_MERGE_BAND_PX)).toBe(130);
      expect(swapHysteresisPx(130, 192, TILE_MERGE_BAND_PX)).toBe(130);
      expect(swapHysteresisPx(192, 101, TILE_MERGE_BAND_PX)).toBe(192);
    });

    it("stays non-alternating on unequal tile widths with no band", () => {
      const widths = [130, 101, 192, 118];
      for (let source = 0; source < widths.length; source += 1) {
        const geometry = geometryFor(widths, source, null, TILE_MERGE_BAND_PX);
  ...
  describe("explicit displacement offsets", () => {
    ...
    it("closes the gap when the tile has left the strip (D5.3)", () => {
      const geometry = geometryFor(
        [130, 101, 192],
        0,
        null,
      ...
      // Everything after the source slides left by the source's advance; the
      // source itself is not displaced - it belongs to another strip now.
      expect(offsets.get("item-1")).toBe(-130);
      expect(offsets.get("item-2")).toBe(-130);
      expect(offsets.has("item-0")).toBe(false);
    });

    it("never displaces anything left of the source when it leaves", () => {
      const geometry = geometryFor(
        [130, 101, 192],
        1,
        null,
      ...
      const offsets = stripOffsetsFor(geometry, null);
      expect(offsets.has("item-0")).toBe(false);
      expect(offsets.get("item-2")).toBe(-101);
    });
