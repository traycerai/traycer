 ...
 * `split` is the primary path: the section owns a `pr.getLocalFileDiff` query addressed by the summary's OID pair, with its own pending/error/ retry states - the Git Diff bundle row's architecture.
 */
...
/**
 * Without this, sections would key only by path: a summary refresh that resolves new OIDs (the local branch advanced), or a whitespace-mode flip, would carry a previously-approved `byteBudget: null` straight onto the NEW comparison and defeat the 256KiB guard - the same identity discipline `useEditableGitDiffSurface` applies via `fullDiffIdentity`.
 */
function sectionStateKey(
  ...
  return [
    ...
    // Tagged per-side identities, not the lossy display strings: two files whose replaced names collide must not share an approval, and a rename side is byte-addressed independently of its partner.
    prLocalDiffPreviousSideKey(file),
    prLocalDiffFileKey(file),
        // NUL-joined: it is the one byte neither a git path nor a base64 token can contain, so two fields can never collide into another identity's key.
  ].join("\0");
}

/**
 * Held at the FILES-VIEW level rather than as row state for one reason: the find session retains a loaded patch after its row unmounts (a collapse, or Virtuoso evicting it), and a retained patch must remount the SAME renderable bytes when a match in it is revealed.
 * Approvals and retention share one lifetime - the files view - and one identity discipline: an approval is granted for a comparison and expires with it, which is what keying by `sectionStateKey` gives (the key embeds the comparison and the whitespace mode), so a summary refresh that resolves new OIDs still cannot carry a `byteBudget: null` onto a comparison it was never approved for.
 */
...
/**
 * The patches come from the checkout the branch was pushed from, not from GitHub - GitHub's GraphQL changed-file list has no patch field at any page size, so this is the only source of a real diff short of a REST sweep per file.
 * That trade has one visible consequence, and making it legible is this component's job: the local checkout can be behind or ahead of what GitHub is showing, and when it is, the banner says so before the reader scrolls a single hunk.
 */
export function PrLocalDiffBody(props: {
  ...
  // The mode seam's normalization: monolith files become view files with `null` sidecars (legacy-unknown - a 1.0 host never reported byte paths), so every consumer below the seam keys files one way and the tagged keys all degrade to `p:` in fallback mode, by construction.
...
/**
 * A host too old for the method, a transport failure, and a PR with no `linkGroupKey` all land here without a real `unavailable` frame; none can name a specific cause, so they get the "no checkout" line rather than a guess.
 */
...
// No error-shaped variant on purpose: a transport failure has no `unavailable` frame, so it lands here with `reason === null` and renders the same "no checkout" line the pre-split view rendered for it - errors and genuine misses have never been told apart on this surface, and the per-file error blocks (split mode) are where a transient failure actually surfaces now.
...
function PrLocalDiffFilesView(props: {
  ...
  // Virtuoso restoration replaces the old native-div scroll restoration; the anchors are stored under a different kind for the same tile instance, so stale native offsets are simply never read again.
  const { virtuosoRef, restoreStateFrom, isScrolling } =
    useBundleDiffScrollRestoration(
      props.node.instanceId,
      props.files.length > 0,
    );
  // One find session for BOTH patch modes - the sections below register their coverage and loaded patches through its context regardless of where the bytes came from, so what "searchable" means cannot diverge between a new host and the old-host fallback.
  // Registered before the empty-range return so the hook order is stable across a range that empties and refills.
...
/**
 * Collapse state lives on the TILE (so it survives a reload and the toolbar's collapse-all can drive it), and a collapsed section renders no diff content at all rather than hiding it - in split mode that also means no fetch: the per-file query only exists while a section body is mounted.
 */
function PrLocalDiffFileSection(props: {
  ...
  // Re-notify when a collapsed section expands (find-driven or manual): the diff body only mounts while expanded, so a mount-only notification would leave a freshly-revealed match painted nowhere.
  useEffect(() => {
    if (collapsed) return;
  ...
  // Deliberately NOT `DiffBundleFileSectionFrame`: that frame carries an "open in editor" button, and a range diff has no file on disk to open - both endpoints are commits, and the working-tree copy of a path may hold neither side of the change.
  // The rest of the frame (sticky header, border rhythm, chevron) is reproduced so the two diff surfaces still read alike - including the two find identity attributes the frame stamps, which are how a revealed match is scoped to this section's DOM.
  return (
    <div
...
/**
 * Split mode: a large file renders a placeholder INSTEAD of fetching - the whole point of the summary knowing line counts up front - and the "Load diff" button swaps in the fetching body on demand.
 */
function PrSplitFileBody(props: {
  ...
  // The placeholder registers nothing with find: the session already files a large file under "large" coverage from the file list, and a find reveal deliberately does NOT press this button for the reader - the guard exists to keep an unbounded patch off the main thread until asked for.
  if (
...
/**
 * Monolith fallback: the patch (or its absence) is already known, so the large-file placeholder reveals inline content rather than fetching, and a `null` patch - a file past the whole-PR byte budget - can only point at GitHub, because a host in this mode has no per-file method to ask.
 */
function PrMonolithFileBody(props: {
  ...
  // `null` and empty are different facts and get different sentences: the byte budget never reached this file, versus the range genuinely changed nothing in it (a pure mode change, say).
  if (props.patch === null) {
    return (
      <PrLocalDiffNote>
...
/**
 * The fetching section body of split mode - the `BundleInlineDiff` of this surface.
 * Owns one `pr.getLocalFileDiff` query and its pending/error states, so one file's transient failure is one section's error block, not the tile's.
 */
function PrLocalFileDiffContent(props: {
  ...
  // Two section-observed facts about the tile's SPINE route to the same recovery, a summary refetch: `ref-unavailable` (the checkout no longer has the summary's OIDs - pruned, or moved and gc'd) re-resolves the range, and `E_HOST_UNSUPPORTED` (the HOST lost the method between the summary and this row - a downgrade, or a reconnect to an older build) makes the refetch itself fail unsupported, which is what flips the tile to the monolith fallback - the summary query is the split view's only capability probe and never re-asks on its own at `staleTime: Infinity`.
  // The recovery is BOUNDED at the tile so a repeatedly-failing range cannot loop.
  ...
  // The session retains a loaded patch past its section's UNMOUNT on purpose (the row can remount and show those bytes again), and a retained patch outranks any coverage state; but a mounted section past "Load Full" will never render the truncated bytes again - the approval only moves forward - so from the moment the new key is pending they are dead for this comparison, and leaving them indexed would have find match text that is not in the DOM, report the file as truncated rather than loading/failed, and park navigation on a skeleton.
  ...
  if (response.kind === "unavailable") {
    // Deliberately NOT the range-level `UNAVAILABLE_SENTENCE` map: "fetch the base branch" is advice about REF NAMES, and this miss is about OIDs the summary already resolved.
    // The tile-level recovery above is the fix.
    return (
      <PrLocalDiffNote>
...
/**
 * Registering here (rather than in each mode's fetch/lookup body) is what keeps "what find can search" identical between a new host and the fallback: a rendered patch is a searchable patch, and stays one after the row virtualizes away.
 */
function PrPatchContent(props: {
  ...
  // Registered BEFORE the hunk check on purpose: a header-only change has no rows to search, but leaving it unregistered would count it as "unloaded" in the coverage message forever, and it is fully loaded.
  useEffect(() => {
  ...
  // A patch with no `@@` hunk is a change git states entirely in headers - a pure rename, or a mode change.
  // Real, but there are no lines to render, and the diff viewer would draw an empty frame for it.
  const hasHunk =
...
/**
 * Mirrors the bundle tile's large-diff placeholder, except the button loads the diff INLINE: git's placeholder opens the file tile, and a range diff has no file on disk to open - both endpoints are commits.
 */
