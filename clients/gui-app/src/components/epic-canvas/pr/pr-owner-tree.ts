...
/**
 * A uniform set gets its own noun; a mixed one has no single true noun, so it falls back to the kind-neutral word rather than picking a side.
 * Lives beside the tree helpers rather than in `pr-owner-label.tsx` because both owner surfaces (the `+N` popover and the row's hover card) need it, and a component module that also exports a plain function breaks fast refresh.
 */
 ...
 * A PR's owner set is flat on the wire, but the nodes in it are not: a chat that spawns sub-agents (`agent.create` sets the child's `parentId` to its sender) contributes itself AND its children, and those children usually carry the parent's title.
 */
...
/**
 * Only owners are rows: an ancestor that is NOT in the set is skipped rather than rendered as context, so every row in the popover is still a chat this PR actually came from.
 * Sibling order is the caller's order, so the popover still lists owners in the order the PR reports them - just re-parented, never re-sorted.
 */
export function buildPrOwnerTree(
  owners: readonly PrOwnerRef[],
  tree: EpicTreeIndex,
): readonly PrOwnerTreeNode[] {
  // An owner's IDENTITY is the pair, matching how the host dedupes its own owner set and how the caller keys the chips.
  // Collapsing on `ownerId` alone would be a narrower key than the producer's, so two owners the host considers distinct could lose a row here while the `+N` on the trigger - counted from the undeduplicated array - still promised it.
  const present = new Map<string, PrOwnerRef>();
  for (const owner of owners) {
    const key = prOwnerKey(owner);
    if (!present.has(key)) present.set(key, owner);
  }
  // First owner wins, which only matters in the case the identity key exists to allow: one node id under two kinds.
  ...
  // Without it a cycle recurses forever, and its members are reachable from no root at all, so the post-pass below is what keeps them from silently vanishing out of a list whose whole job is completeness.
  ...
  // Re-checked as the loop runs, not filtered up front: building one cycle member emits the rest of its ring, and a snapshot taken before the loop would still hold them and emit each a second time.
  for (const owner of present.values()) {
