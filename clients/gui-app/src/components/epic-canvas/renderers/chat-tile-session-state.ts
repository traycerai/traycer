...
/**
 * Fallback harness id used when the inline-edit settings do not carry a resolved harness.
 * This occurs only while the composer settings are being initialised (before the first snapshot resolves the harness from the persisted chat settings or the epic/global run-settings seed).
 */
...
/**
 * The composer's turn-status prop shape - a strict subset of `ChatActiveTurn["status"]` (which also carries terminal values like `"completed"`/`"errored"` that never apply here).
 */
export type ComposerTurnStatus = "running" | "stopping" | null;

/**
 * Maps the host-owned chat `runStatus` onto the composer's turn-status prop shape.
 * `running` shows the stop button, `stopping` shows the "Stopping" affordance, `idle` returns the composer to its send state.
 */
...
/**
 * Background work already has its own "stop all background" control, so rather than show a Stop button that would fail, block a restore that isn't actually unsafe, or duplicate the row indicator after the real turn already settled, this falls back to `null` - exactly as if the chat were idle.
 * When it's null but `runStatus` still reads "running", process of elimination against the queue/background signals is the only way to tell a pre-turn "activating" window (active) apart from a queue-only or background-only one (not active) - both look identical on the wire otherwise.
 */
...
/**
 * Tri-state activity for the chat's progress indicators (sidebar tree, tab icons): is the agent actually processing, or is only background work (Bash `run_in_background` / Monitor / a scheduled wakeup) keeping the chat non-idle?
 * `runStatus` alone can't tell the two apart, and showing the same spinner for both left users unable to see whether the agent was really running.
 */
...
/**
 * `state.messages` cannot answer it on the windowed line.
 * It is a bounded, evictable slice, so "the replacement row is not there" conflates "the edit never landed" with "the reader has scrolled away from it", and the second is the ordinary steady state for an edit made a few turns ago.
 */
...
export function normalizeInlineEditForSession(
  ...
  // The last branch is still right for what it was written for - a REJECTED dispatch, which must return the composer to an editable state - and that is precisely why the success case has to be answered from something that does not expire.
  if (
...
export function canModifyChatMessages(input: {
  ...
  // Narrowed via `resolvedTurnStatus`, not the raw `runStatus`: the raw value also reads "running" while visible background work outlives the settled turn (Bash `run_in_background` / a subagent / Monitor), which would keep edit/delete hidden indefinitely with no turn to wait for - the composer and restore gating already read the narrowed value for the same reason.
  // The narrowed signal still covers the windows `activeTurn` misses: the pre-turn `turnActivating` phase (provider/worktree setup) and stop-during-activation both keep the host's `turnInProgress` true until the run truly unwinds.
  if (
    resolvedTurnStatus(
      input.state,
  ...
  // Queued items do NOT gate history mutation: they carry only content/sender/settings, survive the rewrite untouched, and later send against the new head (the host dropped its QUEUE_NOT_EMPTY guard for the same reason).
...
/**
 * The transcript half of that question cannot be answered from `messages` on the windowed line: it holds what is HYDRATED, so a chat long enough to have been windowed can present no user row and re-trigger title generation on a chat that has had a title for weeks.
 * Both sources are consulted rather than one replacing the other.
 */
...
/**
 * The chat's most recent completed-turn fork boundary, or `null` when none exists yet - the agent has never replied, or its only assistant rows are still live.
 */
...
/**
 * Between the two there is a window in which the gesture is allowed and the boundary still names the PREVIOUS turn, so a fork silently omits the turn the user just watched finish.
 * Two clocks, and only one of them ticks live.
 */
...
// Unlike `forkableAssistantMessageId` it does NOT require the turn to be finished (`completedAt`/`runState`) - question-level fork actions remain available while the assistant resumes after an answer.
...
export function findPendingInterview(
  ...
  // Walk from newest to oldest on BOTH axes.
  // When a single assistant turn contains multiple AskUserQuestion calls in sequence, the latest one is the one currently awaiting input - older ones may still be visible as "streaming" briefly while their resolution event is in flight.
...
// `findUnanswerableInterviews` runs off `renderedMessages`, which changes on every streaming token, so returning a fresh `[]` would churn the composer memo chain on every token - the exact regression chat-tile-composer-rerender.test.tsx pins.
const NO_UNANSWERABLE_INTERVIEWS: ReadonlyArray<UnanswerableInterviewView> = [];

/**
 * `findPendingInterview` renders a card only for a `streaming` interview block, so a host-pending blockId whose block is already settled - or missing from the transcript entirely - yields nothing to answer while the host keeps rejecting sends.
 * This is the disjoint complement of `findPendingInterview` over the host's pending set: every id here has no streaming block, so the two can never name the same block.
 */
export function findUnanswerableInterviews(
  ...
  // Only the ids the host judged UNRENDERABLE.
  // Both other states - judged and placeable, or not judged at all - fall outside this set, which is why the check below is membership rather than a lookup with a default.
 ...
 * `liveTurnUsage` takes precedence over the persisted value so the chip shows live in-flight numbers during a turn and carries the final usage forward across the gap between `turn.completed` and the next snapshot.
 */
export function selectContextUsage(
  ...
  // Deliberately not `?? findLastAssistantUsage(...)`: `latestAssistantUsage: null` is the real answer for a chat where no assistant row has reported usage yet, and the chip's empty form is what that should render.
  // Falling through would put the O(history) scan back on every fresh chat, and let a hydrated row contradict the host on a chat where the host can see further.
