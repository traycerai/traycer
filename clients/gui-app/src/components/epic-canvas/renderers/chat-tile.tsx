...
// Not tied to the send settling - just long enough that a double-click or double-tap can't fire the compaction twice.
const COMPACT_ACTION_LOCK_MS = 1500;

/**
 * Stable identity for the legacy line's "no whole-log partition".
 * A fresh `[]` per render would change the identity of a `useMemo` dependency every time and re-partition the setup lifecycle on every streamed token.
 */
...
interface ChatTileProps {
  ...
  tileId: string;
  /**
   * The value is drilled into `ChatComposer` so only the active tile's composer registers itself with the focused-composer-controls registry that powers the command palette's "Switch model" etc.
   */
  isActive: boolean;
}
...
/**
 * Chat history rendered from `chat.subscribe`.
 * The Epic session still supplies the chat tile identity, title, tree placement, and mention catalog, but chat content now comes from the host-owned per-chat stream.
 */
export function ChatTile(props: ChatTileProps) {
  const { node, viewTabId, isActive } = props;
  const epicId = useOpenEpicId();
  // The gate exists for exactly one race - a chat created here, subscribed to before the create landed - so the bar is evidence of existence, not evidence of a doc record.
  const chatRecord = useChatById(node.id);
  const tabHostId = useTabHostId();
  // A CROSS-HOST live open (a connected peer host's chat, opened from the unified sidebar) may never get a local projection record at all - the chat lives in the owner host's registry, not this device's.
  // The record gate exists only to close the local-first subscribe-first race, and that race is a same-host phenomenon: the chat was just created on the host that was active here.
  const hostBinding = useHostBinding();
  // The three record-gate readers (`tab-group-view`, the route sync, this) resolve the one identity.
  const projectionHostId = useCanvasHostId();
  const [isCrossHostOpen] = useState(() => {
    // Only a KNOWN, different host earns the exemption.
    // RESOLVED against the directory, not the derived id alone, because that is what the active slot answered before P4.2 deleted it: a host whose row has not arrived was `null` here, and the ignorance arm above is written for exactly that state.
  ...
  // Consulted only when the two gates above have already refused, so the ordinary live path costs nothing extra.
  // This cannot reopen the create race it replaces evidence for.
  ...
  // A local chat ref is the viewer's own by construction, so an id-only match could open a collaborator's chat under this tab.
  const isCloudKnown =
  ...
  // The reader cannot act on the difference, so all three get one sentence naming the host - and an end.
  const chatLoad = useBoundedHostLoad({
    hostId: tabHostId,
    hostLabel: resolvedHostLabel(reachability),
    pending: handle === null,
  });
  // The provider is handed `tabHostId` alongside it because this list is evidence about a profile only for anchors minted on THIS host - an anchor a fork carried from another machine names a profile id that is host-local there and can never match here.
  const providersList = useTabProvidersList({
    enabled: true,
    subscribed: false,
  });
  // Mount it only when the banner is actually shown so the live render path does not pay the subscription cost (and tests that omit the host runtime provider stay green).
  const deadTileBanner = (() => {
    ...
    if (reachability.status === "host-starting") {
      // Never offer Clone here - the bound host is most likely this machine, seconds from converging; cloning would fork a healthy thread.
      return (
        <ChatHostStartingBanner
          className={undefined}
  ...
  if (handle === null) {
    return (
      ...
      >
        {deadTileBanner}
        {chatLoad.kind === "ready" ? (
          // Unreachable while `pending` is `handle === null` and we are inside that branch, but written as a fallback rather than a cast: the spinner is the strictly safer thing to render if that ever stops being true.
          <ChatTileLoading />
        ) : (
          <TileHostLoadState
            load={chatLoad}
...
interface ChatDeadTileBannerContainerProps {
   ...
   * The published tile and the canvas substitution do; this live tile does not (the body below it is a load state or a cached live session).
   */
  readonly showsPublishedCopy: boolean;
  /**
   * `""` is treated as absent, not forwarded: the wire field is `z.string().min(1).nullable()`, so an empty owner would turn a clone that should degrade to settings-only into an outright failure.
   */
...
export function ChatDeadTileBannerContainer(
  ...
  const providedOwnerUserId =
    props.sourceOwnerUserId !== undefined && props.sourceOwnerUserId.length > 0
      ...
      : null;
  // A surface that already knows the owner (the published tile's ref names it) passes it instead, and a `null` chatId keeps the hook's cloud query disabled - no lookup runs for an answer the caller already had.
  ...
  // Ownership: only a POSITIVE mismatch against the signed-in user flips the foreign-owner copy - an unknown owner or identity stays on the own-chat sentences, which were this banner's whole vocabulary before collaborators existed.
  // Role: `epic.createChat` is editor-gated host-side, so a known viewer gets the reason in the banner instead of a Clone button that dies on a bare "You don't have permission" toast; an unresolved role (`null`) keeps the offer - the host gate is the backstop, and withholding the way out of a dead tile needs evidence.
...
// Both a plain agent and a workflow run render as a `subagent`-block card (a workflow is a dedicated rendering of that same block, never a distinct persisted type), so either kind opens via the same subagent open-store.
...
/**
 * A nested agent - and anything it owns (commands/monitors, or a workflow's fleet-attributed background work) - has no card of its own in the transcript; it only renders inside its top-level ancestor's "Sub-agents" section.
 * Clicking its panel row must therefore scroll to and expand the ANCESTOR card, walking up `parentTaskId` until a top-level item (null parent) is reached.
 */
...
/**
 * How long a parked cross-tile transcript jump waits for its target row to stream in before it is dropped.
 * Generous enough to cover a cold tile pulling a large transcript, short enough that a stale request cannot fire minutes later and yank the reader somewhere they no longer expect.
 */
const TRANSCRIPT_JUMP_TTL_MS = 30_000;

/**
 * Which open-store a cross-tile block jump should expand.
 * A block that names a live background item follows that item's card kind; anything else (a settled tool card - the usual shape for a file-write anchor) opens as a tool card.
 */
...
export function ChatTileSessionView(props: ChatTileSessionViewProps) {
  ...
  // Viewport → hydration bridge: `ChatMessages` computes which ordinals the reader is looking at; the session store turns that into range requests.
  ...
  // Chat image byte reads are scoped here, once per tile, rather than per rendered image: resolving the routed client is a directory-query subscription, and a transcript can hold a hundred thumbnails.
  const attachmentHostClient = useTabHostClient();
  // The BUILD, not just the id: a host can be upgraded in place under the same `hostId`, and the attachment fetcher remembers its "predates `epic.readChatAttachment`" verdict per build so the upgrade re-probes.
  ...
  // The composer + queue/pinned/agents/background dock now overlays the transcript (decision log #3, #13) instead of pushing it via flex height, so its reply stream can flow visually behind it.
  const {
    setElement: setLowerSurfacesElement,
    element: lowerSurfacesElement,
    height: lowerSurfacesHeight,
  } = useMeasuredElementHeight();
  // Shared transcript jump: resolve the owning message, expand the card via its open-store, and bump the scroll request the messages surface watches.
  // Both the Background panel rows and the autonomous-resume marker route through here so the two navigations behave identically.
  ...
  // Anchor on a message rather than on a card.
  ...
  // Parked in a store rather than called directly because the jump is issued from another tile, possibly before this one exists - `openTile` mounts it and the request is waiting here when it renders.
  ...
  // Reached through the handle's store rather than the `view` projection: this is an ACTION, stable for the store's life, so routing it through the reactive selection would put a new identity in the effect's deps on every frame.
  const requestTranscriptOrdinal = useStore(
    viewHandle.store,
    (s) => s.requestTranscriptOrdinal,
  );
  // A jump target this client cannot place on its own, once it is clear it cannot.
  ...
  const hostLocatedOrdinal = useChatLocateRow({
    ...
    // An ordinal numbered in another one is discarded rather than jumped to - see the hook's own doc.
    // `null` is the legacy line, which has no ordinal space at all - and no cold rows either, so `hostLocatorTarget` is never non-null there and the query never runs.
    epoch: view.transcriptWindow?.epoch ?? 0,
  });
  // The chat transcript streams independently of the graph stream, so a warm tile routinely learns about a message from the timeline BEFORE its own stream delivers the row.
  // Installing the scroll request then would burn it: the transcript marks the request handled and only afterwards discovers it has no index entry for that message, and the row arrives to find nothing parked.
  useEffect(() => {
    ...
    if (target.kind === "end") {
      ...
      // Same release as the resolved-target path below, and it returns before reaching it.
      // An `end` jump that REPLACED a parked request is the case: changing `pendingTranscriptJumpId` clears the previous request's TTL timer, and this pass consumes the new one, so the TTL effect returns early and never runs its release either.
    ...
    const resolveTargetMessageId = (): string | null => {
      ...
      if (target.kind === "first-message") {
        // Ordinal 0 of the WHOLE transcript, not the first row this client happens to hold.
        // On the windowed line `view.messages` is a bounded slice, so `messages[0]` is the top of the hydrated tail - and "jump to the first message" then navigated confidently to the middle of the chat, which is worse than not moving at all.
        const firstRowId =
    ...
    if (messageId === null) {
      // Unresolved, and on the windowed line that is routinely because the target row is COLD rather than because it does not exist yet.
      // Only for a target whose ROW ID is derivable client-side - a user message and an event anchor, whose row ids are the message id and `chatTranscriptEventRowId`.
    ...
    consumeTranscriptJump(hostId, props.node.id, transcriptJump.requestId);
    // Doing this AFTER the consume rather than beside the resolve keeps the request alive across the beat between the two.
    requestTranscriptOrdinal(null);
  }, [
    consumeTranscriptJump,
    hostId,
    // The host's answer arrives asynchronously, so it is the retry signal for the two target kinds that need it - exactly as `view.messages` is for every other kind.
    hostLocatedOrdinal,
    props.node.id,
    requestTranscriptOrdinal,
    scrollToBlock,
  ...
  // ...but a target that never arrives must not wait forever.
  // A jump that cannot land is not an error worth interrupting the user over - the tile is open on the right agent either way, which is the degrade this feature already accepts for anchor-less rows.
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      // The ordinal goes with it, and this is the ONLY place that can release it on this path.
      // Safe without an id check of its own.
      requestTranscriptOrdinal(null);
  ...
  // The chat components receive only inert row handlers; they do not know about canvas stores, tab ids, or tile factories.
      ...
      <ChatDiffTargetContext.Provider value={diffOpener}>
        <ChatScrollToBlockContext.Provider value={scrollToBlock}>
          <div
            ...
            className="flex h-full min-h-0 flex-col"
          >
            {/* A flex CONTAINER (not just an item): ChatSessionMessagesSurface's transcript root relies on `flex-1` from ITS immediate parent to get a definite height (h-full on LegendList needs a real containing block all the way up). The overlay dock below is absolutely positioned, so it does not participate in this flex layout regardless. */}
            <div className="relative flex min-h-0 flex-1 flex-col">
              ...
              />
              {/* A visible split partner keeps rendering its transcript and scroll state, but releases catalog/provider query observers and cannot own palette/composer-global work. Absolutely overlays the transcript (decision log #3) instead of pushing its height via flex, so streamed replies flow visually behind it; `lowerSurfacesHeight` (measured here) feeds the transcript's bottom content inset. */}
              {view.snapshotLoaded ? (
                <div
...
// eslint-disable-next-line complexity -- independent UI concerns for one tile, not reducible nesting
...
/**
 * A send held by the rebind-consent dialog: the input to re-dispatch on confirm, the PATH-SPECIFIC dispatch that must run it - a confirmed compaction still needs its lock and queue promotion, not the composer's title bookkeeping - and the path's LIVE eligibility recheck.
 * The values a path examined at click time may be a whole open dialog older by the time the user confirms (a blocking approval can arrive, the turn can start stopping), so `refusal` reads current state and returns the sentence to surface instead of dispatching, or `null` when the send is still eligible.
 */
type GatedChatSend = {
...
// The composer submit's own eligibility (access + sign-in) is re-checked live by the dialog's confirm handler itself, so its slot carries the always-eligible refusal.
// Module scope for a stable identity.
...
function useChatTileSessionViewModel(props: ChatTileSessionViewProps) {
  ...
  // Surface visibility for the stream-flush coordinator's tiered flush rate: on screen = the pane is shown AND this tab is the pane's front tab.
  ...
  // Label-only, cache-only projection: this tile's own composer (which fetches the TAB host's catalog) owns the fetch; this reads that host-keyed cache (never fetches) so ANY visible transcript - including a restored terminal-focused split with an inactive chat and no live catalog publisher - renders friendly model/reasoning labels immediately, and a host/user switch re-keys the queries and swaps labels.
  // ONE slot, the tab host's - never layered over the default host's.
  ...
  // The handoff state is owned by `useInitialChatHandoffDriver` below; this component does not subscribe to the handoff store to avoid re-rendering the entire tile whenever the handoff transitions.
  const state = useStore(
    ...
    useShallow((s) => ({
      ...
      messages: s.messages,
      events: s.events,
      // Written in the same `set` as the two arrays above, so subscribing costs no extra render and no frame can render rows against the previous hydration's context.
      transcriptRowContext: s.transcriptRowContext,
      // Both already change identity on every windowed frame (they are rebuilt from the window), so subscribing to the window itself costs no extra render.
      // The revert-scope resolution needs it to know whether the two arrays are the whole transcript or a slice of one.
  ...
  // A chat's mention roots are its own working directories, taken from the per-device worktree binding (the source the host workspace selector renders).
  // The epic snapshot's workspace folders are a separate, epic-level set that can be empty even when the chat is bound to a folder.
  const mentionRoots = useMemo(
    () => mentionRootsFromWorktreeBinding(state.worktreeBinding),
    [state.worktreeBinding],
  );
  // Composer-scoped roots: the staged worktree intent layers over the binding (`stagedEntry ?? bindingEntry`), matching what the send path will materialize.
  // Next-message surfaces - the composer's mention search and slash-command discovery, and the inline-edit composer whose resend also carries the staged intent - read these, so a staged replacement stops discovery from probing the superseded (possibly deleted) worktree path.
  ...
  // Explicit folderless mode disables that fallback so workspace file/folder links don't resolve through unrelated global roots.
  const linkResolutionRoots = useWorkspaceMentionRoots(
    mentionRoots,
    !isFolderlessWorkspace,
    activeHostId,
  );
  // The context-usage chip's own catalog lookup shares this rather than the raw `composerMentionRoots`, so it lands on the SAME `agent.gui.listCommands` cache entry the composer already warmed instead of opening a second one with a different (and, on a folder-fallback chat, narrower) working directory set.
  const resolvedComposerMentionRoots = useWorkspaceMentionRoots(
    composerMentionRoots,
    !isFolderlessWorkspace,
    activeHostId,
  );
  // The workspace selector itself stays owner-scoped to the chat's binding so sibling chat folders do not appear in this chip.
  ...
  // Pair the missing-folder send-disable with an on-focus / pane-activation re-check so restoring a deleted folder clears the disable without a send or reload.
  // A locked published copy has no send gate to recover, so its retained surface never enables this activation query.
  ...
  // In-progress UI (restore gating, owner-active, the per-row "Working…" / "Stopping…" indicator below) is driven by the host-owned chat `runStatus` - the single source of truth that covers the first turn and every multi-turn send and flips to `stopping` the moment a stop is requested.
  // We map it onto the composer's turn-status prop shape (`running`/`stopping`/null).
  const activeTurnStatus = composerTurnStatus(state.runStatus);
  // Several consumers below (the row indicator, the composer Stop/Send toggle, restore gating) need a narrower question than the label above: `runStatus` also reads "running" while a queued item is pending or visible background work outlives the turn (Bash `run_in_background` / a subagent / Monitor) - neither of which corresponds to an active turn they can act on or attribute an indicator to.
  // See `resolvedTurnStatus`'s doc comment for the exact derivation.
  ...
  const renderedMessages = useRenderedMessages(
    {
      ...
      // Chat-level rather than per-row, and that is forced: a setup card's row id contains the window index, so a client that renumbered cannot look its own correction up by row id.
      setupCardWindows:
      ...
      // Narrowed, not the raw `state.runStatus`: this drives the per-row "Working…"/"Stopping…" indicator, which belongs to a genuinely active turn - passing the raw value synthesizes a duplicate, live indicator row during background-only phase (no active turn) even after the real row has already settled to its "done" footer.
      runStatus: composerActiveTurnStatus ?? "idle",
      // Binding identity for the in-transcript setup card (replaces the old strip's mount-time tuple): epic + chat owner route the retry mutation and scope the terminal-liveness query; `viewTabId` rides the synthetic segment for the focus-terminal path.
      epicId: currentEpicId,
      ownerId: node.id,
      ownerKind: "chat",
      viewTabId,
  ...
  // Only a prompt item can be loaded into the composer for editing, so narrow here rather than at each consumer: this feeds the composer's settings seed and its remount key, neither of which a content-free managed-command item could supply.
  ...
  // Single owner for the initial-chat-handoff state machine.
  // Replaces five sibling effects that previously coordinated handoff failure detection, failed-send restoration, sending→consumed transitions (via acceptedActions or via persisted messages), and the waitingChat→sendMessage→markSending hop.
  ...
  // Surface the server-spawned setup terminal in the Terminals sidebar while it runs - its PTY isn't created via the renderer, so nothing else refetches `terminal.list`.
  ...
  // Gate on a KNOWN non-owner (access resolved AND not the owner) rather than a positive `role === "viewer"` check, so any non-owner is treated as read-only.
    ...
    () =>
      Object.values(state.pendingActions).some(
        (action) =>
          ...
          // A revert-on-edit runs its cumulative revert under the `editUserMessage` action (before the new turn starts), so include it here to keep the accumulated-changes panel locked during it.
  ...
  // Carried BESIDE the count rather than derived from it downstream, because the two answer different questions and differ in exactly the case that matters.
  ...
  const restoreContext = useMemo(
    () => ({
      ...
      currentUserId,
      activeHostId,
      // Restoring/reverting files while a turn is actively writing is unsafe, but that's a turn-scoped concern, same as the composer's Stop button - `runStatus` alone also reads non-idle during background-only phase (Bash `run_in_background` / a subagent / Monitor with no active turn), which restore/revert can't conflict with.
      // Use the same narrowed value the Stop button uses instead of the raw one, or this would show "Wait for the active turn to finish" and block restore during a window where nothing is actually running against it.
  ...
  // Memoize: `currentSettingsForChatTile` returns a fresh object every call, so without this `currentComposerSettings` churns identity every render.
  ...
  // The tile's own send paths - next steps, compact, inline edit - never touch the composer, so they cannot read the catalog off its picker store.
  // Locked published copies cannot invoke those actions and stay detached.
  ...
  const {
    ...
    // `resolvedComposerMentionRoots`, not the raw roots, for the same reason the context-usage chip above uses it: on a folder-fallback chat the two differ, and the raw set opens a SECOND, narrower cache entry - losing the dedupe and resolving against a catalog the composer never saw.
  ...
  // Null until loaded, which makes a `$` prompt stay plain text rather than chip against a catalog we have not seen yet.
  // A failed `listCommands` leaves TanStack at `isLoading === false` with `data === []`, which would otherwise read as a legitimately EMPTY catalog - "loaded, this workspace has no commands" - and chip nothing while looking resolved.
  ...
  // On the windowed line the rows are the HYDRATED SUBSET and the fold's answer comes from the host's whole-transcript copy instead - a todo created outside the hydrated spans would otherwise vanish from the dock.
  ...
  // Rendered in the composer slot; inline rendering is suppressed in `chat-message-assistant-body.tsx`.
  ...
  // Recomputes only when actions change (not per streaming token), and yields a stable `false` whenever no interview is pending, so the composer memo below never churns during normal streaming.
  ...
  // Host-pending blocks this transcript renders no card for.
  // Yields a stable empty array whenever nothing is stuck, so the composer memo chain below does not churn per streaming token.
  ...
  // All pending approvals route to the composer slot - single or many share one canonical surface.
  // Inline rendering for pending approvals is suppressed; resolved approvals stay inline as turn history.
  ...
  const { messageActionsFor, forkAtAssistantMessage, revertOnEdit } =
    useChatMessageActions({
      ...
      messages: state.messages,
      events: state.events,
      // `transcriptDerived !== null` is the line discriminator: on the legacy line the window is an inert empty value and `messages`/`events` are already whole, so handing it over would make the revert scan think it was looking at an empty transcript and answer "unknown" forever.
  ...
  // The latest completed boundary ID is stable across flushes (a streaming row is never forkable), so the gesture handler hanging off this stays quiet while a turn streams.
  // On the windowed line the scan cannot run here - `renderedMessages` is the hydrated subset, and the latest completed boundary is routinely outside it (scrolled cold, or evicted).
  ...
  // A chat mid-turn has no boundary that includes the turn the user is watching, and one that has never replied has no boundary at all; both say so instead of opening a dialog pointed at something else.
  ...
  const dispatchUserSend = useCallback(
    (input: ChatComposerSubmitInput): boolean => {
      ...
      // Read whole from the store rather than from the tile's `useShallow` selection.
      // The transcript window changes on every windowed frame, so subscribing the tile to it would widen a hot render path for a value only this handler reads - and reading it at SUBMIT time is what the question is about anyway.
  ...
  // A path that calls `chatActions.sendMessage` without passing through here reopens one of two holes: a phantom disclosure on a no-op draft, or a staged rebind committing silently.
  const submitThroughRebindGate = useCallback(
    (send: GatedChatSend): boolean => {
      ...
      // The disclosure is consent for the rebind a send would commit.
      // A send whose draft commits nothing (none staged, or one that restates the committed binding) has no rebind to consent to - gating it would put "Send in the new folder?" in front of every steer of a busy agent.
      if (
        !worktreeDraftCommitsRebind({
  ...
  // The tile contributes what only it knows about its own chat - owner, title, lineage, workspace, pane - and `startSideChat` owns the create, its recovery, and the open.
  ...
  // `canSendNextStep`, re-derived from LIVE store state for the consent dialog's deferred dispatch.
  // The render-scope boolean above is what the click checked, and it can be a whole open dialog stale by confirm time - reading the stores imperatively is the point, not a shortcut.
  ...
  // Never interrupts: with a turn running (or work already queued) the compact command is queued and then promoted to the front so it runs next, and only an otherwise-idle chat compacts outright - queueing there would just park a one-item queue the user has to release by hand.
  // `runNow` has no effect on the host - `deliveryPolicy` only ever branches on `after_safe_point` - it purely decides, client-side, whether the promotion watcher below needs to be armed at all.
  ...
  // The compaction's own dispatch behind the rebind gate: the lock, the queue-or-run-now policy, and the promotion belong to the moment the send actually leaves - a consent dialog may sit between the click and this, so deciding them at click time would act on stale turn/queue state (and take the double-click lock for a compaction that never fired).
  const dispatchCompactSend = useCallback(
    (input: ChatComposerSubmitInput): boolean => {
      ...
      // A cheap re-entrancy guard against a double-click firing two real compactions: the optimistic-queue dedupe only suppresses the second row's on-screen echo, not the frame that already went to the host.
  ...
  // Implement-plan is a submit like any next-step, so it takes the SAME eligibility rule at every point: the button (`canSend` below), this click gate, and the consent dialog's deferred confirm (`nextStepSendRefusal`).
  // A bare `canAct` here once let a click send over a blocking file-edit/tool approval that the composer and every next-step correctly refuse - and worse, made the deferred confirm refuse a send the immediate path allowed: one click, two behaviors.
  ...
  // Durable settings sync: mirror composer selection changes onto the host's per-chat record so headless turns (incoming A2A messages) run on the freshly picked profile.
  // Best-effort - an old host rejects the optional method with E_HOST_UNSUPPORTED and behavior degrades to persist-on-send.
  ...
  // The chip renders as a sibling block below the composer (mirroring the landing page) so the input box stays focused on prompt editing and the binding affordances live alongside it.
  // The selector reads the chat session's `worktreeBinding` (populated from `chat.subscribe`) and the cascading menu drives create / import / re-bind through the existing modals.
    ...
    () => (
      <HostWorkspaceSelector
        ...
        surface={{
          ...
          // Distinguishes WHY the owner reads active, for the disabled-remove tooltip wording only (the disable decision itself stays on the broader `isOwnerActive`, unchanged - a live background Bash/Monitor could still be reading or writing in the folder even with no foreground turn active).
          // `false` here means it's active purely because of visible background work, not a turn the "stop" wording would make sense for.
          hasActiveTurn: composerActiveTurnStatus !== null,
          ownerLabel: node.name,
  ...
  // A second index over the same shells crowded the composer without adding a capability.
  ...
  // Steer capability is a stable boolean (flips only when the running turn's harness changes), so it never churns the memoized composer per streamed token.
  ...
  // A worktree-creating send holds the host's per-chat serializer so the final workspace binding is established before later queue actions run.
  ...
  return {
    ...
    activeTurnId,
    // Same line discriminator as the revert-scope resolution above: on the legacy line the window is an inert empty value whose `rowCount` of 0 would make the merge treat every rendered row as an unplaced tail row.
    ...
    teardownCommit: {
      ...
      onImmediate: () => {
        ...
        // The armed path's OWN eligibility, re-read from live state: a blocking approval or a stopping turn can arrive while the dialog sits open, and the click-time check cannot see it.
        // Peeked before taking the armed submit so a refusal keeps the dialog and its armed send (and the staged draft - consent was for a send that did not happen), matching the access refusals above.
        ...
        // Same bar as arming the dialog: a live draft that commits no rebind has nothing left to re-consent to, so a drift that CLEARED the rebind sends rather than re-disclosing holders forever.
...
function ContextUsageChipForChat(props: {
  ...
  // `workingDirectories` is the composer's own resolved mention roots (`resolvedComposerMentionRoots` in the parent), not the raw chat binding - that's what makes this the SAME `agent.gui.listCommands` cache entry `useKnownSlashCommandNames` already warms, not just a query sharing its activation gate.
  // An actionable tile therefore pays no extra RPC, and an inactive one still fetches nothing and shows no compact affordance - it also has no focusable composer to compact from.
  ...
  // The catalog is matched on `providerKind`, not on name (a differently named compaction command is what this is for), so the literal text this sends has to come from the matched command rather than a hardcoded guess.
  const onCompact =
...
function ChatSessionMessagesSurface(
  ...
  // A fatal close before any snapshot (CHAT_INVALID, CHAT_NOT_VISIBLE, …) means the host will never send one.
  // Surface the reason + a retry instead of an indefinite spinner.
  if (!props.snapshotLoaded && props.fatalClose !== null) {
    return <ChatTileError details={props.fatalClose} onRetry={props.onRetry} />;
  }
  // Show the loading skeleton until the real `chat.subscribe` snapshot lands (~0.5s - the host is local-first).
  // The snapshot then renders the user message + real turn state in one transition; there is no optimistic seed.
  if (!props.snapshotLoaded) return <ChatTileLoading />;
  // Pick the in-progress "thinking" verb once per turn, seeded on the chat plus the RUNNING TURN's id - NOT the indicator row id, which flips from `assistant:live` to `assistant:<turnId>` mid-turn and would otherwise reshuffle the word.
  // On the windowed line `messages` is a bounded, evictable slice rather than the transcript, so that count moved as the reader scrolled through cold history and the verb changed underneath a running turn - against this component's own stability guarantee, and visibly.
...
function useChatTileComposerSettingsSeeds(input: {
  ...
  // This tile's composer is bound to the TAB host for life, so its last-run fallback seeds read that host's buckets and the on-send write below lands in them - another host's remembered settings never leak into this tab.
  const tabHostId = useTabHostId();
...
/**
 * This is what makes the composer's missing-folder send-disable recoverable: `worktree.getBinding` recomputes `missingWorktreePaths` server-side, so restoring a deleted folder and returning to the window lifts the disable without a send or reload (the on-send re-stat is otherwise the only recompute trigger, which a disabled composer can never reach).
 * Scoped to the visible SURFACE (pane visible AND this tab selected) via the `enabled` gate so backgrounded keep-alive chats - including a non-front tab stacked in the same visible pane - don't all re-stat on every window focus; selecting the tab re-enables the query (with `staleTime: 0`) and refetches, which doubles as the surface-activation re-check.
 */
...
/**
 * An imported chat overrides the PROVIDER with the one it was imported from - see `importedChatSettingsSeed` - because a chat whose own settings never resolved would otherwise continue a Codex transcript under whatever the user last ran.
 */
...
/**
 * Committing an unresolved seed would freeze a model this hook only guessed, and `state.currentComposerSettings` outranks the composer's fallback seed for the rest of the chat's life.
 */
...
/**
 * Above that slot is only what the user is doing right now - a live toolbar edit, or the queue item open for editing.
 */
...
function useCachedCollaborators(
  ...
  // Cache-only read (`enabled: false` below) must key where the cache is WRITTEN, not where this tile happens to be bound: `epic.listCollaborators` is only ever filled by the sidebar tree and the sharing panel, both keyed on the Epic SESSION's host (`useEpicSessionHostClient`).
