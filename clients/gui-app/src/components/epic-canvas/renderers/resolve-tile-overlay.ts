...
/**
 * Which loader/terminal surface the tile overlay shows.
 * The wire status union is only `loading | ready | dead`; `stalled` is a client-derived terminal state for a `loading` that went silent, not a new wire phase.
 */
...
/**
 * Pointer blocking is gated on the guest not yet being interactive - never on the same flag that hides the overlay - so a live, presented guest is never click-blocked by a stale loader.
 */
