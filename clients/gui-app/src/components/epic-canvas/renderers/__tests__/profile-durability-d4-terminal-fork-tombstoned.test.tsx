...
/**
 * D4 (durability audit), end-to-end: "Fork dialog seeded from a chat whose profile is now tombstoned: falls back to ambient for the new selection; no crash." Mirrors terminal-agent-fork-dialog.test.tsx's mocking shape (only the picker/agent-mode-toggle/workspace-controls/harness-catalog are stubbed - `useComposerToolbarStore` / `createComposerToolbarStore` are the REAL production store), so this exercises the actual code path that decides what `profileId` gets sent to `createAgent.create`.
 * A cold reviewer flagged the FIRST version of this fix: the seed-resolution hook originally read `providers.list` from the APP-WIDE ACTIVE host (`useProvidersList`) instead of the host the fork's `createAgent.create` call actually targets (this dialog's explicit `hostClient` prop).
 */
...
// This suite's dialog target is always plain-fork intent, so the bulk fork-admission preflight this hook backs is never invoked - stubbed purely so the dialog's unconditional `useValidateTuiForkProfile` call doesn't hit the partially-mocked `use-host-query` module below (which only implements `useHostQuery`, not the mutation-lifecycle helper this hook also imports).
...
// A decoy standing in for "the app-wide active host" (or any other host) - something the dialog must NEVER read profiles from.
// Never passed as the `hostClient` prop in any test below.
...
describe("D4: TerminalAgentForkDialog seeded from a tombstoned profile", () => {
  ...
  it("holds the seeded profileId verbatim while the target host's providers.list hasn't loaded yet (unsettled) - never false-positives an undetermined profile to ambient", async () => {
    // No entry registered for TARGET_HOST_CLIENT at all - the `useHostQuery` mock returns `data: undefined`, the genuine "still loading" signal `resolveSeededProfileId`'s `settled` param must gate on.
    dialogMocks.create.mockResolvedValue("forked-agent");
    render(
      <TerminalAgentForkDialog
        open
  ...
  describe("cross-host: validation must read the fork's OWN hostClient, never the active/decoy host", () => {
    it("source profile alive on the TARGET host but absent on the decoy host must be PRESERVED (no false null)", async () => {
      ...
      // A decoy "active host" - never passed as `hostClient` - claims the profile doesn't exist there.
      // If the dialog ever silently fell back to reading from an app-wide active-host client instead of its own prop, this would wrongly null the profile.
    ...
    it("source profile tombstoned on the TARGET host but alive on the decoy host must resolve to null", async () => {
      ...
      // If the dialog ever silently fell back to reading from an app-wide active-host client, this would wrongly preserve a dead id.
      dialogMocks.providersByClient.set(DECOY_ACTIVE_HOST_CLIENT, [
        claudeState([
          profile("ambient", "ambient", "Terminal account"),
          profile("work-uuid", "managed", "Work"),
