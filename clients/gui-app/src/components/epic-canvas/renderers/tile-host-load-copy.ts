...
/** It also means the copy can be asserted without mounting anything. */
...
/**
 * A table rather than the union member itself so the wire-ish key and the reader-facing word can differ ("shell-output" is not a phrase anyone says).
 */
...
/**
 * A sixth dead reason added to the contract fails to compile HERE, naming its missing key - rather than arriving at runtime and routing silently to a generic fallback.
 * That failure mode is not hypothetical: it is exactly how `activateRefusalMessage` shipped a broker message instead of a compile error during P1.2, and how this very tile family rendered every `plan-restricted` host as "offline" for months.
 */
...
/**
 * The sentence for one bounded load state.
 * Asserted directly by the S1-S6 catalog suite: "no spinner is on screen" passes vacuously when nothing rendered at all, and that is the exact shape an empty mount produces, so the pins assert the WORDS.
 */
export function tileHostLoadMessage(
  ...
  switch (load.kind) {
    ...
    case "timed-out":
      // The host may be fine and merely slow, so a death claim here would be the same lie the `indeterminate` arm of `useHostReachability` exists to avoid.
