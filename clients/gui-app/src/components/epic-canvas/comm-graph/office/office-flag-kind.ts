/**
 * The failure test is `attentionTone`'s OWN first branch, shared rather than restated - an unread NON-terminal failure.
 * Callers ask `attentionTone` whether there is a flag AT ALL and then ask this which one it is, so the two can never disagree about a single agent.
 */
