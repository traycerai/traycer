...
/**
 * Icon + title reuse the exact desktop tab-strip resolution (`TabIcon`, `useEpicTabDisplayTitle`), including the terminal bound-host client so a terminal tab shows its live host title rather than the stored name.
 */
...
function MobileCurrentTileBarBody(
  ...
  // `useHostClientForHostId(null)` returns the DEFAULT client, so the non-terminal branch must pass null explicitly.
  ...
  // An unadopted chat's title must not become EDITABLE.
  // Refusing at commit would silently discard what the user typed; refusing to enter edit mode tells them before they type, which is the same rule the sidebar's disabled Rename entry follows.
