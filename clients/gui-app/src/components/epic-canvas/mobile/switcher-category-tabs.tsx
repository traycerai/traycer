...
/**
 * Tracked from scroll + resize so the edge fades appear only where something is actually cut off (a static both-ends mask would fade the first/last tab even when fully scrolled to that side).
 */
...
/**
 * The category tab bar for the mobile "Switch tab" sheet: a `line`-variant `TabsList` whose triggers take natural width and scroll horizontally when the curated categories overflow phone width.
 * Rendered inside the sheet's `Tabs` root so selection flows through Radix.
 */
export function SwitcherCategoryTabs(props: SwitcherCategoryTabsProps) {
  ...
  return (
    <TabsList
      ref={listRef}
      variant="line"
      // This list is a scroll container (`overflow-x-auto`), which makes overflow-y compute to `auto` too - any vertical spill inside it becomes a user-visible vertical scroller rather than clipping.
      // Every override here has to spell its modifier the way `ui/tabs` spells its own, and the reason is not style. tailwind-merge only drops the base utility when both sides carry the IDENTICAL modifier chain; spell them differently and both ship, at which point the cascade decides - and the bare shadcn variants (`data-horizontal:`, `data-active:`, registered in `shadcn/tailwind.css`) compile `:where()`-wrapped, so they carry ZERO specificity while the `data-[…]` form carries a real attribute.
      className={cn(
      ...
      {visibleSwitcherCategoryDefs(props.hasPullRequests).map((definition) => {
        ...
        return (
          <TabsTrigger
            key={definition.id}
            value={definition.id}
            // `ui/tabs`' line variant already answers this for a `variant="line"` list, but it answers it by out-specifying the default fill rather than removing it, so the guarantee lasts only as long as this list stays on that variant.
