 ...
 * diff, Pull requests, Terminals, Browsers, Sharing and Comments. `pull-requests`
 * sits directly after `git-diff`, `browsers` directly after `terminals`, and
 * `comments` sits last behind `sharing`, exactly as they do in the desktop rail,
 * so the two surfaces read in the same order. Identity (title + icon) is reused
 * verbatim from `LEFT_PANEL_DEFINITIONS` so mobile never forks the category copy.
 *
