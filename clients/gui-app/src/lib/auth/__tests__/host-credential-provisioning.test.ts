...
describe("appHostCredentialMintFlow single-flight policy", () => {
  ...
  it("collapses concurrent notices for the same host into ONE runner call", async () => {
    ...
    // Joiners now get `pending-elsewhere`: a credential WAS minted for this host, so nothing they asked for failed and they must not spend their one attempt on it.
  ...
  it("tells a joiner that did not win what is LEFT of the winner's claim", async () => {
    // The THIRD `pending-elsewhere` producer - the per-caller claim gate, distinct from the stale-ask branch and the ladder.
    // The tests above exercise it but only count `kind`, so a regression returning 0 (spin before delivery could land) or the full TTL (idle a host through a claim already partly spent) passed all of them.
    const deferred: {
      resolve: ((outcome: HostCredentialMintOutcome) => void) | null;
    ...
    try {
      ...
      // Advance the clock BETWEEN the winner stamping its adoption claim and the joiners reading what is left of it.
      // Elapsed time is the whole point: with a frozen clock the remainder equals the full TTL, and the assertion below could not tell a correct answer apart from a producer that ignores how much of the claim is already spent - which is exactly the regression it.
  ...
  it("does not memoize a settled outcome — a later call for the same host re-invokes the runner", async () => {
    // Provisioning is no longer step-up gated and there is no decline memo: an attempt that already settled must not stop a later transport from trying again (e.g. a transient network error on first connect should not strand a host on the client lease for the.
...
/** The window between a mint SETTLING and the host actually adopting the credential. */
describe("appHostCredentialMintFlow adoption claim", () => {
  it("refuses a second mint while a freshly minted credential is still in flight", async () => {
    ...
    try {
      ...
      // `pending-elsewhere`, NOT `unavailable`: the second transport has not spent its one attempt on this, because nothing was attempted and nothing failed - a delivery is simply already in flight. retryAfterMs is what is LEFT of the 60s claim (59s) - a caller.
  ...
  it("allows the next mint once the claim's TTL has passed", async () => {
    // The claim must not become a latch.
    // This used to be driven by an `active` report; that release is gone (see `noteHostCredentialState`) because the report carries nothing to correlate it with, so the TTL is now the only thing that ends a claim.
    const runner = vi.fn(() => Promise.resolve(provisionedOutcome()));
    setHostCredentialMintRunner(runner);
    const realNow = Date.now;
    try {
  ...
  it("does not release the claim on a repeated needs-reauth ack", async () => {
    // A host still asking is not proof of delivery - its ask may simply predate the credential now on its way.
    // Under the TTL-only design NO report releases a claim (`noteHostCredentialState` is inert); `active` is pinned separately below because it is the one report that looks like proof of delivery and still is not.
    const runner = vi.fn(() => Promise.resolve(provisionedOutcome()));
    setHostCredentialMintRunner(runner);
  ...
  it("a held claim answers pending-elsewhere whatever state the host reports", async () => {
    // Requirement (1) of the TTL-only design: while a claim is held, no transport may mint - and none may be told `unavailable` either, or it burns its attempt on a claim it had no part in.
    const runner = vi.fn(() => Promise.resolve(provisionedOutcome()));
    setHostCredentialMintRunner(runner);
    const realNow = Date.now;
    try {
      ...
      // Both reasons probe the SAME held claim at the SAME instant, so both owe the SAME remainder of the 60s claim (10s elapsed -> 50s left) - the reason on the ask does not reset or extend someone else's claim.
  ...
  it("an active report does NOT release the claim - only the TTL does", async () => {
    // The double-mint sequence this closes needs no account switch: sockets report their `openAck` state independently with no cross-socket ordering, so a delayed `active(A)` observed before A was burned can be delivered after B was minted.
    const runner = vi.fn(() => Promise.resolve(provisionedOutcome()));
    setHostCredentialMintRunner(runner);
    const realNow = Date.now;
    try {
      ...
      // retryAfterMs still tracks the ORIGINAL claim's remainder (60s minus the 5s elapsed) - the stale `active` neither released the claim nor re-armed it with a fresh 60s.
  ...
  it("expires the claim after the TTL so a stalled delivery cannot strand the host", async () => {
    // The liveness half.
    // T1 mints and dies before delivery; if the claim never expired, and the only surviving transport had already asked, the host would sit on the client lease until the app restarted.
    const runner = vi.fn(() => Promise.resolve(provisionedOutcome()));
    setHostCredentialMintRunner(runner);
    const realNow = Date.now;
    try {
  ...
  it("does not honour a claim left over from a superseded generation", async () => {
    // The half of the stale-`active` problem that IS closable here: a claim is only ever honoured for the generation it was taken under, so nothing from a previous account can block the current one's mint.
    const runner = vi.fn(() => Promise.resolve(provisionedOutcome()));
    setHostCredentialMintRunner(runner);
...
describe("appHostCredentialMintFlow mint escalation ladder", () => {
  it("imposes no extra wait after the first completed mint, then escalates 120s -> 240s", async () => {
    ...
    try {
      ...
      // One millisecond short of the ladder opening, so retryAfterMs is 1 - the LADDER's remainder, not the adoption claim's, which expired 60s ago.
      // Pinning the number is what tells those two apart; a bare `kind` check reads the same either way.
      ...
      // Past the 60s adoption claim (which alone would already have cleared) but still inside the ladder's 240s wait - isolating the ladder from the adoption claim, since only the ladder can still be blocking here.
      vi.setSystemTime(thirdMintAt + 150_000);
      await expect(
        appHostCredentialMintFlow({ hostId, reason: "missing" }),
        // 240s wait, 150s of it spent: 90s left.
        // A regression handing back the FULL rung wait here would idle the caller for another 240s, and one handing back 0 would spin it against a closed ladder.
  ...
  it("a ~3-minute flap cadence still climbs every reachable rung rather than resetting", async () => {
    // Contrast with the OLD design: a threshold that scaled with the current wait (`max(wait, 60s) * 2`) could be out-waited by a sufficiently slow flap, silently resetting completedMints back to zero on every cycle and pinning the ladder at rung one forever.
    const runner = vi.fn(() => Promise.resolve(provisionedOutcome()));
    setHostCredentialMintRunner(runner);
    vi.useFakeTimers();
    try {
      const hostId = "host-ladder-flap";
      // Required wait before each successive mint (mintBackoffWaitMs of the PRIOR completedMints): 1 -> 0, 2 -> 120s, 3 -> 240s, 4 -> 480s.
      // Flap just past each one - never long enough to approach the 30-minute decay window.
  ...
  it("decays exactly one rung per full 30-minute quiet window, and does not admit until the decayed rung's OWN wait also elapses", async () => {
    ...
    try {
      ...
      // completedMints is now 6.
      // Its ladder wait would be 60_000 * 2 ** 5 = 1_920_000ms - MORE than the 1_800_000ms decay window, so decay always intervenes before that raw wait is ever satisfied: at 1_800_000ms elapsed, one rung decays away (completedMints -> 5, its clock advanced to the.
      ...
      await expect(
        ...
        // This is the one of the three that reads the pre-decay entry, so it is also the one that would silently change if decay ever fired a millisecond early.
      ).resolves.toEqual({ kind: "pending-elsewhere", retryAfterMs: 120_001 });
      expect(runner).toHaveBeenCalledTimes(calls);

      // Exactly at the decay window: one rung decays (6 -> 5), but the decayed rung's own wait (960_000ms) has had ZERO time to elapse from its advanced clock - decaying must not double as admission.
      vi.setSystemTime(sixthMintAt + 1_800_000);
      await expect(
        appHostCredentialMintFlow({ hostId, reason: "missing" }),
        // The FULL 960_000 of the decayed rung, because `mintInBackoff` rewrites the entry's clock to the decay boundary before `remainingBackoffMs` reads it - so the decayed rung's wait starts here rather than being partly spent.
      ...
      await expect(
        appHostCredentialMintFlow({ hostId, reason: "missing" }),
        // 1ms left of the decayed rung.
        // Note this depends on the assertion ABOVE having already run: that call is what persisted the decayed entry, so the remainder here is measured from the decay boundary rather than from the sixth mint.
      ).resolves.toEqual({ kind: "pending-elsewhere", retryAfterMs: 1 });
      expect(runner).toHaveBeenCalledTimes(calls);

      // Now the decayed rung's own wait has elapsed too: admitted.
      // This is also the ladder's practical ceiling under this arithmetic - the mint that lands here increments the (already-decayed) completedMints of 5 back to 6, never organically past it, because any rung whose raw wait exceeds the fixed decay window is.
  ...
  it("resetHostCredentialProvisioning clears the ladder", async () => {
    ...
    try {
      ...
      // Barely past the 60s adoption claim the reset mint itself set - if the ladder had survived the reset this would still be well short of its 120s wait and resolve `pending-elsewhere` with no runner call.
