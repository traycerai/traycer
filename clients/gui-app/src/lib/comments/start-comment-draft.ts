...
export interface StartCommentDraftResult {
  /**
   * True when a non-empty selection produced a draft range and the store was updated.
   * False when the caller should toast / no-op (e.g. the selection was collapsed).
   */
...
/**
 * Snap the editor's current selection into a `DraftRange` and stash it on the comments store.
 * Shared between the toolbar button and the `Cmd+Opt+M` keyboard shortcut so both paths produce the exact same draft shape - the floating draft popover then renders against the saved range.
 */
...
export function startCommentDraft(
  ...
  // First in-doc comment-button click of this session reveals and opens the Comments panel.
