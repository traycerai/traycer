...
/**
 * "Is any local agent in progress" for this window: a local-host chat turn is running (or stopping), or a local-host terminal-agent PTY has not exited.
 * Plain terminals are excluded - an idle shell left open must not pin the machine awake.
 */
...
/**
 * Subscribe each live handle to a level predicate, firing `onChange` only when that predicate flips.
 * Membership churn is handled by the shared `reconcileStoreSubscriptions`; this adds the per-handle activity edge.
 */
