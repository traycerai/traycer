...
/**
 * Markers match on TOKEN boundaries, not as bare substrings.
 * Model slugs are `-`, `.`, `_` and `/` separated, and a bare `includes` reads "mini" inside "gemini" - which made every Gemini model a small one.
 */
...
/**
 * Sonnet is the one family that spans the classes: 3.5 is a mid-size model and 4.5 onwards is not.
 * Version-gated rather than listed, so a future 6 lands on the right side without an edit here.
 */
