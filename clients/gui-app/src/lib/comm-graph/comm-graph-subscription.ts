...
/**
 * FRAME KIND IS TRANSPORT BATCHING, NOT MEANING.
 * `snapshot` is a BOUNDED initial batch, not a completeness claim: rows that predate the subscription legitimately arrive as `event` frames (snapshot overflow past the host's ceiling, and reconnect gap-fills).
 */
export interface CommGraphSubscriptionHandlers {
  readonly onSnapshot: (
    events: ReadonlyArray<EpicCommunicationGraphEvent>,
    /**
     * The host log's head id at handoff (`snapshot.headId` on the wire), null for an empty log.
     * THE arrival boundary - not the snapshot's last row, which a bounded snapshot legally undershoots.
     */
    headId: number | null,
  ) => void;
...
interface HostEntry {
  ...
  cursor: number | null;
  /**
   * This host's initialization line - see `CommGraphHostSnapshotBoundary`.
   * Set once, at the first snapshot handoff, and survives `detach` along with the cursor: a StrictMode remount resumes the same subscription rather than re-initializing it, and re-seeding here would silently reclassify rows the user has already been shown.
   */
   ...
   * Distinct from `failedDials`, which counts sockets that opened and then fell back to reconnecting.
   */
  dialThrows: number;
  ...
  /**
   * Bumped on every open / close.
   * Handlers carry the generation they were opened under, so a frame that arrives from a subscription we already replaced (or detached) is ignored rather than applied out of band.
   */
  generation: number;
}

/**
 * What the canvas should show for a host, which is not always the raw socket status: a host that has failed to dial repeatedly is reported as `unreachable` even while its session is technically mid-reconnect.
 */
function reportedStatus(entry: HostEntry): CommGraphHostStatus {
  // A transport that could not be BUILT reports as such, and keeps reporting it: there is no socket underneath to produce a lifecycle status, so nothing would ever overwrite this until a redial succeeds.
...
export class CommGraphSubscriptionManager {
  ...
  /**
   * Newest row seen above its own host's boundary.
   * Sticky: this layer decides WHAT arrived, never for how long anything should look like it just did.
   */
  ...
  /**
   * Declares the hosts the epic's agents currently live on.
   * Idempotent, so it can be driven straight from a render-derived list.
   */
  ...
  attach(): void {
    ...
    this.attached = true;
    this.reconcile();
    // `isAttached` is part of what subscribers read (the header's feed-health dot gates on it), so an attach that reconciled to no status change must still notify.
    // The snapshot itself is unchanged.
    this.notify();
  }

  /**
   * Closes every socket but keeps the accumulated events and cursors, so a re-attach resumes from the highest seen id per host.
   */
  detach(): void {
  ...
  /**
   * Whether some surface currently holds this epic's sockets open.
   * A detached manager still carries the last statuses it saw, so a reader that reports feed health must gate on this rather than on the retained snapshot.
   */
  isAttached(): boolean {
  ...
  /** Closes and reopens the named hosts' transports, resuming each from its own cursor. */
  ...
  private reconcile(): void {
    ...
    for (const hostId of desired) {
      ...
      if (this.attached && entry.handle === null) {
        // OPPORTUNISTIC RECONCILE, CONTAINED.
        // A host left with no handle by an earlier dial failure - including one that exhausted its retry cap - is picked up here, so the next host-set change or attach recovers it.
  ...
  /**
   * Open one host's transport, degrading a throw to `failed` + a bounded retry instead of propagating it.
   * The ONLY way this manager dials.
   */
  ...
  /**
   * Closes this host's transport and retires its generation.
   * NEVER THROWS.
   */
  ...
  private applySnapshot(
    ...
    const accepted = events.flatMap((event) => {
      ...
      // On the FIRST snapshot the boundary is still unset, so none of these count - which is the point.
      // On a reconnect gap snapshot the boundary is already drawn and its rows appeared while we were away, so they do.
      this.noteArrival(entry, row);
      return [row];
    });
    // The boundary is the HOST'S head at handoff, from the wire - not the snapshot's own last row.
    // The snapshot is bounded, so pre-existing backlog past its bound legally arrives as `event` frames; only the head separates that overflow (history) from genuinely-new rows.
  ...
  /**
   * Draws this host's initialization line, ONCE.
   * A later snapshot on the same entry is a reconnect gap-fill, and the rows in it appeared while we were disconnected - they are arrivals, so the line must not move past them.
   */
  ...
  /** Records a row as the newest ARRIVAL when it sits above its own host's boundary. */
  ...
  private applyEvent(
    ...
    // It is NOT guaranteed to: snapshot overflow and reconnect gap-fills also arrive as `event` frames and can be older than what we hold, and cross-host clock skew can interleave.
  ...
  private applyStatus(entry: HostEntry, status: CommGraphHostStatus): void {
    ...
    entry.status = status;
    // A dial that reached `open` proves the host is there; anything that falls back to `reconnecting` is a failed dial.
    // A host with no dialable endpoint only ever produces the latter, which is what eventually mutes it.
    if (status === "live") entry.failedDials = 0;
    if (status === "reconnecting") entry.failedDials += 1;
    // A drop after we have already applied rows must resume from the highest seen id rather than re-pull the whole log, so re-open the subscription with the advanced cursor.
    // Bounded by construction: the reopen resets `openedCursor` BEFORE it can throw, so a host that stays down (no new rows, cursor frozen) gets exactly one extra dial per outage and then falls back to the session's own backoff - including when this dial fails.
