...
/**
 * Past-tense verb for an artifact operation, shared by the card badge, the per-turn change row, and the accumulated-changes row (one source of truth so the surfaces can't drift).
 * Accepts either the block's `ArtifactOperationAction` ("update") or the manifest's `CheckpointFileOperation` ("edit"); both map to "Updated".
 */
