...
/**
 * Every activation surface (center, toast, native) must emit the same `notification_activation_completed` / `notification_marked_read` pair from the same terminal `onResult` boundary - this is the one definition all of them share so a surface can't drift out.
 */
export function activationResultHandler(input: {
  ...
  return (outcome) => {
    if (input.row === null) {
      ...
      // Local feed mutations are keyed by the stable feed id, so a native click can still acknowledge a row that was pruned from the rendered projection between emission and activation.
      // Cloud mutations need the captured immutable occurrence row; never substitute a potentially reopened entry with the same id.
    ...
    // Preserve the occurrence captured at activation time.
    // A cloud row may reopen under the same feed id before this callback runs.
