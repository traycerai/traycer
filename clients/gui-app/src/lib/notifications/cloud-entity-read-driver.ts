...
/**
 * Matches `NOTIFICATION_FEED_SYNC_RETRY_BASE_MS` in the host's notification feed sync worker - the same kind of unit, a single failed feed request.
 */
export const CLOUD_ENTITY_READ_RETRY_BASE_MS = 1_000;
/**
 * Five minutes, which is both the host worker's `NOTIFICATION_FEED_SYNC_RETRY_MAX_MS` and this app's existing `HOST_STREAM_REOPEN_MAX_BACKOFF_MS`.
 * A recovered server is picked up within minutes; a broken one is a trickle.
 */
 ...
 * Both continuations that can outlive the caller - the armed retry timer and the in-flight loop's own re-entry - capture it and stand down if it moved.
 */
let generation = 0;

/**
 * Jitter over [50%, 100%] of the exponential rather than AWS "full jitter" over [0, exp].
 * Full jitter can draw a near-zero delay repeatedly, which under single-flight turns a persistently failing server into a tight sequential loop - the opposite of the decay this is for.
 */
...
/**
 * Ask the driver to bring `entity` up to date.
 * Safe to call on every presence change and every snapshot: it writes nothing when there is nothing to do, and coalesces into the in-flight pass rather than starting a second one.
 */
    ...
    try {
      for (const entryId of entryIds) {
        try {
          ...
          // Availability state is owned by the mutation's own onError.
          // Here we only decide when this entry may be tried again, and keep going so one bad entry does not strand the rest of the batch.
          const attempts =
      ...
      // A reset during the pass already cleared the claim and moved on.
      // Re- entering here would revive the torn-down generation - and arm a fresh timer holding its dead `deps`.
...
function scheduleNextAttempt(
  ...
  retryTimer = globalThis.setTimeout(() => {
    // Belt and braces with `resetCloudEntityReadDriver`'s `clearTimeout`: a timer that already fired into the macrotask queue before the reset ran cannot be cancelled, so it has to check for itself.
...
/**
 * Drops the in-flight claim, the queued entity and any armed timer, and invalidates every continuation the previous generation could still run.
 */
