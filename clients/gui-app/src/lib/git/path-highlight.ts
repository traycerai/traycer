/**
 * Match ranges for highlighting filter hits in a path.
 * Each tuple is an inclusive `[start, end]` index range into the source string, matching the shape `fuse.js` returns for `includeMatches`.
 */
...
/**
 * The Git panel rows render the basename (`fileName`) and the parent directory (`directoryName`) as separate spans, but the filter matches against the full path.
 * Split full-path match ranges onto each displayed segment so both can be highlighted independently.
 */
