...
/**
 * The chat-tile renders frequently; for ordering we evaluate the `failedSendRestoration` branch FIRST so a stale failed send is cleared before any send→consume transition runs.
 */
export function nextHandoffTransition(
  handoff: InitialChatHandoff | null,
  ctx: HandoffTransitionContext,
): HandoffStep {
  // Failed-send restoration is one-shot: the chat-session-store clears the slot when `ackFailedSendRestoration` runs, so the next render sees `failedSendRestoration === null`.
  // The two-step transition fires `markFailedByAction` first (only when the handoff has not already been moved to `failed`) so the originating handoff sees the failure before the prompt restore consumes the slot.
