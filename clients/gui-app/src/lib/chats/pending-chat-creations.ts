/**
 * The creation surfaces' door to the open-epic store's pending-creation registry (`stores/epics/open-epic/pending-chat-creations.ts`).
 */
...
/**
 * Take it back down: the create failed, so no record will ever arrive to replace it.
 * Idempotent, and safe for a chat that was never registered.
 */
