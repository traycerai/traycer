...
/** Whether the Share / Make private row-menu entry should render, and in which state. */
export function decideChatSharingMenuEntry(input: {
  ...
  return {
    ...
    // Only the unpublished arm explains itself.
    // `!canMutate` greys out every entry in the menu at once, so a per-entry tooltip there would be noise.
    disabledTooltip:
...
/**
 * Own local rows show a Users glyph only when the chat is task-visible AND someone else can actually see it.
 * Private stays quiet; a solo task has no audience, so the glyph would be a lie.
 */
...
/**
 * Initial master-toggle state when the server pref has no read endpoint: all of the viewer's own cloud rows private (or none yet) → off, otherwise on.
 * The toggle always writes `applyToExisting: true`, so the ambiguity is acceptable - the next flip is authoritative either way.
 */
 ...
 * Unknown (query still loading) is treated as solo so the glyph cannot flash on a private-looking task and then vanish.
 */
