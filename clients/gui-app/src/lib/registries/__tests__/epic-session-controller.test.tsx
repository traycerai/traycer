...
describe("EpicSessionController: session lifecycle with no surface mounted", () => {
  ...
  it("a record with a real name is mirrored over by the observed session title", async () => {
    ...
    // name of its own: a user renames the EPIC through the session (the
    // strip's inline rename enqueues `update-epic-title` on the session),
    // never a tab record directly - so the session's title already IS the
    // user's name, and writing it through unconditionally cannot lose one.
    // This generalises what the mounted `renameTab` effect this write-through
    // replaces did for the one tab it could see: it also overwrote
    // unconditionally, once a real title landed. Stage 3 widens that to every
    // open tab record of the epic, named or not.
