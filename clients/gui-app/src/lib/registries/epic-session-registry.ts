...
// The liveness cell moved to the module that owns `acquireMounted`, because that seam is what has to consult it - and this module imports THAT one, so a map declared here could not be read there without a cycle.
export {
...
/**
 * Separates an established Y.Doc session from a host re-point in flight.
 * The old handle remains in `EpicSessionContext` until the replacement has a complete snapshot; consumers use this presentation state to show a bounded recovery result instead of treating a missing effective host as silence.
 */
...
/**
 * The three epic-session contexts are pinned on `globalThis` in Vite's hot runtime, exactly as `HostCompatibilityContext` and the host runtime state are (`lib/host/compatibility-state.ts`, `lib/host/runtime.ts`): Fast Refresh can keep a provider from one.
 */
...
function createStableDevContext<K extends keyof EpicSessionDevGlobals>(
  ...
  // Typed as the dev-globals record alone (not the `typeof globalThis` intersection) so the keyed write below type-checks against the one property this function is generic over.
 ...
 * Session-level provisioning prevents sidebar rows from independently mounting host-directory subscriptions just to address the same Epic host.
 */
...
// The R-1 rotation rationale that used to live here now lives at the acquire effect's comparison in `epic-session-provider.tsx` (`readOwnerIdentityVerdict`)
...
/**
 * The session's host client, stamped by `epic-session-provider.tsx` from the same value it provides through {@link EpicSessionHostClientContext} - for the imperative callers (DnD commits) that run outside that subtree and address the host the session's.
 */
...
export const registry = new OpenEpicSessionRegistry({
  // The shell's retention profile (desktop: `DEFAULT_MAX_LIVE_EPICS`), read on every cap walk so the phone's smaller cap applies whenever its bootstrap selected it.
...
/**
 * Accessor for the module-scoped live-Epic registry.
 * T8 (desktop app-quit intercept) subscribes to this so it can read the aggregated unsynced-edits map without reaching into provider-local state.
 */
...
/**
 * True when the Epic session for `epicId` currently has unsynced edits that the host has not yet proven coverage for.
 * Called synchronously from the tab-close handler to decide whether to pop the discard- confirmation dialog.
 */
...
/**
 * Release (forcibly dispose) the Epic session for `epicId`.
 * Called when the user closes a tab in the strip.
 */
export function releaseOpenEpicSession(epicId: string): void {
  // Tab close is the one release path where a decision was offered: the close confirmation reads `epicHasUnsyncedEdits`, which covers retained buffers, so reaching here means the user has already answered for them too.
...
/**
 * Forcibly dispose every live Epic session.
 * Wired into the auth lifecycle so sign-out, user-switch, or token expiry cannot leave a prior identity's Y.Doc / queue / focus state behind in the registry - the next sign-in starts fresh from a host snapshot.
 */
