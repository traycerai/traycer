 ...
 * restart (or auth rotation) replaces the transport under a STABLE `hostId`
 * and `close()`s the prior client, but the session keyed on that `hostId` is
 * reused - so its factory would otherwise stay bound to the closed client and
 * throw "Cannot subscribe with a closed WsStreamClient" on the next
 * re-subscribe. The provider compares against this map to re-open the stream
 * on the current transport instead.
 */
