...
/**
 * TanStack Query hashes keys through JSON serialization, so a bridge object whose public surface is methods would serialize as `{}`.
 * Give each bridge instance a stable, serializable scope instead.
 */
...
export const runnerMutationKeys = {
  ...
  // A rename is ONE operation on the bridge too: the boundary that loses the second half is the OBSERVER, not the transport, so chaining a delete onto the set's per-`mutate` callback drops it on unmount exactly as the RPC path did before `config.env.rename`.
  ...
  // On-demand bridge read of this machine's host log, for a Doctor report on a host with no `diagnostics.*` family to ask.
  // Mutation-shaped like the RPC card's own logs read: it is a button, not a subscription.
  ...
  // Opens the previewed draft's GitHub issue-form URL in the browser (ticket 07's publish preview - Flow 3b), separate from the fetch above so the preview can render before the user opts to leave the app.
  ...
  // The key identifies the mutation; it does NOT serialize it - TanStack Query only serializes on `scope`, which the caller sets, and which is what keeps a double-click from starting a second renderer-freezing heap walk.
  captureHeapSnapshot: () =>
  ...
  // Force-refresh the registry update probe (bypasses the desktop's 24h on-disk cache).
  // Used by the Settings → Host Updates row's "Check now" / "Retry" buttons so stale cached failures don't survive a fix or a transient outage.
  hostRegistryCheck: () => ["runner.host.registryCheck"] as const,
  // "Clear local app state" wipe (Settings → General).
  // Awaits the windows bridge (IRunnerHost) per-window `clear` RPC, sweeps localStorage, then reloads.
  ...
  // Settings → Notifications → "This phone".
  // Both act on the ONE device this renderer runs on, so a static key is the whole scope - there is no second OS permission to hold a separate entry for.
...
/**
 * Same hazard `supportBridgeQueryScopeId` above exists for: TanStack Query hashes keys STRUCTURALLY (JSON serialization), so two runner-host instances whose enumerable surface serializes identically hash to the same key - and a pending `fetchQuery` for the.
 */
...
export const runnerQueryKeys = {
  ...
  /**
   * Where recovery from a host's client-compatibility rejection should send this user, as decided by the desktop main process.
   */
  ...
  // The shell's durable answer to "which host id belongs to THIS machine", read once while the host directory bootstraps.
  // Takes the SCOPE ID, not the runner object - see `runnerHostQueryScopeId`.
  lastKnownLocalHostId: (runnerHostScopeId: number) =>
    ["runner.host.lastKnownLocalHostId", runnerHostScopeId] as const,
  // `traycerCli: object` keys these queries to a specific runner-host instance so a host swap (test setups, hot reload) invalidates the cache cleanly.
  // Identity comparison only - the object is never serialised.
  ...
  /**
   * A PRIVATE host-status entry, one per mount of a reader that needs a sample it took itself rather than whatever the shared entry holds (`LocalBootstrapAttempts`, mounting on a settled failure).
   */
  ...
  // Live "Add a shell" validation probe, keyed by the candidate path so each debounced value caches independently.
  // Scoped to the runner-host instance like the other traycer queries.
  ...
  // Host-management queries carry the `IHostManagement` instance as a key segment.
  ...
  // Canonical two-lane `HostController` status (Host Update Layer Redesign Tech Plan).
  // Primed once via `getHostControllerStatus()` on mount, then pushed by `HostControllerStatusListener` - never refetched by TanStack's normal mechanisms, since it is entirely event-sourced.
  ...
  // The no-bridge variant of the key above: a disabled placeholder query still needs a key, and an inlined one drifts when this resource's keys change.
  ...
  // Keyed on the HOST ID as well, unlike its neighbours.
  // The `management` segment hashes to `{}` (see the note above) and the bridge is app-wide, so it survives a local host replacement - a report cached for host A would otherwise be served to host B's console, and this report's issues carry the port/pid numbers.
  ...
  /**
   * Stable cache key for the placeholder ServiceStatusSnapshot used by Settings → Host when the shell does not expose `IServiceHost`.
   * The matching query stays `enabled: false`; no fetch ever runs.
   */
  ...
  // Machine-local and effectively static for the session, so a single static key suffices.
  installedFonts: () => ["runner.installedFonts"] as const,
  zoomPercent: (scope: string | null) =>
    ["runner.zoom.percent", scope] as const,
  // Desktop support log viewer (Diagnostics → Logs).
  // Scoped by the support bridge scope so a host/shell swap invalidates cleanly, matching the other runner-host queries.
  ...
  // Report-issue consent panel's log "view" affordance (ticket 07/04): reads the tail FROZEN at report-open, not a live tail, so what the user reviews is exactly what a submit would send.
  ...
  // This phone's OS push permission (Settings → Notifications → "This phone").
  // Machine-local and singular - one renderer, one OS switch - so a static key suffices, like `logLevels` and `installedFonts` above.
  pushPermission: () => ["runner.pushPermission"] as const,
  // Report-issue evidence strip's "Nth time on this install" line (ticket 06 ledger read).
  // Scoped by fingerprint - each distinct defect caches independently.
