 ...
 * reading in its gauge cache, which is what `fetchProviderRateLimits`'s
 * collection attempt picks up.
 */
...
describe("isRateLimitReadStillRunningOnHost", () => {
  ...
  // as "still running" would suppress a failure nothing can resolve and arm a
  // collection with nothing to collect, leaving stale usage looking healthy.
  it.each([
