...
/**
 * Feature-detected access to the desktop-only `appLifecycle` namespace the Electron preload installs on `window.runnerHost`. gui-app must stay browser-safe, so this reads the global defensively and degrades to a plain `window.close()` on shells (web /.
 */
...
/**
 * Quit the desktop app (the removed surface's "Quit Traycer" button).
 * Routes through Electron's normal `before-quit` flow.
 */
