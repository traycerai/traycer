...
export function HostCompatibilityProvider(props: {
  ...
  // The probe's verdict is also SELECTION evidence (D13/C4): reported from here, one level up from the state machine, because that machine is a render function and reporting from render double-fires under StrictMode.
  useHostCompatibilityAuthorityReport(compatibility, effectiveHostId);
  // A host becoming effective re-probes it.
  // Here rather than inside the probe hook because the trigger is the POINTER moving, which the probe cannot see: it only ever knows the host it is currently keyed to.
  useHostStatusReprobeOnRepoint(effectiveHostId);
  // A host parked on dead("incompatible") is unreachable by BOTH triggers above: it is not effective and cannot become effective until a fresh probe clears it.
  // One recovery probe per such host closes that loop - the update that would fix it announces itself as the directory row's version moving, and the verdict reports through the same seam as the main probe.
...
/**
 * A component rather than a loop of hooks, for the usual rules-of-hooks reason: the incompatible set changes size.
 * Renders nothing - its work is the probe it mounts and the verdict that probe reports.
 */
function IncompatibleHostRecoveryProbe(props: {
  ...
  // ARMED only by evidence of an update: a row version that differs from the one the verdict condemned, or - when the verdict carried no version - a row version that MOVED while this watcher was mounted.
