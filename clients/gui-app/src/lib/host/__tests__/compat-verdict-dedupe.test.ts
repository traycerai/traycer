...
/** An `incompatible` verdict whose fatal carries (or omits) the structured requirement. */
...
/**
 * The argument `reportCompatVerdict` takes, spelled out so the spy below is typed.
 * `ReturnType<typeof vi.spyOn>` degrades `.mock.calls` to `any`, and every read off it becomes an unsafe access - the same reason `compatibility-state.test.ts` names its own `CompatVerdictReport`.
 */
...
describe("compat verdict dedupe and the client-compatibility requirement", () => {
  it("reports a null -> structured transition on the same session", () => {
    // A requirement that failed to parse dropped to null (`parseClientCompatibility` is deliberately lossy rather than fatal), and a well-formed one follows on the SAME session - so the anchor is unchanged and cannot discriminate.
  ...
  it("STILL dedupes an identical verdict re-rendered", () => {
    // The other half.
    // This hook runs in a dialog that re-renders on every lease delivery in the app, so a key that changed per render would flood the authority with duplicate verdicts - each of which it ranks and stores.
  ...
  it("distinguishes null from the empty string in a peer-supplied member", () => {
    // `observedClientKind` and `observedClientAppVersion` are the host's normalization of text the PEER sent, so `null` ("the client did not say") and `""` are different observations that a `??
    const reported = driveVerdicts([
  ...
  it.each([
    ...
    (_label, separator) => {
      // THE COLLISION, constructed rather than described.
      // Under a delimiter join these two requirements serialize to the same key: the separator inside `observedClientKind` closes that field early and the remainder is read as the next one.
  ...
  it("does not report anything for a non-verdict state", () => {
    // `checking` / `failed` are not statements about COMPATIBILITY, and reporting one would launder a transport failure into a `dead("incompatible")` lease no reconnection could clear.
    const reported = driveVerdicts([
