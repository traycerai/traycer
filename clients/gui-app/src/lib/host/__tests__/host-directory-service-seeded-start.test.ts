...
/**
 * `startSeeded()` pins - the split `host-directory-service.ts` introduced so boot does not `await` a `GET /api/v3/hosts` round trip.
 * `start()` keeps its existing contract and existing 75 passing tests in the sibling file untouched; this file exists so the NEW resolve-before-the-registry-answers behaviour has its own coverage rather than none.
 */
...
describe("HostDirectoryService.startSeeded", () => {
  ...
  it("emits the unknown -> zero crossing even though an empty directory is byte-identical either side of it", async () => {
    ...
    // Let the not-awaited `refresh()` this started land - a macrotask flush rather than a fixed count of microtask hops, since `performRefresh` awaits more than one thing after the fetch settles (the local-host reseed included).
  ...
  // There is deliberately NO pin here for "a rejecting fetch cannot fail boot".
  // It was written, and it could not be made to fail: `fetchRemoteOutcome` already collapses every fetcher rejection into `{ kind: "failed" }`, and `startSeeded()`'s promise is not derived from the void-fired `refresh()` at all, so the property holds by.
  ...
  it("start()'s contract is intact and costs exactly one fetch - the awaited refresh() joins the one startSeeded() already issued", async () => {
    ...
    const fetcher: RemoteHostFetcher = () => {
      calls += 1;
      // A macrotask-delayed resolution, deliberately: a synchronously resolving fetcher can settle its background `refresh()` in the same microtask batch `startSeeded()`'s own internal awaits drain, which would let `hasSettledFleet()` read `true` by coincidence.
