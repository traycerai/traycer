...
describe("buildHostProgressView", () => {
  ...
  it("a verify stage carrying BYTES still heads 'Setting up Traycer Host…', never 'Downloading'", () => {
    // The `verify` phase now reports hashed bytes, so the card gains a transfer label - "400 MB of 800 MB" - during a local-source install.
    // That is safe only because the download WORDING is gated on `stage === "download"` rather than on the presence of byte fields.
    ...
    // Positive control for the assertions above: the bytes DID reach the view, so "not Downloading" is not being satisfied by a view that never saw them.
    // BINARY MB, and asserted as the exact string because that is what caught the author writing "400 MB of 839 MB" here from a decimal reading of the same two numbers. 400_000_000 B is 381 MiB; 838_860_800 B is exactly 800 MiB.
