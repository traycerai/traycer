...
// G3(c)/(d): the global semaphore bounding concurrent remote reads across the whole fleet, and the hand-off release the module's own comment calls out as load-bearing.
...
describe("runWithFleetReadSlot — global concurrency cap", () => {
  it("never admits more than FLEET_MAX_CONCURRENT_READS tasks at once, counted independently of the module's own accounting", async () => {
    // THE ARRIVAL-AFTER-HAND-OFF SHAPE, which is the whole trick: submitting every task up front (as a naive version of this test would) can never catch the defect, because every one of the first `FLEET_MAX_CONCURRENT_READS` slots is a genuine `acquireSlot()`.
    let active = 0;
    let peak = 0;
    ...
    // Resolve exactly `FLEET_MAX_CONCURRENT_READS` of them, one at a time, each producing a genuine HAND-OFF to the next queued waiter - this is what advances the broken version's `inUse` counter down without any corresponding increment.
    for (let index = 0; index < FLEET_MAX_CONCURRENT_READS; index += 1) {
      firstWaveGates[index].resolve();
      // Give this hand-off's microtask chain (release → shift → resolve → the woken waiter's own continuation) a turn to fully settle before starting the next one, and before the second wave arrives.
      await Promise.resolve();
      await Promise.resolve();
      await Promise.resolve();
    }

    // NOW submit the second wave - arrivals strictly AFTER the hand-offs above.
    // Under the correct gate these queue normally; under the broken (decrement-on-hand-off) version, `inUse` has drifted down and these are admitted immediately on top of whatever is still running.
  ...
  it("releases the slot even when the task THROWS — a failing read must not leak fleet capacity", async () => {
    ...
    // A slot the failing task leaked would still be free for the NEXT caller to acquire immediately - assert that directly rather than reading the module's own counter.
    let secondActive = 0;
...
describe("runWithFleetReadSlot — hand-off order (regression guard, not a barging proof)", () => {
  it("a waiter queued BEFORE a late arrival runs first when a slot frees", async () => {
    // NOTE: this pins release ORDER, and order alone cannot distinguish the correct hand-off from the broken decrement-then-shift version - in both, the early waiter's `resolve()` is called strictly before the late arrival's.
