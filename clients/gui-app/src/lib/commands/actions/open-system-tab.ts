/**
 * Canonical actions for opening / focusing a system tab (history, settings).
 * Mirrors the `new-epic.ts` pattern: pure store mutations here; navigation is the caller's job.
 */
import {
  historyTabIntent,
...
/**
 * Select the History route the caller should open.
 * The navigation controller materializes the system tab after recording its rollback snapshot.
 */
...
export interface OpenSettingsOpts {
  ...
  /**
   * When true, ignore the existing `lastPath` and snap back to `/settings/general`.
   * Used by the UserMenu "Settings" trigger.
   */
  readonly resetToGeneral: boolean;
}

/**
 * Select the Settings route the caller should open.
 * The navigation controller materializes the system tab after recording its rollback snapshot.
 */
