...
function buildSettingsEntryItem(
  ...
  return {
    ...
    group: "navigation",
    scope: "actions",
    // Shortcut is for discoverability - clicking the palette row opens the section picker below, while the chord still navigates directly to `/settings/general` via `app.settings.open`.
    shortcut,
    actionId: null,
...
function buildSectionItem(section: SettingsSection): CommandItem {
  ...
  return {
    ...
    label: section.label,
    description: null,
    // The section `id` rides along as a keyword so a renamed section stays findable under the vocabulary users already learned (e.g.
    // "agents" still reaches "Agent selection").
    keywords: [
      "settings",
    ...
    shortcut: null,
    actionId: null,
    // The rail says which scope a section belongs to with a group heading; this list has no headings, so the badge carries it per row.
    // Rendered on EVERY section, not just the pair that collides: badging only the ambiguous ones makes the absence of a badge mean something, and the next duplicate label would then ship looking unambiguous.
    statusBadge: group,
...
// `undefined`, not `""`, for a group the registry has never heard of: the palette renders any defined `statusBadge`, so an empty string paints an EMPTY chip on every row of that section.
