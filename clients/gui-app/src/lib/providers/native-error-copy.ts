...
const NATIVE_ERROR_COPY: Readonly<Record<ProviderNativeErrorCode, string>> = {
  ...
  // The only LIST-side code: the provider's own config file is malformed or unreadable, so this provider has nothing to show.
  // Deliberately says the config could not be read rather than "no servers configured", which is the wrong-bug answer this code exists to stop the UI from giving.
  config_unreadable:
