 ...
 * `deriveToolInputSummary` is shared with the host harness converters (so the activity view and a sub-agent's progress view can't drift) and re-exported here for the segment headers.
 */
...
// Claude's Edit/Write/NotebookEdit use snake_case (`file_path`, `notebook_path`); codex/cursor/opencode use `path`/`filePath`/`file`.
...
/**
 * Best-effort file path from a tool call's precomputed `inputDetail` (the raw harness input is no longer persisted), used by the activity-group edited-files dedup so the field list can't drift from the host's derivation.
 */
