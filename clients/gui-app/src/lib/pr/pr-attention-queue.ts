/**
 * Derives the PR full view's attention queue: the things about a pull request that need a decision, ranked, each one routable to an agent.
 */
import type {
  PrActivitySection,
  PrActor,
...
export interface PrAttentionItem {
  ...
  /**
   * The one line of evidence we can show.
   * For a check this is its conclusion label, NOT a log excerpt - the wire carries no check logs, so promising an error message here would be a lie the data can't back.
   */
  readonly detail: string | null;
  readonly actor: PrActor | null;
  /**
   * The reporting app's icon, for a row that has no human behind it.
   * A check is attributable to GitHub Actions or Mintlify but not to an actor, and a generic X glyph loses the one mark that makes the row scannable.
   */
...
const KIND_RANK: Record<PrAttentionKind, number> = {
  // A failing check is the most actionable thing on a PR: it names a specific job, and its fix is mechanical.
  // A human's requested changes need reading before they can be acted on.
  "check-failure": 0,
  "changes-requested": 1,
  "review-required": 2,
};
 ...
 * Markdown is deliberately NOT rendered here - the row is a one-line summary and the full body lives one click away on the Feedback tab.
 */
...
export function derivePrAttentionQueue(args: {
  ...
  args.checks.contexts.forEach((context, index) => {
    ...
    items.push({
      // Same key and same name as the Checks tab, for the same reason: `name` is the JOB name, so one reusable workflow reports `build` once per package.
      // Keyed and titled on that alone, five failing packages collapsed into five indistinguishable rows sharing a React key.
  ...
  // Suppressed only by a standing OBJECTION (`blocking`), not by a failing check: beside a concrete "changes requested" this row is noise, since an approving review is implied by resolving the objection.
  ...
  items.sort((left, right) => {
    ...
    // Within a kind, newest first - a reviewer's latest objection is the one still in the author's head.
    // Undated rows (checks) keep frame order, which is the order GitHub reported them in.
...
/**
 * Headline for the queue hero.
 * Separate from the items so the calm state and the blocked state are rendered by one component from one derivation.
 */
