...
/**
 * Trailing-edge debounce for the burst shape the host's freshness sweep produces: one push event PER re-derived row, ~20+ events/second during a wave (providers-list storm RCA, live CDP audit).
 */
export const WORKTREE_CHANGED_INVALIDATION_DEBOUNCE_MS = 300;

/**
 * Upper bound on how long a continuous drizzle of events (gaps under the debounce) can postpone the flush - the sweep's demand-renewal contract and on-screen freshness only need a refetch per wave, but they do need one.
 */
export const WORKTREE_CHANGED_INVALIDATION_MAX_WAIT_MS = 1_000;

/**
 * The union of every scope pushed since the last flush.
 * `root: true` absorbs path precision on purpose: a root event means "row membership may have changed anywhere", which no path set can narrow back down.
 */
...
/**
 * Accumulates `worktree.changed` scopes and delivers them as ONE `onFlush` per burst: trailing-edge `debounceMs` after the last event, bounded by `maxWaitMs` from the first unflushed event.
 */
