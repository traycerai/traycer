...
/**
 * The repo-identity shape shared structurally by worktree listing entries and folder intents.
 * Declared here rather than imported: the protocol keeps its `repoIdentifierSchema` private, and the neighbouring exported aliases (`TaskRepoIdentifier`, `PrRepoIdentifier`) belong to unrelated surfaces.
 */
...
/**
 * What a completed worktree removal (Sweep today; any delete flow that knows what it removed) took off this host: the worktree directories and the branches their teardown deleted.
 */
...
/**
 * Whether a staged/remembered folder intent references a removed worktree and can therefore no longer materialize: an `import` of a removed directory, an existing-branch checkout of a deleted branch, or a new-branch fork whose SOURCE branch was deleted.
 */
