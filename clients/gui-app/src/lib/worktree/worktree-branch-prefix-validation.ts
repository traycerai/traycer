...
// The composed branch name is `${prefix}${suffix}` (single workspace) or `${prefix}${repoSlug}-${suffix}` (multi-workspace), sliced to 80 chars.
// The prefix is capped at 40 chars below, but `repoSlug` (`slugify-branch-seed.ts`) is ALSO capped at 40 chars - a max-length prefix plus a max-length slug already exceeds 80 on its own, so the slice can land inside the repo-slug material, not just the.
const MAX_LENGTH = 40;

/**
 * Light client-side check for the configurable worktree branch prefix - catches values that would make an illegal git ref before they're saved.
