 ...
 * third hint - a notification names a run, this names the cleanup policy - and
 * it is a hint in exactly the same sense: a host that is offline or too old
 * opens no policy popover at all, and the destination still has to be the
 * worktrees panel.
 *
 ...
 * the caller has no machine of its own to name - and it asks for no popover
 * either, because a request that cannot name its host is a request the chip
 * cannot check before acting on.
 */
