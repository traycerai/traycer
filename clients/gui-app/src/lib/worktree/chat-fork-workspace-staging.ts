...
/**
 * Tear down one fork-chat scratch slot: the staged intent AND the live seeded-workspace snapshot beside it.
 * The two stores share a key and are always written together, so clearing one alone leaves the dialog's next open reading half of the last one's state.
 */
...
/** Tear down every fork-chat slot in an epic, across every host one was staged against. */
