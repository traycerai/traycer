...
/**
 * A CSS color expression reduced to the strict `#rrggbb` the terminal-create protocol accepts (the host interpolates it into an OSC report written to the PTY, so nothing wider may cross the wire).
 */
...
/**
 * The spawning client's resolved terminal appearance, sent on `terminal.create` so the host can answer a TUI's OSC 10/11 foreground/background queries (TUI light/dark detection - the query fires at startup, before this client's xterm has subscribed, so only.
 */
...
/**
 * React-side entry point, memoized on the same identity as `useTerminalTheme` so the hint tracks live light/dark and preset switches.
 * The hint only matters at `terminal.create` time - a later theme change does not (and cannot) re-answer a TUI that already probed.
 */
