...
/**
 * A toast "channel" is a single, stable sonner id together with the variant methods that all emit under that id.
 * Because every method forces the same id, firing twice REPLACES the on-screen toast in place instead of stacking a second one - the same behavior the extension relied on, where semantically identical toasts shared one id.
 */
...
/**
 * Builds a family of entity-scoped channels under a shared prefix.
 * Calling the returned factory with a scope (an epic id, a host id, ...) yields the channel whose id is `${prefix}:${scope}`, so each entity replaces only its own toast while staying distinct from other entities' toasts.
 */
