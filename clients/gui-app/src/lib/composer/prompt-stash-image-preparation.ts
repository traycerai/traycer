 ...
 * - the prompt stash prepares with `PROMPT_STASH_PREPARATION_POLICY`, which
 *   keeps the stash's own numbers (2048 px, 975 KiB, animations verbatim
 *   through 5 MiB) so its behaviour is unchanged.
 *
...
const PROMPT_STASH_MAX_LONGEST_EDGE = 2048;
...
export const PROMPT_STASH_PREPARATION_POLICY: PreparationPolicy = {
  sourceCeiling: PROMPT_STASH_IMAGE_MAX_BYTES,
  maxLongestEdge: PROMPT_STASH_MAX_LONGEST_EDGE,
  byteCeiling: PROMPT_STASH_STATIC_IMAGE_MAX_BYTES,
  animationCeiling: PROMPT_STASH_IMAGE_MAX_BYTES,
};
...
export type PromptStashDecodedImage = DecodedImage;
...
export type PromptStashImageCodec = ImagePreparationCodec;
...
export type PreparedPromptStashImage = PreparedImage;
...
export type PromptStashImagePreparationSession = ImagePreparationSession;
...
/** The stash's session: same preparer, the stash's own ceilings. */
export function createPromptStashImagePreparationSession(
  codec: ImagePreparationCodec | undefined,
): ImagePreparationSession {
  return createImagePreparationSession({
    policy: PROMPT_STASH_PREPARATION_POLICY,
    codec,
  });
}
...
function preparedEncoded(
  ...
  return {
    ...
    fileName: canonicalPromptStashImageFileName(fileName, candidate.mimeType),
