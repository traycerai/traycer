/** composer image-preparation: static codecs and the compression ladder */
import { describe, expect, it, vi } from "vitest";

import {
  canonicalPreparedImageFileName,
...
describe("composer image preparation: static codecs and compression", () => {
  describe("canonicalPreparedImageFileName", () => {
    it("rewrites a mismatched extension to the canonical codec extension", () => {
      expect(
        canonicalPreparedImageFileName("screenshot.png", "image/webp"),
      ).toBe("screenshot.webp");
      expect(canonicalPreparedImageFileName("shot.PNG", "image/jpeg")).toBe(
        "shot.jpg",
      );
      expect(canonicalPreparedImageFileName("anim.webp", "image/gif")).toBe(
        "anim.gif",
      );
    });
    ...
    it("preserves directory prefixes and falls back to image.<ext>", () => {
      expect(
        canonicalPreparedImageFileName("dir/sub/photo.jpeg", "image/png"),
      ).toBe("dir/sub/photo.png");
      expect(canonicalPreparedImageFileName("", "image/png")).toBe("image.png");
      expect(canonicalPreparedImageFileName("   ", "image/webp")).toBe(
        "image.webp",
      );
    });
  });
  describe("verbatim static images", () => {
    ...
    it("keeps a static image at exactly 975 KiB verbatim", async () => {
      const bytes = pngBytesOfSize(SPLIT_CEILING_TEST_POLICY.byteCeiling);
    ...
    it("compresses a static image one byte over the 975 KiB threshold", async () => {
      const bytes = pngBytesOfSize(SPLIT_CEILING_TEST_POLICY.byteCeiling + 1);
  ...
  describe("GIF single-frame vs multi-frame policy", () => {
    ...
    it("keeps a two-frame GIF byte-identical through exactly 5 MiB after decode-validate", async () => {
      const bytes = twoFrameGifBytesOfSize(
        SPLIT_CEILING_TEST_POLICY.animationCeiling,
      );
    ...
    it("rejects GIF above 5 MiB before structure work", async () => {
      const bytes = new Uint8Array(
        SPLIT_CEILING_TEST_POLICY.animationCeiling + 1,
      );
  ...
  it("pins the split-ceiling fixture two byte limits", () => {
    expect(SPLIT_CEILING_TEST_POLICY.byteCeiling).toBe(975 * 1024);
