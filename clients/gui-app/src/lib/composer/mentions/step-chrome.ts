...
/** The mention menu's per-step top bar, and the two body affordances that only some steps have. */
...
export interface MentionStepChromeRefresh {
  ...
  /** Identity of what this button refreshes - the button is REMOUNTED when it changes. */
  readonly targetKey: string;
  /** Button label and tooltip; also its accessible name. */
  readonly label: string;
  /**
   * Spinner leash for this step's refresh.
   * Artifacts answer from local state and use the short leash; a GitHub sweep can honestly take ~17s behind two `gh` timeouts plus request spacing, so ending its spinner at 10s would report "done" over a refresh that is still running.
   */
...
export interface MentionStepChromeFilter {
  ...
  /** The filter the list is ACTUALLY applying - already reconciled against `repositories`. */
  readonly selected: GithubMentionFilter;
}

/**
 * A degraded source the user has to be told about in words rather than by a pause countdown.
 * Waiting does not fix these, so they are banners, not ⓘ.
 */
...
/**
 * The appended row below the rows.
 * `busy` decides the glyph: true is the `Loading…` idiom (spinning dots - work is in flight), false is a plain statement (a failed ask that has GIVEN UP must not spin - a spinner beside "Couldn't reach GitHub." would claim progress the read is not making).
 */
...
export interface MentionStepChrome {
  ...
  /**
   * The appended status row below the rows.
   * Null when nothing needs saying behind the already-rendered list.
   */
  readonly appendedStatus: MentionStepChromeStatus | null;
  /**
   * Replaces the provider's generic empty copy when the step can say something truer - "No GitHub repositories found in this task's folders." beats "No matching pull requests" when the scope itself is empty.
   */
...
/**
 * Value equality for a published chrome.
 * The store damps republishes with this so a hook that rebuilds its chrome object every render does not drive a render loop through the store.
 */
...
// Verbatim on purpose, unlike the FOLDED identity comparisons in `github-mention-rows.ts` (here and in `sameRepositories` below): both sides are successive publications of the same pipeline, not two provenances to reconcile, and a casing change in what the.
