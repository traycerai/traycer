/**
 * Pure persisted-record codec/validator for the prompt stash: converts raw IndexedDB values (entry records and blob records) into the repository's typed rows/blobs, with no IndexedDB access of its own.
 */
...
export interface PromptStashEntry {
  ...
  /**
   * Authoritative reference list for this entry's image blobs, captured once at save time from the image-preparation pipeline's own bookkeeping - not re-derived from Tiptap content.
   * Every repository operation that needs to know what an entry references (save, delete, restore) reads this field rather than re-parsing `content`.
   */
...
/**
 * A raw stored record that failed validation - a malformed shape, an image node whose stash-hash reference isn't in `blobHashes`, or a referenced blob missing from the `blobs` store.
 */
...
/**
 * `readPromptStashRestoreBlobs` result.
 * `"missing"` covers both a genuinely absent blob and the general "could not resolve every referenced byte" case; `"corrupt"` means every referenced blob exists but at least one fails runtime-type, measured-length, canonical-MIME, or SHA-256 validation
 */
...
/**
 * Everything this codec can recover from a raw stored record, however broken - used to keep save/delete/load tolerant of a corrupt sibling and to build the visible `PromptStashUnavailableRow` when the record itself doesn't fully validate.
 */
 ...
 * Returns `null` only when the record has no recoverable identity at all (no string `id`) - which should not happen given `entries`' `id` keyPath, but is handled defensively rather than assumed.
 */
...
/**
 * `info.entry` is only ever non-null when the record fully validated structurally; this additionally requires every referenced blob to still be PRESENT (not necessarily byte-valid - that is checked lazily on restore).
 */
...
/**
 * The full restore-critical attr contract for a stash-owned `imageAttachment` node: non-empty `id`/`fileName`, a canonical MIME, a measured positive integer `size`, no inline `b64content` (a persisted stash entry only ever owns its images through the `blobs`.
 */
...
/**
 * Recursive pass over one candidate restorable tree: every node/mark type must be one `KNOWN_STASH_NODE_TYPES`/`KNOWN_STASH_MARK_TYPES` recognizes, and every `imageAttachment` node must resolve a `validStashImageHash`.
 */
...
/**
 * A tree is restorable only when `collectStashImageHashes` accepts every node/mark and image attr AND the set of image hashes actually referenced by content exactly equals the declared `blobHashes` - no hash kept alive that nothing references, no reference.
 */
...
/**
 * Realm-safe Uint8Array check.
 * IndexedDB structured clone (and fake-indexeddb under jsdom) can yield a same-shape view whose prototype is not the current realm's `Uint8Array`, so plain `instanceof` would false-negative a real byte view and mark every restore as corrupt.
 */
...
/**
 * Runtime type, measured length, canonical MIME, MIME-vs-signature agreement, and SHA-256 agreement - the checks the governing architecture calls out for blob-level corruption.
 * Called only on an actual restore attempt (see `prompt-stash-repository.ts`'s `readPromptStashRestoreBlobs`), never on every load.
 */
...
/**
 * Whether an image node's declared `mimeType`/`size` agree with the ACTUAL verified blob resolved for its hash.
 * A stash entry can pass structural validation from its own declared attrs alone (`validStashImageHash`); this additionally catches the entry and its blob store having diverged - e.g. the same hash stored under a different MIME/size than the node declares -.
 */
...
/**
 * The stricter contract `blobHashes` must meet for an entry to count as fully restorable: every element is a canonical lowercase SHA-256 hex string, and there are no duplicates.
 * `isBlobHashesArray` above stays loose (any string array) because save/delete deliberately track a corrupt record's best-effort `blobHashes` too - this stricter check only gates whether the record is a restorable `entry`, not whether its referenced blobs.
 */
