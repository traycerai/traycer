...
function handleBrowserSessionsFrame(args: {
  ...
  switch (frame.kind) {
    ...
    case "tabPreviewResult":
      handleTabPreviewResult(frame, args.pendingPreviews);
      return;
    case "caption":
...
/**
 * The preview answer, settling its request. Extracted for the same reason as
 * {@link handleOpenTabResult} and {@link handleActionAck}: the frame switch
 * routes, the arms do the work.
 */
function handleTabPreviewResult(
  frame: Extract<
    BrowserSessionsUxServerFrame,
    { readonly kind: "tabPreviewResult" }
  >,
  pendingPreviews: PendingRequests<BrowserTabPreview>,
): void {
  const pending = pendingPreviews.get(frame.requestId);
...
/**
 * The host's two recording control frames, on one arm because the switch above
 * is at its complexity ceiling and these two are the same decision seen from
 * both ends: the host owns the helper's lifetime, and the renderer only relays.
 */
function applyRecordingControlFrame(
  frame: Extract<
    BrowserSessionsUxServerFrame,
    { readonly kind: "startTabRecording" | "stopTabRecording" }
  >,
  ...
  if (frame.kind === "startTabRecording") {
    startRecording(frame);
    return;
  }
  // The host tearing the helper down - auto-stop on tab close, the duration
  // cap, or the toolbar. Terminal for the badge whether or not a helper ever
  // reported ready; `recordingEnded` is the desktop's own answer to it, and
  // arriving after this changes nothing.
  stopRecording(frame.recordingId);
  applyTabRecordingEnded({
    recordingId: frame.recordingId,
    status: "ended",
    reason: null,
