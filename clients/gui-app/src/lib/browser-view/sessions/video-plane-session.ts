...
/**
 * `muted` + `playsInline` autoplay is allowed, but a source attached after mount still needs the kick. jsdom (and any runtime without a media stack) throws instead of returning a promise, which is not a plane failure.
 */
...
/**
 * The tile's video plane, as what a render needs to know about it: whether a track exists to mount, and whether it has decoded a frame.
 */
...
export interface VideoPlaneSession {
  /**
   * Consumes `sdpOffer` / `iceCandidate`, and `inputPong` (the DataChannel RTT probe's reply - this session is the only sender of `ping` on the stream); ignores every other frame kind.
   */
  readonly handleServerFrame: (frame: BrowserScreencastServerFrame) => void;
  /**
   * One decoded video frame.
   * `metadata` is the `requestVideoFrameCallback` argument, and `null` on the fallback path (a WebView with no per-frame callback, where `playing`/`timeupdate` is the only decode evidence) - the frame still counts for liveness, it just carries no timings.
   */
...
/**
 * Sampled only while the round is live (a `negotiating` round has no inbound-rtp stats worth reading yet), and both sent to the host (`WebrtcSignalPort.sendVideoStats`) and handed to `onVideoStats` for the tile's debug overlay - one sample, two consumers.
 */
...
export function createVideoPlaneSession(options: {
  ...
  /**
   * The host's measured control-plane RTT for this subscription, `null` until an `rttProbe` has landed.
   * Read when the first-frame deadline is armed.
   */
  ...
  /**
   * The DataChannel RTT probe (ticket 17).
   * One `ping` in flight at a time: the screencast `pong` frame carries no correlation id, so a second outstanding ping would have no way to tell which reply is which.
   */
  ...
  /**
   * How long a round may hold a peer connection without producing a decoded frame.
   * The host's own negotiation deadline stops at "answered" - a connection that never carries pixels (ICE settles, media does not flow) is only visible from this end.
   */
  ...
  /**
   * Sends one `ping` on `input-reliable`, if the channels are up and no probe is already outstanding.
   * It rides the SAME encoding human input rides (a bare wire client frame), so the host parses it on the one input path - `browser-video-plane-broker.ts` admits `"ping"` alongside the input kinds and the screencast plane answers it over the mux as `inputPong`.
   */
  const probeDataChannelRtt = (): void => {
    if (pingSentAt !== null) {
      // A probe still outstanding a whole cadence later got no pong - the subscriber stopped accepting frames mid-flight, or the round changed under it.
      // Abandon it so one lost reply cannot retire the measurement for the life of the session, but do NOT start the replacement in the same breath: the `pong` frame carries no correlation id, so a late reply arriving next to a fresh ping would be credited to the.
      pingSentAt = null;
      return;
    }
  ...
  const sampleStats = (): void => {
    ...
    // Before the stats read, not after it: the probe must fire on a tick whose `getStats()` rejects (teardown overlap) too, or one such tick would retire the measurement.
    // What this sample reports is therefore the PREVIOUS tick's round trip - the one-cadence lag the field documents.
    probeDataChannelRtt();
    void entry
      .getStats()
      .then((report) => {
        if (closed) return;
        // A round superseded while the read was in flight: the report describes a peer that is gone, and `latency` is already the NEW round's window, so publishing here would date one round's stats to the other's.
      ...
      .catch(() => {
        // The interval can overlap teardown by design (it stops on the next `publish()`, not synchronously with the peer closing), and `getStats()` on a closed connection rejects - a skipped sample, not a failure worth surfacing.
      });
  };
  ...
  return {
    handleServerFrame: (frame) => {
      if (frame.kind === "inputPong") {
        // Nothing else on this stream sends `ping`, so an unmatched reply is impossible in practice; guarding anyway keeps a stray one from minting a nonsense RTT out of a stale stamp.
      ...
      if (frame.kind === "sdpOffer") {
        // A new round is a new path: the previous one's latency window and DataChannel round trip describe a connection that no longer exists, and an outstanding ping's reply can never be matched now that the channels it went out on are gone.
    ...
    noteVideoFrame: (metadata) => {
      ...
      // Before the `liveRound` gate: a same-id ICE restart reopens the registry's per-round latch, and only a fresh `live` report cancels the host's restart deadline.
      // The latch keeps this idempotent.
    ...
    close: () => {
      ...
      stopStatsTimer();
      unsubscribe();
      // The registry outlives this subscription (PiP may still hold it), but the reply channel does not: without this the round keeps answering into a stream nobody is reading.
