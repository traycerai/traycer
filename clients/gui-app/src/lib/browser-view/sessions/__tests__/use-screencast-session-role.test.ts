...
/**
 * Security review root cause G: only a shell with a native browser of its own may subscribe at the tier that can drive the tab.
 * Both read-only shells answer `browserView: null` - the web bundle mounts no runner host at all, and `MobileRunnerHost` declares the field `null` - so one predicate covers them, and the desktop stays the single `"tile"`.
 */
