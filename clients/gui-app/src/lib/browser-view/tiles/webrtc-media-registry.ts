...
// Value import; the other module imports only types back, so the cycle is erased at compile time.
import { inboundVideoJitterMs } from "@/lib/browser-view/sessions/webrtc-video-stats";
import { compositeKey } from "./browser-view-keys";

/**
 * Refcounted video-plane peer connections (host + session + tab).
 * One PC outlives tile remounts so tile and PiP share a track.
 */
...
/** The narrow seam to the screencast subscription. */
export interface WebrtcSignalPort {
  sendSdpAnswer(input: {
    readonly negotiationId: number;
    readonly sdp: string;
    /**
     * A12: local candidates gathered before the answer shipped, batched onto this frame instead of one `iceCandidate` frame each.
     * Empty when gathering hadn't produced any yet - never omitted, so an older host (which ignores the field) and a new one see the same shape either way.
     */
  ...
  /**
   * The subscription's measured control-plane RTT (ticket 18), `null` until an `rttProbe` has landed.
   * A12's batch flush window derives from it - see {@link ICE_TRICKLE_BATCH_DEADLINE}.
   */
...
export interface WebrtcVideoStatsSample {
  ...
  /** rVFC-derived timings; see `video-frame-latency.ts`. */
...
/**
 * The input DataChannels the helper (the OFFERER) creates, so they arrive here through `ondatachannel` rather than being opened from this side.
 */
...
/**
 * One inbound DataChannel, adapted the same way {@link MediaPeer} adapts the connection: the registry stays testable without an `RTCDataChannel`.
 */
...
export interface MediaPeerHandlers {
  ...
  /**
   * A12: every local candidate has been gathered (`RTCPeerConnection.iceGatheringState === "complete"`).
   * One of the two batch-flush triggers, alongside the RTT-derived deadline.
   */
...
/**
 * The DOM half of a negotiation round, kept behind an interface so the registry is testable in jsdom (which has no `RTCPeerConnection`) with a fake rather than a patched global.
 */
export interface MediaPeer {
  ...
  /**
   * Passthrough to `RTCPeerConnection.getStats()` (ticket 11) - the registry stays DOM-free otherwise, so the caller (the video-plane session) maps the raw report into the wire's `videoStats` shape.
   */
...
/**
 * The ICE servers a round negotiates against, as the host delivered them on the offer (`sdpOffer.iceServers`).
 * Empty means the host has no TURN configured, and the peer falls back to its built-in STUN literal.
 */
...
export interface BrowserMediaSnapshot {
  ...
  /**
   * Both input channels of the CURRENT round are open, so {@link BrowserMediaEntry.sendInput} can carry human input.
   * Never true for a superseded round - its channels are closed on arrival or at supersede.
   */
...
export interface BrowserMediaEntry {
  ...
  /**
   * Start a round.
   * A higher `negotiationId` supersedes the round in flight (its peer is closed); a lower or equal one is ignored as stale.
   */
  acceptOffer(input: {
    readonly negotiationId: number;
    readonly sdp: string;
  ...
  /**
   * The subscription that supplied a round's reply channel is gone.
   * The round itself survives (the media is refcounted, and another viewer may still be holding it), but nothing is sent on that port again - the next offer brings the port that replaces it.
   */
  detachPort(port: WebrtcSignalPort): void;
  /**
   * Sends one already-encoded client frame on the current round's channel.
   * `false` (a no-op) when the channel is absent, not open, or refuses the payload - the caller falls back to the mux, which is what keeps a discrete frame on exactly one transport.
   */
  sendInput(label: BrowserInputChannelLabel, payload: string): boolean;
  /**
   * The current round's raw stats report, or `null` when no round is in flight (ticket 11's periodic sampler skips a tick rather than throwing).
   */
  getStats(): Promise<RTCStatsReport | null>;
}

/**
 * A remount releases before it re-acquires: React runs the outgoing tree's effect cleanups and the incoming tree's setups in one commit, so the count dips to zero and back within a tick.
 */
export const RELEASE_GRACE_MS = 1_000;

/**
 * A12: how long a round's local-candidate batch waits for {@link MediaPeerHandlers.onIceGatheringComplete} before shipping anyway.
...
/**
 * How many gathered candidates the `sdpAnswer` frame may carry.
 * A pathological gather (many interfaces x many relays) would otherwise ship one unbounded frame; the remainder is not dropped, it trickles.
 */
...
/**
 * Blocker fix (batch-3a review): `connectionState: "failed"` is potentially recoverable - the host may drive a same-id ICE restart on this SAME peer (see `acceptOffer`'s `===` branch) - so it must not fail the round the instant it fires; that races the.
 */
const CONNECTION_FAILED_GRACE_MS =
  VIEWER_CONTROL_PLANE_DEADLINES.firstFrame.floorMs;

/**
 * One negotiation round's live state.
 * Named (rather than inline) because A12 added enough batching fields that an inline literal type stopped reading as one shape.
 */
interface ActiveRound {
  readonly negotiationId: number;
  /**
   * The reply channel; re-pointed on a same-id restart in case it changed, and `null` once the subscription that supplied it has gone (`detachPort`).
   */
  port: WebrtcSignalPort | null;
  readonly peer: MediaPeer;
  /**
   * The offer SDP this round last negotiated against - lets a same-id ICE-restart re-offer (blocker fix, batch-3a review) tell a genuine restart from a duplicate resend of the SDP it already answered.
   */
  lastOfferSdp: string;
  ...
  /**
   * Local candidates gathered before the answer ships (A12), batched onto the `sdpAnswer` frame instead of one `iceCandidate` frame each.
   * Emptied the moment the batch flushes.
   */
...
/**
 * Three segments, deliberately - no `instanceId`, unlike the neighbouring `browserPeekFrameKey`: a runtime restart is covered by the superseding offer it brings, so keying on it would only cost the media its identity.
 */
...
function createRecord(createPeer: MediaPeerFactory): RegistryRecord {
  ...
  /**
   * Latched per STATE, not per round: a round that went live and then lost its track must report both, because "failed" after "live" is the report the host flips `setCaptureEnabled(true)` on.
   */
  const report = (
    state: "live" | "failed",
    ...
    if (state === "live") {
      // The host only accepts `live` on an ANSWERED round, and both ride one FIFO stream: a frame decoded between a restart re-offer and its answer flush would arrive first and be rejected.
  ...
  /**
   * Runs one `answerOffer` round against `started`, whether this is its FIRST offer or a same-id ICE-restart re-offer on the SAME peer (blocker fix, batch-3a review: the host restarts under the SAME `negotiationId` rather than minting a new one, so the round.
   */
  const negotiateRound = (started: ActiveRound, sdp: string): void => {
    ...
    void started.peer
      ...
      .then((answerSdp) => {
        ...
        // A12: ship the answer with whatever local candidates have already gathered, at end-of-gathering or the RTT-derived deadline - whichever comes first.
        // `onIceGatheringComplete` may already have fired (gathering can finish before this promise settles), in which case the flush happens immediately instead of arming the timer.
        const flush = (): void => {
  ...
  const entry: BrowserMediaEntry = {
    ...
    acceptOffer: ({ negotiationId, sdp, port, iceServers }) => {
      ...
      if (round !== null && negotiationId === round.negotiationId) {
        // Blocker fix (batch-3a review): the staleness guard used to drop this outright, silently swallowing the host's same-id ICE-restart re-offer.
        // A bit-for-bit resend of the SDP this round already answered is the one degenerate case left to drop - nothing changed, so there is nothing to renegotiate.
      ...
      if (
        ...
        (negotiationId < snapshot.negotiationId ||
          // A failed round is over, so the host re-offering under the SAME id is a genuine retry of it, not the stale resend the `<=` guard exists to drop.
          (negotiationId === snapshot.negotiationId &&
      ...
      const peer = createPeer(
        {
          onLocalIceCandidate: (candidate) => {
            ...
            // A12: batch until the answer ships, then trickle individually - a late candidate after the batch is gone is still worth sending, just no longer worth holding.
            // `round.port`, not the `port` this offer was accepted with: a same-id restart re-points it, and this handler outlives that (the peer, and its handlers, are never recreated for a restart).
            if (round.answerSent) {
    ...
    sendInput: (label, payload) => {
      // Both or neither: with only one channel up, moves would take the channel while the clicks they precede took the mux, and the two transports have no ordering between them.
...
/**
 * A4/F6: sizes the receiver's jitter buffer off the CURRENTLY measured jitter instead of trusting Chromium's constant default (30-100ms, inflated further by DERP's TCP burstiness).
 * Re-evaluated on every `getStats()` read - the existing 5s sampler in `video-plane-session.ts` - rather than its own timer.
 */
...
/**
 * An actual `a=rtpmap` payload line, not just the substring "h264" appearing anywhere in the SDP (a `cname`/`msid`/tool banner can legitimately contain it) - the payload-type digit is what makes this a real codec offer rather than a coincidence.
 */
const H264_RTPMAP_PATTERN = /a=rtpmap:\d+ h264\//i;

/**
 * A4/F6: prefers H.264 over the offer's default (VP8) ANSWERER-side, and only when both ends can actually use it - this engine's `RTCRtpReceiver.getCapabilities` reports H.264 decode support AND the offer's SDP already carries an H.264 payload (the offerer.
 */
...
/**
 * The real peer.
 * Adapting `RTCPeerConnection` here rather than exposing it is what keeps the registry above DOM-free: track-death detection, the end-of-candidates sentinel and track stopping all live in this function.
 */
export function createBrowserMediaPeer(
  ...
  const connection = new RTCPeerConnection({
    // The host-delivered set (STUN + minted TURN) takes precedence; this literal is only the fallback for a host with no TURN configured - and for an older host that sends no `sdpOffer.iceServers` at all.
    iceServers:
      iceServers.length === 0
  ...
  /**
   * Blocker fix (batch-3a review): armed the moment `connectionState` reads "failed", cleared the moment it reads anything else.
   * "failed" is potentially recoverable - the host may drive a same-id ICE restart on this SAME peer - so it must not report failure the instant it fires; that races the restart and always wins.
   */
  ...
  return {
    ...
    getStats: () => {
      // Piggybacks the existing 5s stats cadence rather than a second timer - see `applyAdaptiveJitterBufferTarget`.
      // One chain, so the caller's catch owns the rejection a closing connection produces.
    ...
    close: () => {
      // `close()` sets `connectionState` to "closed" without dispatching the event, so nothing else disarms the grace timer: left armed it would report a dead peer's failure into whatever round holds its id next.
      clearConnectionFailedTimer();
