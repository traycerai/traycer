...
/**
 * Which bucket dimension the daily chart stacks by.
 * The buckets are day×harness×model rows either way - grouping is a client-side fold, so switching needs no second request.
 */
...
/**
 * Sums the four mutually-exclusive token buckets.
 * Takes just the `tokens` shape (not a whole `UsageBucket`) so every row kind that carries it - day×harness×model buckets, per-chat buckets, per-turn rows - can reuse this one arithmetic instead of each row kind growing its own copy.
 */
...
/**
 * Builds the stacked-bar chart's per-day columns, one segment per series (harness or model per `groupBy`, or "Other" for anything past the slot cap), in the scale's fixed stacking order.
 */
...
/**
 * Zeroes out every segment whose series key is in `hiddenSeries` - the legend-chip filter's effect on the stacked chart.
 * Segments stay present in the returned columns (their key, just a `0` value) rather than being removed, so the stack order and the legend's color assignment never shift as a series toggles - "color follows the entity, never its rank" (dataviz skill).
 */
...
/**
 * Series keys ranked by the window's total known cost, descending.
 * This drives SELECTION only - which keys get a palette slot and which fold into "Other" - so "Other" is always the long tail, never whichever series happened to show up late in the window.
 */
...
/** The series scale for a response's buckets under the given grouping, on two SEPARATE axes: */
