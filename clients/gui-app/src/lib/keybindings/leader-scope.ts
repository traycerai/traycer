/**
 * Leader-scope stack: the single coordination point for "which surface owns a leader modifier (mod/alt) right now".
 * A scope bundles one or more digit actions; the topmost ACTIVE scope that binds a given modifier wins and suppresses every scope below it FOR THAT MODIFIER ONLY (per-modifier fall-through).
 */
...
/**
 * The new-chat / new-terminal modal.
 * It opts out of the keybinding provider's dialog block (`data-leader-scope`) so a leader-aware overlay nested inside it (the model picker) can run its ⌘/⌥ digit shortcuts, but owns no shortcuts of its own.
 */
export const LEADER_SCOPE_NEW_CONVERSATION_MODAL = "new-conversation-modal";
/**
 * The landing page's terminal panel.
 * Owns `mod` digits for its tab strip while the panel is open with at least one terminal - the landing analog of the canvas-tabs scope, which is inactive without an active epic tab.
 */
...
export interface LeaderScopeAction {
  ...
  /**
   * Optional multi-digit dispatcher for surfaces that expose numbered slots beyond 9.
   * Keep null for single-key digit scopes.
   */
  readonly dispatchSequence:
    | ((digits: ReadonlyArray<number>) => boolean)
    | null;
  /**
   * Classifies the currently typed sequence without side effects.
   * Required when `dispatchSequence` is set, null otherwise.
   */
...
/**
 * Push a scope onto the stack (top = highest priority) and return its unregister.
 * Later registrations sit above earlier ones, so the global scope (registered once at provider mount) stays at the bottom and overlays layer on top in open order.
 */
...
/**
 * Register an always-present base scope below any scopes that are already mounted.
 * Use this for app-level surfaces owned by `KeybindingProvider`; normal transient overlays should use `registerLeaderScope` so they sit above base scopes even when child effects run first.
 */
