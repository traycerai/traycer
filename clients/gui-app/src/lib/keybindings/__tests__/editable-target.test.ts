...
describe("isDiffsEditorEvent", () => {
  it("recognizes a contenteditable reached through Diffs' composed shadow path", () => {
    ...
    // jsdom does not compute `isContentEditable` from the attribute (it has no layout/editing-host implementation), so stub the browser-computed property directly - `isDiffsEditorEvent` reads only that, not the raw attribute, so a real browser's `true` here must.
  ...
  it('does not treat an explicit contenteditable="false" node inside the boundary as editable', () => {
    // Regression for treating attribute PRESENCE as editable: a Diffs boundary can contain non-editable chrome (line numbers, gutters) that explicitly opts out via contenteditable="false".
    const boundary = document.createElement("div");
    boundary.setAttribute("data-diffs-editor-boundary", "");
    const host = document.createElement("div");
    const shadow = host.attachShadow({ mode: "open" });
