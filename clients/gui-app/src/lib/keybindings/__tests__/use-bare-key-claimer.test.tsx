...
describe("useBareKeyClaimer", () => {
  it("runs the CURRENT render's handler for a key that lands before passive effects flush", () => {
    // The caller is outside React.
    // `bare-key-owner`'s window listener fires on whatever task the keystroke lands on - including the window between a commit and React's passive-effect flush, which is a separate scheduler task.
    const seen: number[] = [];
  ...
  it("keeps one identity across handler changes, so a claim is not re-prioritized by one", () => {
    ...
    // A factory whose identity moved with the handler would re-claim on every render - pushing an older overlay back above a newer one - and would hand `lastIndexOf` a handler that is no longer the registered one.
