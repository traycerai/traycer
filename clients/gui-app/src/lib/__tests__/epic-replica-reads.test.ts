...
interface FakeState {
  /**
   * The read seam takes the RESIDENT member and no longer the sync one, so the sync member is deliberately absent here: a fixture that still offered it would let a test pin a call `holdArtifactBody` does not make.
   */
  ...
  /**
   * The two attachment legs, and they are DIFFERENT store members - which is the whole property this fixture exists to keep observable.
   */
...
function createHandle(state: FakeState): OpenEpicStoreHandle {
  ...
  return {
    ...
    userId: "user-1",
    hostId: "test-host",
    // No `doc` / `awareness`: a production handle has neither, because the replica lives on the worker thread and a `Y.Doc` cannot cross a structured clone.
    // A fake that offered them would let a test reach for a capability its callers no longer have.
    store,
...
function deferred<T>(): {
  ...
  // Settled through a HOLDER rather than two `let … | null` locals with a guard: TypeScript does not track the executor's assignments, so the guard reads as a comparison between literal values and is rejected as an unnecessary condition.
  const handles: {
    resolve: (value: T) => void;
    reject: (reason: unknown) => void;
...
describe("holdArtifactBody", () => {
  it("resolves the fragment that only becomes readable after residency settles", async () => {
    ...
    // The whole point of the seam: the fragment is read only AFTER residency settles.
    // Reading it in the same tick as the acquire - which is what this function used to do - sees `residentSettled === false` and reports a body that was milliseconds away as `ArtifactBodyUnavailableError`.
  ...
  it("does not release twice when acquiring the lease throws", async () => {
    ...
    // Nothing to release: the acquire never handed a lease back, and the `catch` must not invent one.
    // Kept from before the resident lease existed - only the member that throws changed.
