...
describe("reportableErrorToast", () => {
  ...
  it("tracks report_issue_opened when the toast report action is used", () => {
    ...
    // `surface` is the report context's own name for the failing area, and it is the half that separates two Report buttons on one frame.
    // `source` stays the entry point - asserting both together is what stops a future change re-valuing one into the other's job.
