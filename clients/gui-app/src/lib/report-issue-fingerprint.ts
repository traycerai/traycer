...
/**
 * Reduces a raw stack trace to an ordered list of frame identifiers (function names only - no file paths, line/column numbers, or absolute paths), so the SAME defect fingerprints identically across app versions and platforms whose bundling shifts line numbers.
 */
...
/**
 * `null` for a line that is not a stack frame at all (filtered out); `"<anonymous>"` for a genuine frame with no function name - V8 renders those as a bare `at path:line:col` with no parens - so frame position still contributes even when the function itself.
 */
 ...
 * Not cryptographic - fingerprints only need to be stable and well-distributed, not collision-resistant against an adversary.
 */
...
/**
 * Computes the versioned `fp:v1:<hash>` fingerprint (critique C8).
 * Identity is error code + operation + causal provider/harness + typed subtype ONLY - the parts that survive a fix.
 */
