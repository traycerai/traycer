...
/**
 * Pure priority-table tests for the mapping every output-window state comes from - `UI.md` §4's "one word for what happened" replacing four call sites that used to drift (every fatal close reading as "deleted", an old host blamed for a deletion, an empty log.
 */
...
describe("shellOutputStreamAvailability", () => {
  ...
  it("puts an unsupported host ahead of every other signal", () => {
    // A host too old to serve the method says so before anything else gets a say - deleted, a fatal close and a missing snapshot are all still true here, but none of them is the reason the window has nothing to show.
    expect(
      shellOutputStreamAvailability(
        signals({
  ...
  it("lets a deletion outrank the NOT_FOUND close that follows it", () => {
    // The host telling this window it just destroyed the shell is the stronger claim; a NOT_FOUND close after it is only the reconnect discovering the same thing a second time.
    expect(
      shellOutputStreamAvailability(
        signals({
          deleted: true,
  ...
  it("routes each fatal code to its own reading, by the code alone", () => {
    ...
    // A remote host too old for the method never resolves to `"unsupported"` client-side - its support stays `"unknown"` and the subscription is closed with this code instead.
    // So it must read as the same permanent capability state the local case does, not as a failure with a Retry button that can only fetch the same refusal.
    expect(
      shellOutputStreamAvailability(
  ...
  it("stays connecting until the opening snapshot lands, whatever the transport or line count say", () => {
    // The socket declares itself open the moment the subscribe is acknowledged, but the host serves the snapshot only after its first log read - "open" and "has lines" are both true here and neither wins.
    expect(
      shellOutputStreamAvailability(
        signals({
          snapshotArrived: false,
  ...
  it("only reads as reconnecting once a snapshot has actually landed", () => {
    // Without a snapshot the transport dropping is still "connecting" - the window never had anything to reconnect FROM, so it stays a centred bootstrapping panel rather than a banner over nothing.
    expect(
      shellOutputStreamAvailability(
