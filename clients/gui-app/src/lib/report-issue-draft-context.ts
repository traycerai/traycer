...
/**
 * Structured private cause for an error-triggered report.
 * Real error text lives ONLY here, never in {@link ReportIssueContext} (the public prefill) - see `ReportIssueDraftContext`.
 */
...
export interface ReportIssuePrivateDiagnostics {
  ...
  /**
   * Normalized stack frame family (see `normalizeStackFamily`), for maintainer-side sub-clustering ONLY - deliberately NOT part of `fingerprint`'s identity (critique C8): a one-frame refactor must not re-identify a defect.
   */
...
/**
 * Draft context assembled once at report-open: `publicPrefill` is the existing caller-declared public-safe contract (unchanged); `privateDiagnostics` is the new private branch.
 */
...
/**
 * Assembles the full draft context from an already-normalized public prefill plus an optional error capture: pulls the current support-context registry snapshot (D5) for session state, and reuses the correlation id / fingerprint / stack family already minted.
 */
...
/**
 * Convenience factory for the migrated error surfaces: takes the same input shape as `createReportIssueContext` plus a `capture` (see `captureReportIssueError`), and returns the fully assembled draft.
 */
...
/**
 * Plain-JSON shape of {@link ReportIssuePrivateDiagnostics}.
 * Exported separately (rather than trusting callers to serialize the interface ad hoc) so the exact field list is a single, testable contract for ticket 04's IPC wire shape to match: `cause` (type/message/stack/componentStack/.
 */
