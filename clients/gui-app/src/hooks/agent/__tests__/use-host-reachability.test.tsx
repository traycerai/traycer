...
describe("useHostReachability", () => {
  ...
  it("treats a busy host as reachable", () => {
    // `busy` arrives as `dialable`. Reading it as unreachable locked every chat on a healthy machine.
    list.value = {
  ...
  it("PRESERVED (2026-08-08): a populated directory marking a host unavailable stays unreachable", () => {
    // A genuinely dead host must keep locking.
    // The two-slot live check showed what happens otherwise: an unavailable owner's rows carried no lock, the sidebar routed them to a LIVE tab, and the tile dialed a dead host forever - an eternal spinner instead of the locked published copy.
    list.value = {
      data: [
  ...
  it("reports host-starting for this machine's not-yet-dialable local row", () => {
    // `HostDirectoryService.snapshot()` substitutes the registry's twin of THIS machine as a local entry with no websocketUrl and a hardcoded `unavailable` whenever the local snapshot is absent - boot, restart, or a host busy enough to lose a probe.
    list.value = {
      data: [
        entry({
          hostId: "host-a",
