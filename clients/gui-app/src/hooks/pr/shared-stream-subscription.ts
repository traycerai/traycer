...
/**
 * One transport session per key, however many components mount. Immediate teardown at zero (ADR-0003, no grace).
 */
export interface SharedStreamSubscription<TFrame> {
  ...
  /** A terminal session's `sendRefresh` is inert, so the entry must be REPLACED, not reused, on retry. */
  isTerminal: boolean;
...
/** Read lastEvent via useSyncExternalStore. */
...
export interface SharedStreamSubscriptionResult<TFrame> {
  /** Read through the store (see `entryListenersByRegistry`), never off the entry. */
  readonly lastEvent: TFrame | null;
  /** After a terminal error the session is dead and its `sendRefresh` is a no-op, so a user "refresh"/"Try again" there means RECONNECT: re-run the subscribe effect, which replaces the terminal entry. */
...
export function useSharedStreamSubscription<TFrame>(args: {
  ...
  /** Distinct from `enabled`: a key with `enabled: false` still reads a sibling's live session, it just does not open one of its own. */
  ...
  /** A second consumer joining an ALREADY-LIVE session gets no fresh hydration snapshot from the host - the session never dropped to zero - so this is where a cache slot that was GC'd while unobserved gets refilled. */
  ...
  useEffect(() => {
    ...
    // Replacing a terminal entry is what makes "Try again" actually reconnect instead of spinning against a closed session.
    if (shared === undefined || shared.isTerminal) {
      // Migrate them onto the fresh session instead of leaving them ref-counted against the one we are about to close, which would silently drop their subscription the next time only THIS (retrying) consumer unmounts.
    ...
    return () => {
      // Look up the CURRENT entry at this key rather than closing over the one captured above: a sibling consumer's retry may have replaced a terminal entry with a fresh session in the meantime and migrated THIS consumer onto it (see the migration above), so releasing the stale captured `shared` would decrement/delete the wrong entry and leak this consumer's hold on the session that is actually live.
  ...
  const sendRefresh = useCallback(() => {
    ...
    // Branching on the last ERROR frame instead would swallow the refresh entirely after a NONFATAL error: that leaves the session live and `isTerminal` false, so `retry()` would only bump the nonce, the subscribe effect would reuse the very same entry, and no frame would ever be sent.
