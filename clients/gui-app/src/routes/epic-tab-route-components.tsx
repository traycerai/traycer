...
function PhaseToEpicMigrationGateInner(props: {
  ...
  const openMigratedEpic = useCallback(
    (epicId: string) => {
      // Local state wins for rendering - committed before the navigate so
      // the gate transitions to the EpicSessionProvider immediately,
      // independent of any pending prefetch the route loader kicked off.
      setMigratedEpicId(epicId);
      // Strip migrationSource via the browser history API rather than
      // `navigate(...)` so we don't re-enter the route's loader path
      // and trigger a redundant prefetch.
      if (typeof window !== "undefined") {
        const url = new URL(window.location.href);
        url.searchParams.delete("migrationSource");
        window.history.replaceState(null, "", url.toString());
      }
      // If the migration produced a different epic id than the phase id we
      // arrived with, navigate to that route so the URL reflects the new
      // entity. Same-id migrations (the "phase already resolves to v200"
