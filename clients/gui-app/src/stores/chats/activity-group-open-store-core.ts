...
export function createActivityGroupOpenStore(
  ...
  return createStore<ActivityGroupOpenState>((set) => ({
    ...
    markHeaded: (groupId) =>
      set((state) => {
        if (state.headedIds.has(groupId)) return state;
        ...
        //
        // The growth is bounded by "groups in this tab that have shown a
        // reasoning header", which is proportional to the transcript the tab is
        // already holding in memory, and each entry is one short id.
