...
describe("windowed snapshot seating", () => {
  ...
  it("keeps the scrollback a reader is looking at when the epoch is unchanged", () => {
    // An aux-only re-broadcast (a queue change, an approval) re-sends the snapshot. Dropping hydrated
    // history on one of those would make ordinary chat activity evict the rows on screen.
  ...
  it("seats nothing for an empty tail, which is a real answer and not a bug", () => {
    // The host's tail walks backwards under a hard byte ceiling with no always-serve-one exception, so
    // a chat whose last row is a 1.27 MB tool result ships zero rows.
    const window = applyWindowedSnapshot(
      emptyTranscriptWindow(),
      {
        epoch: 1,
...
describe("skeleton chunks", () => {
  ...
  it("declares the index void when a dropped INTERIOR chunk leaves a hole", () => {
    // The sparse-array trap: the final chunk reaches `rowCount`, so length agrees and the skeleton
    // reads complete while ordinals 3-5 are holes. Length is not coverage.
    const seeded = applyWindowedSnapshot(
      emptyTranscriptWindow(),
      {
        epoch: 1,
  ...
  it("runs the DELAYED identity check on a tail seated before the skeleton arrived", () => {
    // The tail rides the snapshot, which is emitted before any skeleton chunk, so it is seated with no
...
describe("index deltas", () => {
  ...
  it("seats an append at rowCount while the skeleton is still streaming", () => {
    // `windowWithSkeleton` delivers a COMPLETE skeleton, where `length` and `rowCount` happen to agree
    // - so every other case here would pass with either rule.
    const seeded = applyWindowedSnapshot(
      emptyTranscriptWindow(),
      {
        epoch: 1,
  ...
  it("voids the coordinate when a lost frame makes the append growth discontinuous", () => {
    // The host's pump keeps drop-and-continue for a non-deterministic send failure, so an
    // `indexChanged` can be lost while the stream survives.
  ...
  it("applies an append whose rows a snapshot already counted, at the frame's own ordinals", () => {
    // The host's append republish emits the bounded snapshot FIRST - stamped with the post-append
    // `rowCount` and the subscriber's pre-delta `indexRevision` - and the delta for the same append
    const seeded = applyWindowedSnapshot(
      emptyTranscriptWindow(),
      {
        epoch: 0,
    ...
    // And the same baseline governs the stream prefix.
  ...
  it("leaves the skeleton incomplete when the append does not fill what the snapshot got ahead by", () => {
    // The negative that keeps the assertion above from passing for the wrong reason: re-establishing
    // completeness is conditional on the append actually accounting for what the snapshot got ahead
    const seeded = applyWindowedSnapshot(
      emptyTranscriptWindow(),
      {
        epoch: 0,
  ...
  it("drops the whole span containing a row that was rewritten in place", () => {
    // The whole span, not the row: a span's records are a DEDUPLICATED union across its rows, so the
    // client cannot say which records belong to the row that changed.
    const held = applyRangeResponse(
      windowWithSkeleton(4),
      rangeResponse({
        epoch: 1,
  ...
  it("applies an append and an in-place update from ONE frame atomically", () => {
    // A turn finishing is routinely both at once: it appends rows and rewrites the streaming row that
    // preceded them.
    const held = applyRangeResponse(
  ...
  it("voids the index on a non-reindexed delta stamped with a NEWER epoch", () => {
    // The other direction is not a straggler, it is a LOSS: the snapshot (or the `reindexed` beside
    // it) that would have carried the new coordinate space here never arrived.
    const held = applyRangeResponse(
      windowWithSkeleton(4),
      rangeResponse({
        epoch: 1,
  ...
  it("does not seat the newer epoch's appended entries at the old ordinals", () => {
    // The failure this prevents is not "the delta was dropped", it is the delta being APPLIED against
    // a skeleton it does not describe.
...
describe("idempotent voiding", () => {
  ...
  it("pending byte measurements survive an idempotent repeat void", () => {
    // The ledger now carries the debt (`unsettledByteMessageIds` and the entries both survive) through
    // a void, so a repeat with pending measurements is safe to short-circuit: it skips nothing because
    const held = heldWindowWithHydratedSpan();
    const voided = applyIndexChange(held, {
      activeTurnId: null,
      epoch: 2,
    ...
    // `voidedTranscriptWindow` rebuilds from `emptyTranscriptWindow()`, which never carries a pending
    // byte measurement - so this marker can only be organic here if something streamed into the window
...
describe("range responses", () => {
  it("discards a response whose row ids contradict the skeleton", () => {
    // The identity check is what makes the ordinal coordinate safe: a missed invalidation must cost a
    // wasted round trip, never bodies under the wrong rows.
    const window = windowWithSkeleton(4);
  ...
  it("does not invalidate on a superseded epoch, which repairs itself", () => {
    // The contrast that makes the case above a real distinction: a newer epoch is already on its way
    // and will re-seat the coordinate space, so dropping the response is the whole of the correct
    const window = windowWithSkeleton(4);
    const seated = applyRangeResponse(
      window,
      rangeResponse({
  ...
  it("merges abutting spans and keeps one copy of a turn that straddles them", () => {
    // A folded assistant turn belongs to several rows, so a span boundary drawn through it puts the
    // SAME records in both responses.
...
describe("held-copy preference for the active turn", () => {
  ...
  it("yields to a served copy the host stamped newer than the held one", () => {
    // Holding a copy is not the same as having applied every delta to it.
  ...
  it("keeps the held copy when the host stamped it newer", () => {
    // The ordinary streaming case with stamps present on both sides: the client's delta-rewritten copy
    // is ahead, and the freshness gate must not become a reason to discard it.
  ...
  it("prefers the later-served copy when a record sits in two spans", () => {
    // One turn's records reach every span its rows do, and adjacent spans are not always merged, so
    // the same id sits in several spans at different serves.
    const turnId = "turn-two-spans";
    const messageId = "assistant-two-spans";
    const rowId = assistantRowId(turnId);
    // Stamped, so the growth survives its OWN seat: without a version the preference below would
    // substitute the older copy into the later span too, and the fixture would pin nothing.
    const grown = {
      ...assistantMessage(messageId, turnId, 2),
      blocks: [firstBlock, secondBlock],
      blocksVersion: 5,
  ...
  it("a reordered snapshot tail cannot regress the actively streaming body", () => {
    // A bulk snapshot serialized before newer block deltas can arrive after they were applied.
  ...
  it("prefers a copy held only by a STALE span over a delayed range answer", () => {
    // A same-epoch non-echo `updated` demotes the active turn's span while its deltas keep arriving -
    // the stale copy is then the freshest one the client holds, and a delayed bulk-lane answer must
...
describe("gaps and what to request next", () => {
  ...
  describe("required ordinals", () => {
    ...
    it("asks for a required row the window does not hold, one row wide", () => {
      // One row, not the gap around it: the host always serves the first requested row whatever it
      // costs, so a single-row ask cannot be squeezed out by scrollback nobody asked for.
    ...
    it("ignores an ordinal outside the transcript", () => {
      // A judgement carried across a `rowCount` change names a row that does not exist, and a range
      // framed against it comes back empty forever - a hydration loop with nothing on the other end.
      expect(
...
describe("stale spans", () => {
  ...
  /**
   * The split-block / doubled-in-progress-segment regression: a streaming rewrite must reach a copy
   * held ONLY by a stale span.
   */
  ...
  it("keeps the carried span the reader is looking at when the budget rebalances", () => {
    // So the carry ON SCREEN aged while an off-screen one stayed warmer, and the first unrelated range
    // to grow the fresh tier discarded exactly the rows the reader was reading.
    const bulk = "x".repeat(Math.ceil(TRANSCRIPT_WINDOW_MAX_BYTES * 0.47));
    // Seated first, so it is the COLDER of the two by seat order; only the viewport touch can make it
    // the survivor.
    ...
    // The replacement index names the carried row at ordinal 0, which is where the row merger draws it
    // - so that is where "on screen" is decided, not at the span's own stale coordinates.
    const named = applySkeletonChunk(rebased, {
      epoch: 2,
      fromOrdinal: 0,
  ...
  it("does not warm a carry for a row the FRESH tier is the one drawing", () => {
    // Warmth has to mean "this carry is putting a row on screen", because it is the only thing that
    // speaks for a carry under a squeeze.
    const seeded = applyRangeResponse(
      windowWithSkeleton(30),
      rangeResponse({
        epoch: 1,
    ...
    // The replacement serves row-0 only. The carry survives for row-1, which is what makes this the
    // MIXED span the retirement rule deliberately keeps
  ...
  it("does not warm a carry whose rows are all unverified-identity markers", () => {
    // `seatStaleRows` returns on the marker before it considers position, so a marker row is never
    // drawn - not at its old ordinal, not anywhere.
  ...
  it("keeps both carries when their markers collide only across coordinate spaces", () => {
    // Spans are disjoint WITHIN a coordinate space, so two candidates can share an ordinal only when
    // they come from different ones - where the same number names different rows.
  ...
  it("keeps the carry the renderer draws from when two hold one row and warmth ties", () => {
    // `seatStaleRows` draws a duplicated row from the greatest `servedAt`, but `boundedStaleSpans`
    // decides which carry SURVIVES first-come in warmth order - so ordering the two by anything else
    const seeded = applyRangeResponse(
      windowWithSkeleton(30),
      rangeResponse({
        epoch: 1,
    ...
    // A second rebase before the replacement lands, which is what leaves two carries holding `row-5`:
    // the fresher one is admitted for it, the older one for `row-6` beside it.
    ...
    // No per-span serve comparison here any more: both carries reference "row-5" through the single
    // ledger entry, so their derived serve stamps tie BY CONSTRUCTION - there is no per-span copy left
    ...
    // The consequence, stated on the body rather than on span identity: the row still renders from the
    // newer serve, regardless of which span structurally references it.
    expect(
  ...
  it("keeps a locally-rewritten SETTLED record over a delayed older serve", () => {
    // The held-copy preference was active-turn-only, but a settled record is rewritten in place too -
    // a detached subagent's card, an image resolving.
    const settled = {
  ...
  it("clears the retained viewport when the reader reports no placed row", () => {
    // The retained range is a protection, so a stale one keeps exempting carry the reader has scrolled
    // away from - and an exemption is the one thing the budget cannot argue with.
    const seeded = applyRangeResponse(
      windowWithSkeleton(30),
      rangeResponse({
        epoch: 1,
  ...
  // That preference was the defect: "different" is not "newer", and a held copy seated from an OLDER
  // slice differs from a fresher serve in exactly the same way, so the preference pinned the stale
  ...
  // Distinct RESOLVED content per (source, version) - unlike `imageEntry`'s fixed pending/resolved
  // pair, this gives a multi-write scenario as many uniquely-matchable contents per source as it
  ...
  it("a witnessed write defeats the delayed serve that predates it", () => {
    // The witness stream carried BOTH contents, so rule 2 has stamps on both sides: held (rewritten,
    // exact apply stamp) vs served (unique content match on the pre-write content).
  ...
  it("witnesses recorded while the record was unheld stamp its first hydration", () => {
    // The witness is evidence about the source's write stream, not the client's holdings: both writes
    // were recorded before the client held any copy, so the first hydration stamps by unique content
    const witnesses = createImageWitnessStore();
  ...
  it("an ambiguous content match stamps nothing, so the superset rule decides", () => {
    // W1(X), W2(Y), W3(X): the served copy's content X appears at TWO retained occurrences, so
    // matching it to "the newest occurrence" would fabricate "served is later" (3 > 2), break
    const witnesses = createImageWitnessStore();
  ...
  it("superset evidence predating the record's lineage reset is directionless", () => {
    // Same comparison twice around a reset. Before: the held copy's strictly larger source set wins at
    // witness silence (upsert-only, so larger is later).
    const witnesses = createImageWitnessStore();
    const s1 = "https://example.test/s1.png";
    const s2 = "https://example.test/s2.png";
  ...
  it("a snapshot serving a record clears its evidence, and only its", () => {
    ...
    // Read the SEAT decisions off the spans the delayed serves produced - `hydratedRecords` dedupes to
    // the freshest copy, and the snapshot's own tail copy of M would mask what the row-5 seat decided.
  ...
  it("the active turn's held copy substitutes with no witness evidence at all", () => {
    // Arm isolation: the active arm never consults the settled evidence rules.
    const witnesses = createImageWitnessStore();
  ...
  it("multi-source dominance survives alternating applies across two sources", () => {
    // Rule 2's held-wins arm requires EVERY differing source to carry a held-later verdict
    // (dominance).
    const witnesses = createImageWitnessStore();
    ...
    // The delayed slice carries W1's S1 content and W2's S2 content - each uniquely witnessed
    // (versions 0/1/2 per source are each recorded exactly once), so rule 2 has a servedStamp for both
    const delayed = applyRangeResponse(
      afterW4.window,
      rangeResponse({
        epoch: 1,
  ...
  it("a non-image rewrite carries the held copy's image evidence across the swap", () => {
    // W1(S1) stamps the held copy with an exact apply sequence.
    const witnesses = createImageWitnessStore();
    const s1 = "https://example.test/nonimg.png";
    const preWrite = imageEntry("pending", s1);
    const postWrite = imageEntry("resolved", s1);
    // W0: witnessed BEFORE the seed, so the served (pre-write) side gets its own exact stamp later -
    // mirrors "witnesses recorded while the record was unheld stamp its first hydration" above.
  ...
  it("protects every carry drawing the viewport, not just the first admitted", () => {
    // The soft budget exemption was positional: `carried.length > 0` spared whichever contributing
    // span sorted first and dropped the rest.
  ...
  it("warms a shared row's one record and nothing beside it", () => {
    // The pre-ledger pin here - "only the freshest carry warms" - rested on two carries holding two
    // COPIES of the row, where only the fresher copy would render.
    const seeded = applyRangeResponse(
      windowWithSkeleton(30),
      rangeResponse({
        epoch: 1,
...
describe("eviction", () => {
  ...
  it("touchTranscriptRange is identity-stable when nothing overlaps", () => {
    ...
    // The range is RECORDED even when it warms nothing, so the first report of a new range is a new
...
describe("eviction protects the visible span", () => {
  it("never evicts a span overlapping visible, even when it alone exceeds the budget, and drops a cold non-overlapping span first", () => {
    ...
    // A budget of 1 byte: everything alone exceeds it.
    const evicted = evictTranscriptWindowToBudget(
      window,
      1,
  ...
  it("never evicts a span holding a required row", () => {
    // The same re-fetch loop as the visible span, in its purest form: a required ordinal is re-planned
    // with no viewport to scroll away from, so evicting its span means fetching that one row forever.
    let window = windowWithSkeleton(30);
    window = applyRangeResponse(
      window,
      rangeResponse({
...
describe("the record ledger", () => {
  it("accepts staying over budget rather than dropping a protected span, and resets the terminal once a later evict lands under budget", () => {
    // A single span whose end touches `rowCount` is the tail - exempt from eviction outright - so a
    // window that is over budget with NOTHING else to drop cannot make progress.
    ...
    // The reset: a later evict that DOES land under budget must not leave the stale terminal standing,
    // or a caller reading it after the window recovered would keep reporting a squeeze that is long
  ...
  it("names the alias-group as unbreakable when the only unprotected span's every record is shared with the protected tail", () => {
    // The cold span at ordinal 0 holds nothing of its own - its one record IS the tail's record,
    // re-served under the same id.
    ...
    const window = applyRangeResponse(
      ...
      rangeResponse({
        ...
        fromOrdinal: 0,
        rowIds: ["row-0", "row-1"],
        // Same record id as the tail's - this span's ENTIRE reference set is shared, so its own
        // `contextBytes` and exclusive records are both zero.
  ...
  it("evicts an alias closure of two unprotected spans as one unit, and prunes the shared record only once both are gone", () => {
    // Neither cold span has a positive marginal saving alone - each holds ONLY the record they share,
    // so evicting either alone frees nothing (the other still references it).
  ...
  it("charges an evicted span's marginal saving exactly, and re-evaluates it after each drop - never trusting an alias's bytes as savings until it is truly the last holder", () => {
    // Spans A (ordinal 0) and B (ordinal 10) each hold their own exclusive record plus a big record
    // they SHARE; a tiny protected tail (ordinal 18) anchors the window.
    ...
    // The budget only the protected tail's own charge can satisfy: reaching it requires BOTH A and B
    // to go, since A's marginal saving alone (its exclusive record plus its context) is nowhere near
    const budget =
  ...
  it("charges a record shared across three carries once at a rebase, and once more across the fresh/stale boundary after one carry is re-served", () => {
    // All three ranges below name the SAME extra message id ("m-shared5"), so a rebase demotes all
    // three spans to stale in ONE `boundedStaleSpans` pass whose per-record dedupe (`chargedIds`)
    ...
    // Re-serve the shared-only row fresh under the new epoch.
    const reserved = applyRangeResponse(
      rebased,
      rangeResponse({
        epoch: 2,
  ...
  it("keeps two spans apart when the tail's row grew past the merge cap while streaming, and merges them when it did not", () => {
    // The merge ceiling must read the record's TRUE current size, not the ledger's settled `bytes`
    // figure - `streamWindowMessage` leaves that figure stale BY DESIGN (see
  ...
  it("keeps a shared record charged only while a span still references it, and prunes it once the last one is evicted", () => {
    // Two cold spans (ordinals 0 and 8) hold their own exclusive record plus one they share; a small
    // protected tail (ordinal 18) keeps the window itself alive across two separate squeezes.
...
describe("reading a long chat upward from the tail", () => {
  ...
  it("does not coalesce the whole visited history into the tail span", () => {
    ...
    // Every response was adjacent to the last. Before the merge cap this was ONE span [8,20) whose end
    // touches rowCount - the tail span - and the tail exemption then covered all of it.
...
describe("records the index has not placed yet", () => {
  ...
  it("bounds row-less live events instead of growing for the session", () => {
    // Supersession cannot reach this class at all.
    ...
    // The NEWEST are kept: those are the ones with any chance of being live- relevant, and an older
    // one that genuinely belongs to a row is re-served by that row's hydration.
  ...
  it("updates a message wherever it lives, and says so when it lives nowhere", () => {
    ...
    // Outside the window entirely. `held: false` is not an error - it is the case the caller answers
    // by dropping the change and letting hydration serve the host's own version.
  ...
  it("charges the byte delta exactly, in both directions", () => {
    // The bytes are charged incrementally because two callers run at streaming frequency, and a
    // whole-span recompute would serialize every record the span holds once per token.
    const seeded = applyRangeResponse(
      windowWithSkeleton(4),
      rangeResponse({
        epoch: 1,
    ...
    // The from-scratch recompute this pins the incremental `hydratedBytes` delta against: every record
    // referenced by a FRESH span, deduped, plus each fresh span's structural bytes (0 in this fixture
...
describe("the streaming row's byte charge", () => {
  ...
  it("evicts on the settled figure, not the stale one", () => {
    // The whole design rests on this. Eviction is the ONLY reader of the byte figure, so deferring is
    // sound exactly as long as eviction settles first.
    const seeded = windowWithColdAndTail();
  ...
  it("re-bounds the stale tier when a streamed copy grows it past the shared budget", () => {
    // The stale tier is bounded where it is BUILT, and that holds only while its spans keep their
    // size.
    const bulk = "x".repeat(Math.ceil(TRANSCRIPT_WINDOW_MAX_BYTES * 0.55));
    // The streaming row is seated FIRST, so the idle row is the warmer of the two by seat order. Only
    // the rewrite makes the streaming span the warmest
  ...
  it("carries live events across a void", () => {
    // `provisionalLiveEventsForSnapshot` already states the rule for this question: an INVALIDATING
    // transition retains what the client holds live.
  ...
  it("drops live events when the void's authority says no rows remain", () => {
    // The other half, and the reason this is gated rather than unconditional: a zero-row authority
    // says those rows no longer exist, so retaining what would render them publishes deleted history.
    const seeded = appendLiveRecords(windowWithSkeleton(10), {
      messages: [],
      events: [event("evt-live", 5)],
    });
  ...
  it("keeps the warmest carry when the fresh tier alone is over budget", () => {
    // The fresh tier sits over budget between a seat and the eviction that answers it - and STAYS over
    // for as long as protected spans alone exceed the limit, since eviction is deliberately soft
    const bulk = "x".repeat(Math.ceil(TRANSCRIPT_WINDOW_MAX_BYTES * 0.6));
    const seeded = applyRangeResponse(
      windowWithSkeleton(30),
      rangeResponse({
  ...
  it("carries a deferred figure's debt across a rebase, unsettled", () => {
    // `unsettledByteMessageIds` records what still owes a measurement.
    const bulk = "x".repeat(Math.ceil(TRANSCRIPT_WINDOW_MAX_BYTES * 0.6));
    const seeded = applyRangeResponse(
      windowWithSkeleton(30),
      rangeResponse({
    ...
    // The figure this span WOULD have carried had it been settled in place. Pinned as an equality
    // against that, not as "larger than before": the understated value is also larger than zero.
  ...
  it("re-measures a stale copy a remap rewrote", () => {
    // `boundedStaleSpans` trusts `span.bytes`, so a remap that rewrites the stale records and leaves
    // the figure behind puts the carry's share of the shared budget out by whatever the rewrite
    const grow = (message: Message): Message =>
...
describe("holdsEveryRecordFrom", () => {
  /**
   * 30 rows: a cold span at the top (0-4) and a live tail (25-29), with a 20-row hole between them.
   * The shape a reader who scrolled back to the start of a long chat is actually in.
   */
  ...
  it("does NOT hold everything from a message above the hole", () => {
    // The message itself is hydrated - the user can see it and click edit on it - and everything below
    // it is not.
...
/**
 * What a merge, a re-seat and a skeleton chunk do to bodies they already hold. All three are cases
 * where the window holds TWO answers for one row and has to pick.
 */
describe("what an overlap keeps", () => {
  ...
  it("drops a retained tail the new snapshot could not serve", () => {
    ...
    expect(seeded.spans).toHaveLength(1);
    // Same epoch, and the last row has since grown past the inline budget - so the host legitimately
    // ships an EMPTY tail.
  ...
  it("leaves a transcript with no tail rows alone", () => {
    // The other `null` from the same helper, which must NOT drop anything: `fromOrdinal === rowCount`
    // is "there is no tail", not "the tail was withheld".
    const seeded = applyRangeResponse(
      windowWithSkeleton(3),
      {
        ...rangeResponse({
  ...
  it("fills the inline tail's empty ids from the skeleton that names them", () => {
    // The snapshot precedes the skeleton, so the tail is always seated with no ids to carry. Until the
    // chunk backfills them the row merge cannot match these ordinals to their models at all.
    const seeded = applyWindowedSnapshot(
      emptyTranscriptWindow(),
      {
        epoch: 1,
  ...
  it("fills them from an appended delta when no chunk will reach them again", () => {
    // The other authority, and the only one for a row appended AFTER the skeleton stream finished: a
    // chunk backfilled the tail above, but nothing re-streams for a row the delta itself introduces.
    const seeded = applyWindowedSnapshot(
      emptyTranscriptWindow(),
      {
        epoch: 1,
...
describe("one record across several spans", () => {
  /**
   * A steered turn is MANY rows over ONE record set, and `SPAN_MERGE_MAX_BYTES` stops touching spans
   * from always merging - so one turn's slices can sit in several spans, each holding its own copy
   */
  ...
  it.each(["earlier-first", "later-first"] as const)(
    ...
    (order) => {
      // Ordinals 2-3 are a gap, so these never touch - which is exactly the shape a turn straddling an
      // evicted slice produces.
  ...
  it.each(["earlier-first", "later-first"] as const)(
    ...
    (order) => {
      // The question the dropped-sharer rule was really answering.
  ...
  it("drops every COPY of a rewritten row's records, not only its own span - kept stale for display", () => {
    // The reader is parked on an EARLIER slice of one turn while a later slice is rewritten.
  ...
  it("leaves an unrelated span alone when a row is rewritten", () => {
    // The other half of the same rule: following the records must not become "drop everything". A span
    // holding none of the stale turn's records is untouched.
    const held = spanCarrying(windowWithSkeleton(10), {
      fromOrdinal: 0,
      rowIds: ["row-0", "row-1"],
      messages: [userMessage("m-0", 1)],
  ...
  /**
   * One turn's rows across ordinals 4-6, with only the FIRST slice hydrated. `assistant:t-1:part:0`
   * (4), its steer bubble `steer:q-1` (5), and `assistant:t-1:part:1` (6).
   */
  ...
  it("drops a sibling slice's copy when the rewritten row is NOT hydrated", () => {
    // The case the containing-span seed cannot see.
  ...
  it("stops at the turn boundary rather than invalidating the transcript", () => {
    // The other half, and the one that would make this rule too expensive to keep: widening must not
    // walk past the user rows that bound the turn, or every streaming `updated` would drop every span
  ...
  it("widens nothing for a rewritten USER row, which shares no turn", () => {
    // `rowRecordIds` gives a user row its own message and nothing else, so the conservative widening
    // must not fire here - it would cost a discard and a refetch on the most common `updated` there
...
describe("what a span charges the byte budget", () => {
  ...
  it("keeps the context charge when a streaming row settles", () => {
    // `settleWindowBytes` re-measures the RECORDS of a span holding an unsettled row.
    const grow = (message: Message): Message =>
  ...
  it("keeps the context charge when records are remapped", () => {
    ...
    // Same gap, same reason: a remap rewrites records and leaves the context map untouched, so the
    // difference after it must still be exactly the context - not merely "more than zero".
    expect(withContext.spans[0].contextBytes).toBeGreaterThan(0);
    expect(withContext.hydratedBytes - bare.hydratedBytes).toBe(
      withContext.spans[0].contextBytes,
    );
  ...
  it("measures a merged span's context once rather than summing its members", () => {
    // The merge DEDUPES the context map exactly as it dedupes records, so a charge summed from the
    // members would bill a re-served row twice and leave the window reporting bytes it does not hold.
    ...
    // And the merged span's own charge still INCLUDES that context.
  ...
  it("evicts a span whose context is what put the window over budget", () => {
    // The point of charging it. A window whose records fit and whose retained context does not must
    // still evict, or the bound is a number that agrees with itself and not with memory.
