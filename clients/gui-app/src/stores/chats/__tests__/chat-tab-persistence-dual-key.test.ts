...
describe("ticket 15 dual-key scroll cache", () => {
  it("RESTORE-FIRST: saved following-end and free-scrolling both round-trip (tab-key)", () => {
    ...
    expect(restoreChatTabState(id, messages.map((message) => message.id))).toEqual({
      mode: "following-end",
      anchorMessageId: null,
      anchorIndex: null,
      offset: 0,
  ...
  it("RESTORE-FIRST via chat-key only (simulates reopen after tab eviction)", () => {
    ...
    expect(restoreChatTabState(reopened, messages.map((message) => message.id))).toEqual({
      mode: "free-scrolling",
  ...
  it("multi-view: independent tab-keys; durable chat-key is last-writer-wins", () => {
    ...
    expect(restoreChatTabState(viewA, messages.map((message) => message.id)).anchorMessageId).toBe(
      messages[1]?.id,
    );
    expect(restoreChatTabState(viewB, messages.map((message) => message.id)).anchorMessageId).toBe(
      messages[4]?.id,
    );
    ...
    expect(restoreChatTabState(third, messages.map((message) => message.id))).toEqual({
      mode: "free-scrolling",
  ...
  it("sweep evicts tab-keys only, never chat-keys", () => {
    ...
    expect(peekSavedChatTabState(id) !== null).toBe(true);
    expect(
      restoreChatTabState(chatIdIdentity("reopen-new"), messages.map((message) => message.id)).mode,
    ).toBe("free-scrolling");
  });
  ...
  it("chat-delete eviction drops the durable chat-key entry and tombstones it (ticket 15 review round 3)", () => {
    ...
    expect(restoreChatTabState(reopened, messages.map((message) => message.id)).mode).toBe("following-end");
...
describe("ticket 15 dual-key registries (round 3: sweep-simulated promotion)", () => {
  it("scroll cache: reopen-after-close restores free-scrolling", () => {
    ...
    evictChatTabState([closed.tileInstanceId]);
    expect(
      restoreChatTabState(chatIdIdentity("reopen-new"), messages.map((message) => message.id)).offset,
    ).toBe(7);
  });
...
describe("ticket 15 review round 4: real close order drives the canvas sweep (F3/finding 5)", () => {
  ...
  it("A writes, B writes, B closes (own sweep), A closes LAST (own sweep) -> A's durable commit wins", () => {
    ...
    expect(restoreChatTabState(reopenedA, messages.map((message) => message.id))).toEqual({
      mode: "free-scrolling",
