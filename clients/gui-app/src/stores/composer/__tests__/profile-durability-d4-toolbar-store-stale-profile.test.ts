...
describe("D4: composer-toolbar-store trusts its seeded profileId (validation is the caller's job)", () => {
  it("a seed with a profileId that no longer exists anywhere else still survives untouched, with no crash - by design, the store has no profiles[] channel to judge it against", () => {
    ...
    // No crash, and the store presents a fully-resolved, catalog-confirmed selection - it just happens
    // to be pinned to a profile that no longer exists anywhere else in the system.
  ...
  it("an UNKNOWN harness for the tombstoned profile still survives cleanly (availability reroute only touches harnessId, never profileId)", () => {
    // The seed's harness itself is unavailable, forcing the availability reroute in
    // `effectiveSelectionFromHarnesses`.
    const store = createComposerToolbarStore({
    ...
    // No eligible harness at all: `effectiveSelectionFromHarnesses` returns the selection UNCHANGED
    // (not rerouted) - so the dead profileId still rides along. No crash either way.
