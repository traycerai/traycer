...
/**
 * Whether a given `(hostId, epicId)` is known to have at least one PR - the only signal the rail
 * needs to decide whether the Pull Requests panel earns a slot (`isAutoVisible`).
 */
export interface PrPresenceStore {
  readonly hasItemsByScopeKey: Readonly<Record<string, boolean>>;
  /** Record what the newest list frame observed. */
...
export function migratePrPresencePersistedState(persisted: unknown): {
  ...
  for (const [scopeKey, value] of Object.entries(
    ...
    // Only a literal `true` survives: anything else (a stale shape, a null, a truthy non-boolean)
    // reads as "not known to have PRs", which is the safe default - it hides an icon rather than
...
export const usePrPresenceStore = create<PrPresenceStore>()(
  persist(
    (set) => ({
      ...
      recordPrPresence: (hostId, epicId, hasItems) => {
        ...
        set((state) => {
          ...
          // Identity-stable no-op on an unchanged frame.
    ...
    }),
    {
      ...
      // `migrate` runs ONLY on a version mismatch, and `basePersistOptions` already pins this store at
      // the current version - so the ordinary rehydrate path skips it entirely and zustand's default
