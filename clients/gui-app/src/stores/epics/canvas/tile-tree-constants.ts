/** Geometry constants for the N-ary canvas split tree. */

/** Minimum normalized fraction a pane may occupy inside its group. */
export const MIN_SPLIT_SIZE = 0.1;

/**
 * Pixel floor applied on top of {@link MIN_SPLIT_SIZE} at render time: a pane's effective minimum
 * is `max(MIN_SPLIT_SIZE * containerPx, MIN_PANE_PX)` when the container is large enough to honor
 */
export const MIN_PANE_PX = 240;

/**
 * Maximum split-tree depth (a bare pane has depth 1). Edge-drops that would nest beyond this are
 * rejected; same-direction drops merge into the parent group and therefore never deepen the tree.
 */
