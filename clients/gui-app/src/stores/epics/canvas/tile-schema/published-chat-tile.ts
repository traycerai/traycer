/**
 * Schema + factory for the `published-chat` tile: a chat read from the last copy its owning host
 * published, because that host is out of reach.
 */
...
/**
 * The tile id for one published chat. Prefixed so it can never be mistaken for - or collide with -
 * a live chat tile, whose id is the bare `chatId`.
 */
...
function parsePublishedChatTileRef(
  ...
  return {
    ...
    chatId,
    ownerUserId,
    // Unlike the triple, a missing owner only costs the composer's reason its host name - the read
    // still resolves - so it degrades instead of dropping the tab a user had open.
