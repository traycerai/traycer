/**
 * Per-tab, per-panel left-panel tree expansion. Hoisted out of panel body local state so expansion
 * survives panel remount (header tab switches, epic navigation).
 */
...
// tabIds are uuids and panelIds are fixed slugs, so this separator never collides.
// `tabScopePrefix` is what `copyTabState` matches to move every panel scope belonging to a tab.
...
// Remap every `${sourceTabId}::<panel>` scope onto `${targetTabId}::<panel>`, leaving unrelated
// tabs untouched.
...
/**
 * Derive the effective expanded-id set: implicit roots + active-artifact ancestors, merged with
 * user-driven explicit expansions, minus user collapses.
 */
export function useEpicSidebarEffectiveExpanded(
  ...
  // Memoize the derived Set on its inputs.
