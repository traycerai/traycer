/**
 * An authority-epoch replacement over an already-open lane session must keep both transport legs
 * OPEN, and the sync pill must never read "reconnecting" for it.
 */
...
interface LaneRig {
  ...
  /** The two lane sockets' `onConnectionStatus("open", null)`. */
...
function statusSnapshot(authorityEpoch: string): EpicStatusSnapshotFrame {
  const parsed = epicStatusSubscribeServerFrameSchemaV10.parse({
    ...
    authorityEpoch,
    securityEpoch: 1,
    // EDITOR: this suite's write-gate assertions are incidental to its real subject (the transport
    // legs and the pill), but a viewer role would still shut writes for a second, unrelated reason.
...
function openLaneRig(options: LaneRigOptions): LaneRig {
  // The LATEST callbacks only, reassigned on every factory call - which is exactly what a re-dial
  // through `requestFreshSnapshot` produces: the adapter's `openTransport()` calls the factory
  ...
  const laneSelection: EpicLaneSelectionSources = {
    // Declared support, as the sibling open-cycle suite does: this suite is about what a lane-arm
    // SESSION does once installed, not about how it gets chosen.
    support: () => "supported",
    subscribeSupport: () => () => {},
...
async function settle(handle: OpenedStoreForTest): Promise<void> {
  // As many drains as the sibling open-cycle suite names: the command crosses to the worker queue,
  // the send crosses back, the answer crosses again to resolve the record.
  await handle.flush();
  await handle.flush();
  await handle.flush();
}

/** Replicates `selectHostDirtyState` from `@/lib/epic-selectors.ts`, which is module-private. */
...
describe("an authority-epoch replacement over open lanes keeps both transport legs open and the pill never reads reconnecting", () => {
  it("keeps hostTransportStatus and recordsTransportStatus open across the replacement, and the pill stays synced", async () => {
    ...
    // The replacement itself: NO transport event accompanies it - the sockets stay exactly as they
    // are, and only the two lanes' next snapshots carry the new epoch.
...
describe("a client-driven requestFreshSnapshot still resets the legs, because it redials and the reopened lanes report open again", () => {
  it("drops both legs to connecting on the redial, then recovers once the reopened sockets report open", async () => {
    ...
    // The CONTRAST: this path genuinely closes and reopens the sockets, so resetting to "connecting"
    // is honest here - unlike the authority replacement above, these legs really do have to report
