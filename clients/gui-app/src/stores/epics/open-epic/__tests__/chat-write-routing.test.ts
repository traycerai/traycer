...
/**
 * A host with a chat record plane but no terminal-agent remainder - the middle of the three
 * classes.
 */
...
describe("chat write routing - floor-era host (no record plane)", () => {
  it("ALLOWS every row, including a doc-projected one", () => {
    // `epic.renameChat` and friends are on the released floor and resolve a pre-registry chat through
    // the host's own storage seam.
...
describe("chat write routing - host WITH a chat record plane", () => {
  ...
  it("DISABLES a row the plane stated is doc-homed", () => {
    // Not a doc write: on a host with a record plane the doc is no longer the authority, so
    // record-wins would snap the edit back and the affordance would read as working while changing
  ...
  it("DISABLES a row whose home the delivering plane never stated", () => {
    // `host.chatRecords.subscribe` carries the base row and the host announces doc-homed chats on it
    // (`hydrateLegacyDocSecondary(…, true)`), so `null` is genuinely "could be either" - and a guess
  ...
  it("re-enables the moment the home is STATED, with nothing else changing", () => {
    // The `null` window is one poll round trip wide, and closing it must not require anything to
    // remount or re-open: the same inputs but for the stated home flip the verdict.
...
describe("chat write routing - the terminal-agent arm is not consulted", () => {
  it("reads only the CHAT arm, so a TUI-only doc arm cannot open the chat gate", () => {
    // The two populations have different coverage conditions (the chat plane has a hydration shim, the
    // terminal plane does not), so a predicate that read the wrong member would allow a doc-homed chat
