/**
 * Regression pin for the doc-over-doc waiver in `chatRowSupersedesOnSnapshot`
...
describe("createChatRecordTable - refreshing a doc-resident row", () => {
  it("takes the later of two doc reads, which both report revision 0", () => {
    ...
    // A second list call. `readDocResidentSummaries` re-read the live map and found a rename; the row
    // still reports `revision: 0`, because a doc entry has no seq to report.
    ...
    // THE REDDENING ASSERTION - and it reddens twice over. Under `0 > 0` the row is rejected, so the
    // slice is unchanged and `applyRecords` publishes NOTHING at all.
    if (second === null) {
      throw new Error(
    ...
    // The archive travels too - the projection carries it as `archivedAt`. Two fields, because a
    // rename and an archive reach the sidebar through different affordances and the freeze hid both.
  ...
  it("still refuses a stale in-flight answer between two REGISTRY rows", () => {
    // The control that keeps the waiver narrow.
    const table = freshTable();
  ...
  it("does not let an IN-FLIGHT answer roll back a row a push advanced, waiver or not", () => {
    ...
    // The delta plane states no home, so this seeds `docResident: null` on the held row - the
    // pre-existing waiver's exact trigger, and reachable without any doc-resident chat at all.
    ...
    // THE REDDENING ASSERTION. `held.docResident === null` waived the revision guard, so revision 5
    // replaced revision 6 and nothing was owed to put it back until the next poll.
    expect(stale).toBeNull();
    expect(table.current().byId[CHAT_ID].title).toBe("Pushed");
  });

  it("still admits an answer issued AFTER the push it carries", () => {
    // The control. A fence that rejected every carried row would pass the case above and freeze the
    // table against its own poll - which is the backup half of this design, not an optional one.
    const table = freshTable();
...
describe("createChatRecordTable - confirmed mutation caller revision guard", () => {
  it("rejects an older owner answer after a newer unknown-home delta, then admits an equal stated-home fill", () => {
    // The sole caller (`applyConfirmedMutation`) refuses revision < held before `applyPointRead`.
    const table = freshTable();
    expect(
      table.applyDelta({
        kind: "upsert",
