...
interface CountingArtifactFactory {
  ...
  openCount(): number;
  /**
   * How many constructed body clients have had `close()` invoked, summed across every artifact this
   * factory has ever built one for.
   */
  closeCount(): number;
  /**
  ...
  /**
   * Resolve the named body as method-incompatible, the way the mux does when `artifact.subscribe`
   * itself is refused - a statement about the ARM, not the tile.
   */
  deliverMethodUnsupported(artifactId: string): void;
}

/**
 * The body-lane factory, capturing each constructed body's OWN callbacks - keyed by artifact id -
 * so a test can deliver frames to one tile among several without disturbing the others.
 */
...
function createCountingStatusFactory(): CountingStatusFactory {
  ...
  return {
    ...
    closeCount: () => closes,
    /**
     * Deliver the control lane's first frame, the way a served subscription does. This is the SUCCESS
     * signal the probe reads: a frame can only arrive if the host is serving the method.
     */
    deliverSnapshot(): void {
    ...
    /**
     * Resolve the subscribe as method-incompatible, the way the mux does - a fatal close carrying
     * `INCOMPATIBLE`, which `isMethodIncompatibleClose` is the shared predicate for.
     */
    deliverMethodUnsupported(): void {
      if (live === null) throw new Error("no status client was constructed");
...
function statusSnapshotFrame(): EpicStatusSnapshotFrame {
  const parsed = epicStatusSubscribeServerFrameSchemaV10.parse({
    kind: "snapshot",
    // The text-only marker every frame on the two RECORD lanes carries. Omitting it is a parse error,
    // which is the point of building this through the schema instead of by hand.
    hasBinaryPayload: false,
    authorityEpoch: "epoch-1",
    securityEpoch: 1,
...
interface CountingStateFactory {
  ...
  closeCount(): number;
  /**
   * Resolve the records lane as method-incompatible, the way the mux does for a lane behind a flag
   * or a rolling upgrade - a fatal close carrying `INCOMPATIBLE`.
   */
  deliverMethodUnsupported(): void;
}
...
interface CountingLegacyFactory {
  ...
  openCount(): number;
  /** How many `@1` clients have been closed. */
  closeCount(): number;
}
...
describe("lane adapter probe - forever-unknown support must still reach the lanes", () => {
  ...
  it("(a) the relay case: forever-unknown support installs the lanes off the PROBE'S OWN OUTCOME, adopting its stream", () => {
    ...
    // Exactly one status stream client was constructed (the probe), and ZERO state clients - the
    // records lane must not open before the arm is settled.
    expect(rig.status.openCount()).toBe(1);
    expect(rig.state.openCount()).toBe(0);
    expect(rig.legacy.openCount()).toBe(0);

    // The host serves the subscription: its first control frame arrives. This is the ONLY thing that
    // happens.
    ...
    // ...and the status client count is STILL exactly 1 - the probe's stream was adopted, not
    // replaced.
    expect(rig.status.openCount()).toBe(1);
    expect(rig.legacy.openCount()).toBe(0);
  ...
  /**
   * (b2) The RE-PROBE, and the reason the arm's single answer is per ATTACHMENT rather than per
   * runtime.
   */
  it("(b2) a relay host upgraded in place re-probes on the reconnect edge and moves the legacy tab onto the lanes", () => {
    ...
    // The re-handshake. Support has NOT moved and cannot - this source has no `set` - so nothing here
    // resolves a manifest verdict.
    support.notify();
  ...
  /** (b3) A refused RE-probe has to clean up after itself, or there is exactly ONE re-probe ever. */
  ...
  it("(e) one epoch change reported TWICE rebuilds the replica exactly once", () => {
    ...
    // ONE frame, which the adapter reports through TWO paths: its translated `doc-unavailable` (which
    // `lane-body-translation` turns into `replace-replica`) and its own explicit `requestReplacement`
  ...
  it("(f) status success -> state INCOMPATIBLE -> legacy, while support stays unknown throughout", () => {
    ...
    // The records lane is REQUIRED, and support on this rig never moves off "unknown" - see
    // `FOREVER_UNKNOWN_SUPPORT`. Nothing but this typed close could have produced the fallback below.
  ...
  it("(g) status + state success -> first body INCOMPATIBLE -> legacy, exactly ONE replacement with N>1 tiles", () => {
    ...
    // Only ONE of the three open tiles reports the refusal - the arm's contract is to coalesce across
    // every body under it, not to fall back per tile.
    rig.artifacts.deliverMethodUnsupported("art-1");

    // Exactly one whole-epic legacy install, not one per open tile.
    expect(rig.legacy.openCount()).toBe(1);
    // `toBe`, not `toBeGreaterThan`: a per-tile fallback that replaced once per open body would still
    // "work" (end up on legacy) and must fail here on the generation step count alone.
    expect(rig.runtime.replicaGeneration()).toBe(before + 1);
  ...
  it("(h) the typed manifest answer agrees with the close answer: artifact.subscribe unsupported installs legacy, not lanes", () => {
    ...
    // Same destination as (g), reached through the manifest instead of a close: one required lane
    // refused is enough to keep the arm off "lanes" entirely - never a partial install with two lanes
    expect(rig.legacy.openCount()).toBe(1);
    expect(rig.status.openCount()).toBe(0);
    expect(rig.state.openCount()).toBe(0);
    expect(rig.artifacts.openCount()).toBe(0);
  ...
  it("(i) the required-lane latch is per ARM, not per session: two transitions on one arm object", () => {
    // The "controllable" rig, not the frozen one: this pin needs a SECOND manifest resolution after
    // the first fallback to legacy, and `FOREVER_UNKNOWN_SUPPORT.subscribeSupport` is a hardcoded `()
    const rig = buildRuntimeRig("controllable");
    runtimes.push(rig.runtime);
    const support = requireSupport(rig);
    ...
    // Lanes are attached again as arm #2, on the object that already fell back once: a fresh state
    // client was opened, and legacy has not opened a second time from this alone.
    expect(rig.state.openCount()).toBe(2);
    expect(rig.legacy.openCount()).toBe(1);

    // Transition 2: the records lane is refused again, on arm #2.
    rig.state.deliverMethodUnsupported();

    // Asserted on the COUNT, not on a boolean "is legacy installed" - a boolean would already read
    // true from transition 1 and pass under the bug.
  ...
  it("(j) acquireArtifactBodyLease's lease closure is idempotent on the demand half, not just the tier half", () => {
    ...
    // Two independent runtime leases on the SAME artifact - a canvas tile and the mobile switcher's
    // preview both wanting "art-1" open at once is the ordinary case `EpicArtifactBodyLanes.release`'s
    const releaseA = rig.runtime.acquireArtifactBodyLease("art-1");
    ...
    // Lease A's own closure invoked TWICE - the `finally` backstop shape the fix's doc comment names
    // (an early release followed by a defensive re-release on the same holder).
    releaseA();
    releaseA();

    // Still open: lease B never released and still wants this body.
    expect(rig.artifacts.closeCount()).toBe(0);
...
// ── Replacement coalescing (FIX B) and resume-too-old scope (FIX C) ──────── A separate rig from
// `buildRuntimeRig` above: those factories only ever deliver ONE hardcoded epoch-1 snapshot
...
/**
 * A lane-capable runtime with CONTROLLABLE status/state factories and a recording write-command
 * sender, for driving replacement coalescing (FIX B) and the `resume-too-old` scope (FIX C)
 */
...
describe("replacement coalescing across the state and status lanes (FIX B)", () => {
  ...
  it("the state lane and the status lane both reporting the SAME authority epoch produce exactly ONE replica-generation bump", () => {
    ...
    // ONE authority transition, reported by BOTH lanes.
    ...
    // Exactly one rebuild for the one true occurrence.
    expect(rig.runtime.replicaGeneration()).toBe(before + 1);
  });
  ...
  it("two reports naming DIFFERENT reasons for the same epoch still collapse to ONE rebuild", () => {
    ...
    // epoch-1, with a migration RUNNING - the fact that makes the status lane call the upcoming epoch
    // change "migration-completed" rather than a bare "authority-epoch-changed".
    rig.status.deliverSnapshot(
    ...
    // The status lane names this transition "migration-completed" (a migration was in flight under the
    // previous epoch); the state lane, which has no memory of the migration, always calls the same
    rig.status.deliverSnapshot(statusSnapshotFrameAt("epoch-2", null));
    rig.state.deliverSnapshot(
      stateSnapshotFrameAt("epoch-2", "authorityEpochChanged", 2),
    );
  ...
  it("a LATER, genuinely different epoch still rebuilds - the anti-latch control", () => {
    ...
    // epoch-2 -> epoch-3: a SECOND, genuinely different transition.
...
describe("resume-too-old replacement scope (FIX C)", () => {
  ...
  it("does not clear the write gate and does not bump the generation, unlike an authority-epoch-changed replacement", async () => {
    ...
    // Transport BEFORE the snapshots - opening after would wipe the freshness the status snapshot is
    // about to establish (see `lane-arm-open-cycle.test.ts`'s `openLaneRig` for the same rule).
    rig.status.open();
    rig.state.open();
    // The status lane's snapshot is what adopts the role and opens the write gate on the lane arm; the
    // state lane's own snapshot never touches control facts at all (FIX D's own point).
    rig.status.deliverSnapshot(statusSnapshotFrameAt("epoch-1", null));
    rig.state.deliverSnapshot(stateSnapshotFrameAt("epoch-1", "cold", 1));
    ...
    // Contrast, driven the SAME way (through the state lane alone): a genuine authority-epoch-changed
    // replacement DOES both. This is the point of pairing the two cases in one test.
    rig.state.deliverSnapshot(
    ...
    await flushMicrotasks();
    // The write gate is shut: `beginAuthorityReplacementCycle` reset this cycle's snapshot freshness
    // (the transport legs stay as the still-open sessions reported them), and nothing has
    expect(rig.sentCommandCount()).toBe(2);
  });
});
