...
interface DemoteRecord {
  ...
  /**
   * Answer with an ERROR reply rather than a verdict, on a worker that stays alive - which is what
   * `serve()` does with a handler that threw, and what a malformed response decodes to.
   */
  fail(message: string): void;
}

/**
 * A worker side that answers materialize immediately and parks every demote until the test settles
 * it - which is what makes "before the ack" a state a test can actually stand in.
 */
  ...
  const unsubscribe = pair.worker.subscribe((message) => {
    ...
    if (call.kind === "body/demote") {
      ...
      demotes.push({
        ...
        bytes: [...update],
        settle: (answer) => {
          // The wire answer carries a REASON now, and the decoder refuses a response missing it - correctly,
          // since a verdict this side cannot read is not one to act on.
          respond({
            ...answer,
...
/** A scheduler the test drives, so the linger window is a call rather than a real minute. */
...
/**
 * For the constructions that never reach an awaiting body - their worker side always answers with
 * bytes or with nothing at all, so no retry can stall.
 */
...
function setup() {
  ...
  // Recorded for the same reason: a stalled retry keeps waiting rather than failing, so the report
  // is the only observable that the disagreement between the availability map and the tier was
...
describe("constraint 1 — the doc stays hot until the ack", () => {
  it("does not drop the doc, nor settle the charge, before the worker answers", async () => {
    ...
    // The accountant knows it is on its way out, and has NOT been told the bytes are free. A doc
    // reported cold here is one the accountant believes it can spend while the store still holds it.
    expect(budget.calls).toEqual(["chargeHot:artifact-1:3"]);
  ...
  it("keeps the body hot while another holder remains, whatever the releaser asked for", async () => {
    // The retention is the LAST releaser's call, and this is why that is safe: an earlier return on
    // `leases > 0` means the question is never reached while anyone still holds the body.
    const { leases, worker, scheduler } = setup();
...
describe("constraint 2 — a worker that dies mid-demote", () => {
  it("keeps the doc, and re-sends once to the replacement", async () => {
    ...
    // The re-send is driven from the state the ORIGINAL bridge holds, so this asserts the observable
    // half: the doc survived with its bytes intact and is re-sendable.
    void nextLeases;
...
describe("constraint 3 — re-acquire before the ack wins locally", () => {
  it("cancels the pending drop, does no round trip, and ignores the stale ack", async () => {
    ...
    expect(docs.installed).toEqual(["artifact-1"]);
    // The re-acquire DISARMED the pending demote.
    expect(leases.unacknowledgedDemoteKeys()).not.toContain("artifact-1");

    // It names a generation the lease has outlived, so it must do nothing at all - dropping here takes
    // the doc out from under an editor that is bound to it right now.
    worker.demotes[0]?.settle({ accepted: true, settledBytes: 4_096 });
    await Promise.resolve();
    await Promise.resolve();
...
describe("constraint 3 (legacy @1 arm) — a room-keyed re-acquire revives the same way", () => {
  it("disarms the pending demote when the re-acquire arrives through materialize", async () => {
    // On `@1` the doc key is a ROOM id, so a held entry is not findable by artifact id and EVERY
    // re-acquire goes through `body/materialize` - the post-materialize arm, not the fast path.
    const pair = createFakeBridgePair("sync");
...
/**
 * The LINGER - the hot doc's own reclaim window, relocated with the hot doc. Pre-flip the tier ran
 * this timer because the tier owned the hot doc.
 */
describe("the linger window", () => {
  it("re-acquiring inside the window costs no materialize and no demote", async () => {
    ...
    // THE assertion: still ONE materialize. The doc was never released, so the second acquire is a
    // reference count going back up rather than a round trip.
    expect(worker.materializes).toEqual(["artifact-1"]);
    expect(worker.demotes).toEqual([]);
    expect(docs.dropped).toEqual([]);
    expect(docs.has("artifact-1")).toBe(true);
  ...
  it("flushes lingering docs immediately at teardown", async () => {
    // A linger is a bet that the user is coming back.
    const { leases, worker, scheduler } = setup();
    const grant = await leases.acquire("artifact-1", "linger");
    if (grant.kind !== "granted") throw new Error("expected a grant");
...
describe("the linger window — re-arming", () => {
  it("restarts the window on each release rather than firing on the first one's clock", async () => {
    // Written after ablating `cancelLinger` in `reviveAndHold` left the pins above GREEN.
    const { leases, worker, scheduler } = setup();
    const first = await leases.acquire("artifact-1", "linger");
    if (first.kind !== "granted") throw new Error("expected a grant");
...
describe("the hot-doc cap", () => {
  it("evicts the least-recently-held lingering doc once the population exceeds it", async () => {
    // The BACKSTOP, and it moved here for the same reason the linger did: it bounds how many HOT docs
    // exist, and the hot docs are main's now.
    const { leases, worker, docs, scheduler } = setup();
  ...
  it("never evicts a doc that is still leased, even past the cap", async () => {
    // A pinned doc is not evictable: taking a body out from under a bound editor is data loss, not
    // reclamation.
...
describe("the forward-only release", () => {
  ...
  it("KEEPS the doc when the tier refuses, and looks again next window", async () => {
    // A collaborator is present in the room.
    const { leases, docs, scheduler, releases } = forwardOnlySetup({
      released: false,
      reason: "pinned",
    });
...
describe("the forward-only release — generation fencing", () => {
  it("ignores a release reply belonging to a PREVIOUS acquire/release cycle", async () => {
    const pair = createFakeBridgePair("sync");
    const releases: string[] = [];
    // Held, not answered: this is the whole stimulus.
    ...
    // The FIRST reply finally lands. Length, not an `=== undefined` test: `noUncheckedIndexedAccess`
    // is off, so the element type carries no `undefined` for that comparison to narrow.
...
describe("a demote that fails on a LIVE worker", () => {
  ...
  it("a re-acquire inside the re-armed window cancels the retry", async () => {
    // The re-arm must not become a second way to demote a doc someone is holding.
    const { leases, docs, worker, scheduler } = setup();
    const first = await leases.acquire("artifact-1", "linger");
    if (first.kind !== "granted") throw new Error("expected a grant");
    first.release();
...
/**
 * A body reassigned to a different room WHILE STILL AWAITING ITS SEED. The moved-key rule was
 * already applied where bytes arrived.
 */
describe("an awaiting body reassigned to another byteless room", () => {
  ...
  it("releases the room the worker actually retained, not the one the acquire asked about", async () => {
    ...
    // BEFORE the holder releases anything: room-1's demand comes off at the moment the move is
    // adopted, not at unmount.
    expect(releases).toEqual(["room-1"]);

    grant.release();
    await settle();

    // BOTH rooms, and each for its own reason. room-2 is the re-key: without it this reads
    // `["room-1"]` and room-2's demand is retained with nothing left that can release it.
  ...
  it("releases every room a body vacates when it moves twice", async () => {
    // Reassignment is not once-per-body.
