...
// The pre-fix shape: bypass -> bare children; ready -> children under a wrapper component.
...
describe("back stays functional after promoting a system overlay to a tab", () => {
  ...
  it("back click #1 after promotion escapes the overlay entry instead of re-pushing the tab route", async () => {
    ...
    // The core trap: pre-fix, the guard's cold-load-only redirect re-arms on the remount this Back
    // triggers and immediately pushes /settings/general right back on top, leaving `rendered`
