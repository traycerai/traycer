...
/**
 * Dial timeout for per-request WebSocket connections. 5 s is generous enough
 * to absorb a cold desktop host spin-up on first use while still short
 * enough that an unreachable localhost surfaces as a transport failure in a
 * single user gesture, not a multi-tab hang.
 */
const DEFAULT_WS_DIAL_TIMEOUT_MS = 5_000;
...
>(): TypedHostRuntime<Registry> {
  ...
  function HostRuntimeProvider(
    ...
    useEffect(() => {
      ...
      const rawMessenger: IHostMessenger<Registry> =
        messengerFactory !== null
          ...
          : new WsRpcClient<Registry>({
              ...
              dialTimeoutMs: DEFAULT_WS_DIAL_TIMEOUT_MS,
      ...
      const messenger: IHostMessenger<Registry> = createAuthAwareMessenger(
        rawMessenger,
        auth,
        null,
      );
