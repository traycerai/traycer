// Conflict groups for `cn` (src/lib/utils.ts), compiled into
// `src/lib/cn-tables.ts` by `bun run cn:build`. A new spacing or size token
// added to `src/index.css` belongs here too, followed by that rebuild -
// `src/__tests__/cn-tables-up-to-date.test.ts` fails otherwise.

// The app's custom `--text-*` typography tokens (src/index.css). Without
// this, the merger cannot classify `text-ui-sm` & friends, falls back
...
// The app's safe-area spacing tokens (src/index.css). The stock scales accept
// numbers, keywords and arbitrary values, so a bare token like `safe-bottom`
// matches nothing and the class falls through UNCLASSIFIED. Unclassified is
// safe - the class is never dropped - but it also never conflicts, so
// `h-safe-dvh` does not displace an `h-full` from earlier in the same merge and
// both reach the stylesheet, where source order decides. That failure is
// invisible on desktop, because every safe-area token collapses to zero there.
// Registering them is what makes the tokens ordinary overrides.
