...
/** `traycer agent send` - hand a prompt to another agent (`agent.sendMessage`). - `--expect-reply` opens (or reuses) a thread keyed on the (sender, receiver) pair; the host returns a `responseId` the receiver echoes back on its final reply. - `--response-id <id>` on a send with `--expect-reply` omitted is the final reply that closes that thread; omit both for a one-shot, no-reply message. */
