...
// `traycer host uninstall [--all]`: default → remove the installed + staged host bytes and the install record, and NOTHING else.
// The OS service stays registered, and a host that is already running keeps running until it exits - after which the surviving registration has no valid install to launch.
...
export interface RunHostUninstallDeps {
  ...
  /** The host process this environment last published, read BEFORE the teardown - because the teardown destroys it. On Windows the uninstall removes pid metadata even when its `taskkill` calls failed, so a probe that runs afterwards has nothing left to look at. */
  readPublishedHost(environment: Environment): Promise<PublishedHost | null>;
  /** Did THAT process exit? Compares the OS process against the start identity pid.json published, so a recycled pid reads as gone rather than alive. */
  probeProcessExited(
    pid: number,
...
// Did the stop CALL resolve?
// Necessary for the runtime purge but not sufficient - the caller pairs this with process-identity, registration and successor checks, because on Linux and Windows every teardown call tolerates its own failure and resolving proves nothing about the process.
...
export async function runHostUninstallWithAttempt(
  ...
  const contenderOptions: WithCliUpdateContenderOptions = {
    ...
    // The maintenance lease creates the capability for the sudo CALLER's canonical home while this process runs elevated.
    // Revalidation must name the same home, or every in-attempt verify resolves the elevated process's own home and refuses with wrong-host-home.
...
async function runHostUninstallWithActuators(
  ...
  // Captured FIRST, before anything is torn down.
  // The teardown destroys this evidence: on Windows `uninstallService` removes pid metadata even when its `taskkill` calls failed or timed out, so a probe that runs afterwards is guaranteed to find nothing and would read a surviving host as gone.
  const published = args.all
    ? await readPublishedHostBestEffort(deps, ctx)
    : null;
  let liveness: LivenessObservation = "unknown";
  // Observed BEFORE anything is removed, on the default path: it is the only way this command can say what it is about to leave behind.
  // `--all` reads its registration AFTER acting instead (see below), since what matters there is what the teardown actually left.
  ...
  if (args.all) {
    ...
    // Deregister BEFORE waiting for the process to exit.
    // On macOS the running job stays under launchd's `KeepAlive` supervision until its registration is torn down (`uninstall` -> `launchctl bootout`); stopping first and deregistering after leaves a window where a non-clean SIGTERM exit gets treated as a failed/crashed exit and launchd respawns the host before we ever reach `uninstall`.
    ...
    // Install removal stays best-effort, but runtime files are preserved unless the process is confirmed GONE.
    // A host that is still serving is still writing its pid metadata and rotating its log.
    const stopArgs = {
      environment: ctx.environment,
      label,
      logger: ctx.logger,
    ...
    // The resolved `stop()` is necessary but NOT sufficient - see `findLiveHost`.
    // Both halves are required: a stop that threw means the host was never asked/consented to die, and a probe that still finds one means the kill did not land.
    liveness = await observeLiveness(deps, ctx, published);
    // Registration is verified too, for the same reason: on Linux the unit file is removed even when `disable --now` timed out, and on Windows `/Delete` tolerates failure.
    // Ask the controller what is actually registered now rather than inferring it from a resolved call.
    retainedAfterAll = await readServiceStateBestEffort(deps, ctx);
    // Re-read at the PURGE BOUNDARY, after every probe above has had its chance to take time.
    // `published` describes the child we captured; this asks whether anything is publishing NOW - a supervisor relaunch loop can produce a successor while these probes are running.
    ...
    if (!successorRead.answered) {
      ...
      // Identity-probe B rather than inheriting A's verdict.
      // A successor record is not proof of life either - it can be stale - so it gets the same treatment A got.
      liveness = await observeLiveness(deps, ctx, successor);
    }
    registrationClear = retainedAfterAll?.state === "not-installed";
    // Runtime purge is withheld unconditionally: an uninstall must not delete a live host's data dir.
  ...
  if (!args.all) {
    // AFTER the removal, deliberately.
    // The default path stops nothing, so a still-registered service or a foreground supervisor can publish a successor while the install and staged directories are being deleted - and a read taken before that would report the machine clean while the successor serves the install just removed.
  ...
  // Both fields report only what was OBSERVED.
  // Neither is derived from a backend call having resolved: on Linux and Windows every teardown call tolerates failure, so a resolved `uninstall()`/`stop()` is not evidence that the unit is gone or the process exited (see `findLiveHost`).
  const observedService = args.all ? retainedAfterAll : retainedService;
  // TRI-STATE, and the asymmetry is the point.
  // A positive registration is a real observation - `launchctl print` naming a loaded label, a systemd unit reported active.
  const registrationRetained: boolean | null =
    observedService === null
      ...
      : observedService.state === "not-installed"
        ? null
        : true;
  // NO platform can verify this today. macOS looked like the exception, but `inspectLaunchdOwnership` tolerates non-zero too (a timeout collapses to "not-loaded") and an unloaded SMAppService record is invisible to it - which is exactly the record `uninstallService` warns may respawn at next login on macOS <= 25.
  // Linux re-reads the manifest this command deleted; Windows maps every `/Query` failure to `not-installed`.
  const serviceRegistrationRetained = registrationRetained;
  // Liveness comes from PROCESS IDENTITY on both paths, never from `ServiceStatus.state`.
  // That field is about REGISTRATION: `busy-check.ts` keys liveness off pid metadata instead, and `externally-managed` (the Desktop-owned macOS label) deliberately folds in no run state at all and reports no pid.
  ...
  return {
    data: {
      ...
      removedInstallDir: result.removedInstallDir,
      removedStagedDir: result.removedStagedDir,
      // Request semantics, VETOED by a positive readback.
      // Desktop's `parseUninstallResult` projects this straight to `deregisteredService`, whose contract is "actually accomplished", so a readback that positively found the registration still there must not be published as success.
      serviceUninstalled: serviceUninstalled && registrationRetained !== true,
      deregisterRequested: serviceUninstalled,
      purgedRuntime: result.purgedRuntime,
      // What the machine is left holding, so an automated caller does not have to infer the default path's end state from the absence of `serviceUninstalled`.
      // Both are `null` when the platform probe could not answer - see the tri-state note above.
    ...
    human: humanSummary({
      ...
      // The READBACK, not the request.
      // Saying "deregistered OS service" while the same result reports `serviceRegistrationRetained: true` had the prose and the payload contradicting each other in one breath, and an unanswerable probe must not count as agreement either.
      deregisterRequested: serviceUninstalled,
      serviceRegistrationRetained,
      hostStillRunning,
    }),
...
// Best-effort like the status probe, and for the same reason: this exists to DESCRIBE the end state, so an unreachable endpoint degrades to "unknown" rather than failing a removal that already happened.
...
/** Distinguishes "nothing is published" from "the read did not answer" - `readHostPidMetadata` collapses both to null, and only the first is evidence. */
...
/** Is the process this environment published still alive? Every arm defaults toward `"unknown"`, because `"gone"` is the only answer that licenses deleting another process's runtime files. */
async function observeLiveness(
  ...
  // `readHostPidMetadata` accepts any JSON number, while the identity helper answers `dead` for anything non-integral or <= 0.
  // A record carrying `pid: -5` would otherwise read as proof the host exited and license the purge.
...
// Never fails the uninstall: this read exists to DESCRIBE the end state, and a platform probe that cannot answer (launchctl missing, a systemd user manager that was never started) must not turn a working removal into an error.
// An unanswerable probe simply drops the extra disclosure.
...
function humanSummary(args: {
  ...
  // Keyed on the liveness evidence, not on the purge - the purge is now always withheld, so using it would warn on every clean teardown.
  // What an operator needs to know is whether anything is still serving.
  if (args.deregisterRequested) {
    ...
    // Deliberately says what THIS command did, not what the machine now holds.
    // Windows' own teardown removes pid metadata unconditionally after kills that tolerate their own failure (`platforms/windows.ts`), so a blanket "runtime was kept" is false there - and correcting that belongs to the backend quiescence work, not to this summary.
    parts.push(
