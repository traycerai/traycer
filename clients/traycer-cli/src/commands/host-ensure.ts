...
// `traycer host ensure [--release <v> | --from <path>]` - idempotent "make the host installed + registered + running" command.
// This is the single host-lifecycle call the desktop shell makes after the user signs in; the desktop never registers services or calls launchctl itself.
export interface HostEnsureArgs {
  ...
  // Install the host bytes only; leave OS-service registration to the host.
  // The desktop passes this because it registers the macOS login item via SMAppService (only the .app can attribute the row).
...
export function buildHostEnsureCommand(args: HostEnsureArgs): CommandFn {
  return async (ctx): Promise<CommandResult> => {
    // This command can install + register + START a host (three of its four actions), so it owns the same sign-in pre-flight and post-start credential provisioning as `host install` and `host service install` - see install-auth.ts for the start-capable command inventory.
    // Unlike those two, ensure is idempotent and its FOURTH action is a no-op, so the pre-flight cannot run unconditionally: the desktop calls ensure on every launch, and a signed-out operator whose host is already healthy would be prompted to sign in for a command that then does nothing.
    ...
    // After ensureHost returns - and with it the cli-lock it took - exactly like host-install's post-lock probe.
    // `serviceLifecycle` is non-null on every branch that started or cycled the service, so the two `"none"` cases below are both deliberate: - `serviceLifecycle === null` is the noop branch: already installed, registered AND running, so this command started nothing (and, per `beforeMutate` above, never even ran the pre-flight).
...
function describeAction(result: HostEnsureResult): string {
  // Prefer the archive's own build stamp: `version` is the caller's idempotency identity and can lag the installed bytes when an older CLI installs a newer archive (the echo then names a build that isn't the one that just started).
