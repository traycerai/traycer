...
// Reason prefixes the host rides on the generic `RPC_ERROR` message rather than a typed wire code - see `epic-set-chat-archived-resolver.ts`, `AgentArchiveBlockedError`, and `EpicChatRecordMissingError`.
// The 409-vs-500 distinction never crosses the wire, so this is the only signal available.
...
/** `traycer agent archive --agent-id <id> [--unarchive]` - toggle the durable archive flag on a GUI chat or terminal agent (`epic.setChatArchived`). One RPC covers both record kinds; the host resolves the id against chats first, then terminal agents. */
...
function archiveHumanMessage(
  ...
  // Deliberately noncommittal about WHY nothing changed.
  // This host throws `RECORD_NOT_FOUND:` for an absent id, but `epic.setChatArchived@1.0`'s response contract also permits `updated: false` for a record that did not match, so a compatible host may answer `false` where this one throws.
...
function remapArchiveError(err: unknown, agentId: string): unknown {
  ...
  if (err.message.startsWith(AGENT_BUSY_PREFIX)) {
    return cliError({
      ...
      // Carry the host's OWN busy text through rather than substituting a single "stop it first" line.
      // `archiveBlockedMessage` has two arms and they prescribe DIFFERENT remedies: a running turn is cleared by stopping, whereas detached subagents/workflows/scheduled wakes survive a stop and must be waited out or stopped individually.
      message: `traycer: ${stripBusyPrefix(err.message)}`,
      details: null,
      exitCode: 1,
    });
  }
  // Cross-host archive refusal.
  // `E_AGENT_NOT_LOCAL` already exists as the CLI surface for "this agent runs elsewhere" (see `mapHostRpcError`), and the GUI gives this same refusal its own copy - without the remap a script sees only `E_UNEXPECTED` plus raw internal text for an actionable condition.
...
/** Drops the wire-level `AGENT_BUSY:` reason prefix, leaving the host's human-readable explanation (which already names the applicable remedy). */
