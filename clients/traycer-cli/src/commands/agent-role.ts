...
/** `traycer agent role claim|list|relinquish` - the Claude TUI surface of the Task role registry (`agent.roles.*`). Every command validates the request it builds through the PROTOCOL request schema (`parseUserInput`) BEFORE any transport: role/scope normalization (NFC, whitespace fold, trim) and claimId UUID validation happen client-side, a bad value reports the offending field as a clean `E_INVALID_ARGUMENT`, and NO request frame is sent for invalid input. */
