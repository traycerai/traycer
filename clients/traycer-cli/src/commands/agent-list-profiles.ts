...
/** `traycer agent list-profiles <harness>` - the provider profiles available for a harness (the ambient CLI login plus any managed subscriptions), each with its cached rate-limit status and a `--profile` value that reselects it in a later `create` / `profile-rate-limits` / `configure` call. `agent.listProviderProfiles` is an optional host method: against a host that doesn't advertise it, the transport raises `E_HOST_UNSUPPORTED` and the shared agent error boundary reports it as `E_HOST_UNSUPPORTED` with host-upgrade guidance. */
