...
describe.skipIf(process.platform === "win32")(
  "host free-port - genuine cli-lock contention",
  () => {
    beforeEach(() => {
      workHome = mkdtempSync(
        join(tmpdir(), "traycer-host-free-port-lock-test-"),
      );
    ...
    it("blocks behind a foreign lock holder (e.g. an in-progress apply) - cannot kill inside its critical section - and proceeds only after it releases, without ever restarting", async () => {
      ...
      const holdBarrierDir = mkdtempSync(
        join(tmpdir(), "traycer-free-port-lock-barrier-"),
      );
      ...
      try {
        ...
        const { buildHostFreePortCommand } = await import("../host-free-port");
