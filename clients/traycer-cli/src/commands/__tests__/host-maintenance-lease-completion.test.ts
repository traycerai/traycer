...
// TWO SOURCE-ORDER SUITES AND ONE BEHAVIOURAL ONE, labelled so the difference is not something a reader has to infer.
// `superviseRootMaintenanceExecutor` is module-private and its completion bookkeeping only becomes observable by spawning a real supervisor process that spawns a real detached actuator.
...
describe("root maintenance executor completion is a box, not a value sentinel", () => {
  ...
  it("never uses `undefined` to mean 'no completion frame arrived'", () => {
    // `JSON.stringify({ value: undefined })` drops the key, so an executor that completes with nothing sends exactly the shape the old sentinel read as "never completed" - and a clean exit 0 was then reported as `maintenance executor exited (0, none)`.
  ...
  it("boxes the completion and tests the box for null", () => {
    ...
    // Resolving `completion.value` rather than the box is what keeps an actuator's own `null` - the Linux platform install's "left for the developer to install by hand" - reaching the caller intact.
  ...
  it("subscribes to close/error before the first await — Node emits them once, never replayed", () => {
    // The defect this guards: with the first subscription sitting AFTER the liveness rebind (a filesystem round trip), an executor that failed to exec, or died immediately, could emit `error`/`close` into no listener at all before the subscription landed - the `error` became an uncaught event that killed this process mid-lease.
  ...
  it("reconciles an early death once the promise body owns settlement", () => {
    // The executor can have terminated while the liveness rebind above was in flight; nothing between the subscriptions and this point could act on that evidence, so the promise body re-checks it once it is the thing holding `onTermination`.
    expect(supervisor).toContain("if (termination !== null) onTermination();");
  });

  it("recordTermination keeps only the first evidence", () => {
    // A failed spawn emits `error` and then `close`; the error names the cause while that close would otherwise overwrite it with a bare null code.
    // Guard must live INSIDE `recordTermination`, not merely somewhere in the file.
  ...
  it("captures stdout chunks at spawn — before the liveness rebind, not after", () => {
    // The stdout `data` subscription is attached in the same unbroken subscription block as `error`/`close`, before the rebind await below it.
    // A `data` handler attached only after that filesystem round trip would miss whatever the executor wrote while it was in flight.
  ...
  it("subscribes to close/error before every throwing guard, not only before the rebind await", () => {
    // The pid guard is not the only throw in this function: the pipe guard above it throws synchronously too, for a spawn that can still emit `error` asynchronously.
    // Listeners must precede it as well.
    const errorIdx = supervisor.indexOf('child.once("error"');
    const closeIdx = supervisor.indexOf('child.once("close"');
  ...
  it("the stdout data handler defers dispatch until dispatch is armed", () => {
    // Chunks are captured unconditionally into `buffer`; frames are only drained once `frameDispatchArmed` is set.
    // Pinning the literal guards against a rewrite that drains eagerly on every chunk again.
  ...
  it("arms dispatch, drains, and only then reconciles the recorded termination — in that order", () => {
    // A completed executor that died mid-rebind has its `complete` frame sitting in the buffer and its `close` already recorded in `termination`.
    // Draining before reconciling is what lets the classification see the completion it earned; reordering either step resurrects the "confident failure over finished root work" defect.
  ...
  it("serializes frame handlers on a single dispatch tail instead of firing and forgetting", () => {
    // A fire-and-forget dispatch let the close path restore the holder and reject while a destructive handler (an `execute`'s service stop, a `bind-actuator`'s publication) was still mid-flight.
    // `drainFrames` must chain each handler onto `dispatchTail` rather than invoking it bare.
  ...
  it("settleFailure fences on the dispatch tail before any teardown or restoration", () => {
    // The tail must be awaited right after `settled = true`, and the actuator-group/restore section - restoring the holder under a still-running handler re-opens the race this settlement exists to close - now lives in `settleFailureAfterTail`, split out so `onTermination`'s completion path can reuse the same post-tail teardown (see the completed-after-tail test above).
    // The fence this test guards is therefore two-part: `settleFailure` must await the tail before ever calling into `settleFailureAfterTail`, and the teardown itself must live in the callee that name promises it does.
  ...
  it("the clean-completion path fences on the dispatch tail before waiting on the process group", () => {
    // Same fence, other settlement path: the resolve IIFE inside `onTermination` must await `dispatchTail` before `waitForProcessGroupExit`/`restoreHolder`, not after.
  ...
  it("kills the child and rethrows when the initial liveness rebind fails", () => {
    // The executor is parked at its start gate waiting for frames that will now never come; a bare rethrow with no kill would leave a privileged child process alive with nothing supervising it.
  ...
  it("bind-actuator's pid guard floors at > 1, not > 0 - pid becomes a negated process-group kill target", () => {
    // `bind-actuator`'s `value.pid` is later passed as `supervisedProcessGroupId` and negated for `terminateAndReapProcessGroup`'s `kill(-groupId, ...)`.
    // `kill(-1, ...)` means "every process this user may signal", not group 1 (init's) - the same floor the lock parsers already apply (cross-process-lock, host-update-attempt-liveness).
  ...
  it("reads `completed` only AFTER the dispatch tail settles, not synchronously at close time", () => {
    // Pre-fix, `completed` was read into `completion` BEFORE the async IIFE even started (synchronously, at `close` event time).
    // An executor that wrote its `complete` frame and closed DURING the initial liveness rebind has that frame's handler still queued on `dispatchTail` right here - reading `completed` before awaiting the tail sees it still `null` and classifies a finished platform install/uninstall as `maintenance executor exited (0, none)` instead of resolving the real completion.
    const onTerminationIdx = supervisor.indexOf(
      "onTermination = (): void => {",
    );
...
describe("assertPathHelpersBoundToTarget", () => {
  // Behavioural, via the module's public entry point.
  // The guard runs before anything else in `runHostMaintenanceLease`, so a refusal surfaces as a rejected promise with nothing else having happened.
  ...
  it("refuses when the sealed target names a different account than the path helpers resolve", async () => {
    // A directory that exists (so `realpathSync` succeeds) but is not this process's home - the win32 shape, where `os.homedir()` ignores the `HOME` the root script sets and the binding silently does not take.
    // `tmpdir()` instead of a hardcoded "/tmp" so this also exists on Windows.
  ...
  it("does NOT refuse an ordinary invocation with no sealed target", async () => {
    // The positive control, and the one that matters most: the guard is scoped to the root-script path.
    // An unscoped refusal would break every ordinary `host maintenance-lease` call, and the two assertions above would still pass.
