...
// Genuine two-process regression coverage for `host stop`'s `cli-lock` wiring (Host Update Layer Redesign Tech Plan, "Lifecycle lock coverage"): a terminal stop must not enter another actor's apply/install/activation critical section and kill the process it just started - only real OS-level file contention (the same worker `cli-lock.test.ts`/`host-restart-lock.test.ts` use) can be trusted to prove this, not an in-process simulation.
...
describe.skipIf(process.platform === "win32")(
  ...
  () => {
    ...
    it("blocks behind a foreign lock holder (e.g. an in-progress apply) and proceeds only after it releases", async () => {
      ...
      const holdBarrierDir = mkdtempSync(
        join(tmpdir(), "traycer-stop-lock-barrier-"),
      );
