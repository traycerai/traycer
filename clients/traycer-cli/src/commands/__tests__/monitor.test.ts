...
describe("runMonitor recovery", () => {
  it("states durable queued work from the @1.1 structural outcome flag", async () => {
    const stdout = vi
    ...
    session.serverFrame({
      ...
      notice: {
        ...
        outcome: "exited",
        isCorrective: false,
        durableQueuedWorkRemains: true,
      },
    });
    expect(stdout).toHaveBeenCalledWith(
      expect.stringContaining(
        "durable queued work remains and may run if the agent returns",
      ),
    );
    void monitor;
  });

  it("drops a malformed @1.1 frame instead of reinterpreting it as legacy", async () => {
    const stdout = vi
      .spyOn(process.stdout, "write")
      .mockImplementation(() => true);
    const monitor = runMonitor({ agentId: "a1", epicId: "e1" });
    ...
    stdout.mockClear();
    // Carries @1.1 delivery metadata but an invalid outcome: the @1.1 parse
    // fails, and the frozen @1.0 schema would strip the metadata and render
    // this as a legacy inactivity notice — losing the delivery truth. It
    ...
    const rendered = stdout.mock.calls.map(([t]) => String(t)).join("");
    expect(rendered).not.toContain("[traycer inbox]");
    // A genuinely legacy frame (no @1.1 keys) still renders via the fallback.
    session.serverFrame({
      kind: "notice",
      hasBinaryPayload: false,
      notice: {
    ...
    const renderedAfter = stdout.mock.calls.map(([t]) => String(t)).join("");
    expect(renderedAfter).toContain("[traycer inbox]");
    void monitor;
  });
