...
const decisionSkippedNonInteractive: AutoBootstrapDecision = {
  status: "skipped",
...
describe("loginCommand auto-bootstrap wiring", () => {
  it("runs auto-bootstrap before the browser sign-in (Core Flow 7)", async () => {
    const autoBootstrap = await import("../../host/auto-bootstrap");
    const loginFlow = await import("../../auth/login-flow");

    let order: string[] = [];
    (autoBootstrap.maybeAutoBootstrap as Mock).mockImplementation(async () => {
      order.push("bootstrap");
      return decisionInstalled;
    });
    (loginFlow.runLoginFlow as Mock).mockImplementation(async () => {
      order.push("login");
      return {
        token: "t",
    ...
    expect(order).toEqual(["bootstrap", "login"]);
    expect(autoBootstrap.maybeAutoBootstrap).toHaveBeenCalledWith(
      expect.objectContaining({ trigger: "login" }),
    );
    expect((result.data as { bootstrap: AutoBootstrapDecision }).bootstrap.status).toBe(
      "installed",
    );
  });

  it("respects --no-bootstrap: still signs in but surfaces a skipped decision", async () => {
    const autoBootstrap = await import("../../host/auto-bootstrap");
    const loginFlow = await import("../../auth/login-flow");
    ...
    const data = result.data as { bootstrap: AutoBootstrapDecision };
    expect(data.bootstrap.status).toBe("skipped");
    expect(data.bootstrap.reason).toBe("explicit-no-bootstrap");
  });

  it("respects CI / TRAYCER_NONINTERACTIVE: surfaces a noninteractive skipped decision", async () => {
    const autoBootstrap = await import("../../host/auto-bootstrap");
    const loginFlow = await import("../../auth/login-flow");

    (autoBootstrap.maybeAutoBootstrap as Mock).mockResolvedValue(
      decisionSkippedNonInteractive,
    );
    (loginFlow.runLoginFlow as Mock).mockResolvedValue({
      token: "t",
      user: { id: "u", email: "a@b", name: "A" },
    ...
    const { loginCommand } = await import("../login");
    const ctx = makeCtx(makeRuntime({ nonInteractive: true }));
    const result = await loginCommand(ctx);
    const data = result.data as { bootstrap: AutoBootstrapDecision };
    expect(data.bootstrap.reason).toBe("noninteractive-cannot-prompt");
  });

  it("surfaces a failed bootstrap as structured data without aborting sign-in", async () => {
    const autoBootstrap = await import("../../host/auto-bootstrap");
    const loginFlow = await import("../../auth/login-flow");
    ...
    const ctx = makeCtx(makeRuntime({}));
    const result = await loginCommand(ctx);
    // Bootstrap failure must NOT prevent sign-in.
    expect(loginFlow.runLoginFlow).toHaveBeenCalled();
    const data = result.data as { bootstrap: AutoBootstrapDecision };
