...
// This file exists because the equivalent gap was a certified HIGH in Ticket 05 and must not be recreated.
...
// The command tree, the option definitions, and the callback under test are real.
...
describe("--ack-nonce reaches the builder through the Commander bridge", () => {
  // SEAM PROOF, asserted first.
  // Every capture-based assertion below reads `captured.args`; if the mock were not on the path the callback imports, that array would stay empty and the assertions would fail for the WRONG reason - a later edit could then "fix" them by loosening the expectation.
  ...
  it("forwards the nonce, with every other field intact", async () => {
    ...
    // The bridge's job is to translate every option, and a test watching one field would not notice a sibling being dropped or crossed over in the same callback.
  ...
  // The NEGATIVE CONTROL, and it is load-bearing rather than decorative: a bridge hard-coded to some constant nonce would satisfy the test above while breaking every ordinary invocation.
  // The pair pins the MAPPING, not a value.
  ...
  it("carries the nonce alongside the other options, not instead of them", async () => {
    // A dispatch really does arrive with more than one flag set.
    // Pinned so a bridge that handles the nonce by rebuilding the args object cannot drop its siblings unnoticed.
    await buildProgram().parseAsync(
