...
// `traycer host start --environment <ch>` is the single supervisor entry point.
// There is one launch path: read the environment's HostInstallRecord, refuse to spawn with stable machine-readable codes when the record is missing / the executable is gone, and otherwise spawn `record.executablePath` directly.
...
describe("resolveHostStartTarget", () => {
  ...
  it("returns the install record's executablePath as the spawn target", async () => {
    ...
    // The host home dir, never the executable's own directory: a CWD inside `install/` is an open handle children inherit, and on Windows any such child that outlives the pre-update kill fails the install swap rename with EBUSY.
...
/** The ONE place this file bridges `StubChild` to `ChildProcess`. `StubChild` is an EventEmitter standing in for a real child process, and no structural type bridges those. */
...
/** Capture-only stderr tee: never opens host.log. Typed as StderrTee so no cast is required (concrete StderrLogTee private fields stay out of deps). */
...
function makeRunStubs(
  ...
  // The stub implements only the surface `runHostStart` touches; route it to `ChildProcess` through an explicit `unknown` intermediate rather than a chained `as unknown as` assertion.
  ...
  const deps: Partial<RunHostStartDeps> = {
    // `runHostStart` falls back to the REAL `createCliLogger` (writing to the actual production `~/.traycer/cli/cli.log`) via `deps.logger ?? createCliLogger(...)` whenever this is left unset - `??` treats `undefined` as nullish just like a missing key, so it must be supplied explicitly here, not left to the `Partial` default.
    // Spy wraps noop for crash-diagnostics message assertions.
    logger: spyLogger,
    // Same hazard as `logger` above: left unset, the `Partial` default falls through to the REAL `findLiveIncumbentHost`, which reads the developer's actual `~/.traycer/host/pid.json` and probes whatever host is live on this machine.
    // Every test below would then decline instead of spawning.
    ...
    // Same hazard class as `findIncumbentHost` above, now for the relaunch loop: - unset, `hasStopIntent` falls through to the real implementation and reads the developer's actual `~/.traycer/host/stop-intent.json`, so a stray file left by a real `traycer host stop` would silently suppress every relaunch assertion below; - unset, `sleep` is a REAL timer, and one exhausted budget costs 1+5+15+30+60 = 111s of wall clock, which no 10s test timeout survives.
    hasStopIntent: async () => false,
    // Same hazard as `hasStopIntent` above, and easier to miss because it is read ONCE at startup rather than per attempt: unset, the `Partial` default falls through to the real `readStopIntentIdentity`, which reads the developer's actual `~/.traycer/host/stop-intent.json`.
    // A stray record from a real `traycer host stop` would then be remembered as "served at startup", silently changing what every intent assertion below means.
    readStopIntentIdentity: async () => null,
    sleep: async () => undefined,
    closeLogFd: async () => undefined,
    // Pin the shared harness to a SINGLE attempt.
    // Every test built on `makeRunStubs` predates the relaunch loop and asserts what one attempt records and exits with; letting them relaunch would silently change what they mean.
    maxRelaunches: 0,
    ...
    // The real supervisor calls process.exit which never returns.
    // In tests we record the requested code and resolve any pending waiter so event-handler-driven exit paths surface to the test runner without an uncaught throw.
    exit: (code) => {
...
/** Schedule the stub child's terminal event AFTER spawn returns so the supervisor has already attached its `exit` / `error` listeners. A plain `setTimeout(..., 0)` before `runHostStart` races the pre-spawn awaits (`rotateLog`, `pruneCrashReports`, marker write, open fd). */
...
/** One host per data dir. launchd runs at most one instance of a LABEL, but the CLI label (`ai.traycer.host`) and Desktop's SMAppService agent label (`ai.traycer.host.agent`) are distinct labels that both invoke this supervisor against the same data dir, both with `RunAtLoad`. A machine carrying both registrations spawned two hosts at every login, racing the same pid.json and stores. */
describe("runHostStart - incumbent host guard", () => {
  it("declines with exit 0 and never spawns when a live host owns the data dir", async () => {
    ...
    // Exit 0 is load-bearing: the macOS plists set `KeepAlive.SuccessfulExit = false`, so a clean exit leaves the job down.
    // Any non-zero code would have launchd relaunch this supervisor immediately and flap against the incumbent forever.
  ...
  // A live incumbent settles what this invocation should do regardless of whether THIS label can resolve its own install target.
  // Resolving first exited 69, which `KeepAlive.SuccessfulExit = false` treats as restartable, so launchd relaunched the job into a throttled crash loop while a healthy host was serving.
...
describe("runHostStart - label-derived reclaim probe", () => {
  it("derives live probe authority from a label-only registration and bypasses the incumbent guard", async () => {
    ...
    // The status transport is authorised EXPLICITLY, never inferred.
    // The supervisor opened the pipe at stdio[3], so it must also name that descriptor - and the two must agree, or the host writes its frame into a descriptor nobody is reading (or, worse, one somebody else is).
...
describe("runHostStart - installed-record launch path", () => {
  it("spawns record.executablePath directly with no shell wrapping and the environment env exported", async () => {
    ...
    // The supervisor passes the slot dir as `--host-data-dir` (NOT an `--environment` arg or env): path-only, so the host writes pid.json in this environment's slot while its cloud target stays baked.
    ...
    // No status pipe was opened on this path, so the host must NOT be authorised to write status frames.
    // The flag IS the authorisation: passing it speculatively is what let the host sniff fd 3 by type and inject frames into an unrelated Node IPC channel.
  ...
  it("rotates the log for this environment before anything appends to the run", async () => {
    ...
    // Ordering is load-bearing, not incidental, so assert the actual SEQUENCE rather than just that each step ran.
    // The `starting` marker must land in the POST-rotation file, and the stdio fd opened after it lives for the child's whole lifetime - an fd follows the inode across a rename, so rotating any later would divert the host's own stdout into the rotated-away file and split one session across two inodes.
  ...
  it("dev wrapper-script executablePath spawns through the same code path", async () => {
    // The dev orchestrator stages a small POSIX wrapper at `~/.traycer/host/dev/runtime/traycer-host` that internally exec's `node <bundle>`.
    // The supervisor sees only the wrapper path - it does NOT branch on bundle / node-bin args anymore.
...
describe("runHostStart - signal/exit propagation", () => {
  ...
  it("enriches a crashed marker with exitMeaning, report, and stderrTail via injected deps", async () => {
    ...
    // Fleet crash telemetry: exactly one report, carrying the decoded exit status and the INSTALL RECORD's version (not a possibly-different running host version, which only identity resolution would know).
    ...
    // The crashed child's pid, as spawned by the stub - the crash telemetry uses it to decide whether the pid.json on disk was written by THIS child before attributing its published version to the crash.
  ...
  it("writes a crashed marker without report when findCrashReport never resolves", async () => {
    ...
    // The report is built AFTER the 2 s scan bound expired, but its uptime is spawn-to-exit: the diagnostic wait must not be booked as time the child ran.
    expect(recorded.crashReports).toHaveLength(1);
  ...
  it("does not hold the relaunch decision on a reportHostCrash that never resolves", async () => {
    // The reporter is dispatched WITHOUT awaiting it: the relaunch decision and its backoff must start the moment the marker is written, not after HOST_CRASH_REPORT_TIMEOUT_MS.
    // Real timers, so the elapsed-time bound below is the invariant itself - a run that waited on the reporter's 1.5 s race would fail it.
    const exec = "/opt/traycer/host/install/traycer-host";
    const { child, recorded, deps } = makeRunStubs(sampleRecord(exec), null);
...
// --------------------------------- service manifest sanity tests SCOPE.
// These are text-level negatives about the manifest layer only: that no flag which does not exist on `host start` leaks into a generated definition.
...
describe("service manifests never leak a flag `host start` does not have", () => {
  ...
  it("macOS plist", async () => {
    ...
    // launchd execs ProgramArguments[0]; the compatibility program is the per-label launcher FILE (macOS names the login item after it - the former inline `/bin/sh -c` vector surfaced as "sh" in Login Items).
...
// --------------------------------------------------------------------------- Crash-relaunch loop (int #4826, from OSS #916) A host that crashes while the desktop app is CLOSED had no guardian at all: the health monitor is not running, launch converge needs a launch, and on Windows the Scheduled Task did not restart a task whose action exited 0xC0000409.
// The supervisor now relaunches its own child.

/** Drive N attempts whose child outcome is scripted per attempt. `outcomes[i]` is applied to the i-th spawn; the last entry repeats, so a "crashes forever" host is `[{ code: 7 }]`. */
...
describe("runHostStart - a child ending during admission's own post-spawn await", () => {
  // `admitHostStartSpawn` owns real awaits AFTER `run()` returns the spawned child - an adoption grant's `acknowledgeSpawn()`, or `withUpdateContender`'s post-callback capability verification.
  // The child's terminal-evidence listeners are attached synchronously inside `run()`, at the spawn site itself, precisely so an ending emitted in that window is not lost: an `error` used to be an uncaught event that crashed the supervisor, and an `exit` here left `await childEnding` pending forever with the attempt lock held.
  const exec = "/opt/traycer/host/install/traycer-host";
  ...
  it("settles a spawn-error emitted during admission's post-spawn await as a normal spawn failure", async () => {
    ...
    // Reaches the normal async-spawn-failure path - a marker and a stable exit code - instead of hanging or taking the test process down with an uncaught exception.
    expect(recorded.exited).toBe(66);
    expect(
      recorded.markers.some(
        (marker) =>
  ...
  it("settles an exit emitted during admission's post-spawn await instead of hanging forever", async () => {
    ...
    // A clean exit recorded before `childEnding` even had listeners of its own must still reach the terminal-marker path, not strand the supervisor waiting on an ending it will never observe again.
    expect(recorded.exited).toBe(0);
...
describe("runHostStart - crash relaunch loop", () => {
  ...
  it("lets a sustained run forgive the crash budget even when it ends in a requested restart", async () => {
    // The restart branch returns to the top of the loop directly, so it never reaches the sustained-uptime reset the abnormal path applies.
    // Without its own copy, a host that burned budget, then ran well past `SUSTAINED_UPTIME_RESET_MS`, then was restarted ON PURPOSE would hand its replacement the stale crash history that the sustained run had already earned off - so an operator-requested restart quietly shortens the next real crash allowance.
  ...
  it("stops respawning a host that requests a restart faster than it can start", async () => {
    // Exit 87 deliberately skips the crash budget AND the backoff - it is a hand-off, not a failure.
    // But "skips the budget" must not mean unbounded: a host whose restart handshake fires during startup would otherwise spawn / exit 87 / respawn with no delay for the supervisor's lifetime.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const originalSpawn = deps.spawn;
    if (originalSpawn === undefined) {
      throw new Error("test spawn dependency missing");
  ...
  it("relaunches a crashed host and gives the new attempt its own identity", async () => {
    // `spawn-evidence.ts` pairs a post-baseline `starting` marker with its terminal marker.
    // A relaunch that reused the first attempt's id would read as a second ending for the first attempt.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
  ...
  it("relaunches a fatal signal death but never a forwarded shutdown signal", async () => {
    ...
    // The forwarded case has to actually FORWARD: the supervisor is signalled, latches, and passes it on.
    // Scripting a SIGTERM child death without signalling the supervisor does not exercise this - it is the direct-kill case below, and asserting "no relaunch" on it was pinning a bug.
        ...
        runHostStart(
          { environment: "production", cwd: null },
          {
            ...
            spawn: (command, args, options) => {
              ...
              setImmediate(() => {
                // `currentChild` is assigned once this spawn call returns, so by now the handler has a child to forward to.
                // The handler runs synchronously inside `emit`.
    ...
    // Exit 0, NOT 128+SIGTERM.
    // This is the ordinary macOS `host stop`: launchd signals the supervisor, it forwards, and the child dies by that signal.
    expect(term.recorded.exited).toBe(0);
  });

  it("escalates a raced stop to SIGKILL when the child ignores SIGTERM", async () => {
    // Without this the supervisor awaits `childEnding` with no deadline, and nothing else intervenes: the stop announced itself on disk, `host stop` has already returned, and no service manager is watching this process.
    // A child that never handles SIGTERM would leave the supervisor waiting forever while the host it was told to stop kept serving.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const originalSpawn = deps.spawn;
    if (originalSpawn === undefined) {
      throw new Error("test spawn dependency missing");
  ...
  it("relaunches a host the OOM killer took, which no diagnostic whitelist names", async () => {
    // Recoverability and diagnosability are different questions, and deciding one with the other's answer is the bug this pins.
    // `describeFatalSignal` lists native-crash signals so a support log can explain them; SIGKILL is deliberately absent because there is nothing to explain.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [
  ...
  it("does not relaunch when a stop was requested", async () => {
    ...
    let intentChecks = 0;
    // The stop is requested while the FIRST child is running - the ordinary shape.
    // Returning `true` from the start would instead exercise the spawn-boundary guard and never let a child run at all, which is a different test (see the setup-window cases below).
    ...
    // Three halves, really: the guard was consulted, nothing respawned, AND the exit code does not ask the platform to respawn on our behalf.
    // Asserting only the spawn count would pass if the loop never ran - and asserting the child's code, as this used to, passed while the stop was undone one process later.
  ...
  it("abandons the relaunch when another host claims the data dir during backoff", async () => {
    // Backoff holds a claim on the data dir for seconds-to-minutes while nothing serves.
    // `traycer host ensure` can legitimately win that race, and stacking a second host on top is what the incumbent policy exists to prevent.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
  ...
  it("re-resolves the install target before each attempt and inside admission", async () => {
    // An install swap renames `install/` aside mid-life.
    // A supervisor holding the first attempt's path would relaunch a binary that has moved - today a fresh launchd-spawned supervisor re-resolves, and the loop must keep that.
    ...
    // The outer setup resolves once for diagnostics, then the admitted callback resolves again before each spawn to bind the selected binary to the live contender boundary.
    expect(reads).toBe(4);
  ...
  it("reopens the host log fd for each attempt", async () => {
    // The host rotates host.log BY RENAME.
    // An fd held across a rotation writes the next child's stdout into host.log.1 - the same hazard oss #982 fixed for the stderr tee.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [
      { code: 5, signal: null },
      { code: 0, signal: null },
  ...
  it("registers one set of signal listeners no matter how many attempts run", async () => {
    // Five relaunches must not install five listener sets on `process` - and the one set that IS installed must not outlive the call.
    // Production never notices the second half, because `deps.exit` is `process.exit` there; an injected `exit` returns, and every such call used to strand three listeners.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
  ...
  it("resets the budget only after a child that actually ran for the sustained window", async () => {
    // The governor's hard-won rule, transplanted: a host that dies shortly after boot every time answers "it started" every time, and treating that as recovery is what let the original respawn loop re-arm itself forever.
  ...
  it("measures that window at the child's DEATH, not after the diagnostics", async () => {
    // `ranForMs` used to read the clock where it was compared, which is on the far side of the whole post-mortem: the stderr end wait (2s), the tee flush (1s) and the bounded crash-report scan (2s).
    // A child that died a second short of the window was therefore credited with five seconds it never ran, cleared the bar, and reset the budget - every time.
    vi.useFakeTimers({ toFake: ["Date"] });
    try {
      vi.setSystemTime(Date.parse("2026-08-06T00:00:00.000Z"));
      const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
      // The 6th outcome is a CLEAN exit, purely so the buggy reading terminates: without it, a budget that keeps resetting never stops and this test would hang instead of failing.
      const scripted = withScriptedAttempts(deps, [
        { code: 9, signal: null },
        { code: 9, signal: null },
        { code: 9, signal: null },
      ...
      // The post-mortem writes through `writeTerminalMarker`, NOT `writeMarker` - `writeMarker` only carries "starting" and "failed-to-spawn", so hooking it puts the clock advance AFTER `ranForMs` is computed and makes this test inert.
      // It was, until a mutation probe survived.
...
// --------------------------------------------------------------------------- Regressions found by cold review of the relaunch loop.
// Each of these passed the original loop tests and was still wrong. ---------------------------------------------------------------------------
...
describe("runHostStart - relaunch loop, guards re-checked across the backoff", () => {
  ...
  it("honours a stop that lands DURING the backoff, not just one already present", async () => {
    // The original code sampled stop intent once and then slept up to a minute, so a `traycer host stop` arriving mid-backoff was decided against before it happened - and the supervisor respawned a host the user had just stopped.
    // The backoff window is exactly when a stop is most likely to land, because the host is already down.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [{ code: 1, signal: null }]);
    let stopRequested = false;
    ...
    expect(recorded.spawnCalls).toHaveLength(1);
    // Exit 0, NOT the crashed child's 1.
    // This assertion used to read `toBe(1)`, which quietly gave back everything the refusal above had just won: launchd (`KeepAlive{SuccessfulExit:false}`), systemd (`Restart=on-failure`) and the Scheduled Task all treat a nonzero exit as a crash and start a REPLACEMENT supervisor, and that one reads the stop intent as older than its own invocation - already served - and spawns the host.
    expect(recorded.exited).toBe(0);
  });

  it("honours a shutdown signal that lands DURING the backoff", async () => {
    // Same race through the POSIX door: `launchctl bootout` signals this process mid-backoff.
    // A boolean snapshot taken before the await cannot see it, which is why the decision reads a live getter.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [{ code: 1, signal: null }]);
...
describe("runHostStart - a stop exits 0 wherever it is honoured", () => {
  ...
  it("still exits 69 when target resolution runs out of budget instead", async () => {
    // The other half, and the reason this is a `cause` rather than a blanket "stop means 0": exhaustion deliberately hands the machine back to the platform, and it needs the nonzero code to do that.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [{ code: 0, signal: null }]);
...
describe("runHostStart - relaunch loop, per-attempt isolation", () => {
  ...
  it("arms the transition probe on the first attempt only", async () => {
    // The probe answers "did THIS transition succeed" - a one-shot verdict owned by the install/restart lifecycle.
    // A relaunch re-arming it would overwrite that verdict with the outcome of unrelated recovery work.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const markerWrites: ProbeMarker[] = [];
    const originalSpawn = deps.spawn;
    if (originalSpawn === undefined) {
    ...
    // Every attempt gets a child with a READABLE fd 3.
    // Without one the probe observer has nothing to attach to and writes no marker at all - which is verdicts and passed with the observer reading the wrong context.
    let spawns = 0;
    const scripted = {
      deps: {
        ...deps,
      ...
      () =>
        runHostStart(
          ...
          },
          {
            ...
            // Answer with a frame for whichever attempt is being observed right now.
            // `observeProbeStatus` discards a frame whose `attemptId` does not match, so a fixed id here would make the test vacuous in exactly the direction that hides the bug.
    ...
    // Attempt 2 must carry neither, or the host writes a status frame for a transition that was already decided.
    ...
    // THE LOAD-BEARING ASSERTIONS. argv and stdio alone are satisfied by the spawn sites using the per-attempt context while the OBSERVER still reads the outer one - exactly the defect this test is named for, and it passed that way.
    // Two writes belong to the PROBE attempt: the observation (`awaiting-readiness`, from its Layer-0 frame) and the finalization that resolves it once the child's fate is known.
  ...
  it("releases the supervisor's log descriptor for every attempt", async () => {
    // The supervisor now outlives many attempts, and a sustained-uptime reset can extend the loop indefinitely - so an fd opened per attempt and never closed is an unbounded leak, not a bounded one.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
  ...
  // The test above exercises ordinary child EXITS only, which is why removing the close on the synchronous-throw path left it green.
  // Each attempt-end route gets its own case: an fd is a per-attempt resource, and "some paths close it" is the shape of a leak, not the absence of one.
...
describe("runHostStart - relaunch loop, spawn-failure policy", () => {
  ...
  it("retries a synchronous spawn throw on a relaunch, like the async one", async () => {
    // Whether `spawn()` throws or reports through the `error` event is a platform detail.
    // Letting the two disagree meant a transient EBUSY mid-swap could still exit a supervisor that had budget left.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const originalSpawn = deps.spawn;
    if (originalSpawn === undefined) {
      throw new Error("test spawn dependency missing");
  ...
  it("leaves terminal evidence when a relaunch cannot resolve its target", async () => {
    // A retry that wrote only a `starting` marker left host.log claiming an attempt began with nothing to say why it never spawned - int #4839 reads exactly these markers to explain a failed start.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [{ code: 3, signal: null }]);
    let reads = 0;
  ...
  it("retries a FIRST-attempt spawn failure when the service manager started it", async () => {
    // The Windows hole an attempt-number gate left open: a Scheduled Task action that hits a transient EBUSY mid-swap on its very first spawn exits, handing the machine back to the `RestartOnFailure` mechanism this loop exists precisely because it cannot be relied on.
    // Nothing is listening to a service start, so it spends the budget instead.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const originalSpawn = deps.spawn;
    if (originalSpawn === undefined) {
      throw new Error("test spawn dependency missing");
  ...
  it("surfaces a FIRST-attempt spawn failure immediately for an interactive start", async () => {
    ...
    // A person - or Desktop, which parses the result envelope and is waiting on it - must not sit through the full backoff ladder before being told the install is broken.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const originalSpawn = deps.spawn;
    if (originalSpawn === undefined) {
      throw new Error("test spawn dependency missing");
  ...
  it("retries a FIRST-attempt target-resolution failure only for a service start", async () => {
    // Same split, on the other failure route: a task action whose install record is momentarily absent mid-swap retries, an interactive start reports E_HOST_NOT_INSTALLED now.
...
describe("runHostStart - production defaults", () => {
  ...
  it("keeps the backoff timer referenced so the supervisor survives the wait", async () => {
    // Every other test injects `sleep`, so the REAL one is otherwise never exercised - and it is the only thing holding the event loop open during a backoff.
    // By then the child is dead, its stderr is closed and the log descriptor has been released; signal listeners do not ref the loop and neither does an awaited promise.
  ...
  it("keeps the raced-stop escalation timer referenced, and cancels it on demand", async () => {
    // Same argument as the backoff timer, one path over: this timer is the only thing that ends an unbounded wait on a child that will not honour SIGTERM.
    // Unref'd, Node could decide the supervisor had nothing left to do and exit while the host it was told to stop kept serving.
  ...
  it("gives a raced stop longer than the host's own force-exit watchdog", async () => {
    // Derived, not chosen.
    // A functioning host arms its own force-exit watchdog, so a grace at or below it would SIGKILL hosts that were moments from completing the exact shutdown we asked for.
...
describe("runHostStart - stop signals outside the child-running window", () => {
  ...
  it("does not spawn when a shutdown signal arrives during per-attempt setup", async () => {
    // Installing the signal handlers once, up front, SUPPRESSES Node's default "die on SIGTERM".
    // The single-shot supervisor relied on that default: it registered its handler only after spawning, so a signal during setup killed the process.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
  ...
  it("ignores the stop intent it saw at startup, so the next crash is recoverable", async () => {
    // Stop the host, log back in inside the sentinel's freshness window, and the new supervisor must not read the previous stop as still in progress and refuse to recover its OWN child's first crash.
    // The record is REMEMBERED, never deleted: an earlier version cleared the file at startup, which erased any stop that landed between this invocation and the clear.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [
      { code: 5, signal: null },
      { code: 0, signal: null },
        ...
        runHostStart(
          { environment: "production", cwd: null },
          {
            ...
            hasStopIntent: async (_environment, _nowMs, servedAtStartup) => {
              ...
              sawIdentity = true;
              // The record on disk IS the one this supervisor started with, so its own start already answered it.
              // Deliberately no clock here: that is the whole point of the identity rule.
  ...
  it("does not spawn when stop intent lands during the FIRST attempt's setup", async () => {
    // The Windows shape, and the one a POSIX-only latch cannot see: `schtasks /End` never signals this process, so `shuttingDown` stays false forever.
    // The stopper writes intent and scans for a child while the supervisor is still opening its log - finding nothing to kill, because nothing has been spawned yet.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [{ code: 0, signal: null }]);
    let stopRequested = false;
    ...
    expect(recorded.spawnCalls).toHaveLength(0);
    expect(recorded.exited).toBe(0);
    // The attempt already wrote `starting` before the guard ran, and `spawn-evidence.ts` pairs that against a terminal marker.
    // Leaving it unpaired makes a cleanly stopped host read as an attempt still in progress.
  ...
  it("does not spawn when stop intent lands during a RELAUNCH's setup", async () => {
    // Same hole one attempt later: `decideRelaunch` re-reads intent around the backoff, but per-attempt setup is a chain of awaits AFTER that read, and a stop can complete inside it.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [{ code: 9, signal: null }]);
    let attempts = 0;
    let stopRequested = false;
...
describe("runHostStart - a stop that lands INSIDE the pre-spawn read", () => {
  ...
  it("does not spawn when the signal arrives during the intent read itself", async () => {
    // The narrowest window in the pre-spawn guard, and the one operand order decides.
    // `||` short-circuits: asking `shuttingDown` FIRST reads it as of before the intent await, so a signal delivered while that read is in flight is invisible to the condition it is supposed to trip.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
...
describe("runHostStart - a marker write must never cost the host", () => {
  ...
  it("still recovers a crash when every marker write fails", async () => {
    ...
    // Awaited bare inside the loop, a transient EBUSY/EACCES/ENOSPC rejected straight out of `runHostStart`; the entrypoint turned that into exit 1, and exit 1 is precisely what the Windows Scheduled Task does NOT answer with a fresh supervisor.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [
      { code: 7, signal: null },
      { code: 0, signal: null },
...
describe("runHostStart - per-attempt setup failures stay inside the budget", () => {
  ...
  it("spends an attempt and retries when opening the log fd fails mid-ladder", async () => {
    // `openLogFd` is an `fs.open`, and a Windows scanner holding the file or a momentary EACCES is not a statement about whether the host can run.
    // It used to reject straight out of `runHostStart`: the entrypoint turned that into exit 1, and the Scheduled Task does not answer exit 1 with a fresh supervisor - so a transient filesystem error ended the ladder and left the machine hostless, mid-recovery from a crash the loop had already decided to fix.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const scripted = withScriptedAttempts(deps, [
      { code: 7, signal: null },
      { code: 0, signal: null },
...
describe("runHostStart - a SERVICE launch refused as busy exits non-zero so it is retried", () => {
  // Codex round 3, P1 (download-stage).
  // `host download` holds the update-attempt execution segment across its whole transfer, and mutual exclusion on that lock is unconditional on admission - decided before `dispositionFor` is consulted.
  const exec = "/opt/traycer/host/install/traycer-host";
  ...
  it("INTERACTIVE launch + busy -> still exit 0, unchanged", async () => {
    // The scoping half.
    // An interactive or Desktop-driven start has a caller watching that can decide for itself, so its exit semantics must not move; turning every busy refusal non-zero was the fleet-wide change this was deliberately narrowed away from.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const refused: Partial<RunHostStartDeps> = {
      ...deps,
      admitHostStartSpawn: async () => busy,
  ...
  it("service launch refused for a NON-busy reason -> still exit 0", async () => {
    // Only `busy` is transient.
    // `lock-not-live`, `nonterminal-attempt`, `record-fail-closed` and `held-in-process` are states a relaunch cannot clear, so retrying them would be a throttled crash loop rather than a recovery - the exact failure this suite already records for exit 69.
    const { recorded, deps } = makeRunStubs(sampleRecord(exec), null);
    const refused: Partial<RunHostStartDeps> = {
      ...deps,
