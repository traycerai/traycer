...
function defaultArgs(
  ...
  return {
    ...
    controller,
    label: { id: "ai.traycer.host.production" } as never,
    parentPid: 4242,
    platform: "linux",
    spawnImpl: (() => {
      throw new Error("spawnImpl should not be invoked on POSIX still-locked path");
    }) as RestartArgsBaseline["spawnImpl"],
    writeImpl: (() => {
      throw new Error("writeImpl should not be invoked on POSIX still-locked path");
...
describe("restartWithPendingCliUpgradeFinalize", () => {
  it("stops, finalises the staged binary, then starts (lifecycle order matters)", async () => {
    ...
    const { restartWithPendingCliUpgradeFinalize } = await import(
      "../host-restart"
    );
  ...
  it("on Windows still-locked, schedules a detached helper, skips controller.start(), and writes the helper script", async () => {
    ...
    try {
      ...
      const spawnStub: RestartArgsBaseline["spawnImpl"] = (
        command,
        args,
      ) => {
