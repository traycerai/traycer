...
interface HostAvailableArgs {
  /** The catalog override: `true` explicitly includes release candidates, `false` explicitly filters them out, and `null` means the caller stated nothing and wants the default derived from the installed host. Three states, not two, because "unchecked" and "never touched" are different requests once an RC host includes its own line by default: collapsing them would leave a user on an RC host with no way to get a stable-only listing. */
...
/** The CLI is the catalog-default authority: it is the only process that reads this environment's install record, so it - not the GUI, and not the host resolver - decides what an unstated override means. An installed canonical `X.Y.Z-rc.N` includes pre-releases so the RC can see its own line. */
...
/** Reads the installed host's version for the derivation above, failing closed. Every failure mode collapses to `null` (stable-default), deliberately. */
...
// `traycer host available` - explicit registry probe per Flow 6.
// With neither flag it derives RC inclusion from the installed host; `--include-pre-releases` and `--no-include-pre-releases` override that in either direction.
  ...
  return async (ctx): Promise<CommandResult> => {
    ...
    return {
      data: {
        ...
        manifestUrl: urlInfo.url,
        platformKey,
        // Both, not one: the resolved inclusion says what the rows were filtered by, and the provenance says why - which is what lets a caller explain an RC-inclusive default without claiming the user saved a preference.
        includePreReleases: resolved.includePreReleases,
        includePreReleasesSource: resolved.source,
      },
      human: listing.human,
...
// The human listing states the provenance too, not just the filter.
// An RC host lists RC rows with no flag passed, and without this line that reads as the CLI ignoring its own documented default.
...
// Emit only the asset for this CLI's platform. Windows arm64 resolves to the x64 host runtime.
...
// Project the CLI floor into availability, with the registry client's OWN evaluator so the listing and the install path cannot disagree about one value: `client.ts` rejects a floored target with HOST_CLIENT_FLOOR_UNMET only at download time, AFTER the RPC has answered `accepted` - so a listing that ignored the floor advertised installs that could only terminate as floor failures.
// The unreleased-CLI waiver applies here exactly as there (a dev build is below every floor by SemVer and is deliberately let through).
...
// `isPreReleaseVersion` is the shared catalog predicate rather than a local one: Desktop's `HostController` predicts this exact filter to decide whether a staged build would still appear in the default listing, and a second copy of the rule here is what would eventually purge one.
