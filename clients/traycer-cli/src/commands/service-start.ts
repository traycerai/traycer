...
//
// The update-contender bridge for the same reason `host stop` rides it: a
// start must not land inside another actor's install/apply critical section
// and race the process that section is about to swap out, and the contender
// (which wraps cli-lock with attempt awareness) is the one arbitration layer
// every host-service mutator goes through - see
