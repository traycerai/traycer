...
// Runner-aware `traycer config shell add`.
// Unlike the permissive `config shell set` (absolute-path-only), `add` is the picker's gated entry point: the path must be absolute AND point at something that exists and is executable, otherwise it fails with a config-invalid-value error so junk never joins the remembered list.
