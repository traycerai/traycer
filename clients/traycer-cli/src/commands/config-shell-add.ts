...
// junk never joins the remembered list. On success it appends the path to
// `shell.added` and selects it in one write. JSON mode emits the persisted
...
export function buildConfigShellAddCommand(args: {
  ...
  return async (ctx): Promise<CommandResult> => {
    ...
    return {
      data: { path: next.path, added: next.added },
