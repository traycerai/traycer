...
/** `traycer agent turn-ended-from-hook --provider <provider>` - invoked by a provider `Stop` hook when a terminal-agent finishes a turn. Signals the host (`agent.tui.turnEnded@1.0`) so the inter-agent broker can fire a `turn-ended` inactivity notice for any thread the agent still owes a reply on. */
