...
// Mirrors stage-reconcile.test.ts's seam: forces `unlink`/`rm`/`rename` to fail for one specific path while every other path proxies through to the real implementation - the exact operations layered invalidation and the commit rename depend on.
// `forceRenameFailureForDestination` matches on the rename's destination (`to`) argument, since the source is a dynamically-generated staging dir the test can't hardcode.
const mocks = vi.hoisted(() => ({
  ...
  // Finding-2 regression (reconcile-before-commit): the reconcile restore rename and atomicSwap's own swap-in rename can share the exact same destination (`install/`), so distinguishing "let the Nth call to this destination succeed, fail a LATER one" needs a call-index gate on top of the plain destination match above.
  // `null` preserves every existing test's behavior (fail on every call to the matched destination).
  forceRenameFailureForDestinationOnCall: null as number | null,
  // Prefix-matched variant of the destination match above, for renames whose destination the test cannot hardcode - atomicSwap's aside rename targets `install.old-<Date.now()>`.
  // Call counting keys on the prefix.
  forceRenameFailureForDestinationPrefix: null as string | null,
  // Error code the simulated failure carries.
  // "EIO" (non-retryable) by default so a matched rename fails on the first attempt instead of spending the swap retry schedule; the swap-lock-recovery tests set "EBUSY" to exercise the retries.
  forceRenameFailureCode: "EIO" as string,
  // Retry-until-success gate: when set, calls 1..N to the matched destination fail and later calls succeed.
  // Mutually exclusive with the exact-call gate above.
  ...
  return {
    ...
    rename: async (from: PathLike, to: PathLike) => {
      ...
      if (to === mocks.forceRenameFailureForDestination || prefixMatched) {
        ...
        if (shouldFail) {
          // "EIO" (non-retryable) unless a test overrides the code, so `renameWithRetry` fails on the first attempt instead of spending the retry schedule on EBUSY/EPERM/etc.
...
describe("sweepOldTrash", () => {
  ...
  it("invalidates install.old-* litter even when the sidecar unlink AND the recursive removal both fail", async () => {
    // Same shape as stage-reconcile.test.ts's identical staged-aside test: the rename-to-`.dead-*` primary layer is left real (not forced to fail), so forcing exactly the two operations that were the pre-parity implementation's whole defense to fail no longer matters - the candidate is already structurally invisible to the `.old-*` scan before unlink/rm are ever reached.
...
describe("commitInstallFromSource", () => {
  ...
  it("materializes install.json inside the source tree BEFORE the commit rename, so it survives a failed swap", async () => {
    // Proves the crash-safety property the commit tail exists for: the record and the bytes travel in ONE rename, not a rename followed by a separate post-swap write (which could land bytes with no record on a crash in between).
    // Forcing the commit rename itself to fail here means the source dir is never consumed - if the write happened AFTER the rename (the pre-refactor ordering), it would never have been attempted at all.
...
describe("atomicSwap - swap-lock recovery", () => {
  ...
  it("re-kills lingering slot processes between retryable aside-rename attempts and then succeeds", async () => {
    ...
    // The aside rename targets `install.old-<Date.now()>` - only the prefix is predictable.
    // Two EBUSY failures, then the rename goes through: the shape of a lingering handle that a re-kill releases.
  ...
  it("gives rollback the same verified retry schedule without re-killing from compensation", async () => {
    // CodeRabbit finding on PR #829: the restore rename (trash -> target, the worst failure path - it runs when the swap-in itself already failed, and its own failure leaves NO install dir at all) used the plain `renameWithRetry` default (~2.5s) while a comment claimed it "gets the same retry" as the forward renames.
    // Rollback now shares the bounded schedule and per-attempt verifier, but deliberately does not re-kill: compensation may restore old bytes, never kill another process on behalf of a stale forward transaction.
    await installVersion("v1", "1.0.0");
    ...
    // Compensation may restore old bytes but must never kill another process as part of a stale forward transaction.
    // The restore's first failure is destination attempt 4; no kill may occur at or after that boundary.
    expect(killAfterRenameAttempts.filter((attempt) => attempt >= 4)).toEqual(
      [],
    );
  ...
  it("gives an actionable fallback when EBUSY exhausts every retry and the scan names nothing", async () => {
    // The field cohort the scan is structurally blind to: a CWD-only holder has no install path in its exe or command line, so the re-kill runs on every retry, releases nothing, and the detail scan comes back empty.
    // The user must get told what actually unblocks them instead of a bare errno.
    await installVersion("v1", "1.0.0");
    setSwapRenameDelaysForTests([1, 1]);
    mocks.forceRenameFailureForDestinationPrefix = `${installDirFor(ENV)}.old-`;
    mocks.forceRenameFailureCode = "EBUSY";
...
describe("swap rename schedule contract", () => {
  it("stays far above the default ~2.5s retry and is never truncated by its own ceiling", () => {
    // The whole point of the schedule is outlasting AV scans and orphan handle release that the previous ~2.5s budget could not - pin the floor so a refactor cannot quietly regress it.
    // The ceiling exists for slow RE-KILLS, so it must clear the sleep schedule itself with room for healthy (couple-of-seconds) hook passes.
...
describe("commitHostInstallSource - reconcile runs BEFORE the commit (Finding 2)", () => {
  ...
  function seedRecoverableAside(version: string): string {
    // `install/` itself does NOT exist - only a valid `install.old-*` aside does.
    // `validateInstallAsideCandidate` requires `executablePath` to resolve (relative to the FINAL install dir, not the aside's own path) to a file that actually exists in the aside.
    const installDir = installDirFor(ENV);
  ...
  it("restores a missing install/ from its .old-* aside before atomicSwap's entry sweep can destroy it, so a later swap failure still leaves a restorable install", async () => {
    ...
    // Let the FIRST rename onto `install/` succeed (reconcile's own target-missing recovery, restoring the aside) and force the SECOND one (atomicSwap's swap-in of the fresh staged tree) to fail - the exact "sweep already ran, then the new rename also failed" window Finding 2 closes.
    // Verified by temporarily reverting the fix: WITHOUT it, `install/` is still missing when atomicSwap runs, so only ONE rename ever targets it (the swap-in) - this gate's "fail on call 2" never fires, the swap-in (call 1) succeeds using the fresh v2.0.0 tree, and the promise resolves instead of rejecting, failing the assertion below.
    mocks.forceRenameFailureForDestination = installDirFor(ENV);
    ...
    // With reconcile running first, `install/` already existed (restored from the aside) when atomicSwap ran, so `targetExists === true` meant the failed swap-in (call 2) triggered atomicSwap's own rollback (`renameWithRetry(trash, target)`) - the recovered v1.0.0 install survives rather than being lost.
  ...
  it("cleans the staged source when pre-commit reconciliation throws", async () => {
    // A present but malformed install record makes the first reconcile throw before `commitInstallFromSource` begins.
    // The staged tree still belongs to this call and must be cleaned by commitHostInstallSource's finally.
  ...
  // Change 12 (fixup ticket): `cleanupStagingArtifacts`'s finally-block cleanup used to let a rejecting `verifyMutationCapability` throw straight out of `discardStagedHostInstallSource`/`commitHostInstallSource`'s `finally` - which, from a REAL `finally`, replaces whatever the caller was already returning or throwing.
  // Best-effort cleanup losing its capability must degrade to "leave the leftovers for the next admitted run's sweep", never surface as this call's own failure.
  it("discardStagedHostInstallSource does not throw when verifyMutationCapability rejects during cleanup, and leaves the staging dir for a later sweep", async () => {
    ...
    // The capability loss skipped the destructive removal rather than throwing through it - the staged tree is still on disk for the next admitted run to sweep.
    expect(existsSync(staged.stagingDir)).toBe(true);
  });
});
