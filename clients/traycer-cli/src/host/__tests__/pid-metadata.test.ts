...
describe("host pid metadata endpoint validation", () => {
  it("accepts local host websocket endpoints", () => {
    expect(isValidLocalHostWebsocketUrl("ws://127.0.0.1:7100/rpc")).toBe(
      true,
    );
    expect(isValidLocalHostWebsocketUrl("wss://127.0.0.1:7100/rpc")).toBe(
      true,
    );
  });
  ...
  it("rejects non-local or malformed websocket endpoints", () => {
    ...
    expect(isValidLocalHostWebsocketUrl("ws://localhost:7100/rpc")).toBe(
      false,
    );
    ...
    expect(
      isValidLocalHostWebsocketUrl("ws://attacker.example:7100/rpc"),
    ).toBe(false);
    expect(
