...
// Dispatch admits a new attempt only; a continuation field on the dispatch shape is an architecture failure.
...
/** Production files permitted to reach `dispatchAttemptExecutor`. **Empty today, and that is the assertion, not a placeholder.** Nothing in production admits a new schema-v2 attempt before Ticket 07's cutover. When the cutover adds the new-attempt dispatcher, exactly that one admission owner joins this list - and a second entry should be a conversation, not a diff. */
const ALLOWED_DISPATCH_IMPORTERS: readonly string[] = [];

/** Files that own a CONTINUATION or RECOVERY flow. Each must reach the segment and must never reference dispatch. */
...
/** Fields that would mean dispatch is being handed a continuation. `hostHomeDir` is on the list for a reason worth stating: it is not itself a continuation, but it is the canonical-record path, and the only reason dispatch would want one is to answer a question about an existing attempt. */
...
// Dependencies and gitignored build outputs (`**/dist/` etc. - no tracked source lives under these names) are pruned: scanning them would make the gate's verdict depend on whether a local build has run, and CI never sees them.
...
describe("dispatchAttemptExecutor is admission-only new-attempt transport", () => {
  it("its production importer set is EXACTLY the allowed admission owners", async () => {
    ...
    // Exact equality in both directions on purpose.
    // An unexpected importer is a continuation leaking into the admission path; an allowlist entry with no importer is a stale permission nobody removed, which is how an allowlist quietly stops describing the code it governs.
  ...
  it("DispatchAttemptExecutorOptions carries no continuation-implying field", async () => {
    ...
    // If the interface cannot be found the gate is inert, so its absence is a failure rather than a skip - the shape of a check that silently stops checking is exactly what this epic keeps finding.
