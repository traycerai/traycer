...
vi.mock("@traycer-clients/shared/host-update", async (importOriginal) => {
  ...
  return {
    ...
    withUpdateContender: (
      ...
      entries.admissions.push(options.admission);
    ...
    withSupervisorRelaunchContender: (
      ...
      entries.relaunchEntered += 1;
      entries.admissions.push("supervisor-relaunch-maintenance");
...
beforeEach(() => {
  ...
  entries.admissions.length = 0;
  entries.relaunchEntered = 0;
  vi.resetModules();
});
...
describe("a parked update does not defer the lifecycle teardown, and is resumed by the next start", () => {
  it.each([
    ...
    async (phase, install) => {
      ...
      expect(entries.admissions).toEqual(["lifecycle-teardown-maintenance"]);
      ...
      expect(entries.relaunchEntered).toBe(1);
      expect(entries.admissions).toEqual(["supervisor-relaunch-maintenance"]);
  ...
  it("an applying record refuses the teardown: retry, no commit, no signal, record untouched", async () => {
    ...
    expect(result).toEqual({ kind: "retry", reason: "update-attempt-active" });
    expect(entries.admissions).toEqual(["lifecycle-teardown-maintenance"]);
