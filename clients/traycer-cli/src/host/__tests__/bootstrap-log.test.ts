...
describe("bootstrap-log crash diagnostic fields", () => {
  ...
  it("formatFields renders exitMeaning, report, and stderrTail and parse round-trips them", async () => {
    ...
    writeBootstrapTerminalMarker("production", "crashed", {
      ...
      error: undefined,
      exitMeaning:
        "0xC0000409 STATUS_STACK_BUFFER_OVERRUN (fail-fast abort: a native module or CRT abort when stderr is empty, else V8 fatal/OOM, which prints FATAL ERROR to stderr first)",
