...
/** These tokens are a compatibility surface, not an implementation detail: an emitted launchd plist / systemd unit / Scheduled Task asks an OLDER CLI about them, so renaming one silently un-labels every registration that probes it. Pinning the literal strings is the point of this file. */
...
/** Executed against the real commander program, because the contract the emitted scripts depend on is the PROCESS behaviour - exit code and stdout - not the pure function above. In particular the emitted probes branch on the exit code alone, so a command that printed the right thing while exiting non-zero would silently un-label every registration. */
describe("`traycer host capabilities` as a subprocess", () => {
  ...
  // The whole reason this contract exists: `--service-label` used to be detected by grepping `host start --help`, so applying the file's own `.hideHelp()` convention to an "Internal:" option silently disabled the identity binding everywhere.
  // The options ARE hidden now - assert that, and assert the capability answer is unaffected by it.
  ...
  /** Closes the loop between the two halves of the contract. Everywhere else the emitted script is executed against a STUB CLI, which can only prove the script's own branching. */
