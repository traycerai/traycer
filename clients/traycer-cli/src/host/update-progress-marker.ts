...
// Cross-process handoff file `traycer host update` writes so the host daemon (which spawns the update detached and does NOT wait for it) can learn the outcome without polling process exit codes.
// Deliberately mirrored (by contract, not by import - this CLI lives in a separate repo/package graph from `traycer-host/`) at `traycer-host/src/paths.ts::hostHomeDir`, and shape-compatible with `packages/common/src/types/host/index.ts`'s `HostUpdateProgress` in the internal monorepo (also not importable from here for the same reason).
...
export interface HostUpdateProgress {
  ...
  /** Identity of the process that wrote the record - what lets a conditional clear or replace tell "the marker I wrote" from an identical-looking one another updater wrote in the same millisecond. Additive: the host daemon reads `state` and `error` only, and a marker written by an older CLI reads back as `null`. */
  readonly writerId: string | null;
}

// One identity per process.
// Two `host update` invocations racing for the same target can write byte-identical `state`/`targetVersion`/`updatedAt` (millisecond clock), and content alone would then let the first to finish clear or fail-stamp the second's live marker - which the second could never clear, because the stamp no longer matches what IT wrote.
...
// Best-effort clear on confirmed success.
// Never throws - a failure to delete the marker just leaves a stale "updating" marker the daemon will eventually reconcile once it observes the new `appVersion`; it must not fail the otherwise-successful update command.
...
/** Identity of two marker records: every field, `writerId` included. Content alone is not identity - two updaters racing for the same target can write the same `state`, `targetVersion` and millisecond `updatedAt` - so the writer's identity is what tells "mine" from "an identical one of theirs". */
...
/** Write `progress` to a private sibling of `target` and return its path. The name is unique per writer: a shared `${target}.tmp` let two concurrent writers overwrite each other's staging file, so one renamed the OTHER's bytes into place and the other's rename failed on a file that had moved. */
...
/** Delete the marker only if it still reads as `expected` - a compare-and- delete that cannot erase a marker another writer landed in between. For a clear that decided from a marker it READ ("this `failed` is stale, the host is current") or WROTE (this invocation's own `updating`, by the time the update finished). */
...
/** Replace the marker with `next` only if it still reads as `expected` - the failure stamp's compare-and-swap. `host update` stamps `failed` over the `updating` IT wrote, and by then another updater may have landed its own `updating` at the same path (its write precedes its lock wait, so holding the lock proves nothing about the marker). */
...
/** Compare-and-swap for progress markers. Fail closed on an unreadable file. */
async function swapMarkerIfUnchanged(
  ...
  // `link(from → target)` lands `from` at the live path only if that path is empty; EEXIST means a newer marker is there and wins.
  // A filesystem that refuses hard links falls back to an exclusive create (`wx`) of the same bytes - still create-if-absent, so a marker landed in between still wins with EEXIST.
  const landAtomically = async (
    ...
    try {
      ...
      try {
        ...
        // Neither route could land `from` (no hard links AND the create failed - ENOSPC, EIO).
        // `from` may be the only complete copy of another updater's live marker, taken out of the live path in step 1; deleting it here would erase that marker while reporting `changed`.
        logger.warn(
  ...
  try {
    ...
    if (absent) {
      // An empty live path is NOT proof that nobody else is in flight: another conditional swap may hold the current marker in its scratch at this instant (its step 1), and a stamp landed here now would win its restore's `link` with EEXIST - our stale failure standing over the live marker it then drops.
      // Absence is a changed marker as far as a replace is concerned; the failure is still reported by exit code and log, and the only marker it can lose is one that was already gone.
      return changed(null);
    }
    await ensureHostHomeDir(environment);
...
// Read-only accessor for Doctor / tests.
// Returns `null` when absent or malformed (a malformed marker is treated the same as "no marker" - it is advisory UI state, not an authoritative record worth failing loudly on).
