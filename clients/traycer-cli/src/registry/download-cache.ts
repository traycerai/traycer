...
// Cross-invocation staging for registry archive downloads (traycer#585, 586, #588).
// The archive used to land in a fresh `mkdtemp` dir per CLI process, so a re-spawned CLI could never see the previous invocation's partial file: every retry restarted from byte zero, and a 700MB host archive over a link that resets every few MB could never finish.

const OWNER_SUFFIX = ".owner";
const PRIVATE_DIR_PREFIX = "private-";
const BREAK_SUFFIX = ".break";
// Every suffix that means something to `sweepDownloadCache`.
// A slot file name must not end in one - see `foldReservedSuffixes`.
...
// Longest a publisher-controlled path component may contribute to a slot name.
// `sanitizeSegment` already reduces the alphabet; this bounds the length so a pathological manifest cannot push the resolved path past the filesystem's limit and fail every download with ENAMETOOLONG.
const MAX_SLOT_SEGMENT_LENGTH = 96;

// How long an entry whose owner CANNOT be identified must sit completely idle before another process may take it over.
// Only reached when `verifyProcessIdentity` answers "indeterminate" - a probe that positively says dead or alive-different frees the entry at once, and "alive-same" holds it regardless of age.
...
export interface DownloadSlot {
  ...
  // False when another live process already owns the shared slot and this caller got a private per-invocation path instead.
  // Callers do not need to branch on it - it exists for logging and tests.
...
// Claims the shared slot for `version`+`sha256`, falling back to a private temp path when a live process already holds it.
// Also sweeps entries no live process owns, so a superseded partial cannot pin hundreds of MB.
export async function acquireDownloadSlot(
  ...
  // The shared slot for THIS version is spared even when this caller fell back to a private path - falling back is precisely what proves another process is working on it.
  // Without that, the loser of a race would sweep the entry the winner is mid-takeover of: claim it in the window between the winner's unlink and its own marker write, delete the partial, and leave both processes on private paths with the resumable bytes gone.
...
// Removes the archive and its claim.
// For the terminal paths: the caller consumed the archive, or it is verified-bad and must not be resumed.
...
// Marks the claim as still being worked on.
// The idle rule that frees an unidentifiable owner measures the newest mtime across the marker and the entry it claims, and the phases AFTER the transfer - hashing, signature verification, and above all the caller's extract - only ever READ the archive, so its mtime stops advancing while the slot is still legitimately held.
...
// Whether the marker at `ownerPath` is still this process's own claim.
// A pid is unique among live processes, so a marker recording our pid can only be ours.
...
// A shared slot is claimed at the archive path itself; a private slot is claimed at its containing `private-*` directory, so removing the claim removes the whole directory the archive sits in.
// The private case escalates a targeted delete into a RECURSIVE one, so it is admitted only for a directory that provably resolves to a child of this environment's own download cache.
...
async function createPrivateSlot(
  ...
  // A private path costs this invocation a full re-download, but two writers appending to one file would corrupt both.
  // Claim BEFORE creating the directory.
  ...
  try {
    ...
    // Claiming first is what makes this safe, but it also means the claim outlives a failed creation.
    // Nothing would ever collect it: the sweep judges an orphaned marker by its own freshness, and this one is fresh.
...
// `<version>-<sha-prefix>-<publisher basename>`.
// The sha prefix is what makes resume safe across a re-publish: a changed asset resolves to a different path, so a stale partial is never appended to a new entity.
...
// The basename comes from the manifest URL's last path segment, which is publisher-controlled.
// Everything outside a conservative allowlist is folded away so it can never contribute a separator, a parent-dir hop, or a leading dot to the resolved path.
function sanitizeBasename(value: string): string {
  const cleaned = sanitizeSegment(value)
    // A run of dots is never part of a real archive name, and a leading dot would make the entry hidden.
    // Neither can traverse anywhere once the separators are gone, but both are folded away so the resolved name stays boring and greppable.
    .replace(/\.{2,}/g, ".")
    .replace(/^[.]+/, "");
  const bounded =
    cleaned.length === 0
...
// This module distinguishes an archive from its own bookkeeping purely by suffix, so a slot file must never END in one the sweep would act on.
// Nothing upstream prevents it: the basename is the manifest URL's last path segment, and `sanitizeSegment` deliberately preserves `.` and `-`, so a published asset named `…owner` or `…break` would land as `<version>-<digest> -x.owner`.
...
// How this process came to hold an entry.
// The sweep needs the distinction: "created from nothing" against an entry whose marker WAS listed a moment ago means the marker vanished in between, which is another process mid-takeover rather than litter.
type ClaimOutcome = "created" | "took-over" | "denied";

// Claims `claimedPath` for this process.
// Every path to a non-"denied" outcome ends in an EXCLUSIVE create, so at most one process can hold a given entry at a time; a loser falls back to a private slot.
...
// Serializes the removal of a marker proved stale, then re-verifies the decision under that exclusivity.
...
async function breakStaleMarker(
  ...
  try {
    // A read failure is not evidence, and a marker whose bytes changed belongs to a holder that appeared after the decision.
    // Neither may be unlinked.
...
// Returns this acquisition's nonce, or null if another breaker holds the sub-lock.
// Recovery from a sub-lock left behind by a CRASHED breaker requires positive evidence that its writer is gone - age alone is not evidence.
...
// Compare-and-delete, mirroring `cross-process-lock.ts`'s `releaseBreakLock`: a recoverer that correctly took this sub-lock over from a presumed-crashed breaker must not have its ownership silently clobbered by that breaker's own late release.
...
// The sub-lock carries this process's identity (so a contender can tell a crashed breaker from a live one) plus a per-acquisition nonce (so the release above can tell its own claim from a successor's).
...
// One `acquireDownloadSlot` call judges the slot it wants plus every entry in the sweep, and each judgement can shell out to `ps`/`tasklist` with a 3s timeout and retries - synchronously, on the event loop, before the download is allowed to start.
// Entries abandoned by the same process share a token, so memoizing per token collapses that to one probe per distinct owner.
...
async function identityVerdictFor(
  ...
  // Keyed on the operand the verdict is actually derived from.
  // Keying on `startedAtMs` (which nothing reads any more) would let two tokens with the same pid but different creation stamps share one cached verdict.
...
async function markerIsStale(
  ...
  if (token !== null) {
    const verdict = await identityVerdictFor(token, verdicts);
    // "alive-same" is the only verdict that proves the recorded downloader still exists.
    // "alive-different" means the pid was recycled onto an unrelated process, which is positive evidence the owner is gone.
    if (verdict === "alive-same") return false;
    if (verdict === "dead" || verdict === "alive-different") return true;
  }
  // The marker says nothing usable - it is corrupt, or the identity probe itself failed.
  // Fall back to whether the claim is still being WORKED ON rather than to how old the marker is.
  const idleMs = await entryIdleMs(ownerPath);
  return idleMs !== null && idleMs >= UNIDENTIFIED_OWNER_IDLE_TAKEOVER_MS;
}

// How long since anything last touched this claim: the newest mtime across the marker and the entry it claims.
// A private slot claims a DIRECTORY whose own mtime does not move while the archive inside it grows, so a directory resolves to the newest mtime among its children too.
...
// `null` means the marker could not be read at all - it is absent, or the read failed.
// Neither is evidence about an owner, and neither may be compared against a staleness decision.
...
// Best-effort removal of every entry no live process owns, except the one the caller just claimed.
// A partial for a superseded version is dead weight the moment a newer one resolves - the only file worth keeping across invocations is the one being downloaded now.
async function sweepDownloadCache(
  ...
  for (const name of entries) {
    ...
    if (name.endsWith(OWNER_SUFFIX)) {
      // A marker whose claimed entry still exists is decided together with that entry, below; only an orphaned marker is judged on its own, and removing one is still an unlink another contender could be racing, so it goes through the same arbitration.
    ...
    // Claim the entry before deleting it.
    // Proving it stale is not enough on its own: another process can be taking the very same entry over to RESUME it, and a sweep that deleted the archive out from under a successful claim would throw away exactly the partial this cache exists to preserve.
    const outcome = await claimEntry(entryPath, verdicts);
    if (outcome === "denied") continue;
    // The listing showed a marker, yet the claim created one from nothing: the marker vanished in between, which is a contender that has already unlinked it and is about to write its own.
    // Sparing the CURRENT version's slot is not enough to cover this - the entry being taken over belongs to whatever version that other process wants, not ours.
