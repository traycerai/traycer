...
// The staged-store sidecar (`staged.json`) - Host Update Layer Redesign Tech Plan, "CLI: two-phase split with a staged store".
// Carries EVERYTHING needed to materialize `install.json` at apply time except the fields minted at materialization (`installId`, `installedAt` - the latter not modeled here yet; ticket 2 wires apply's materialization).
...
// Reads and validates the sidecar at an explicit directory (rather than always the canonical `hostStagedDir(environment)`) so the same tolerant parser can validate a `staged.old-*` aside candidate during reconcile's aside-recovery step, not just the live `staged/` dir.
...
// Writes the sidecar atomically at an explicit directory - used both to write into a not-yet-promoted temp dir (before it is renamed wholesale into `staged/`) and, in principle, directly at `hostStagedDir`.
