/** NSIS uninstall macro and CLI hidden-launcher naming must stay lockstep. */
...
/** Resolves the macro through the SAME field electron-builder reads, rather than by hardcoding the filename here. `nsis.include` is resolved against `directories.buildResources` (app-builder-lib `getResource`), so a move of either the file or the buildResources dir has to keep this readable. */
...
describe("windows OS-native uninstall macro", () => {
  ...
  it("deletes the launcher at the path the Scheduled Task's action actually runs", () => {
    // `hiddenHostLauncherPath` is module-private in windows.ts, so rebuild it from its two real inputs: `cliInstallHomeDir("production")` and the launcher filename.
    // The home dir is absolute and platform-native, so compare the part BELOW the user profile, spelled the way NSIS spells it (`$PROFILE\...`).
  ...
  it("never delimits an nsExec argument with a quote it also needs inside", () => {
    // NSIS accepts `"`, `'` and backtick as string delimiters but has NO escape for the delimiter itself - doubling it (`''`) does not escape, it ends the string early.
    // The folder-cleanup command needs both `"` and `'` for PowerShell, so it must be backtick-delimited.
  ...
  it("does NOT tear the autostart down during an in-place update", () => {
    // The catastrophic-regression guard. electron-builder runs the OLD uninstaller with `--updated` as part of every update (templates/nsis/include/installUtil.nsh), and `customUnInstall` is inserted near the top of the un.Uninstall section, so it fires then too.
    // Unguarded, upgrading Traycer would delete the logon task and the host would never start again after an update - strictly worse than the defect the macro fixes.
    const macro = readUninstallMacro();
    expect(macro).toContain(`\${GetOptions} $R0 "--updated" $R1`);

    // `${GetOptions}` sets the error flag when the option is ABSENT, so `${ifNot} ${Errors}` is the arm taken when `--updated` WAS passed - the update - and every teardown command has to live in its `${else}`.
    // Both arms are sliced and asserted, rather than only checking that the delete follows the guard: that weaker form stays green when the two arms are SWAPPED, which is precisely the inversion this test exists to catch.
