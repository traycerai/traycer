...
// Cooperative shutdown of a Desktop-managed host, through the host's own lifecycle-claim RPCs instead of launchd.
// Who registered the OS service is irrelevant here by design: we ask the RUNNING HOST to stand down (claim -> commit -> wait for real exit), which protects in-flight work (a busy host denies the claim) and mutates no registration the CLI does not own.
...
export type CooperativeShutdownOutcome =
  ...
  // Pid metadata could not be read at all - absent, unreadable, or malformed.
  // This is NOT proof that no host is running, and conflating the two is how a host that is still booting (loaded agent, endpoint not published yet) gets reported as successfully stopped: `host stop` returns success while it keeps serving, and an install swaps bytes underneath it.
  | { readonly kind: "no-metadata" }
  // The host denied the claim: it has work in progress.
  // Callers must surface this rather than escalate - a denied claim is retryable, a killed working host is not.
  | { readonly kind: "busy" }
  ...
  // The claim path could not run at all (RPC dial/auth failure, invalid endpoint, or a commit denial after expiry).
  // The live process may or may not be serving; callers choose the escalation appropriate to their operation.
...
/** `intent` tells the host what happens AFTER it exits, which is the one thing it cannot see for itself: a cooperative stop and the stop half of a restart are byte-identical from inside the process. A host told `"restart"` publishes a restart tombstone to every attached client so a deliberate bounce does not read as death in every window on every machine (D5/M1); one told `"shutdown"` behaves exactly as it always has. */
export async function requestCooperativeShutdown(
  ...
  /** Best-effort return of a granted claim. Deliberately swallows its own failure: we are already on an abort path, and the caller's outcome is about the shutdown, not about the cleanup. */
  ...
  // A granted claim closes the host to new work until it is committed, released, or expires.
  // Every abort path below therefore has to hand the token back: `CLAIM_TTL_MS` is ~62s, so a transient dial failure between claim and commit would otherwise leave a HEALTHY host refusing work for a full minute - a self-inflicted outage produced by the code that exists to shut down cleanly.
...
export type ForcedShutdownOutcome =
  ...
  // the signal landed).
  | { readonly kind: "stopped" }
  // PROVEN absent: pid metadata was read and the process it names is gone - either nothing occupies the pid, or an identity comparison proved the occupant is an unrelated process on a recycled pid.
  | { readonly kind: "no-host" }
  // Pid metadata could not be read at all, so there is no process to signal.
  // Same non-proof caveat as `CooperativeShutdownOutcome`: a booting host may not have published its endpoint yet.
  | { readonly kind: "no-metadata" }
  // pid.json RECORDS a start identity but the live occupant's could not be read/compared right now.
  // Something holds the pid and we cannot prove it is the host, so nothing may be signalled: the error directions are not symmetric - a refused force stop is retryable, a SIGKILL delivered to a recycled pid's unrelated occupant is not.
...
/** Forced shutdown of a Desktop-managed host: kill the HOST CHILD process directly instead of asking it to stand down. This deliberately signals the child named by pid.json and never touches launchd: the SMAppService registration stays exactly as Desktop left it, and the supervisor - which reads the stop-intent record the calling command's `withStopIntent` decorator wrote before this ran - treats the death as asked-for rather than a crash, so nothing relaunches the host. */
export async function forceStopHostProcess(
  ...
  // A force stop's success must not leave a resurrect signal behind.
  // The desktop health monitor reads pid.json AFTER its endpoint probe fails and treats ABSENCE as "deliberate stop" - metadata still present with a dead endpoint is read as a crash and respawned.
...
// The outcome plus the pid.json record the signals were aimed at (`null` only for `no-metadata`).
// The caller's purge needs the record to guarantee it removes the STOPPED instance's file and never a replacement's.
...
async function signalHostForForcedStop(
  ...
  // A pid.json that survived a host crash can name a RECYCLED pid, and liveness alone would aim SIGTERM/SIGKILL at whatever unrelated process occupies it now.
  ...
  // The invariant that gated the first signal gates the second.
  // The host can exit in the last instants of the SIGTERM grace and the OS can hand its pid to a stranger before the final liveness poll - at which point "still alive" describes the STRANGER, and this SIGKILL is the irreversible one.
  if (metadata.processStartIdentity !== null) {
...
/** `true` when the signal was delivered; `false` when the process is already gone (ESRCH). Any OTHER failure - EPERM most plausibly - throws: the process is still alive and unsignalable, and reporting it "stopped" would be the one lie this path must never tell. */
