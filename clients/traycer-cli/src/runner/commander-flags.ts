...
export function addRunnerFlags(cmd: Command): Command {
  return cmd
    .option("--json", "Emit NDJSON events on stdout instead of human-readable text")
    .option("--quiet", "Suppress non-essential human output")
    .option("--no-progress", "Suppress progress events / progress lines")
    .option("--no-bootstrap", "Skip implicit bootstrap actions (e.g. auto-start)");
}
...
export function extractRunnerFlags(raw: Readonly<Record<string, unknown>>): RawRunnerFlags {
  return {
