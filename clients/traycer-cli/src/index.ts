...
import {
  buildConfigDiagnosticsClearTemporaryCommand,
  buildConfigDiagnosticsSetCommand,
  buildConfigDiagnosticsTemporaryCommand,
  configDiagnosticsGetCommand,
  configDiagnosticsResetCommand,
} from "./commands/config-diagnostics";
import {
  buildDiagnosticsExportCommand,
  parseDiagnosticsExportTailBytes,
} from "./commands/diagnostics-export";
...
import { LOCAL_CLI_VERSION, resolveCliVersion } from "./cli-version";
...
// `LOCAL_CLI_VERSION` / `resolveCliVersion` live in `./cli-version` so command
// modules (e.g. `commands/config-diagnostics.ts`) can resolve the CLI version
// without importing this entrypoint - which imports those command modules and
// would create a cycle. Re-exported for existing importers and the version
// resolution test.
export { LOCAL_CLI_VERSION, resolveCliVersion };
...
function collectValueOptionFlags(root: Command): Set<string> {
  ...
  const visit = (cmd: Command): void => {
    for (const opt of commandOptions(cmd)) {
      // Boolean flags (`required` and `optional` both false) take no value.
      if (!opt.required && !opt.optional) continue;
      if (opt.long !== null) flags.add(opt.long);
      if (opt.short !== null) flags.add(opt.short);
    }
...
function commandOptions(cmd: Command): readonly RuntimeCommandOption[] {
  ...
  return rawOptions.flatMap((option) => {
    ...
    const required = "required" in option ? option.required : null;
    const optional = "optional" in option ? option.optional : null;
    const long = "long" in option ? option.long : null;
    const short = "short" in option ? option.short : null;
    if (typeof required !== "boolean" || typeof optional !== "boolean") {
      return [];
    }
    return [
      {
        required,
        optional,
        long: typeof long === "string" ? long : null,
        short: typeof short === "string" ? short : null,
      },
    ];
  });
}
...
function registerDiagnosticsCommands(program: Command): void {
  const diagnostics = program
    .command("diagnostics")
    .description("Collect diagnostics bundles for support");

  withRunner(
    diagnostics
      .command("export")
      .description("Write a redacted diagnostics bundle")
      .option("--output <path>", "Write the bundle JSON to this path")
      .option(
        "--tail-bytes <bytes>",
        "Maximum bytes to collect from each log file",
      ),
    (opts) =>
      buildDiagnosticsExportCommand({
        outputPath: typeof opts.output === "string" ? opts.output : null,
        tailBytes: parseDiagnosticsExportTailBytes(
          typeof opts.tailBytes === "string" ? opts.tailBytes : null,
        ),
      }),
  );
}
...
function registerConfigCommands(program: Command): void {
  ...
  const diagnostics = config
    .command("diagnostics")
    .description("Diagnostic log levels for CLI, desktop, and host");
  withRunner(
    diagnostics
      .command("get")
      .description("Print configured and effective diagnostic log levels"),
    () => configDiagnosticsGetCommand,
  );
  withRunner(
    diagnostics
      .command("set")
      .description("Set permanent diagnostic log levels")
      .option(
        "--level <level>",
        "General level: trace, debug, info, warn, error, off",
      )
      .option(
        "--host-level <level>",
        "Host level: inherit, trace, debug, info, warn, error, off",
      ),
    (opts) =>
      buildConfigDiagnosticsSetCommand({
        level: typeof opts.level === "string" ? opts.level : null,
        hostLevel: typeof opts.hostLevel === "string" ? opts.hostLevel : null,
      }),
  );
  withRunner(
    diagnostics
      .command("temporary")
      .description("Set temporary diagnostic log levels for support")
      .option("--level <level>", "Temporary General level")
      .option("--host-level <level>", "Temporary Host level")
      .option("--duration <duration>", "Duration such as 30m or 2h"),
    (opts) =>
      buildConfigDiagnosticsTemporaryCommand({
        level: typeof opts.level === "string" ? opts.level : null,
        hostLevel: typeof opts.hostLevel === "string" ? opts.hostLevel : null,
        duration: typeof opts.duration === "string" ? opts.duration : null,
      }),
  );
  withRunner(
    diagnostics
      .command("clear-temporary")
      .description("Clear temporary diagnostic log-level overrides")
      .option(
        "--scope <scope>",
        "Temporary scope to clear: general, host, or all",
      ),
    (opts) =>
      buildConfigDiagnosticsClearTemporaryCommand({
  ...
  withRunner(
    diagnostics
      .command("reset")
      .description("Reset diagnostic log-level settings to defaults"),
    () => configDiagnosticsResetCommand,
  );
}
