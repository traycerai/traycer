    ...
    program
      ...
      .addHelpText(
        ...
        [
          ...
          "",
          "Exit codes: 0 both removed - 1 signed out, but the cache directory could",
          "not be removed. The sign-out itself has landed either way (`--json`",
          "reports `data.loggedOut`, and `data.chatCache` carries the path,",
          "`cleared`, and the reason); exit 1 exists so an unattended caller does",
    ...
    program
      ...
      .addHelpText(
        ...
        [
          ...
          "'-unconfirmed' values, meaning the write was attempted and the commit did",
          "not confirm - the stored credentials may or may not have changed, so run",
          "`traycer login` if a later command fails to authenticate. A validation",
          "that ends in a network error can also have spent the refresh token",
