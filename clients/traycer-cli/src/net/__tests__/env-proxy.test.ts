...
/** A real tunneling proxy that records the authority of every CONNECT it is asked for, and then answers the tunneled request itself. This is what makes the routing assertions real rather than shape checks. undici tunnels by default (`proxyTunnel` is `true` even for `http:` origins), so a recorded authority PROVES the dispatcher routed through the configured proxy, and an empty recording proves it went direct. */
...
describe("installEnvProxyDispatcher", () => {
  ...
  // The seam has to be honest: `EnvHttpProxyAgent` falls back to `process.env` for every option it is not given, so an installer that only READ the injected env would install an agent routing on the runner's environment instead.
  // Pointing `process.env` at a dead port proves which one won.
  ...
  // NO_PROXY is the half that decides a request must NOT be tunnelled.
  // On a managed workstation it is routinely the reason the loopback host and the corporate proxy can coexist, so it has to come from the same source.
