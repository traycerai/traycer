...
// npm-publishable JS build. Protocol is inlined at build time; do not leave a runtime protocol dependency.
...
// The bundle MUST be named `traycer` (no extension).
// The CLI entry's script-mode guard `isTraycerCliEntrypoint(process.argv[1])` only runs `parseAsync` when argv[1] basename is `index.ts`, `traycer`, or `traycer.exe` (see src/index.ts) - the same name the SEA binary uses.
...
// Broad runtime floor: the bundle only touches stable `node:` builtins (fs/net/http/child_process/crypto/os/path/url/util/events), so it runs on every active Node LTS. esbuild downlevels syntax to this target.
...
function main() {
  ...
  // Version precedence: the release workflow injects TRAYCER_CLI_VERSION (from the `cli-v<version>` tag); local builds fall back to the workspace manifest version, then the shared `0.0.0-local` sentinel (kept identical to LOCAL_CLI_VERSION in src/index.ts).
  const cliVersion =
  ...
  // Sentry DSN, baked exactly as build-cli-sea.cjs bakes it.
  // `sentry.ts` reads `process.env.TRAYCER_CLI_SENTRY_DSN` directly (a form esbuild's define substitutes), and only calls `Sentry.init()` when the value is non-empty.
  const cliSentryDsn =
    typeof process.env.TRAYCER_CLI_SENTRY_DSN === "string"
      ? process.env.TRAYCER_CLI_SENTRY_DSN
      : "";

  // Bundle everything (no externals) so the published artifact resolves nothing at the user's runtime.
  // Node builtins are auto-externalised by `--platform=node`.
  const args = [
    cliEntry,
    "--bundle",
    "--platform=node",
    ...
    // esbuild `--define` value must be a JS expression; a single JSON.stringify yields the quoted string literal "1.5.0" (matches build-cli-sea.cjs).
    // Double-encoding would inject the quotes into the value and break version resolution.
  ...
  // The CLI entry (src/index.ts) carries its own `#!/usr/bin/env -S bun` hashbang, which esbuild preserves verbatim on line 1.
  // The npm bin must launch on the user's *Node*, so strip whatever hashbang the bundle starts with and pin the Node one.
  ...
  const publishManifest = {
    ...
    // No "type": the single extensionless `traycer` bundle is CommonJS (it opens with `"use strict";`); omitting the field keeps the manifest unambiguous regardless of host defaults.
