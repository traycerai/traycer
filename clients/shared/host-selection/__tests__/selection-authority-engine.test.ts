...
/**
 * Only the engine's own {@link LOCAL_EXPECTED_OUTAGE_CEILING_MS}-equal B2 ceiling ever moves a lease held by a port like this one; the port itself never will.
 */
...
describe("SelectionAuthorityEngineImpl - SEAM: late attach and handover races", () => {
  ...
  it("an issued generation that NEVER claims retires the held attachment at the handover ceiling - its stale inventory stops suppressing death", () => {
    // Codex #1243 (engine): a reload's preload allocates, then the renderer's bootstrap fails while the process stays alive - no render-process-gone, no window destruction, no claim.
    // Before this bound, window A's retired document kept H `ready` in every window for as long as the app lived.
    const clock = createFakeAuthorityClock(0);
    const authority = createTestAuthority({
      initialFleet: {
        identityGeneration: 0,
  ...
  it("a hard detach during the handover ends the HANDOVER ceiling with it, and arms the CORPSE ceiling for the sessions it dropped", () => {
    ...
    // The hard detach: `render-process-gone`, no `sessionLost` ever sent.
...
describe("SelectionAuthorityEngineImpl - death aggregation", () => {
  ...
  it("plan-restricted provenance is sticky until proof of life; ordinary refusals still derive offline", () => {
    ...
    // An entitlement refusal is deterministic, unlike a reachability failure: one observed verdict is conclusive and avoids manufacturing two more network attempts merely to make the UI publish the known reason.
    ...
    // The authority owns the matching deadline too.
...
describe("SelectionAuthorityEngineImpl - compat freshness", () => {
  it("compat freshness is anchored to session observation order: a later session's verdict recovers the lease, and a delayed verdict re-anchored to the older session is dropped", () => {
    ...
    // s1 is lost; s2 is observed for the first time now, so its ordinal is strictly later than s1's - a legitimate downgrade / same-version restart case, not a version-string comparison.
    engine.ingestEvidence(
      "A",
      attachA.incarnationId,
      sessionEvidence("H", "s1", "lost", 0),
...
describe("SelectionAuthorityEngineImpl - bounded per-incarnation state", () => {
  it("evicting session ordinals never promotes a stale verdict: an anchor for a session the reporter no longer holds ranks at the floor, not as the newest", () => {
    ...
    // Before the floor rule, an unknown anchor was minted as the newest ordinal, so eviction alone would have flipped this lease to dead.
    engine.ingestEvidence(
      "A",
      attachA.incarnationId,
...
describe("SelectionAuthorityEngineImpl - expected-outage budgets are bounded", () => {
  // Every other test OF these holds scales with them.
  // Pinned here because an expected outage is a bounded lie: for its whole length the authority reports a host it cannot reach as merely cycling, suppressing the death evidence that would otherwise fail the window over.
  it("the restart-intent episode covers a restart cycle without becoming a long-lived lie", () => {
    // Lower: a restart/apply cycle has to fit, or the exemption never does its job and a deliberate restart still reads as death.
    // Upper: a host that never comes back must reach `dead` promptly - the user is waiting on ∅ or a failover, and neither can happen while this holds.
  ...
  it("the local mutation-lane ceiling sits between the quiet and max host budgets", () => {
    // The upper bound IS deliberately tight - it IS §3's documented max budget, so any increase fails here.
    // That is the intent, not an oversight: this band pins the documented contract rather than a comfortable range, and a legitimate upward tune means amending §3 first.
...
describe("SelectionAuthorityEngineImpl - local outage signal", () => {
  it("holds the local host's lease in restarting-expected while the signal is true, and the ceiling caps it", async () => {
    ...
    const engine = new SelectionAuthorityEngineImpl({
      fleet,
      identity,
      // Provisionable, and settled before the outage edges below.
      // The subject here is a deliberate restart of a host that is up - which is the only shape a user restart can have - so the construction-time ensure D14 draws for a never-dialed local host must have answered first.
  ...
  it("COMPOSITION: an ensure the ENGINE started outranks the outage signal it busies - the local lease stays connecting and ∅ never shows while provisioning", async () => {
    ...
    // Self-healing: once the request the engine was waiting on answers, the arm is gone and the lane's own signal governs the lease again.
    // The window keeps pointing at the cycling host (the D5/M6 hold), which is the whole reason `restarting-expected` is a hold rather than eligibility.
  ...
  it("COMPOSITION: a live session outranks the engine's own in-flight ensure - the warm host reads ready, not connecting, for the whole converge", async () => {
    // The launch-time ensure fires on every cold boot (never-dialed is trivially true at t=0), and on a machine whose host is already UP it is a 30-45s CLI converge that proves nothing about the host's ability to serve.
    const clock = createFakeAuthorityClock(0);
    const ensure = createDeferredEnsure();
    const authority = createTestAuthority({
      initialFleet: {
  ...
  it("a DEFERRED ensure paces the next request without deadening the lease", async () => {
    // Deferral means the lifecycle lane or its CLI lock was busy - another launch actor mid-work - so nothing ran and nothing was learned about the host.
    const clock = createFakeAuthorityClock(0);
    const ensure = createDeferredEnsure();
    const authority = createTestAuthority({
      initialFleet: {
...
describe("SelectionAuthorityEngineImpl - A3: compat ranks are incarnation-scoped", () => {
  it('two windows that both call their session "s1" rank by the authority\'s own observation order, not a shared ordinal', () => {
    ...
    // A's older session reports incompatible on its "s1".
    engine.ingestEvidence(
      "A",
      attachA.incarnationId,
      compatIncompatible("H", "s1", INCOMPAT_DETAIL),
...
describe("SelectionAuthorityEngineImpl - A4: identity transition keeps an active local outage", () => {
  it("a deliberate local restart spanning a sign-out is not blindly cleared, and the ceiling still counts from the ORIGINAL start", async () => {
    ...
    // The new-generation fleet is already available by the time the identity transition runs, so the local host is still a member afterwards.
    ...
    // The ceiling counts from the original start (t=0), not from the transition (t=1000): 1000ms elapsed already, so only ceiling - 1000 more is needed to lapse it.
...
describe("SelectionAuthorityEngineImpl - closure round: drain guard orders delivery across listeners", () => {
  it("every listener receives revision N before ANY listener receives revision N+1, even when listener 1 re-enters synchronously", () => {
    ...
    let reentered = false;
    // Listener 1 re-enters the engine mid-delivery by publishing a fleet snapshot, which stages a second leasesChanged event.
...
// The next snapshot does not repair this - if the same durable host id re-registers, the prune finds it a member and deletes nothing, so it inherits its pre-removal state.
// `dropsAsOutsideFleet` closes the gap; session evidence is deliberately exempt (invariant 5), and it composes with `pruneEvidenceOutsideFleet` rather than narrowing C4's live-session suppression inside `ingestDial`.
...
describe("SelectionAuthorityEngineImpl - dropsAsOutsideFleet gates late evidence for a removed host (:803 fix)", () => {
  it("R2a-control: prune genuinely clears a verdict when no late report arrives", () => {
    ...
    // X rejoins with NO late report in between.
  ...
  it("R2b: a late restart-intent re-anchors an episode on a re-registered host", () => {
    ...
    fleet.publish(0, "L", [fleetHost("L", "local")]);
    // Replay of the same tombstone: ordinarily an ignored duplicate (mechanism 7), but without the gate the prune erased the memory of having seen it, so the replay anchors a fresh episode.
    engine.ingestEvidence(
      "A",
      incarnation,
      restartIntent("X", "tomb-1", null, 0),
...
describe("SelectionAuthorityEngineImpl - hasFleetAnswer distinguishes an answered [] from no answer yet (:803 fix)", () => {
  ...
  it("a cold start whose first answer is an empty fleet drops a late verdict", () => {
    ...
    // Nothing about the fleet's content changed here - only that it is now known, which is the discriminator R2d's empty seed cannot draw: a published snapshot is always an answer, even an empty one.
    fleet.publish(0, null, []);
  ...
  it("construction seeded with a non-empty fleet counts as answered; an empty seed does not", () => {
    ...
    // The seed already names a host, so - unlike R2d's empty seed - this counts as answered from construction: a late report for a host outside it is dropped immediately, with no publish needed to arm the gate.
    engine.ingestEvidence(
      "A",
      incarnation,
      compatIncompatible("X", null, INCOMPAT_DETAIL),
  ...
  it("an identity transition resets the answer flag: a fresh window accepts evidence again", () => {
    ...
    // B's first answer names Y: the verdict ingested during B's unknown window must have been kept, which is only visible once it is credited into a real lease on the far side.
...
describe("SelectionAuthorityEngineImpl - derivation precedence (P1.2)", () => {
  it("a usable preferred host outranks a usable local host: effective is the preferred host", async () => {
    ...
    const authority = createTestAuthority({
      ...
      initialIdentityKey: "acct-1",
      clock,
      // A provisionable local host, which is what makes this a precedence test at all.
      localHostEnsure: readyLocalHostEnsurePort(),
    });
    const { engine } = authority;
    const seqA = engine.allocateAttachSeq("A");
    ...
    // L is usable by `isUsableForSelection` (only `dead` and `restarting-expected` are excluded).
    // Both P and L are therefore usable at the moment of activation, which is what makes this a precedence test rather than a "no other candidate" test.
    ...
    // Still true after the activation - L was never made unusable, so this pins order (the preferred arm runs before the local arm), not merely "local was unusable so preferred won by default".
...
describe("SelectionAuthorityEngineImpl - F14 deregister-clear (P1.2)", () => {
  it("a non-empty fleet that omits the preferred host clears preferred and emits cause deregister-clear; an empty fleet does not clear", async () => {
    ...
    // An empty fleet must not clear the preference (module header: "no hosts" is what this port publishes before its first genuine registry answer, and while an identity transition is in flight).
    fleet.publish(0, null, []);
    expect(engine.snapshot().preferredHostId).toBe("H");
    expect(preferredStore.load("acct-1")).toBe("H");
    const selectionEvents = events.filter(
...
describe("SelectionAuthorityEngineImpl - P1.3 failover scenarios", () => {
  it("A1: preferred remote dies → fallback immediately, no stability window", async () => {
    ...
    const authority = createTestAuthority({
      ...
      initialIdentityKey: "acct-1",
      clock,
      // L IS the fallback, so this machine's host has to be startable.
      localHostEnsure: readyLocalHostEnsurePort(),
    });
    const { engine } = authority;
    const incarnation = attachReporter(engine, "A");
  ...
  it("A3: fallback restart while FailedOver → no third-host hop (M6)", async () => {
    ...
    const authority = createTestAuthority({
      ...
      initialIdentityKey: "acct-1",
      clock,
      // L is the fallback the failover lands on; C is the third host M6 must keep the window from hopping to.
      localHostEnsure: readyLocalHostEnsurePort(),
    });
    const { engine } = authority;
    const incarnation = attachReporter(engine, "A");
    ...
    // Advance past both stability windows (failover-candidate and return-to-target) while the restart episode (60s) is still open.
    // Once damping alone would admit a hop, the hold rule is the only thing that can still keep the window on L instead of jumping to the third usable host C.
    expect(
  ...
  it("A6: a healthy remote preferred still ensures a DOWN local host (target-independent lifecycle), and stays on the remote while it boots", async () => {
    // Decision 2026-08-19: the local host's lifecycle does not depend on which host a window is pointed at.
    const clock = createFakeAuthorityClock(0);
    const ensure = createDeferredEnsure();
    const authority = createTestAuthority({
      initialFleet: {
    ...
    // L dies later (something pinned to it dialed it and was refused).
  ...
  it("does not hold a return-to-target behind an incumbent that has itself died", async () => {
    ...
    const authority = createTestAuthority({
      ...
      initialIdentityKey: "acct-1",
      clock,
      // L is the fallback the return-to-target hold sits on; it has to be startable, or the failover in step 2 lands on ∅ instead of L and there is no incumbent for step 4 to kill.
      localHostEnsure: readyLocalHostEnsurePort(),
    });
    const { engine } = authority;
    const incarnation = attachReporter(engine, "A");
    ...
    // 3.
    // Premise that the hold is real: a few seconds into the window, still L - without this check the arm under test could pass vacuously because the hold never engaged in the first place.
    engine.ingestEvidence(
      "A",
      incarnation,
      dialOutcome("P", "revive", "success", clock.now()),
    ...
    // 5.
    // The engine must not keep serving a host it has itself called dead (and is now merely booting) for the remaining ~17s of the 20s window (clock is at 3_000ms, well inside RETURN_TO_TARGET_STABILITY_MS).
    expect(engine.snapshot().effectiveHostId).toBe("P");
    expect(lastSelectionChange(authority.events).cause).toBe("recovery");
  ...
  it("returns to a revived target at once when the incumbent is a local host held usable only by the engine's own in-flight ensure", async () => {
    // The consequence of the incumbent rule stated for the decided lifecycle: a booting local host is a candidate (∅ never shows for it) but not something to keep serving a window from once the target can.
    const clock = createFakeAuthorityClock(0);
    const ensure = createDeferredEnsure();
    const authority = createTestAuthority({
      initialFleet: {
    ...
    // 4.
    // Nothing above is asserted again, so those callbacks could not change a verdict - but a second engine reacting to the first one's deadlines is not a fixture anyone should have to reason about.
    authority.dispose();
...
// The production guard is `isUsableForSelection`, which excludes every `dead` reason and therefore excluded `incompatible` from the day the arm landed.
// These are that scenario - a green run here is not evidence of new production behavior, it is evidence the behavior was never asked.
describe("SelectionAuthorityEngineImpl - P3.2 D13: incompatible is never a candidate", () => {
  it("failover skips an incompatible remote and takes the healthy host BEHIND it in fleet order", async () => {
    // No local host, so the choice is made purely among remotes by the third derivation arm - mru first, then fleet order.
    // X is seeded before Y, so an engine that forgot D13 would take X: the assertion discriminates rather than merely agreeing with the current implementation.
    const clock = createFakeAuthorityClock(0);
    const authority = createTestAuthority({
      initialFleet: {
        identityGeneration: 0,
  ...
  it("a PREFERRED host that goes incompatible after an app update falls over, with the switch cause the toast reads", async () => {
    // C4 hole 2, arm one.
    // Derivation must move - and the move is what the surface chip and the one-line "Switched to X" toast narrate (D11), which is why the cause is asserted and not just the destination.
    const clock = createFakeAuthorityClock(0);
    const authority = createTestAuthority({
      initialFleet: {
        identityGeneration: 0,
...
describe("SelectionAuthorityEngineImpl - local proof-of-life clears the ensure cooldown (P5.2)", () => {
  ...
  it("T4/addendum: a cooldown cleared by a live session does not resurface once that session is lost", async () => {
    ...
    // A session establishes for the local host - stronger evidence than a dial, and it clears the cooldown the same as T1.
    engine.ingestEvidence(
      "A",
      incarnation,
      sessionEvidence("L", "s1", "established", 1),
    ...
    // The discriminator: once the session goes away, the cooldown must not resurface.
    engine.ingestEvidence(
      "A",
      incarnation,
      sessionEvidence("L", "s1", "lost", 1),
...
// ------------------------------------------------------- :1799 owed pins `completeLocalEnsure` used to match a completion to its request by object identity alone.
...
describe("SelectionAuthorityEngineImpl - completeLocalEnsure fences on the local host across a swap (:1799 fix)", () => {
  it("R1a: a successful ensure for A credits A after the local host became B", async () => {
    ...
    // Premise, asserted positively before the harm: the local host really is A right now, and A's lease reads `connecting` only because the engine's own ensure is in flight (D14) - never because anything observed A alive.
    expect(engine.snapshot().targetHostId).toBe("A");
    ...
    // The harm the fence closes: A was credited with proof of life by a request that was about A but completed into a fleet whose local host is B.
  ...
  it("R1b/R1c: a failed ensure for A arms the cooldown against B, and B cannot ask for its own", async () => {
    ...
    // B is local and never dialed: it wants an ensure.
    // A second call means B got to ask for itself; one call would mean A's failure armed the cooldown over it instead - and clearing the token is what lets B ask at all rather than waiting out A's ceiling.
    expect(ensure.calls.count).toBe(2);
...
describe("SelectionAuthorityEngineImpl - compat eligibility (B3)", () => {
  /**
   * B3's eligibility half, which `ade7fe0f` did not touch.
   * This is the other half: `isUsableForSelection` answers on lease status alone, and a host whose compatibility has never been established derives as `connecting`, which is usable.
   */
   ...
   * Making unknown ineligible - the obvious reading of "never a candidate" - would make every host ineligible and ∅ universal.
   */
...
describe("SelectionAuthorityEngineImpl - a dead host reaches `dead` (B1/C6)", () => {
  /**
   * B1's corpse path.
   * The measured half was "48 s, zero updates"; the *unboundedness* was plausible - derived from the absent producer, never observed to a bound.
   */
  it("a host whose session dies with no further dial evidence still leaves `connecting`", async () => {
    ...
    const { engine } = authority;
    const incarnation = attachReporter(engine, "A");
    // The cold-boot ensure for L fires at attach whichever host is preferred (target-independent lifecycle).
    await Promise.resolve();
    await Promise.resolve();
  ...
  /** The false-death guard, and the reason the ceiling is scoped to the effective host at all. */
  ...
  /**
   * The arm-time half of that guard, which the bystander case above does not prove: removing the effectiveness check at arm time leaves this suite green, because the derive-time check still catches the bystander.
   */
  ...
  /**
   * The reselection half (Codex #1243): the derive-time check above stops a lapsed ceiling from killing P while the app is elsewhere - but P is then carrying an already-expired deadline.
   * Without a restart the app sits on P forever - the unbounded exit the ceiling exists to close, reached one selection later.
   */
...
describe("SelectionAuthorityEngineImpl - an in-flight ensure is bounded (B2)", () => {
  /**
   * B2 was filed plausible - derived end to end, never executed.
   * `nextDeadline()` has an arm for the cooldown after a failed ensure and no arm for an ensure still running, and the in-flight arm of `deriveHostStatus` reports `connecting`, which is usable.
   */
...
describe("SelectionAuthorityEngineImpl - compat anchor must displace, not merely differ (P5.2 T5/P7)", () => {
  ...
  // Inverted, and the inversion is the record of a D13 defect this test used to pin as correct.
  // The engine now orders two verdicts only when both name a session.
  ...
  // Recovery.
  // If it cannot displace an incumbent that names no session, the host is dead("incompatible") forever and no amount of fixing it helps.
 ...
 * These exist because "a pinned host stopped answering and its lease never reached `dead`" was not diagnosable from a production log.
 */
describe("selection authority dial-evidence instrumentation", () => {
  ...
  it("warns once dial failures stop advancing the streak - the reported pathology", () => {
    ...
    // A live session suppresses every refusal (invariant 5), so the host fails repeatedly while its lease can never move.
    engine.ingestEvidence(
      "A",
      incarnationId,
      sessionEvidence("H", "s1", "established", 0),
  ...
  it("warns ONCE per stall episode, not once per dial past the threshold", () => {
    ...
    // The test above stops exactly AT the threshold, so it cannot tell "fire at the crossing" from "fire at or above it".
    engine.ingestEvidence(
      "A",
      incarnationId,
      sessionEvidence("H", "s1", "established", 0),
  ...
  it("names the crossing ONCE - later refusals stay ordinary `counted`", () => {
    ...
    // The lease decision stays `>=` - the host is dead and stays dead - but the label marks the single report that crossed.
    const dispositions = dialLogs(log.records).map(
      (record) => record.detail.disposition,
    );
    expect(
  ...
  it("clears the stall on proof of life that is not a dial - a session", () => {
    ...
    // `onHostProvedAlive` is the funnel for every kind of proof, not just a dial success.
    // Without clearing there, inert reports from before a recovery combine with one after it and warn about an episode that ended.
    engine.ingestEvidence(
      "A",
      incarnationId,
  ...
  it("warns again after a host recovers and stalls a SECOND time", () => {
    ...
    // Latching on the host forever would be the wrong cure for the flood: a machine that recovers and then strands a surface again is a new incident, and the second one is exactly as worth reporting as the first.
    engine.ingestEvidence(
      "A",
      incarnationId,
      sessionEvidence("H", "s1", "established", 0),
  ...
  it("does not materialise an evidence record for a host whose dial was dropped", () => {
    // The instrument reads the streak without `hostEvidence()`, because the evidence map's emptiness is load-bearing ("never dialed" gates the launch-time ensure).
    const log = createRecordingAuthorityLog();
    const { authority, incarnationId } = attachedAuthority(log);
...
describe("SelectionAuthorityEngineImpl - cold-start hold: a restarting LOCAL target is waited for, not failed over", () => {
  /**
   * Builds an engine with a controllable {@link LocalHostOutageSignal}, seeded on an empty fleet (`localHostId: null, hosts: []`).
   * `initialOutage` and `localHostEnsure` are required, not defaulted (this repo's type-safety rules: no optional/defaulted params) - every caller states its own premise rather than inheriting one silently.
   */
  ...
  it("COLD START HOLD: a never-served process holds ∅ for a restarting LOCAL target instead of failing over to a usable remote", () => {
    ...
    // The mutation lane was already cycling the local host before the fleet ever answered - the launch-reconcile-races-boot shape the hold exists for.
    ...
    // Still held with no new evidence, comfortably inside the 20s ceiling - this is what distinguishes a genuine hold from a one-shot ∅ that just happens to be the first answer.
  ...
  it("P1 FIX - ONE WINDOW PER EPISODE: after the ceiling lapses onto R, a later ∅ adopts a newly-usable Q at once instead of re-arming a fresh hold on the still-restarting L", () => {
    ...
    // The regression: a second full 20s of ∅ - and this one narrates as the hard "No host is available" card, not as a launch, because the window has been served since.
    // Q must be adopted on the spot.
  ...
  it("P1 FIX - NEVER A FIRST WINDOW AFTER SERVICE: a local restart that begins only after the app has been serving gets no ∅ hold at all, so a newly usable Q is adopted at once", async () => {
    ...
    // The app works: nothing is cycling, and derivation names a host.
    // R, because this machine's host cannot be provisioned at all.
    fleet.publish(0, "L", [fleetHost("L", "local"), fleetHost("R", "remote")]);
    await Promise.resolve();
    await Promise.resolve();
    expect(engine.snapshot().effectiveHostId).toBe("R");
    ...
    // Now the local host starts cycling - the first restart episode this process has seen, so there is no lapsed record to stop a hold arming.
    // The episode guard cannot help here; only "derivation has named a host before" can.
    clock.advance(1_000);
    ...
    // A usable machine appears.
    // Holding ∅ for it would put the ∅ modal in front of a user who was working a moment ago, for the full ceiling, with a host right there - the hold buying a modal rather than preventing a flicker.
    fleet.publish(0, "L", [
      fleetHost("L", "local"),
      fleetHost("R", "remote"),
      fleetHost("Q", "remote"),
  ...
  it("BOUNDED: the ceiling forces a fallback to R at 20s even with the LOCAL OUTAGE signal (a 15-minute hold) still true - the exact 'stuck on the startup screen' regression this ceiling prevents", () => {
    ...
    // The ceiling itself, reached with NO new evidence - the engine's own deadline timer, not a report, is what wakes this transition.
    clock.advance(1);
    expect(findLease(engine.snapshot().leases, "L")?.status).toBe(
      "restarting-expected",
    );
  ...
  it("after the ceiling forces R, a later-usable local target recovers only through the ordinary RETURN_TO_TARGET_STABILITY_MS window - no special-casing", async () => {
    ...
    // The outage ends; L becomes usable.
    authority.setOutage(false);
    expect(findLease(engine.snapshot().leases, "L")?.status).not.toBe(
      "restarting-expected",
    );
    ...
    // Let the ensure the outage's end just triggered settle.
    await Promise.resolve();
    await Promise.resolve();
  ...
  it("P1 FIX - UNPROVEN INCUMBENT IS BOUNDED: a local host that became effective only because the engine's own ensure is in flight - never proven alive - gets the BOUNDED hold once it starts restarting, not the unbounded D5/M6 one", () => {
    ...
    // First look at L (local) and R (a fine, usable remote).
    // No outage yet, so the never-dialed local host draws the launch ensure and derives `connecting` - usable - and is picked as both target and effective.
    fleet.publish(0, "L", [fleetHost("L", "local"), fleetHost("R", "remote")]);
    expect(findLease(engine.snapshot().leases, "L")?.status).toBe("connecting");
    expect(engine.snapshot().effectiveHostId).toBe("L");

    // The mutation lane starts cycling L a moment later - after the ensure was already minted.
    // This is exactly the case the module header's "composition" pin protects: the outage signal alone cannot flip L's lease while this engine's own ensure is still outstanding for it, so nothing observable changes yet.
    ...
    // L is still the effective host - the D5/M6 exemption never newly selects a restarting host, but it does not throw the app off an already-effective one either.
    // What changed is which arm is holding it: the incumbent has never proved itself, so this is the bounded cold-start hold, not the unbounded one.
    expect(engine.snapshot().effectiveHostId).toBe("L");
  ...
  it("P1 FIX - PROVEN INCUMBENT STAYS UNBOUNDED: a local host proved alive by its own successful ensure keeps the unbounded D5/M6 hold once it starts restarting", async () => {
    ...
    // Let the launch ensure settle: a `{ ok: true }` completion is firsthand proof of life (`onHostProvedAlive`) - the one thing the previous pin's incumbent never got.
    await Promise.resolve();
    await Promise.resolve();

    // Now the mutation lane starts cycling L.
    authority.setOutage(true);
    expect(findLease(engine.snapshot().leases, "L")?.status).toBe(
      "restarting-expected",
    );
    ...
    // Unbounded: well past the 20s cold-start ceiling that bounded the unproven incumbent above - this is the D5/M6 hold at full strength, because `hasProvedAliveAtLeastOnce("L")` is true.
  ...
  it("P1 FIX - RE-KEY ON HOST REPLACEMENT: a fleet republish that swaps the LOCAL identity mid-hold gives the new host its own full 20s window, not the old host's remaining time", () => {
    ...
    // The desktop republishes on a local-identity change (PID metadata, re-enrolment): a different host, L2, is now `localHostId`, and it is also cycling (the outage signal is a port-level fact, not scoped to one host id).
    fleet.publish(0, "L2", [
      fleetHost("L2", "local"),
      fleetHost("R", "remote"),
    ]);
    ...
    // 10 more seconds - 25s total since L1's window opened, past what would have been L1's 20s ceiling.
  ...
  it("P1 FIX - NO RE-ARM AFTER LAPSE: once the ceiling has forced a fallback, further passes for the SAME still-restarting host do not re-engage the hold", () => {
    ...
    // A further pass for the same host, still restarting: the registry re-publishes the same membership (a periodic refresh, not a change).
    fleet.publish(0, "L", [fleetHost("L", "local"), fleetHost("R", "remote")]);
    expect(findLease(engine.snapshot().leases, "L")?.status).toBe(
      "restarting-expected",
    );
...
describe("SelectionAuthorityEngineImpl - cold-start hold does not apply once the process has served", () => {
  it("NOT A COLD START: serving remote R once, a later-cycling local L stays on R (the ordinary D8/M6 arms decide, unaffected by the hold)", async () => {
    ...
    const { engine } = authority;
    const incarnation = attachReporter(engine, "A");
    // L is never even the target here - the local-target-only half of the gate is pinned separately below.
    expect(engine.snapshot().effectiveHostId).toBe("R");

    await Promise.resolve();
    await Promise.resolve();

    // L now starts a deliberate restart while R - not L - is the current effective host.
    engine.ingestEvidence(
      "A",
      incarnation,
  ...
  it("P1 FIX - A SERVING FALLBACK IS NEVER DROPPED: with the local host as the TARGET, a later restart intent for it leaves the failed-over R serving instead of holding ∅", async () => {
    ...
    // NO preference, so the target IS the local host (M5).
    // That is the whole difference from the case above - there R was preferred and L was never the target, so `targetHostId === localHostId` was false and the target-side arm could not fire whatever it did.
    expect(engine.snapshot().targetHostId).toBe("L");

    // L is unprovisionable (the harness default port answers "unavailable") and then refuses its dials outright, so the app fails over to R exactly as D8 says.
    await Promise.resolve();
    await Promise.resolve();
    killHostWithRefusals(engine, "A", incarnation, "L");
    expect(engine.snapshot().effectiveHostId).toBe("R");
