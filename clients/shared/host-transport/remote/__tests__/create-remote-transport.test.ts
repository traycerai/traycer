...
describe("createRemoteHostTransport bearer gate", () => {
  it("refuses to build - and so to cache - a session with no auth context", () => {
    // The bearer thunk is a live read, so a session built while it is null could later dial once a context appears, while keyed under an epoch label divorced from that context.
    // Building must degrade to the same `null` as an unconnectable target, not mint a cache entry.
    expect(transportFor(null)).toBeNull();

    // A released lease is the same verdict through a different shape: the source object survives (still labelling the retired epoch) but its bearer read throws.
    // Letting it into the cache would present the stale epoch as newest and supersede the live context's entry, while the session it builds could never mint.
    ...
    // The third enumerated refusal: a source that reads fine but holds an empty token.
    // It cannot authorize a grant mint any more than a released lease can - only the failure would come later, from authn, after the session was already cached under that epoch.
    ...
    // Non-vacuity contrast: the identical options with a live auth context build fine - the null verdicts above came from the bearer gate, not from some other option being malformed.
