 ...
 *
 * `drop` is the third case and the one that owes nothing: an ANSWER the live
 * line added, settling a server frame this line never sends. Nothing on a
 * v1.3.0 host asked for it, so nothing there is waiting on it, and minting an
 * `actionAck` for it would answer a `requestId` that does not exist.
 */
...
export function projectBrowserSessionsClientFrameToV10(
  ...
  switch (frame.kind) {
    ...
    // The recording answers. A `@1` host never sends `startTabRecording`, so
    // there is no recording on this line for one of these to settle and no
    // `requestId` to refuse - dropping is the whole of the right answer.
    case "recordingHelperReady":
    case "recordingEnded":
      return { kind: "drop" };
