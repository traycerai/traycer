...
describe("createDialGate: the concurrent-handshake cap and its queueing rules", () => {
  ...
  it("drains interactive fully before background, and lets a late interactive cut ahead of an earlier background", async () => {
    ...
    // The reserve caps background at MAX_BACKGROUND_CONNECTING (3), so 6 background dials alone can no longer fill all six slots the way this case's setup used to.
    // Fill them with 3 interactive + 3 background instead - still enough to show that interactive drains first - and leave the "late interactive cuts ahead of earlier-queued background" half of the claim to the release below.
  ...
  it("reserves capacity for interactive: at most MAX_BACKGROUND_CONNECTING background dials start before a late interactive one", async () => {
    ...
    // The start order, not just a count, says which dial went first: only 3 of the 6 background dials got a slot before the interactive one did - background-3..5 are still queued behind it.
    expect(started).toEqual([
      "background-0",
      "background-1",
      "background-2",
...
describe("the WHATWG factories: native construction stays behind the dial gate", () => {
  class FakeWebSocket {
    ...
    /**
     * Mirrors the platform: closing a socket - connecting or open - always ends in a `close` event, synchronously for this fake.
     */
  ...
  afterEach(() => {
    vi.unstubAllGlobals();
    // Every case below must leave the shared singleton idle - a slot leaked here starves every other host socket for the life of the process.
  ...
  it("constructs at most MAX_CONNECTING native sockets before the first one opens", async () => {
    ...
    // Drain everything so the gate is idle for the afterEach assertion.
  ...
  it("never lets the platform see a socket closed before its dial started, and still delivers exactly one onclose", async () => {
    ...
    // The first 6 hold the slots; the 10th has not dialed yet.
    // Close it synchronously, before microtasks flush, so it is dequeued rather than torn down mid-handshake.
    ...
    // Release the 6 held slots so the still-queued entries (6, 7, 8) get a turn - proving the cap, not just the queue order, governs how many of the surviving 9 ever reach the platform.
  ...
  it("the stream factory aborts a still-CONNECTING socket on close() and frees its slot without waiting for the platform", async () => {
    // The host that produces this is one that never answers the handshake: `WsStreamClient` gives up at `dialTimeoutMs` and calls `teardownSocket(4000, "dial-timeout")`, then re-dials.
    // The platform is still sitting on the abandoned socket, so nothing releases the gate ticket on its own - which is the whole failure.
    ...
    // Half 1: the socket is closed for real, not merely forgotten.
    // A fix that released the ticket without closing would satisfy Half 2 alone while leaving a live pending connection against the browser's own ceiling - the gate would then admit a replacement dial on top of it.
    expect(native.closeCalls).toEqual([{ code: 4000, reason: "dial-timeout" }]);

    // Half 2: the slot is back now.
    // The platform has delivered no `close` event (that is what `blackHoleConnectingCloses` models), so the native listeners have not run and this can only be `close()`'s own release.
    await flush();
    expect(constructedSockets).toHaveLength(7);

    // A late `open` on the abandoned socket is not announced to the consumer that already gave up on it.
    // That is the only state this guard exists for, and it has to be staged deliberately.
