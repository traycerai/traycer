...
describe("WsStreamClient", () => {
  ...
  describe("retryable-close log dedup (P2-5)", () => {
    ...
    it("delivers a retryable fatal as ('reconnecting', {kind: 'retryableClose', details}) with the bounded code", async () => {
      vi.useFakeTimers({ shouldAdvanceTime: false });
      const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
      ...
      if (last.reason === null || last.reason.kind !== "retryableClose") {
        throw new Error("expected a retryableClose reason");
      }
      expect(last.reason.details.code).toBe("CHAT_STORE_TOO_NEW");
      expect(last.reason.details.reason).toBe(
        "Chat store was written by a newer build",
      );
    ...
    it("bounds a retryable close's code - an unprintable remote code is delivered as <unprintable>, never rendered raw", async () => {
      vi.useFakeTimers({ shouldAdvanceTime: false });
      const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
      ...
      expect(last.reason.details.code).toBe("<unprintable>");
    ...
    it("delivers an ordinary reconnect as ('reconnecting', null), and does not repeat stale retryable details on the next ordinary drop", async () => {
      vi.useFakeTimers({ shouldAdvanceTime: false });
      const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
      ...
      // Episode 1: a retryable fatal - this session's `reconnecting` carries
      // the details.
      completeHandshake(sockets[0].socket);
      sockets[0].socket.fireText(RETRYABLE_STORE_FATAL);
      const afterRetryable = calls[calls.length - 1];
      if (
        afterRetryable.reason === null ||
        afterRetryable.reason.kind !== "retryableClose"
      ) {
        throw new Error("expected a retryableClose reason");
      }

      // Episode 2: an ordinary transport drop on the redial - a plain socket
      // close, not a fatal frame. `pendingRetryableClose` must have been
      // consumed by the first `reconnecting` transition, so this one must not
      // repeat it.
      await vi.advanceTimersByTimeAsync(5);
      expect(sockets).toHaveLength(2);
      completeHandshake(sockets[1].socket);
      ...
      const afterOrdinary = calls[calls.length - 1];
      expect(afterOrdinary.status).toBe("reconnecting");
      expect(afterOrdinary.reason).toBeNull();
