...
function openSession(options: SessionOptions): Session {
  ...
  socket.onerror = (event: WebSocketErrorEvent) => {
    failAll(
      new HostRpcError({
        code: "RPC_ERROR",
        message: `WebSocket transport error: ${event.message}`,
        requestId,
        method,
        fatalDetails: null,
      }),
  ...
  socket.onclose = (event: WebSocketCloseEvent) => {
    ...
    if (frameResolver !== null) {
      failAll(
        new HostRpcError({
          code: "RPC_ERROR",
          message: `WebSocket closed before next frame (code=${event.code}, reason='${event.reason}')`,
          requestId,
          method,
          fatalDetails: null,
        }),
      ...
      return new Promise<void>((resolve, reject) => {
        const timer = setTimeout(() => {
          failAll(
            new HostRpcError({
              code: "RPC_ERROR",
              message: `WebSocket dial timed out after ${dialTimeoutMs}ms`,
              requestId,
              method,
              fatalDetails: null,
            }),
      ...
      return new Promise<HostFrame>((resolve, reject) => {
        const timer = setTimeout(() => {
          failAll(
            new HostRpcError({
              code: "RPC_ERROR",
              message: `WebSocket frame timed out after ${frameTimeoutMs}ms`,
              requestId,
              method,
              fatalDetails: null,
            }),
