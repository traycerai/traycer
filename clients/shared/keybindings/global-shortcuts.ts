...
/**
 * Static table of global (OS-level) shortcuts the app offers.
 * Today there is one entry (`summon`); the id union exists so a second global shortcut never re-invents the intent/status/reconcile machinery - see the desktop's `electron-main/app/shortcuts.ts` for the registry itself.
 */
...
/**
 * What the OS actually granted for one shortcut, as reconciled by main: - `registered` - the OS accepted the registration; the chord is live.
 * - `disabled` - the user turned the shortcut off (or reconciliation is currently suppressed), so it was never offered to the OS.
 */
