...
async function supervisor(): Promise<void> {
  ...
  // Bound once, right after the guard: `wrapper.pid` is a property read that TypeScript cannot narrow across the closure below, which is what forced the `wrapper.pid!` non-null assertion.
  // A local `const` carries the guard's proof instead of re-asserting it at every use site.
  ...
  const reapDescendantGroup = async (): Promise<void> => {
    ...
    try {
      ...
      // D may already have died; E is still in the same process group when the wrapper death was the first event, and the group kill is best effort on platforms without negative-PID process groups.
    }
    ...
    wrapper.stdout.on("data", (chunk: string) => {
      ...
      void (async () => {
        try {
          ...
          // Awaited before the stdout publish below: `helper` waits for the "bind-actuator" stdout line and then immediately reads this file.
          ...
          // The lock envelope is C, not transient D.
          process.stdout.write(
            `${JSON.stringify({
              kind: "bind-actuator",
              pid: process.pid,
