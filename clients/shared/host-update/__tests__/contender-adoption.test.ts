...
// `probeAttemptHolder` is mocked (spy-wrapping the real implementation by default) so one test can force the "indeterminate" arm deterministically.
vi.mock("../lock", async () => {
...
// The load-bearing property is conjunctive liveness re-taken on every call - identity matching the lock file on disk is necessary but never sufficient, because a dead parent leaves that identity behind unchanged.
...
// Explicit rather than defaulted: the repo's type-safety rules ban default parameters so a caller cannot silently inherit a bound it never chose - a wait budget is exactly the kind of value a reader should see at the call.
...
describe("withUpdateContenderAdoption - dead-parent case (real subprocess)", () => {
  it("stale-but-present lock file with matching identity is NOT live - identity alone would have admitted, liveness is what refuses", async () => {
    ...
    // Read the real holder metadata off the lock file the child wrote, while it is still alive - this is exactly what a genuine adoption proof would carry.
    ...
    // Now kill the child without letting it release the lock, so the file on disk is untouched - same token, same pid, same start identity - and only the process behind it is gone.
    worker.kill("SIGKILL");
    ...
    // Identity alone (no liveness) still matches: this is the specific hole the conjunctive check exists to close.
    await expect(
  ...
  it("never authorizes from a cached positive verdict, even one warmed a moment earlier against the same still-present lock", async () => {
    ...
    // Warm the fleet-status-style cache with a long ttl while the holder is genuinely alive.
    ...
    // Called immediately - well inside the 60s window just warmed above - so a cache-consulting implementation would still be serving the stale positive verdict here.
    const adopted = await withUpdateContenderAdoption(
      adoption,
      {
        hostHomeDir,
...
describe("withUpdateContenderAdoption - structural: no record write, no handle", () => {
  ...
  // The fix is a `finally` that deletes both map entries on every exit path.
  it("an escaped capability captured on successful completion verifies dead once the segment has returned, even though the parent lock is still live", async () => {
    ...
    const outer = await withUpdateContender(
      ...
      async (capability) => {
        ...
        // The segment has returned.
        // The parent lock (`capability`, from the outer `withUpdateContender`) is still genuinely live - only the child segment ended.
