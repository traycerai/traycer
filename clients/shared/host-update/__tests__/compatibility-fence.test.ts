...
describe("compatibility fence — the SHIPPED floors", () => {
  it("both floors ship UNPINNED, and the fence refuses while they are", () => {
    // This pins the deviation from the plan's "land with the current dev version" wording, and the reason for it.
    // A dev version is below every real release, so an unpinned floor would admit the whole fleet including the lock-blind binaries the fence exists to refuse - and forgetting the re-pin would be silent and fleet-wide.
  ...
  it("an unpinned floor refuses even a fleet that would otherwise pass", () => {
    // The ordering inside the function matters: the unpinned check has to come first, because every comparison after it is meaningless against a sentinel.
    expect(
      decideCompatibilityFence(
...
describe("compatibility fence — the preventive matrix, floors pinned", () => {
  ...
  it("ADMITS when no CLI is recorded — the named residual, asserted so it is a decision", () => {
    ...
    // Refusing on absence would not close that gap; it would only refuse machines with no installed CLI, which are not the dangerous ones.
    expect(
      decideCompatibilityFence(
  ...
  it.each([
    ...
    (_which, input, reason) => {
      // "Cannot compare" is not evidence of compliance.
      // The fence exists because an unverified actor is the dangerous one, so an unorderable version is exactly the case that must not be admitted.
...
describe("compatibility fence — the detective half", () => {
  ...
  it.each(["restarting", "verifying"] as const)(
    ...
    (phase) => {
      // Not a severity choice.
      // Once `restarting` is committed the record has promised a return and the graph offers {verifying, failed, superseded} and no park at all, so terminal-with-diagnostics is the only honest close.
      const verdict = decideLegacyMarkerConcurrency({
        legacyMarkerPresent: true,
        attemptPhase: phase,
      });
  ...
  it.each(["complete", "failed", "superseded"] as const)(
    ...
    (phase) => {
      // A terminal record (complete/failed/superseded) has no legal successors, so an abort/park disposition would be unapplyable.
      // This is distinct from the post-tombstone (`restarting`/`verifying`) case above, which still aborts: those phases are live and non-terminal, only barred from parking.
      expect(
        decideLegacyMarkerConcurrency({
          legacyMarkerPresent: true,
          attemptPhase: phase,
...
describe("cohort policy (O4) — failure degrades TO the fence, never through it", () => {
  ...
  it.each([
    ...
    // The load-bearing property, and the reason all three share one branch: a kill switch whose failure mode is "admit" is not a kill switch.
