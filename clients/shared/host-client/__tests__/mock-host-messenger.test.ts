...
describe("MockHostMessenger", () => {
  ...
  it("surfaces missing handlers as HostRpcError", async () => {
    ...
    await expect(
      messenger.request("host.echo", { message: "x" }, null, authority()),
    ).rejects.toSatisfy(
      (err: unknown) =>
  ...
  it("emits phase hooks in the WS lifecycle order on the happy path", async () => {
    ...
    await messenger.request("host.echo", { message: "hi" }, null, authority());
    unsubscribe();
  ...
  it("rejects a held handler when request authority is aborted", async () => {
    ...
    const request = messenger.request("host.echo", { message: "x" }, null, {
      ...authority(),
