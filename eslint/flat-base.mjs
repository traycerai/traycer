/**
 * Shared flat-config building blocks for the monorepo.
 * Per-workspace `eslint.config.mjs` files import from here instead of depending on the eslint packages individually.
 */
...
// `.cjs`/`.mjs` are excluded to match the old `--ext .ts[,.tsx]` scope (deploy scripts and tool configs like vite/vitest configs were never linted).
