/**
 * Dumps this tree's protocol surface (handshake manifest + version graph + per-version wire schemas for the unary and stream host registries) as JSON on stdout.
 * Imports must stay RELATIVE (package-alias subpaths may not exist in old trees) and the runtime dependency set must stay { zod, this repo's host index }.
 */
...
async function resolveUnaryFloorMethodNames(): Promise<readonly string[]> {
  try {
    ...
    // Backfill path: this script is copied into older released tags that do not have the production floor module yet.
