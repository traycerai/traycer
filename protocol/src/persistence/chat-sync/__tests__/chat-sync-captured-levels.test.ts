...
// Importing the barrel builds every schema reachable from both records, which is what populates the registration list below.
...
describe("chat-sync captured residual levels", () => {
  ...
  it("registers each level exactly once", () => {
    // `reprojectResidualCapture` exists so the reader schemas can reuse a level without double-counting it.
    const registered = listCapturedLevelRegistrations().map(
      (level) => level.id,
    );
