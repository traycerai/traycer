 ...
 *
 * `row-projection.ts` as it stood before the row fold (`foldTranscriptRows`)
 * replaced its enumeration, with the steer-lifecycle fold it called inlined as
 * it stood then. The fold's parity suites compare every projection - the
 * whole-array entry and every incremental step - against THIS, so a bug that
 * the refactor put into both the full and the incremental path is still
 * caught: agreeing with itself proves nothing.
 *
 ...
 */
export function legacyProjectTranscriptRows(
