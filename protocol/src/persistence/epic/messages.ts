...
export const assistantMessageSchema = assistantMessageSchemaV18.extend({
   ...
   *
   * It lives on the LIVE extension, not on the shared `V18` historical base:
   * `assistantMessageSchemaV18` is what `chat.subscribe@1.7`/`@1.8` serve, and
   * a released line does not grow a key. `@1.9` and persistence read this
   * shape and see it.
   *
   ...
   * historical base plus the hand-frozen wire copies below simply omit the key,
   * so a released peer neither receives it nor has to know it exists. Adding it
   * here is a same-major persistence addition under `COMPATIBILITY.md`'s "new
   * key whose input schema accepts absence" rule.
   */
