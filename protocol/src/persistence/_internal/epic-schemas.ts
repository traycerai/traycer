...
const epicNonChatFields = {
  artifacts: lazySchema(() => z.record(z.string(), epicArtifactSchema)),
  deletedArtifacts: lazySchema(() =>
    z.record(z.string(), deletedEpicArtifactSchema),
  ),
  tuiAgents: lazySchema(() => z.record(z.string(), tuiAgentSchema).default({})),
  roleClaims: lazySchema(() => roleClaimsSchema.default({})),
};

/** Epic 2.0 as shipped before the Reasonix persisted enum/union variants. */
export const epicSchemaPreReasonix = lazySchema(() =>
  z.object({
    ...epicIdentityFields,
 ...
 */
export const epicSchema = lazySchema(() =>
  z.object({
    ...epicIdentityFields,
