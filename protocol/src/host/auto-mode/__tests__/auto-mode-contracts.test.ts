...
describe("auto-mode protocol change", () => {
  ...
  it("registers all six settings methods off the released floor", () => {
    ...
    //
    // `autoJudge.get` / `autoJudge.set` head at `1.2` now (`1.1`'s `fallback`
    // response arm, plus `1.2`'s `reasoningEffort` key on the selection); the
    // other four are still `1.0`.
    const methods = [
      { method: "providers.setAutoJudge", latestMinor: 0 },
      { method: "autoJudge.get", latestMinor: 2 },
      { method: "autoJudge.set", latestMinor: 2 },
  ...
  it("keeps the judge selection's harness id an open string", () => {
    ...
    // The frozen `1.0`/`1.1` selection has the same openness, minus the effort.
    expect(
