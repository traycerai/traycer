...
/**
 * The chat-side doors into the managed-command UI: the queue chip and the resume divider.
 * Both name ONE entity - a shell - and carry `monitoring` as the state that decides its glyph, never as a second sort of thing.
 */
...
describe("managed-command queue chip", () => {
  ...
  it("reads an item persisted before the field existed as unrecorded", () => {
    // The queue is durable: an item rehydrated after a host restart must still parse.
    // Absent means "not recorded", which the chip renders generically - never a flag guessed on its behalf.
    const parsed = chatQueuedManagedCommandItemSchema.parse({
      kind: "managed-command",
      queueItemId: "q-1",
      commandId: "cmd-1",
...
describe("resume divider trigger", () => {
  ...
  it("reads a divider persisted before `monitoring` existed as not watching", () => {
    // Chats written while this key still carried a monitor/shell `kind` lose that value on parse.
    // The trigger must survive it: a shell whose watching state is unrecorded renders as a plain one rather than failing the chat.
    const parsed = autonomousResumeTriggerSchema.parse({
      kind: "monitor",
      title: "db migration",
      status: "completed",
  ...
  it("defaults to absent for every trigger that is not a managed command", () => {
    // A subagent or wakeup trigger has no command to open, and every trigger persisted before this key existed has none recorded.
    const parsed = autonomousResumeTriggerSchema.parse({
      kind: "subagent",
      title: "reviewer",
      status: "completed",
  ...
  it("keeps the persisted source enum closed", () => {
    // This is the constraint that forced the shell's identity into a keyed property rather than a `kind: "shell"` variant: an unknown value in a PERSISTED enum fails the WHOLE chat's safeParse on an older host, while an.
    expect(() =>
