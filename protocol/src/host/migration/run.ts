...
export const migrationRunOpenRequestSchema = lazySchema(() => z.object({}));
export type MigrationRunOpenRequest = z.infer<
  typeof migrationRunOpenRequestSchema
>;

const taskChainOutcomeSchema = lazySchema(() =>
  z.enum(["complete", "skipped", "failed"]),
);
const epicOutcomeSchema = lazySchema(() => z.enum(["complete", "failed"]));
const replayEntityKindSchema = lazySchema(() => z.enum(["chain", "epic"]));

const migrationCompleteCountsSchema = lazySchema(() =>
  z.object({
...
export const migrationRunServerFrameSchema = lazySchema(() =>
...
export const migrationRunClientFrameSchema = lazySchema(() =>
  z.discriminatedUnion("kind", [
    z.object({
      kind: z.literal("ping"),
      hasBinaryPayload: z.literal(false),
