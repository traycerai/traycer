...
const HOST_STREAM_RPC_REGISTRY_OTHER_DEFINITION = {
  ...
  "browser.sessions": {
    ...
    // The epic-media-pipeline recording control frames
    // (`startTabRecording` / `stopTabRecording` out, `recordingHelperReady` /
    // `recordingEnded` back - text only, the clip's chunks go over HTTP) extend
    // this live 2.0 in place, never the frozen @1.0.
    2: {
      latestMinor: 0,
      versions: {
        0: {
