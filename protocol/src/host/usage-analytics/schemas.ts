...
export const usageSummaryOutcomeSchema = lazySchema(() =>
  z.enum(["completed", "stopped", "interrupted", "abnormal_exit"]),
);

export const usageSummaryCompletenessSchema = lazySchema(() =>
  z.enum(["measured", "partial", "absent"]),
);
...
export const usageSummaryCostProvenanceSchema = lazySchema(() =>
  z.enum(["unpriced", "modelPriced", "providerReported"]),
);

const nonNegativeIntSchema = lazySchema(() => z.number().int().nonnegative());

export const usageSummaryTokenTotalsSchema = lazySchema(() =>
  z.object({
 ...
 */
const usageDayKeySchema = lazySchema(() =>
  z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
);

export const usageSummaryBucketSchema = lazySchema(() =>
  z.object({
...
export const usageSummaryWindowBoundsSchema = lazySchema(() =>
  z.object({
...
export const usageProvenanceSplitEntrySchema = lazySchema(() =>
  z.object({
...
export const usageProvenanceSplitSchema = lazySchema(() =>
  z.object({
...
export const usageSummaryTotalsSchema = lazySchema(() =>
  z.object({
    factCount: nonNegativeIntSchema,
    tokens: usageSummaryTokenTotalsSchema,
    knownCostUsd: z.number().finite(),
...
export const usageSummaryChatBucketSchema = lazySchema(() =>
  z.object({
 ...
 */
export const usageSummaryHostBucketSchema = lazySchema(() =>
  z.object({
...
export const usageTurnRowSchema = lazySchema(() =>
  z.object({
...
export const usageSummaryOutcomeBreakdownSchema = lazySchema(() =>
  z.object({
...
export const usageSummaryCompletenessBreakdownSchema = lazySchema(() =>
  z.object({
...
export const usageSummarySchema = lazySchema(() =>
  z.object({
...
export const usageCostCoverageSchema = lazySchema(() =>
  z.object({
 ...
 */
const hostUsageSummaryRequestFields = lazySchema(() =>
  z.object({
    timezone: z.string().min(1).max(100),
    windowDays: z.number().int().positive(),
    epicId: z.string().min(1).max(191).nullable(),
...
export const hostUsageSummaryRequestSchemaV10 = lazySchema(() =>
 ...
 */
export const hostUsageSummaryRequestSchema = lazySchema(() =>
  hostUsageSummaryRequestFields
 ...
 */
export const hostUsageSummaryResponseSchemaV10 = lazySchema(() =>
  z.object({
