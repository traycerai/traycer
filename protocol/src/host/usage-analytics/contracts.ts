...
/**
 * D21/D27: adds an optional `profileId` filter to narrow the totals to one
 * profile. Response is UNCHANGED - `usageSummarySchema` itself gains no
 * per-profile buckets here; W5-T2 owns that if it turns out to need one.
 *
...
>({
  ...
  to: { major: 2, minor: 0 },
  // An old client only ever meant every profile.
  upgradeRequest: (request) => ({ ...request, profileId: null }),
  upgradeResponse: (response) => response,
});

/**
 * `profileId` only ever NARROWS a read (see the request's own doc comment),
 * so downgrading to a host that cannot filter is an honest degradation - it
 * returns every profile's totals instead of refusing. Never
...
>({
  ...
  downgradeRequest: (request) => {
    const { profileId: _profileId, ...rest } = request;
