...
describe("hostStatusUpgradeV10ToV11", () => {
  it("upgrades a v1.0 response with busySessionCount: null, NOT a fabricated 0", () => {
    ...
    // The upgraded shape must itself satisfy v1.1's schema - a nullable count is only useful downstream if the contract it upgrades INTO actually accepts it.
  ...
  // The review's point: a test that asserts the SAME observable shape for both an upgraded (never-reported) count and a genuine zero would still pass if the upgrade normalized `null` back to `0` with `??
  ...
  it("rejects a v1.1 response schema that still fabricates 0 for an absent count", () => {
    // Documents WHY the upgrade must not go back to `??
    const parsed = hostStatusV10.responseSchema.parse(V10_RESPONSE);
    const upgraded = hostStatusUpgradeV10ToV11.upgradeResponse(parsed);
    expect(upgraded.busySessionCount).not.toBe(0);
  });
