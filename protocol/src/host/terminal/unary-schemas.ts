...
// `terminal.subscribe@1.4` - Session scope: `{ kind: "epic" }` sessions belong to an epic; `{ kind: "independent" }` sessions are landing-scope (epic-less).
// Used by the major-2 unary lines and `terminal.subscribe@1.4` (see `canonicalTerminalSessionInfoSchema` below); the frozen `terminalSessionInfoSchema` predates scoping and stays `epicId`.
...
// Frozen `epicId: string` session-info shape, shared by every RELEASED terminal contract (`create@1.0`/`list@1.0` responses; `subscribe@1.0`- `1.3` frames).
// Never mutate in place - replacing `epicId` with a scope union would change an already-shipped host->client slot, which the compat checker blocks.
export const terminalSessionInfoSchema = z.object({
  ...
  // Lifetime is the session's - PTYs don't survive host restarts, so neither does the title.
  title: z.string().nullable(),
  // Host-observed foreground process name for the PTY.
  // `null` means the terminal is idle or the host cannot determine a foreground process.
  activeProcessName: z.string().nullable().optional(),
});
export type TerminalSessionInfo = z.infer<typeof terminalSessionInfoSchema>;

// Canonical session-info shape for the scope-bearing terminal lines: `scope` replaces `epicId` - `{ kind: "independent" }` denotes a landing-scope (epic-less) session.
// Every other field is identical to the frozen `terminalSessionInfoSchema` above - this is a parallel export, not a replacement, so released contracts keep parsing the frozen shape untouched.
...
// `terminal.list@2.0` - Latest canonical session-info shape.
// The wire schema accepts an empty compatibility value because the frozen v2.1 `cwd` field did; clients treat it as unavailable.
...
// `terminal.list@2.3` - Who owns the session's lifetime.
// This is a parallel schema: the v2.2 currentCwd shape already shipped and stays frozen.
...
// `cwd` is the renderer-selected working directory; `shellCommand` and `shellArgs` remain nullable so interactive terminals can use the host's configured shell.
export const createTerminalRequestSchema = z.object({
  ...
  // Worktree paths the launching caller is committing the PTY to using for its lifetime.
...
// `terminal.create@2.0` - `scope: { kind: "independent" }` requests a landing-scope (epic-less) session; every other field is unchanged from `@1.0`.
// The response's `session` carries the canonical (scope-bearing) session info instead of the frozen `@1.0` shape.
...
// Spawning client's resolved terminal appearance, carried on `terminal.create@2.1` so the host can answer a TUI's OSC 10/11 foreground/background queries (which otherwise time out - no client is subscribed yet when a TUI.
...
// `terminal.create@2.1` - additive request-side `themeHint`.
...
// `terminal.kill@1.0` - terminates a session and evicts it from the host's in-memory map.
...
// `terminal.list@1.0` - lists sessions the host currently knows about for the given epic, including ones in the post-exit grace window (so the renderer can show "Process exited (code N) - Restart" instead of silently.
...
// `terminal.list@2.0` - `scope: { kind: "independent" }` lists landing-scope (epic-less) sessions instead of an epic's.
// Sessions carry the canonical (scope-bearing) session info instead of the frozen `@1.0` shape.
...
// `terminal.list@2.1` - additive `homeCwd` on the response.
// `null` is reserved for compatibility: the v2.0 → v2.1 response upgrade supplies `homeCwd: null` because an older host cannot authoritatively provide it.
...
// `terminal.list@2.2` - additive live `currentCwd` on every session.
// That frozen field allowed an empty compatibility value, which current clients interpret as "directory unavailable" rather than inventing a path.
...
// `terminal.list@2.3` - additive lifetime-owner discriminator on every session.
// Genuinely older hosts remain full `terminal.list` after positive legacy negotiation and do not consult the field.
...
// `terminal.readOutput@1.0` - read-only access to one session's output for a caller that is an AGENT rather than a renderer.
// The file is a regenerable projection of live emulator state, rewritten on every call, so a path is never worth caching past the read that returned it.
...
// `terminal.rename@1.0` - overrides the session's display title.
// The wire schema and `updated` response semantics remain frozen.
