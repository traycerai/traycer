...
// identity" section). Consumed by the CLI (`clients/traycer-cli`), desktop
// main (`clients/desktop`) and the host itself (through `store-formats`)
// so update/stage/promote decisions never diverge between the processes.
// It lives in the protocol package because the host cannot import
// `@traycer-clients/shared`, and the store-format floor it needs is a
// function of version ORDER; `clients/shared/host-version` re-exports it
