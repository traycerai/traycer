/**
 * Version-aware projections for the `chat.subscribe` stream that both ends need and neither owns alone.
 * A new client sends live frames; a peer that negotiated `1.4`-`1.6` must receive the shape ITS contract declares.
 */
import type { SchemaVersion } from "@traycer/protocol/framework/versioned-stream-rpc";
import type { ChatSubscribeClientFrame } from "@traycer/protocol/host/agent/gui/subscribe";

/** A client frame already reduced to its wire form for a specific negotiated line. */
...
/**
 * The minor that introduced interview settlement on the wire: selection evidence on answers, Skip intent plus saved drafts on `interviewError`.
 */
 ...
 * The GUI must then not claim drafts were saved - they were not sent, and nothing durable recorded them.
 */
 ...
 * Agent payloads (`kind: "agent"`) never carry these fields and are left alone.
 */
...
/**
 * Normalize live-only message fields on a snapshot that took the `1.6` SHALLOW path.
 * The frames that arrive OUTSIDE a snapshot get no frozen parse at all - see `normalizeV16BrowserPayloadsInFrame`.
 */
...
/**
 * Neutralize the `1.7`-only browser arrays on a NON-snapshot server frame that arrived on the `1.6` line.
 * Empty, never `undefined` - consumers are typed as if both arrays are present.
 */
...
/** Drop `relaunchOnHostRestart` from a list of managed commands. */
...
function projectSnapshot(
  ...
  // Only rewrite the key when the snapshot carries it: a pre-`1.6` peer's snapshot has already had the set stripped, and must not grow it back as an explicit `undefined`.
...
export function projectChatServerFrameForVersion(
  ...
  // BEFORE the 1.7 identity return: `chat.imported` shipped on the 1.8 line, and a released 1.7 client's strict event enum fails the WHOLE snapshot on an unknown member - so every pre-1.8 peer must never see the event.
  // A client that cannot render an import provenance row has nothing to do with the value anyway.
...
/**
 * The ONE metadata key a `1.7`+ host may use to attach interview settlement facts to a durable chat event.
 * PHASE 2 OBLIGATION: the host must write `DurableInterviewSettlement` under this key.
 */
export const INTERVIEW_SETTLEMENT_METADATA_KEY = "interviewSettlement";

/**
 * The opaque delivery envelope paired with a detached settlement.
 * It is only durable-repair input for the host (identity, owner, and exact provider payload); pre-`1.7` peers must not observe this new metadata surface.
 */
export const INTERVIEW_DELIVERY_METADATA_KEY = "interviewDelivery";

/**
 * Metadata keys whose value is an array of interview ANSWERS, each of which may carry `selection` under `1.7`.
 * Enumerated rather than discovered, so this never walks into unrelated provider metadata looking for something answer-shaped.
 */
 ...
 * A snapshot instead filters the event out of the durable log, since the snapshot itself must still land.
 */
