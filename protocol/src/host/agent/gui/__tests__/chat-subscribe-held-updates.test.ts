...
/**
 * The Stop fence's held updates on `chat.subscribe@1.6` - the snapshot's `heldUpdates` and the `heldUpdatesChanged` frame that gives `managedCommand.deliverHeld` something to act on (see `unary-schemas.ts` and.
 * Note what those tests do NOT cover: the host serializes frames as-is, so a frozen schema only STRIPS on parse.
 */
...
describe("chat.subscribe@1.6 (held updates)", () => {
  ...
  // THE regression guard, aimed at `1.5` - the newest line with peers in the field.
  // It is bound to hand-frozen schemas exactly so a live addition cannot leak onto it, and nothing else catches a leak, since the live serverFrame union happily accepts either shape.
