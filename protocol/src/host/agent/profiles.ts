/**
 * Agent-facing provider-profile discovery, on-demand rate-limit reads, and atomic GUI-agent reconfiguration - the additive RPC family the A2A profile-awareness epic adds alongside `agent.create@2.0`'s `profileSelection`.
 */
import { z } from "zod";
import {
  defineDowngradePath,
...
/** One selectable row: the harness's provider's ambient login, or one of its managed profiles. */
...
/**
 * Frozen `agent.listProviderProfiles@1.0` response, pinned to the pre-Hermes provider id set (`providerIdSchemaV40`) so an already-shipped v1.0 caller's strict decode never sees a post-v4.0 provider (`hermes`, `omp`).
 * Do NOT widen this schema - extend the latest schema and use the v2 bridge instead.
 */
...
/**
 * Frozen `agent.listProviderProfiles@2.0` response, pinned to the pre-omp provider id set (`providerIdSchemaV50`) so an already-shipped v2.0 caller's strict decode never sees `omp`.
 * Do NOT widen this schema - extend the latest schema and use the v3 bridge instead.
 */
...
/**
 * Frozen `agent.listProviderProfiles@3.0` response, pinned to the pre-Hugging-Face provider id set (`providerIdSchemaV60`) so an already-shipped v3.0 caller's strict decode never sees `huggingface`.
 * Do NOT widen this schema - extend the latest schema and use the v4 bridge instead.
 */
...
/**
 * Frozen `agent.listProviderProfiles@4.0` response, pinned to the pre-Reasonix provider id set (`providerIdSchemaV70`) so an already-shipped v4.0 caller's strict decode never sees `reasonix`.
 * Do NOT widen this schema - extend the latest schema and use the v5 bridge instead.
 */
...
>({
  ...
  downgradeResponse: (response) => {
    // A v1.0 caller only ever configures/lists a pre-hermes provider, so the common case reparses cleanly through the frozen schema.
    // Fails closed (rather than silently mis-decoding) for any provider unrepresentable on the frozen v1.0 wire (Hermes and omp today) - the message deliberately names no single provider so it stays honest as the enum grows.
    const parsed =
...
>({
  ...
  downgradeRequest: (request) => ({ ok: true, value: request }),
  downgradeResponse: (response) => {
    // A v2.0 caller only ever lists a pre-omp provider, so the common case reparses cleanly through the frozen schema.
    // Fails closed (rather than silently mis-decoding) for any provider unrepresentable on the frozen v2.0 wire (omp today).
    const parsed =
...
>({
  ...
  downgradeRequest: (request) => ({ ok: true, value: request }),
  downgradeResponse: (response) => {
    // A v3.0 caller only ever lists a pre-Hugging-Face provider, so the common case reparses cleanly through the frozen schema.
    // Fails closed (rather than silently mis-decoding) for any provider unrepresentable on the frozen v3.0 wire.
    const parsed =
...
>({
  ...
  downgradeRequest: (request) => ({ ok: true, value: request }),
  downgradeResponse: (response) => {
    // A v4.0 caller only ever lists a pre-Reasonix provider, so the common case reparses cleanly through the frozen schema.
    // Fails closed (rather than silently mis-decoding) for any provider unrepresentable on the frozen v4.0 wire.
    const parsed =
...
// `last_used`/`inherit_sender` are not accepted: the caller must already know which concrete profile it wants a fresh read for.
...
/**
 * Frozen `agent.getProviderProfileRateLimits@1.0` response, pinned to the pre-Hermes `rateLimits.provider` enum (`providerRateLimitsSchemaV40`, whose `available: false` arm carries `providerIdSchemaV40`) so an.
 * Do NOT widen this schema - extend the latest schema and use the v2 bridge instead.
 */
...
/**
 * Frozen `agent.getProviderProfileRateLimits@2.0` response, pinned to the pre-omp `rateLimits.provider` enum (`providerRateLimitsSchemaV50`, whose `available: false` arm carries `providerIdSchemaV50`) so an.
 * Do NOT widen this schema - extend the latest schema and use the v3 bridge instead.
 */
...
/**
 * Frozen `agent.getProviderProfileRateLimits@3.0` response, pinned to the pre-Hugging-Face `rateLimits.provider` enum (`providerRateLimitsSchemaV60`, whose `available: false` arm carries `providerIdSchemaV60`) so an.
 * Do NOT widen this schema - extend the latest schema and use the v4 bridge instead.
 */
...
/**
 * Frozen `agent.getProviderProfileRateLimits@4.0` response, pinned to the pre-Reasonix `rateLimits.provider` enum (`providerRateLimitsSchemaV70`, whose `available: false` arm carries `providerIdSchemaV70`) so an.
 * Do NOT widen this schema - extend the latest schema and use the v5 bridge instead.
 */
...
// The LIVE line: ranges over `providerRateLimitsSchema` rather than a frozen snapshot, because `5` is the newest major and no released peer has ever negotiated it (the newest released baseline tops out at `4`).
...
export const agentGetProviderProfileRateLimitsDowngradeV20ToV10 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // Grok is representable in the frozen provider enum (it predates Hermes), so a grok-available snapshot degrades to the unavailable `unsupported_provider` shape (via the shared bridge map) - the exact row a v1.0 host.
      // A post-v4.0 provider's rate-limit read (Hermes, omp) stays unrepresentable on the frozen v1.0 wire and still fails closed below.
      const rateLimits = mapGrokAvailableToUnavailable(response.rateLimits);
      // A v1.0 caller only ever reads pre-hermes rate limits, so the common case reparses cleanly through the frozen schema.
      // Fails closed (rather than silently mis-decoding) for any provider unrepresentable on the frozen v1.0 wire (Hermes and omp today).
      const parsed =
...
export const agentGetProviderProfileRateLimitsDowngradeV30ToV20 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // No grok degrade here: the frozen v2.0 union keeps the grok available arm (grok rate limits predate the v1.1.8 tags), so a grok snapshot reparses as-is.
      // Only `omp` is unrepresentable, and this response carries exactly one provider - so fail closed rather than mis-decode.
      const parsed =
...
export const agentGetProviderProfileRateLimitsDowngradeV30ToV10 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // Unlike the v3->v2 bridge this one DOES degrade grok: the frozen v1.0 union predates the grok available arm, so a grok-available snapshot becomes the `unsupported_provider` row a v1.0 host returns for grok today (shared.
      // Post-v4.0 providers (Hermes, omp) stay unrepresentable and still fail closed.
      const rateLimits = mapGrokAvailableToUnavailable(response.rateLimits);
      const parsed =
        agentGetProviderProfileRateLimitsResponseSchemaV1.safeParse({
          ...response,
...
export const agentGetProviderProfileRateLimitsDowngradeV40ToV30 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // No grok degrade here: the frozen v3.0 union keeps the grok available arm, so a grok snapshot reparses as-is.
      // Only a post-v6.0 provider is unrepresentable, and this response carries exactly one provider - so fail closed rather than mis-decode.
      const parsed =
...
export const agentGetProviderProfileRateLimitsDowngradeV40ToV20 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // Same rule as the v4->v3 bridge against the narrower v2.0 enum: the frozen v2.0 union keeps grok, so only post-v5.0 providers (omp, huggingface) fail closed here.
      // Cursor degrades for the same reason it does there - `"cursor"` is in the frozen v5.0 provider enum too.
      const parsed =
...
export const agentGetProviderProfileRateLimitsDowngradeV40ToV10 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // Unlike the v4->v3 / v4->v2 bridges this one DOES degrade grok: the frozen v1.0 union predates the grok available arm, so a grok-available snapshot becomes the `unsupported_provider` row a v1.0 host returns for grok.
      // Cursor degrades here too: `"cursor"` is in the frozen v4.0 provider enum.
...
export const agentGetProviderProfileRateLimitsDowngradeV50ToV40 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // NO degrade maps here, unlike every bridge below: the frozen v4.0 union keeps every available arm the live union carries at this cut (grok, huggingface, opencode and cursor all shipped on 4.0), so an available snapshot.
      // This response carries exactly one provider, so fail closed rather than mis-decode.
      const parsed =
...
export const agentGetProviderProfileRateLimitsDowngradeV50ToV30 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // Same shape as the v4->v3 bridge: the frozen v3.0 union keeps grok, so only OpenCode/cursor degrade, and anything post-v6.0 (huggingface, reasonix) fails closed.
      const parsed =
        agentGetProviderProfileRateLimitsResponseSchemaV3.safeParse({
          ...response,
          rateLimits: mapCursorAvailableToUnavailable(
...
export const agentGetProviderProfileRateLimitsDowngradeV50ToV20 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // Same rule against the narrower v2.0 enum: the frozen v2.0 union keeps grok, so only post-v5.0 providers (omp, huggingface, reasonix) fail closed.
      const parsed =
        agentGetProviderProfileRateLimitsResponseSchemaV2.safeParse({
          ...response,
          rateLimits: mapCursorAvailableToUnavailable(
...
export const agentGetProviderProfileRateLimitsDowngradeV50ToV10 =
  ...
  >({
    ...
    downgradeRequest: (request) => ({ ok: true, value: request }),
    downgradeResponse: (response) => {
      // Like the v4->v1 bridge this one also degrades grok: the frozen v1.0 union predates the grok available arm.
      // Post-v4.0 providers (Hermes, omp, huggingface, reasonix) stay unrepresentable and fail closed.
      const rateLimits = mapGrokAvailableToUnavailable(
        mapCursorAvailableToUnavailable(
          mapOpenCodeAvailableToUnavailable(response.rateLimits),
        ),
...
export const agentConfigureSettingsSchema = z.object({
  ...
  /**
   * RETAINED even though Epic Mode was removed from the product.
   * It goes when the released floor passes this version, together with the equally-blocked `chatActiveTurn.agentMode` and `createAgentRequestSchemaV30.agentMode`.
   */
...
/**
 * Frozen `agent.configure@1.0` settings/response, pinned to the pre-Hermes harness id set (`guiHarnessIdSchemaV40`) so an already-shipped v1.0 caller's strict decode never sees a post-v4.0 `harnessId` (`hermes`, `omp`).
 * Do NOT widen this schema - extend the latest schema and use the existing v2 bridge instead.
 */
...
/**
 * Frozen `agent.configure@2.0` settings/response, pinned to the pre-omp harness id set (`guiHarnessIdSchemaV50`) so an already-shipped v2.0 caller's strict decode never sees `harnessId: "omp"`.
 * Do NOT widen this schema - extend the latest schema and use the v3 bridge instead.
 */
...
/**
 * Frozen `agent.configure@3.0` settings/response, pinned to the pre-Hugging-Face harness id set (`guiHarnessIdSchemaV60`) so an already-shipped v3.0 caller's strict decode never sees `harnessId: "huggingface"`.
 * Do NOT widen this schema - extend the latest schema and use the v4 bridge instead.
 */
...
/**
 * Frozen `agent.configure@4.0` settings/response, pinned to the pre-Reasonix harness id set (`guiHarnessIdSchemaV70`) so an already-shipped v4.0 caller's strict decode never sees `harnessId: "reasonix"`.
 * Do NOT widen this schema - extend the latest schema and use the v5 bridge instead.
 */
...
>({
  ...
  downgradeResponse: (response) => {
    // `settings.harnessId` echoes the configured agent's harness; a v1.0 caller only ever configures a pre-hermes harness, so the common case reparses cleanly through the frozen schema.
...
>({
  from: { major: 2, minor: 0 },
  to: { major: 3, minor: 0 },
  // Request shape is identical across both majors (v3.0 reuses `agentConfigureRequestSchemaV20`) - only the response's `harnessId` enum grows (omp).
  upgradeRequest: (request) => request,
  upgradeResponse: (response) => response,
});
...
>({
  ...
  downgradeRequest: (request) => ({ ok: true, value: request }),
  downgradeResponse: (response) => {
    // `settings.harnessId` echoes the configured agent's harness; a v2.0 caller only ever configures a pre-omp harness, so the common case reparses cleanly through the frozen schema.
    // An omp-configured response (unreachable from a v2.0 REQUEST today, but this bridge must still hold if that ever changes) cannot be represented on the frozen v2.0 wire, so this fails closed instead of silently.
...
>({
  from: { major: 3, minor: 0 },
  to: { major: 4, minor: 0 },
  // Request shape is identical across both majors (both reuse `agentConfigureRequestSchemaV20`) - only the response's `harnessId` enum grows (huggingface).
  upgradeRequest: (request) => request,
  upgradeResponse: (response) => response,
});
...
>({
  ...
  downgradeRequest: (request) => ({ ok: true, value: request }),
  downgradeResponse: (response) => {
    // `settings.harnessId` echoes the configured agent's harness; a v3.0 caller only ever configures a pre-Hugging-Face harness, so the common case reparses cleanly through the frozen schema.
    // A response on a post-v6.0 harness (unreachable from a v3.0 REQUEST today, but this bridge must still hold if that ever changes) cannot be represented on the frozen v3.0 wire, so this fails closed instead of silently.
...
>({
  from: { major: 4, minor: 0 },
  to: { major: 5, minor: 0 },
  // Request shape is identical across both majors (both reuse `agentConfigureRequestSchemaV20`) - only the response's `harnessId` enum grows (reasonix).
  upgradeRequest: (request) => request,
  upgradeResponse: (response) => response,
});
...
>({
  ...
  downgradeRequest: (request) => ({ ok: true, value: request }),
  downgradeResponse: (response) => {
    // `settings.harnessId` echoes the configured agent's harness; a v4.0 caller only ever configures a pre-Reasonix harness, so the common case reparses cleanly through the frozen schema.
    // A response on a post-v7.0 harness (unreachable from a v4.0 REQUEST today, but this bridge must still hold if that ever changes) cannot be represented on the frozen v4.0 wire, so this fails closed instead of silently.
...
>({
  from: { major: 5, minor: 0 },
  to: { major: 1, minor: 0 },
  // Same refusal as every other bridge onto v1.0: v1.0 has no `permissionMode` field, so the explicit choice a v2.0+ caller makes cannot be carried to a v1.0 host.
  downgradeRequest: () => ({
    ok: false,
    error: {
      code: "DOWNGRADE_UNSUPPORTED",
