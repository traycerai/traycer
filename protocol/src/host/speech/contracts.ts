 ...
 * These contracts only manage the model files the recognizer needs: a status read and an idempotent "ensure downloaded" trigger.
 */
import { z } from "zod";
import { defineRpcContract } from "@traycer/protocol/framework/index";
import { speechModelStatusSchema } from "@traycer/protocol/host/speech/schemas";

// `speech.getModelStatus@1.0` - reports whether the dictation model is present on disk and, while downloading, how far along.
...
// `speech.ensureModel@1.0` - idempotently kicks off the model download and returns the status snapshot immediately; the renderer polls `speech.getModelStatus` for progress and completion.
...
// `speechDictateV10` is intentionally NOT re-exported here - it lives in `./subscribe.ts` and the barrel (`./index.ts`) already surfaces it from there.
