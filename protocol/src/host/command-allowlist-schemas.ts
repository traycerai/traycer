...
import {
  ...
  commandAllowScopeSchema,
} from "@traycer/protocol/host/agent/gui/agent-runtime";

/**
 * Wire schemas for the per-device command allowlist management RPCs
 ...
 * shape is reused from {@link commandAllowRuleSchema} so the stored, matched,
...
export const commandAllowlistListResponseSchema = z.object({
  rules: z.array(commandAllowRuleSchema),
});
...
export const commandAllowlistClearRequestSchema = z.object({
  scope: commandAllowScopeSchema.optional(),
});
