...
describe("epic.listTuiAgents@1.2 origin union", () => {
  ...
  it("accepts a cloud row whose harness the cloud projection never carried", () => {
    // `harnessId` is read off `runSettingsSummary`, which a row written before that field existed does not have.
    // Nullable here and required on the local arms: the roster shows the agent without a harness mark rather than dropping it.
  ...
  it("keeps the @1.1 row parseable, so a @1.1 client is unaffected by the widening", () => {
    // The registry arm is the `@1.1` row plus one key, and an added plain object key is stripped by an older peer's schema.
...
describe("epic.listTuiAgents v1.1 -> v1.2 upgrade path", () => {
  ...
  it("leaves the request untouched", () => {
    // `hasDocReplica` answers what the caller already HOLDS; `@1.2` gates on what it can PARSE, which is the negotiated version.
    expect(
