...
describe("epic.subscribe@2 registry and open request", () => {
  it("registers @2.0 beside the frozen @1 line rather than replacing it", () => {
    ...
    // This branch mints @1.4-@1.6 above the mainline's @1.3, so the @1 line is
    // six minors tall here. What the assertion is about is that it SURVIVES
    // beside @2.0 rather than being replaced by it - both its mainline floor
    // and its current top are pinned so a regression either way is visible.
    expect(line1.latestMinor).toBe(6);
    expect(line1.versions[3]?.contract).toBe(epicSubscribeV13);
    expect(line1.versions[6]?.contract).toBe(epicSubscribeV16);
