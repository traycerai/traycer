...
export const organizationVersionSchema = lazySchema(() =>
  z
    .string()
...
export const labelNameSchema = lazySchema(() =>
  z.string().trim().min(1).max(80),
);
export const organizationColorSchema = lazySchema(() =>
  z.string().regex(/^#[0-9a-fA-F]{6}$/),
);
export const SYSTEM_LABEL_OWNER = "__system_labels__";
export const systemLabelKeySchema = lazySchema(() =>
  z.enum(["imported", "automation"]),
);
export const labelDefinitionSchema = lazySchema(() =>
  z.object({
    ownerId: id,
    labelId: id,
...
export const taskLabelSchema = lazySchema(() =>
  labelDefinitionSchema.extend({
    assignmentId: id,
  }),
);
export type TaskLabel = z.infer<typeof taskLabelSchema>;
export const labelCatalogCommandSchema = lazySchema(() =>
...
export const taskLabelOperationSchema = lazySchema(() =>
  z.discriminatedUnion("operation", [
    z.object({
...
export const applyTaskLabelsRequestSchema = lazySchema(() =>
  z.object({
    taskId: id,
...
>;
const position = lazySchema(() => z.number().int().min(0).max(2147483647));
export const groupOperationSchema = lazySchema(() =>
  z.discriminatedUnion("operation", [
    z.object({
      operation: z.literal("create"),
      groupId: id,
...
export const applyGroupOperationsRequestSchema = lazySchema(() =>
  z.object({
    operationId: id,
    expectedVersion: organizationVersionSchema,
...
>;
export const personalGroupsSchema = lazySchema(() =>
  z.object({
    version: organizationVersionSchema,
    groups: z.array(
      z.object({
...
export const personalAppearanceSchema = lazySchema(() =>
  z.object({
    taskId: id,
    version: organizationVersionSchema,
    color: organizationColorSchema.nullable(),
...
export const patchPersonalAppearanceRequestSchema = lazySchema(() =>
  z
    .object({
      taskId: id,
      operationId: id,
...
>;
export const readOrganizationTasksRequestSchema = lazySchema(() =>
  z.object({
...
export const publishAutomationRequestSchema = lazySchema(() =>
  z.object({
    taskId: id,
    version: organizationVersionSchema,
...
>;
export const publishAutomationResponseSchema = lazySchema(() =>
  z.object({
    version: organizationVersionSchema,
...
export const organizationMutationResultSchema = lazySchema(() =>
  z.object({
...
export const taskLabelStateSchema = lazySchema(() =>
  z.object({
...
export const cloudTaskFiltersSchema = lazySchema(() =>
  taskFiltersSchema.extend({
...
export const cloudListTasksRequestSchema = lazySchema(() =>
  listTasksRequestSchema.extend({
    filters: cloudTaskFiltersSchema.nullable(),
  }),
);
export type CloudListTasksRequest = z.infer<typeof cloudListTasksRequestSchema>;
export const taskOrganizationSchema = lazySchema(() =>
  z.object({
    labels: z.array(taskLabelSchema),
...
export const cloudListTasksResponseSchema = lazySchema(() =>
  listTasksResponseSchema.extend({
