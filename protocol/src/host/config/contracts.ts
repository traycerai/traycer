...
// v1.1 adds the optional `wslHealth` annotation to each row (a Windows wsl.exe that cannot host a terminal: the installer stub, or no registered distribution).
// Shipped as a minor rather than an in-place change to v1.0 because cli-v1.2.0 released the v1.0 response shape: a released peer never sends the key, so it may only exist at a version that peer does not negotiate.
...
// The request is empty either way, and a v1.0 response is already a valid v1.1 response - `wslHealth` is optional and its absence means "no verdict", exactly what a host that never probed should express - so both.
