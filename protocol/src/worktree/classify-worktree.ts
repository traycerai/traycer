/**
 * The canonical worktree presentation tier ladder, owned by the protocol so
 * the GUI, the CLI, and the HOST all read one set of labels and one precedence
 * order. It moved here verbatim from `clients/shared/worktree/classify-worktree.ts`
 * (which now re-exports it) when host-side automatic cleanup needed the same
 * ladder: two implementations of "which tier is this worktree in" would let an
 * unattended deletion disagree with the pill the user was shown.
 *
 * This module stays PURE PRESENTATION. It classifies the facts a host already
 * put on the wire and nothing else - it is deliberately NOT the automatic
 * cleanup eligibility decision, which additionally needs host-internal
 * freshness/actor evidence (probe outcomes, activity leases, holder inventory)
 * that must never become a wire compatibility contract. A green tier here is a
 * necessary, never a sufficient, condition for deleting a worktree unattended.
 */
import type {
