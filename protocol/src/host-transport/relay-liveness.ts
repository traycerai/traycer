 ...
 * ## Shape
 *
 * Only the per-socket SAMPLING state lives here. The arithmetic is
 * {@link deriveRttDeadlineMs}, shared with the screencast control plane
 * (ticket 18) rather than re-derived: the relay simply passes the variance
 * term that plane leaves at zero, because THIS deadline is a liveness verdict
 * rather than a budget and has to cover the path's jitter tail.
 *
 ...
 */
import { deriveRttDeadlineMs } from "../host/browser/control-plane-rtt";
...
export type RelayPathEstimator = {
  /** Folds one completed `relay-ping` -> `relay-pong` round trip in. */
  readonly noteRoundTrip: (sampleMs: number) => void;
  /**
   * `max(floorMs, roundTrips x (srtt + 4 x rttvar))`. Returns `floorMs`
   * unchanged until a round trip has completed.
   */
  readonly deadlineMs: (floorMs: number, roundTrips: number) => number;
  /** The smoothed round trip, or `null` before the first sample. Trace only. */
