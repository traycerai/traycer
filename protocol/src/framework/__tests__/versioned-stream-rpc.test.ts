...
describe("validateVersionedStreamRpcRegistry", () => {
  it("accepts the combined hostStreamRpcRegistry", () => {
    ...
    // `editUserMessage` and on the queued prompt item took @1.17. Skeleton
    // resume (the open request's `resume` claim and `retainedRows` on the
    // `skeletonChunk` frame) took @1.18.
    //
