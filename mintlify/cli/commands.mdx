...
| `traycer agent fork`                | Fork an agent from its latest checkpoint.       |
| `traycer agent configure`           | Change a GUI agent's settings for future turns. |
...
| `--permission-mode <mode>`              | `agent create`, `agent fork`, `agent configure`                                                  | GUI permission mode: `supervised`, `auto_accept_edits`, `auto`, or `full_access`. Defaults to `full_access`. |
...
`auto` is [Auto mode](/panels/agents#auto-mode): a judge approves routine commands and sends risky ones to you. On `agent configure`, the mode applies from the agent's next turn. Omitting `--permission-mode` there does not keep the agent's current mode: it resets the agent to `full_access`. Not every coding agent supports every mode; see [Agents & Models](/agents-and-models/coding-agents).
